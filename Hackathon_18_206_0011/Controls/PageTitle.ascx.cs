...
public partial class User_PageTitle : TitlePanel, ITitleModuleController
{
	...
	private void AddFavoriteLegacy(string title, Guid? siteId)
	{
		...
		using (PXDataRecord record = PXDatabase.SelectSingle<PX.SM.SiteMap>(new PXDataField("Title"), new PXDataFieldValue("NodeID", siteId)))
		{
		...
		if (!string.IsNullOrEmpty(neutralTitle))
		{
			using (PXDataRecord rec_id = PXDatabase.SelectSingle<Favorite>(
					new PXDataField(new Column("Position").Max() + new PX.Data.SQLTree.Constant(1)),
					new PXDataFieldValue("UserID", PXAccess.GetUserID())))
			{
				int pos = 1;
				if (rec_id != null)
