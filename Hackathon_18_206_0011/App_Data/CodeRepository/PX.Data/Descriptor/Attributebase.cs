...
namespace PX.Data
{
	...
	public abstract class PXDBInterceptorAttribute : Attribute
	{
		...
		{
			return ((description.Expr as SQLTree.Column)?.Table() as SimpleTable)?.Name == table.Name;
		}
	...
	public class PXTableAttribute : PXDBInterceptorAttribute
	{
        ...
        private sealed class BqlRowSelection : BqlCommand, IPXExtensibleTableAttribute
		{
			...
			private void CommandPreparing(PXCache sender, PXCommandPreparingEventArgs e, Type table)
			{
				...
				e.BqlTable = table;
				e.Expr = new Column(fieldName, new SimpleTable(tableName));
			}
			...
			public override SQLTree.Query GetQueryInternal(PXGraph graph, BqlCommandInfo info, Selection selection) 
			{
				...
				foreach (string field in cache.Fields) {
					...
					if (single) {
						outerQuery.GetSelection().Add(new Column(field, mainTable.Name));
				...
				if (extensions != null) {
					foreach (Type ext in extensions) {
						...
						Joiner j = new Joiner(jt, new SimpleTable(ext.Name), query);
				...
				if (single) {
					...
					SQLExpression innerWhere = new SQLExpression();
					SQLExpression outerWhere = new SQLExpression();
					for (int i = 0; i < wherefields.Length; i++) {
						SQLExpression expressionThere;
						if (preparedColumnsByAlias.TryGetValue(wherefields[i], out expressionThere)) {
							innerWhere=innerWhere.And(expressionThere.Equal(Literal.NewParameter(i)));
						}
						outerWhere=outerWhere.And(new Column(wherefields[i], mainTable.Name).Equal(Literal.NewParameter(i)));
					}
			...
			public override void Parse(PXGraph graph, List<IBqlParameter> pars, List<Type> tables, List<Type> fields, List<IBqlSortColumn> sortColumns, StringBuilder text, Selection selection)
			{
				...
				List<Type> extensions = cache.GetExtensionTables();
			    var mainTableName = mainTable.Name;
			    foreach (string field in cache.Fields)
				{
					if (selection.Restrict && !IsFieldRestricted(cache, selection, field)) continue;
					...
					PXCommandPreparingEventArgs.FieldDescription description;
					cache.RaiseCommandPreparing(field, null, null, PXDBOperation.Select, null, out description);
					if (description == null || description.Expr == null) continue;

					bool shouldWriteFieldName = !(description.Expr is Column) ||
					...
					string fieldName = shouldWriteFieldName ? description.Expr.SQLQuery(graph.SqlDialect.GetConnection()).ToString() : Null;
					
					if (single)
					{
						selection.Add(sqlDialect.quoteTableAndColumn(mainTableName, field));
						if (firstfield)
							firstfield = false;
						else
							text.Append(", ");
				...
				for (int i = 0; i < this.tables.Count; i++)
				{
                    ...
                    text.Append(TableChangingScope.GetTableName(sqlDialect, () => sqlDialect.quoteDbIdentifier(tableName), tableName));
					text.Append(" ");
					text.Append(sqlDialect.quoteDbIdentifier(tableName));
					joinedTables.Add(this.tables[i]);
				...
				if (single)
				{
					...
					text.Append(") ").Append(mainTableName);

					int j = 0;
				        text.AppendJoined(wherefields, " WHERE ", " AND ",
				            k =>
				                sqlDialect.quoteTableAndColumn(mainTableName, wherefields[j]) + " = " +
				...
				for (int i = 0; i < links.Count; i++) {
					for (int j = 0; j < bound.Count; j++) {
						...
						if (k < bound.Count) {
							...
							if (description?.Expr == null) continue;
							exp = exp.And(pdescription.Expr.Equal(description.Expr));
			...
			private IEnumerable<string> MakeLinks(List<Type> bound, PXCache cache)
			{
				for (int i = 0; i < links.Count; i++)
				{
					for (int j = 0; j < bound.Count; j++)
					{
						PXCommandPreparingEventArgs.FieldDescription pdescription;
						cache.RaiseCommandPreparing(links[i], null, null, PXDBOperation.Select, bound[j], out pdescription);
						if (pdescription != null && pdescription.Expr != null)
						{
							int k = j + 1;
							if (k < bound.Count)
							{
								...
								if (description != null && description.Expr != null)
								{
								    ...
								    sb.Append(pdescription.Expr.SQLQuery(cache.Graph.SqlDialect.GetConnection()).ToString());
								    sb.Append(" = ");
								    sb.Append(description.Expr.SQLQuery(cache.Graph.SqlDialect.GetConnection()).ToString());
            ...
            for (int i = 0; i < length; i++)
            {
				if (pars[i] != null && pars[i].Count > 0)
				{
					...
					if (tc._NoteIDName != null)
					{
						...
						if (note?.Value != null && tc._KeyValueAttributeNames != null)
						{
							...
							object[] vals = tc.GetSlot<object[]>(row, tc._KeyValueAttributeSlotPosition);
							object[] origs = tc.GetSlot<object[]>(row, tc._KeyValueAttributeSlotPosition, true);

							if (vals != null)
							{
								foreach (KeyValuePair<string, int> pair in tc._KeyValueAttributeNames)
								{
									if (pair.Value < vals.Length)
									{
										PXDataFieldAssign a =
											new PXDataFieldAssign(pair.Key,
											tc._KeyValueAttributeTypes[pair.Key] == StorageBehavior.KeyValueDate ? PXDbType.DateTime
											: (tc._KeyValueAttributeTypes[pair.Key] == StorageBehavior.KeyValueNumeric ? PXDbType.Bit
											: PXDbType.NVarChar), vals[pair.Value]
											);
										a.Storage = tc._KeyValueAttributeTypes[pair.Key];
										if (a.IsChanged = origs != null && pair.Value < origs.Length && !object.Equals(a.Value, origs[pair.Value]))
										{
											a.NewValue = tc.AttributeValueToString(pair.Key, a.Value);
											a.OldValue = origs[pair.Value];
