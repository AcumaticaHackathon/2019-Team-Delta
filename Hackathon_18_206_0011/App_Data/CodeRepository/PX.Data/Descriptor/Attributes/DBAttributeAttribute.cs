			...
			else
			{
				...
				if (operationAllowsSelectForXml) {
					if (aggregateAttributes && (e.Operation & PXDBOperation.Option) != PXDBOperation.External) {
						...
						e.Expr = new SQLTree.SubQuery(GetAttributesJoinedQuery(sender.Graph, bld, init, ((IBqlSearch)_SingleSelect).GetField(), _PureWhere));
						return;
					}
					else {
						e.BqlTable = _BqlTable;

						Query q = new Query().Field(new SQLTree.Constant(_Fields.Length));
						e.Expr = new SubQuery(q);
					}
				}
				else {
