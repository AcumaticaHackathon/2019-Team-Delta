...
namespace PX.Data
{
    ...
    /// <summary>Binds a DAC field of <tt>Guid?</tt> type to the database column that keeps note identifiers and enables attachment of text comments, files, and activity items
    ...
    public class PXNoteAttribute : PXDBGuidAttribute, IPXRowPersistingSubscriber, IPXRowPersistedSubscriber, IPXRowDeletedSubscriber, IPXReportRequiredField
    {
        ...
        internal const string _NotePopupTextField = "NotePopupText";
        ...
        internal const string _NotePopupTextExistsField = "NotePopupTextExists";
        ...
        {
            return PXFieldState.UnwrapValue(cache.GetValueExt(data, _NotePopupTextField)) as string;
        }
        ...
        public virtual void notePopupTextFieldSelecting(PXCache sender, PXFieldSelectingEventArgs e)
        {
            if (PopupTextEnabled)
            {
                ...
                NoteTextGenericFieldSelecting(sender, e, GetPopupOriginalCountsItem, SetPopupNoteTextExists, getText, _NotePopupTextField);
            }
        }
        ...
        {
            Action<PXCache, Guid, string> updateNoteRecord = (c, g, s) => UpdateNoteRecord(c, g, s);
        ...
        public virtual void notePopupTextFieldUpdating(PXCache sender, PXFieldUpdatingEventArgs e)
        {
            if (PopupTextEnabled)
            {
                Action<PXCache, Guid, string> updateNoteRecord = (c, g, s) => UpdateNoteRecord(c, g, popupText: s);
        ...
        /// <exclude/>
		public static void UpdateNoteRecord(PXCache sender, Guid id, String text = null, String popupText = null)
        {
            Note note = new Note { NoteID = id };
        ...
        protected virtual void notePopupTextCommandPreparing(PXCache sender, PXCommandPreparingEventArgs e)
        {
            if (PopupTextEnabled)
            {
                NoteTextGenericCommandPreparing(sender, e, _PopupTextRequired, _NotePopupTextField);
            }
        }
        ...
        private void NoteTextGenericCommandPreparing(PXCache sender, PXCommandPreparingEventArgs e, bool textRequired, string noteTextField)
        {
            ...
            if (null != subqueryKey)
            {
				...
				q.Field(new Column(noteTextField, noteDoc)).From(noteDoc).Where(new Column("NoteId", noteDoc).Equal(GetExpressionForSubquery(sender, e, _TextRequired, false)));
				e.Expr = new SubQuery(q).Embrace();
            }
            else
            {
				e.Expr = new SQLTree.SQLExpression(SQLTree.SQLExpression.Operation.NULL);
            }
        }
            ...
            if (tableFound)
            {
                ...
                if (null != subqueryKey)
                {
					...
					q.Field(SQLExpression.Count()).From(act).Where(
						...
						? new Column("BAccountID", act).Equal(keyExpr)
						: new Column("RefNoteID", act).Equal(keyExpr)).And(
						new Column("UIStatus", act).NotEqual("CL").And(
						new Column("UIStatus", act).NotEqual("RL")))
					);
			...
			if ((e.Operation & PXDBOperation.Command) == PXDBOperation.Select && fieldRequired) {
				...
				if (!_BqlTable.IsAssignableFrom(sender.BqlTable)) {
					if (sender.Graph.Caches[_BqlTable].BqlSelect != null &&
						...
						e.DataValue = e.Value;
						e.BqlTable = _BqlTable;
						return new Column(_DatabaseFieldName, new SimpleTable(tableName ?? _BqlTable.Name));
					}
					else {
						PXCommandPreparingEventArgs.FieldDescription description;
						e.Cancel = !sender.Graph.Caches[_BqlTable].RaiseCommandPreparing(_DatabaseFieldName, e.Row, e.Value, e.Operation, e.Table, out description);
					...
					if (forCRActivity && ActivitiesCountByParent) {
						string field = EntityHelper.GetIDField(sender.GetItemType());
						return new Column(field, new SimpleTable(tableName ?? (e.Table != null ? e.Table.Name : _BqlTable.Name)));
					}
					else {
						return new Column(_DatabaseFieldName, new SimpleTable(tableName ?? (e.Table != null ? e.Table.Name : _BqlTable.Name)));
					}
				}
			}
			return null;
        ...
        protected virtual void noteFilesCommandPreparing(PXCache sender, PXCommandPreparingEventArgs e)
        {
            ...
            if (null != subqueryKey)
            {
				...
				q.Field(SQLExpression.Count()).From(noteDoc).Where(new Column("NoteId", noteDoc).Equal(GetExpressionForSubquery(sender, e, _FilesRequired, false)));
				e.Expr = new SubQuery(q).Embrace();
            }
            else
            {
        ...
        public override void CacheAttached(PXCache sender)
        {
            ...
            if (PopupTextEnabled)
            {
                sender.Fields.Insert(++fieldIdx, _NotePopupTextField);
                field = _NotePopupTextField.ToLower();
                ...
                sender.Fields.Insert(++fieldIdx, _NotePopupTextExistsField);
                sender.FieldSelectingEvents[_NotePopupTextExistsField.ToLower()] += notePopupTextExistsFieldSelecting;
            }
    ...
    public static class PopupNoteManager
    {
        ...
        private const string _slotPrimaryViewWarningKey = "PopupNoteManagerPrimaryViewWarningKey";
        private const string _slotPreserveCopyPasteWarningsKey = "PopupNoteManagerPreserveWarningsKey";
        private const string _customInfoKey = "PopupNoteManagerCustomInfoKey";
        ...
        private static PXUIWarningInfo PrimaryViewWarning
        {
            get
            {
                return PXContext.GetSlot<PXUIWarningInfo>(_slotPrimaryViewWarningKey);
            }
            set
            {
                PXContext.SetSlot(_slotPrimaryViewWarningKey, value);
            }
        }
        ...
        public static void ShowPrimaryViewWarning(PXGraph graph)
        {
            PXUIWarningInfo warning = PrimaryViewWarning;

            if (warning != null)
            {
                PXCache cache = graph.Caches[warning.CacheItemType];

                ShowWarningOnField(cache, cache.Current, warning.FieldName, warning.WarningText);
            }
        }

        private static void ShowWarningOnField(PXCache cache, object data, string fieldName, string text)
        {
            PXUIFieldAttribute.SetWarning(cache, data, fieldName, text);
        }
        ...
        {
            List<PXUIWarningInfo> warnings = PXLongOperation.GetCustomInfoForCurrentThread(_customInfoKey) as List<PXUIWarningInfo>;

            if (warnings == null)
            {
                warnings = new List<PXUIWarningInfo>();
                PXLongOperation.SetCustomInfoInternal(PXLongOperation.GetOperationKey(), _customInfoKey, warnings);
            }

            warnings.Add(new PXUIWarningInfo(cache.GetItemType(), data, fieldName, text));
        }

        public static void StorePrimaryViewWarning(PXCache cache, string fieldName, string text)
        {
            PrimaryViewWarning = new PXUIWarningInfo(cache.GetItemType(), null, fieldName, text);
        }

        public static List<PXUIWarningInfo> GetImportFromExcelWarning(object key)
        {
            return PXLongOperation.GetCustomInfo(key, _customInfoKey) as List<PXUIWarningInfo>;
        }

        public static void ShowImportFromExcelWarnings(PXGraph graph, IEnumerable<PXUIWarningInfo> warnings)
        {
            foreach (PXUIWarningInfo w in warnings)
            {
                PXCache cache = graph.Caches[w.CacheItemType];

                ShowWarningOnField(cache, w.CacheItem, w.FieldName, w.WarningText);
            }
        ...
        public static void RegisterText(PXCache cache, object data, string fieldName, string text)
        {
            ...
            if (!ignorePopup)
            {
                ...
                {
                    StoreImportFromExcelWarning(cache, data, fieldName, text);
                }
                else if (graph.IsCopyPasteContext && errorTextIsEmpty)
                {
                    if (graph.PrimaryItemType == cache.GetItemType())
                    {
                        StorePrimaryViewWarning(cache, fieldName, text);
                    }
                    else
                    {
                        ShowWarningOnField(cache, data, fieldName, text);
