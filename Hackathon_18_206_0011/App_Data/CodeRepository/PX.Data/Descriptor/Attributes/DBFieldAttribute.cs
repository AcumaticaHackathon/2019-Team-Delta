...
namespace PX.Data
{
	...
	public class PXDBFieldAttribute : PXEventSubscriberAttribute, IPXRowSelectingSubscriber, IPXCommandPreparingSubscriber
	{
		...
		protected virtual void PrepareFieldName(string dbFieldName, PXCommandPreparingEventArgs e)
		{
			...
			e.Expr=new SQLTree.Column(dbFieldName, new SQLTree.SimpleTable(tableName), e.DataType);
		}
	...
	public class PXDBLocalizableStringAttribute : PXDBStringAttribute, IPXRowPersistedSubscriber
	{
		...
		public static PXDataField GetValueSelect(string tableName, string fieldName, bool isLong)
		{
			...
			for (int i = 0; i < ordered.Count; i++)
			{
				...
				locSwitch.Case(fieldCol.Equal(fieldName + value), new SQLTree.Constant(i));
			}
			...
			if (provider != null)
			{
				...
				{
					SQLExpression seq = new SQLTree.Constant(cid);
			...
			for (int i = 0; i < ordered.Count; i++)
			{
				var col = new SQLTree.Constant(fieldName + ordered[i].ToUpper());
			...
			query.Field(new Column(isLong ? "ValueText" : "ValueString", extTable))
				...
				.Where(new Column("RecordID", extTable).Equal(new Column("NoteID", tableName)
				...
				if ((e.Operation & PXDBOperation.Command) == PXDBOperation.Select)
				{
					...
					if (description != null)
					{
						...
						if ((e.Operation & PXDBOperation.Option) == PXDBOperation.External)
						{
							...
							SimpleTable sqlTable = new SimpleTable(description.BqlTable.Name + SqlDialectBase.KvExtSuffix);
							Column sqlField = new Column("FieldName", sqlTable);
							SQLExpression locSeq = new SQLExpression();
							SQLSwitch locSwitch = new SQLSwitch();

							for (int i = 0; i < ordered.Count; i++)
							{
								var value = ordered[i].ToUpper();
								locSwitch.Case(sqlField.Equal((IsProjection ? _DatabaseFieldName : _FieldName) + value), new SQLTree.Constant(i));
							}
							...
							q.Field(sqlValueField).From(sqlTable).Where(
								new Column("RecordID", sqlTable).Equal(new Column(sender._NoteIDName, (e.Table ?? description.BqlTable).Name))
	...
	public class PXDBLocalStringAttribute : PXDBStringAttribute
	{
		...
		protected override void PrepareCommandImpl(string dbFieldName, PXCommandPreparingEventArgs e)
		{
			...
			{
				string tableName = (e.Table == null ? _BqlTable.Name : e.Table.Name);
				e.Expr = new Column(dbFieldName, tableName).Coalesce(new Column(_DatabaseFieldName, tableName));
			}
		}
		#endregion
	}
	...
	public class PXDBDateAttribute : PXDBFieldAttribute, IPXRowSelectingSubscriber, IPXCommandPreparingSubscriber, IPXFieldUpdatingSubscriber, IPXFieldSelectingSubscriber
	{
		...
		private static Constant<string>[] dateParts = 
		{
			new DatePart.month(),
			new DatePart.day(),
			new DatePart.hour(),
	    ...
	    private void AddFieldForDatePart(PXCache cache, Constant<string> datePart, string datePartName)
	    {
			...
			cache.CommandPreparingEvents.Add(partFieldName, (sender, e) =>
			{
				if (e.IsSelect() && (e.Operation & PXDBOperation.Option) == PXDBOperation.External)
				{
					...
					e.Expr = SQLExpression.DatePartByTimeZone(datePart, new Column(fieldName, e.Table.Name), isNeedConvertTime ? GetTimeZone() : null);
				}
			});
		}
			...
			if ((e.Operation & PXDBOperation.Command) == PXDBOperation.Select && (!sender.BypassCalced || sender.BqlSelect != null))
			{
				...
				if (!typeof(IBqlCreator).IsAssignableFrom(_OperandType)) {
					if (String.Compare(_OperandType.Name, _FieldName, StringComparison.OrdinalIgnoreCase) != 0) {
						...
						exp = new Column(_OperandType.Name, new SimpleTable(tables[0].Name));
					}
				}
				else {
					if (_Operand == null) {
					...
					var calcedTables = BqlCommand.GetFields(_OperandType).Where(t => t.DeclaringType != null && typeof(IBqlTable).IsAssignableFrom(t.DeclaringType)).Select(t => t.DeclaringType).Distinct().ToList();
					foreach (var table in calcedTables)
					{
						var cache = sender.Graph.Caches[table];
    ...
    /// <summary>Maps a DAC field to the database column and automatically sets the field value to the data record's last modification date and time.</summary>
	...
	public class PXDBBinaryAttribute : PXDBFieldAttribute
	{
		...
		protected override void PrepareCommandImpl(string dbFieldName, PXCommandPreparingEventArgs e)
		{
			base.PrepareCommandImpl(dbFieldName, e);
			if (_DatabaseFieldName != null && (e.Operation & PXDBOperation.Option) == PXDBOperation.GroupBy)
				e.Expr = new SQLExpression(SQLExpression.Operation.NULL);
	...
	public class PXDBBinaryStringAttribute : PXDBBinaryAttribute
	{
		...
		{
			if (e.Value != null)
				e.Value = SerializeValue(e.Value);
			base.CommandPreparing(sender, e);
		}
	}
