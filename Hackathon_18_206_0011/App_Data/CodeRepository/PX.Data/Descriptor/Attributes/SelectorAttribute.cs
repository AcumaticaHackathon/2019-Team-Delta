...
namespace PX.Data
{
	...
	public class PXSelectorAttribute : PXEventSubscriberAttribute, IPXFieldVerifyingSubscriber, IPXFieldSelectingSubscriber, IPXDependsOnFields
	{
        ...
        public virtual bool ShowPopupMessageOnSelecting { get; set; }
        public virtual bool ShowPopupMessageOnVerifying { get; set; }
		...
		internal sealed class GlobalDictionary : IPXCompanyDependent
        {
			...
			public static void ClearFor(Type table, byte keysCount) => PXDatabase.ResetLocalizableSlot<GlobalDictionary>(_GetSlotName(table, keysCount), table);
		...
		internal static object GetItem(PXCache cache, PXSelectorAttribute attr, object data, object key, bool unconditionally)
		{
		    ...
		    if (row == null && (key == null || key.GetType() == cache.GetFieldType(attr._FieldName)))
			{
				...
				{
					CheckIntegrityAndPutGlobal(globalDictionary, view.Cache, attr.ForeignField.Name, row, attr.CreateGlobalCacheKey(cache, data, key), false);
				}
			}
			return row;
		}
	    ...
	    internal static object GetItemUnconditionally(PXCache cache, PXSelectorAttribute attr, object key)
	    {
            if (key == null)
                return null;
	        GlobalDictionary dummy;
            var row = LookupGlobalDictionary(cache, attr, null, key, out dummy);
            ...
            if (row == null && fieldType == null || key.GetType() == fieldType)
	        {
	            ...
	            row = SelectSingleBound(select, new object[0], key);
	            return row;
            }
	        return row;
	    }
		...
		internal static void CheckIntegrityAndPutGlobal(GlobalDictionary globalDictionary, PXCache foreignCache, String foreignField, Object foreignRow, Object ownKey, Boolean isRowDeleted, Boolean putSingle = false)
		{
				...
				|| String.Equals(foreignCache.Identity, foreignField, StringComparison.OrdinalIgnoreCase)))
			{
				lock (globalDictionary.SyncRoot)
				{
					globalDictionary.Set(ownKey, foreignRow, isRowDeleted);
					if (putBoth)
					{
						Object valKey = compositeKey == null ? val : Composite.Create(otherKeys.Append(val));
						globalDictionary.Set(val, foreignRow, isRowDeleted);
					}
				}
			}
		}
		...
		public virtual void FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
		{
            ...
            if (sender.Keys.Count == 0 || _FieldName != sender.Keys[sender.Keys.Count - 1])
            {
                ...
                else if (ShowPopupMessageOnVerifying)
                {
		...
		public virtual void FieldSelecting(PXCache sender, PXFieldSelectingEventArgs e)
		{
            ...
            if (_AttributeLevel == PXAttributeLevel.Item || e.IsAltered)
            {
                ...
                if (ShowPopupMessageOnSelecting && e.Row != null)
                {
                    object p = sender.GetValueOriginal(e.Row, _FieldName);
                    ViewWithParameters view = GetViewWithParameters(sender, p);
		...
		foreach (PXDBFieldAttribute attr in sender.GetAttributes(_FieldName).OfType<PXDBFieldAttribute>()) {
				...
				q.Field(new Column(_SubstituteKey.Name, extSQLTable))
					...
					.Where(
						...
						.Equal(
						new Column(sender.BqlSelect == null ? attr.DatabaseFieldName : _FieldName,
							e.Table?.Name ?? (isInnerSubselect ? attr.BqlTable.Name : _BqlTable.Name)))
					);
				...
				(e.Value == null || e.Value is string))
			{
				...
				foreach (PXEventSubscriberAttribute attr in sender.GetAttributes(_FieldName))
				{
					if (attr is PXDBFieldAttribute)
					{
						e.BqlTable = _BqlTable;
						string extTable = _Type.Name + "Ext";
						string extTableQuoted = e.SqlDialect.quoteDbIdentifier(_Type.Name + "Ext");
						...
						sq.Field(sqlField).From(sqlTable).Where(
							new Column(ForeignField.Name, extTable).Equal(new Column(sender.BqlSelect == null ? ((PXDBFieldAttribute)attr).DatabaseFieldName : _FieldName, e.Table == null ? _BqlTable.Name : e.Table.Name))
						);
...
#pragma warning restore CS0618 // Type or member is obsolete
    ...
    {
        public bool ShowPopupMessageOnSelecting { get; set; }
        public bool ShowPopupMessageOnVerifying { get; set; } = true;
        public override void CacheAttached(PXCache sender)
        {
            ...
            {
                s.ShowPopupMessageOnSelecting = ShowPopupMessageOnSelecting;
                s.ShowPopupMessageOnVerifying = ShowPopupMessageOnVerifying;
            }
        }
    }
}
