...
namespace PX.Data
{
	...
	public class PXStringAttribute : PXEventSubscriberAttribute, IPXFieldUpdatingSubscriber, IPXFieldSelectingSubscriber, IPXCommandPreparingSubscriber
	{
		...
		public virtual void CommandPreparing(PXCache sender, PXCommandPreparingEventArgs e)
		{
			if ((e.Operation & PXDBOperation.Command) == PXDBOperation.Select && e.Value != null)
			{
				...
				if(e.Expr == null) e.Expr = new Column(_FieldName, _BqlTable.Name);
	...
	public class PXShortAttribute : PXEventSubscriberAttribute, IPXFieldUpdatingSubscriber, IPXFieldSelectingSubscriber, IPXCommandPreparingSubscriber
	{
		...
		public virtual void CommandPreparing(PXCache sender, PXCommandPreparingEventArgs e)
		{
			if ((e.Operation & PXDBOperation.Command) == PXDBOperation.Select && e.Value != null)
			{
				e.BqlTable = _BqlTable;
				if (e.Expr == null)
					e.Expr = new SQLTree.Column(_FieldName, e.BqlTable.Name);
    ...
    /// <remarks>
    /// 	<para>The attribute configures a drop-down list that will represent the
					...
					if (descr != null && descr.Expr != null)
					{
						...
						for (int i = 0; i < allowedValues.Length; i++)
						{
							...
							if (value == null) {
								...
								whenexp = descr.Expr.Equal(SQLTree.Constant.SQLConstant(value));
							}

							swexp.Case(whenexp, SQLTree.Constant.SQLConstant(_AllowedLabels[i]));
						}
						e.Expr = swexp;
		...
		{			
			if (_Dependencies == null)
			{
				...
				if (_Formula != null)
				{
					...
					var exp = new PX.Data.SQLTree.SQLExpression();
	...
	public class PXDBScalarAttribute : PXDBFieldAttribute
	{
		...
		public override void CommandPreparing(PXCache sender, PXCommandPreparingEventArgs e)
		{
			if ((e.Operation & PXDBOperation.Command) == PXDBOperation.Select && 
                ...
                !_recursiveCall)
			{
				bool isWhereExternal = (e.Operation & PXDBOperation.Option) == PXDBOperation.External &&
				                       (e.Operation & PXDBOperation.Place) == PXDBOperation.WhereClause;
				if (((e.Operation & PXDBOperation.Option) == PXDBOperation.Normal ||
				(e.Operation & PXDBOperation.Option) == PXDBOperation.Internal) || isWhereExternal)
				{
					...
					{
						e.Expr = new SQLTree.Column(_FieldName, tables.First().Name);
					}
					else
					{
						string fieldtext;
						...
						if (_Search is IBqlAggregate &&
							...
							fieldtext = BqlCommand.GetSingleField(field, sender.Graph, new List<Type>(), selection,
								BqlCommand.FieldPlace.Select);
							query.ClearSelection();
							query.Field(BqlCommand.GetSingleExpression(field, sender.Graph, null, selection, BqlCommand.FieldPlace.Select));
	...
	public class PXDBDataLengthAttribute : PXEventSubscriberAttribute, IPXCommandPreparingSubscriber, IPXRowSelectingSubscriber
	{
		...
		public virtual void CommandPreparing(PXCache sender, PXCommandPreparingEventArgs e)
		{
			...
			{
				e.BqlTable = _BqlTable;
				e.Expr = new SQLTree.Column(this._TargetFieldName, tableName).BinaryLength();
			}
		}
    ...
    /// <summary>Indicates a DAC field of <tt>int?</tt> type that represents a duration in time as the number of minutes and that is not mapped to a database column.</summary>
    /// <remarks>The attribute is added to the value declaration of a DAC field. The field is not bound to a table column.</remarks>
    /// <example>
    ///   <code title="" description="" lang="CS">
    ...
    /// <summary>Used for calculated DAC fields that contain references to
