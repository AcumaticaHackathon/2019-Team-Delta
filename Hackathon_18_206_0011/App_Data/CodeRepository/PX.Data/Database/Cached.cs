...
namespace PX.Data
{
	...
	{
		/// <summary>
		/// A <see cref="PXView"/> that always returns only a predefined set of rows
		/// </summary>
		...
		/// <summary>
		/// Gets the value indicating whether a view delegate should retrieve total row count.
		/// </summary>
					...
					if (_Select is IBqlAggregate && !String.IsNullOrEmpty(fieldName))
					{
						...
						else
						{
							...
							if (descr != null && descr.Expr != null)
							{
								...
								newname = _Selection.Get(newfieldName);
						...
						case PXCondition.ISNOTNULL:
							try
							{
								...
								else
								{
									...
									if (descr != null && descr.Expr != null && descr.Expr.Oper() != SQLTree.SQLExpression.Operation.NULL
										&& descr.Expr is SQLTree.SubQuery)
									{
										fr.Description = descr;
										fr.UseExt = true;
									}
	        ...
	        if (filters != null)
	        {
	            ...
	            for (int i = 0; i < filters.Length; i++)
	            {
	                ...
	                if (field != null && !conditionViolation)
	                {
	                    ...
	                    {
							string fieldName = fr.Description.Expr.SQLQuery(cache.Graph.SqlDialect.GetConnection()).ToString();
							SQLTree.SQLExpression fieldExpr = fr.Description.Expr?.Duplicate();
							int idxdot, idxscore;
...
//			if (SelectQueries.IsCommandMutable && queryKey.CommandText == null && queryKey.BadParamsQueryNotExecuted) 
			...
			)
		{
			...
			if (_IsReadOnly || !Graph.Caches.ContainsKey(CacheType)) 
				return false;
...
//			IBqlParameter[] selpars = _Select.GetParameters();
			...
			try
			{
			    ...
			    if (list is IPXDelegateCacheResult delegateCache && delegateCache.IsResultCachable)
			    {
			        ...
			        list = LookupCache(cacheKey, ref topCount);
			        if (list == null)
			        {
                        delegateCache.EmitRows();
                        StoreCached(cacheKey, list);
			        }
	    ...
	    protected static List<object> ToList(IEnumerable src)
	    {
            ...
            PXDelegateResult delegateResult = src as PXDelegateResult;
