...
namespace PX.Objects.FS
{
        ...
        where TPostLine : class, PX.Data.IBqlTable, IPostLine, new()
    {
        ...
        protected virtual Job CreatePostingBatchAndInvoicesJob(Guid currentProcessID, int billingCycleID, DateTime? upToDate, DateTime? invoiceDate, string invoiceFinPeriodID, string postTo, List<FSPostDoc> invoiceList, List<TPostLine> postLineRows, ExecutionContext executionContext, PXQuickProcess.ActionFlow quickProcessFlow)
        {
            ...
            step = new Step(PXMessages.LocalizeFormat(TX.Messages.CREATE_FSPOSTBATCH, billingCycleID), StepProcessingType.WaitStepCompletionBeforeNextStep, job);
            ...
            foreach (FSPostDoc invoiceItem in invoiceList)
            {
                ...
                step = new Step(PXMessages.LocalizeFormat(TX.Messages.CREATE_INVOICE_BILLING_CYCLE, billingCycleID, invoiceItem.GroupKey), StepProcessingType.Independent, job);
                job.StepList.Add(step);
            ...
            step = new Step(PXMessages.LocalizeFormat(TX.Messages.APPLY_PREPAYMENT_BILLING_CYCLE, billingCycleID), StepProcessingType.WaitCompletionOfAllPreviousStepsBeforeRun, job);
            job.StepList.Add(step);
            ...
            step = new Step(PXMessages.LocalizeFormat(TX.Messages.COMPLETE_FSPOSTBATCH_BILLING_CYCLE, billingCycleID), StepProcessingType.WaitCompletionOfAllPreviousStepsBeforeRun, job);
            job.StepList.Add(step);
        ...
        public static void CreatePostingBatch(MethodParm parm)
        {
            ...
            postBatchShared.FSPostBatchRow = postBatchShared.PostBatchEntryGraph.CreatePostingBatch(iparm.BillingCycleID, iparm.UpToDate, iparm.InvoiceDate, iparm.InvoiceFinPeriodID, iparm.Module);
        }
        ...
        public virtual void CreateInvoiceDocument(MethodParm parm)
        {
            ...
            processShared.Initialize(iparm.Module, iparm.BillingBy);
            ...
            if (iparm.Exception == null)
            {
                using (var ts = new PXTransactionScope())
                {
                    UpdatePostInfoAndPostDet(docLinesGrouped, postBatchShared.FSPostBatchRow, processShared.PostInfoEntryGraph, processShared.CacheFSPostDet);
                    ts.Complete();
                }
        ...
        public static void SetGroupKeyErrorInfoInLines(InvoicingParm iparm)
            ...
            foreach (TPostLine postLineRow in iparm.PostLineRows.Where(x => x.GroupKey == iparm.GroupKey &&
                ...
                if (errorInfo != null)
                {
                    ...
                    errorMsgBuilder.Append(PXMessages.LocalizeFormat(
                    ...
                    errorMsgBuilder.Append(Environment.NewLine + errorInfo.ErrorMessage);
                }
        ...
        protected virtual void CreatePostingBatchesForBillingCycle(Guid currentProcessID, int billingCycleID, CreateInvoiceFilter filter, List<TPostLine> postLineRows, JobExecutor<InvoicingProcessStepGroupShared> jobExecutor, PXQuickProcess.ActionFlow quickProcessFlow)
        {
            ...
            else if (filter.PostTo == ID.Batch_PostTo.SO && PXAccess.FeatureInstalled<FeaturesSet.distributionModule>())
            {
                ...
                Job job = CreatePostingBatchAndInvoicesJob(currentProcessID, billingCycleID, filter.UpToDate, filter.InvoiceDate, filter.InvoiceFinPeriodID, ID.Batch_PostTo.SO, soInvoiceList, postLineRows, jobExecutor.MainContext, quickProcessFlow);
                jobExecutor.JobList.Add(job);
            }
        }
        ...
        {
            IEnumerable<IGrouping<int?, DocLineExt>> OrderList = docLines.GroupBy(x => x.fsServiceOrder.SOID);
        ...
        */
        ...
        public class InvoicingParm : MethodParm
        {
            ...
            public InvoicingParm(Step myStep, ExecutionContext executionContext, string module, Guid currentProcessID, int billingCycleID, string groupKey, List<TPostLine> postLineRows, string billingBy)
    ...
    public class InvoicingProcessStepGroupShared : StepGroup.StepGroupShared
    {
        ...
        public virtual void Initialize(string module, string billingBy)
        {
            ...
            {
                InvoiceGraph = InvoicingFunctions.CreateInvoiceGraph(module);
            }
            else
            {
                InvoiceGraph.Clear();
