...
namespace PX.Objects.FS
{
    public class ServiceOrderEntry : PXGraph<ServiceOrderEntry, FSServiceOrder>, IGraphWithInitialization
    {
        ...
        public virtual FSServiceOrder CalculateExternalTax(FSServiceOrder fsAppointmentRow)
        {
        ...
        #endregion
        #region InventoryItem_ActionType
        ...
        [PXHidden]
        public PXSetup<BAccount,
        Where<
            BAccount.bAccountID, Equal<Current<FSServiceOrder.customerID>>>> BAccount;
        ...
        public PXSetup<SOOrderType, Where<SOOrderType.orderType, Equal<Current<FSSrvOrdType.allocationOrderType>>>> AllocationSOOrderTypeSelected;
            ...
            if (_licenseLimits != null)
            {
                OnBeforeCommit += _licenseLimits.GetCheckerDelegate<FSServiceOrder>(
                    new TableQuery(TransactionTypes.LinesPerMasterRecord, typeof(FSSODet), (graph) =>
                    {
                        ...
                        {
                                new PXDataFieldValue<FSSODet.srvOrdType>(((ServiceOrderEntry)graph).ServiceOrderRecords.Current?.SrvOrdType),
        ...
        public PXSelect<FSServiceOrderTax,
                Where<
                    ...
                    And<FSServiceOrderTax.lineNbr, NotEqual<intMax>>>,
                OrderBy<
        ...
        public PXSelectJoin<FSServiceOrderTaxTran,
                ...
                OrderBy<
                    Asc<FSServiceOrderTaxTran.entityID,
        ...
        [PXUIField(DisplayName = "Open Appointment Screen", MapEnableRights = PXCacheRights.Select, MapViewRights = PXCacheRights.Select)]
        ...
        public virtual void OpenAppointmentScreen()
        {
                    ...
                    && !SharedFunctions.isThisAProspect(this, ServiceOrderRecords.Current.CustomerID))
            {
                ...
                try
                {
                    ...
                    graphAppointmentEntry.AppointmentRecords.Cache.SetValueExt<FSAppointment.soRefNbr>(graphAppointmentEntry.AppointmentRecords.Current, ServiceOrderRecords.Current.RefNbr);
        ...
        [PXUIField(DisplayName = "Create Purchase Order", MapEnableRights = PXCacheRights.Select, MapViewRights = PXCacheRights.Select)]
        [PXButton]
        public virtual void CreatePurchaseOrder()
        {
            ...
            graphPOCreate.Filter.Current.SrvOrdType = fsServiceORderRow.SrvOrdType;
            graphPOCreate.Filter.Current.ServiceOrderRefNbr = fsServiceORderRow.RefNbr;
            ...
            using (var ts = new PXTransactionScope())
            {
                ...
                var resultSet = PXSelectJoin<FSSODet,
                            ...
                            OrderBy<
                                ...
                                Asc<FSSODet.sOID,
                                Asc<FSSODet.sODetID>>>>>
                            .Select(dummyGraph, CurrentServiceOrder.Current.SOID);
        ...
        {
            return ServiceOrderCore.ViewDirectionOnMap_Handler(this, adapter, ServiceOrderRecords.Current);
        }
        #endregion
        ...
        {
            if (ServiceOrderRecords.Current != null && ServiceOrderRecords.Current.AddressValidated == false)
            {
                if (!string.IsNullOrEmpty(ServiceOrderRecords.Current.CountryID)
                        && !string.IsNullOrEmpty(ServiceOrderRecords.Current.City)
                            && !string.IsNullOrEmpty(ServiceOrderRecords.Current.State)
                                && !string.IsNullOrEmpty(ServiceOrderRecords.Current.PostalCode))
                {
                    string addressString = ServiceOrderCore.GetAddress(
                                                                    ServiceOrderRecords.Current.AddressLine1,
                                                                    ServiceOrderRecords.Current.AddressLine2,
                                                                    ServiceOrderRecords.Current.City,
                                                                    ServiceOrderRecords.Current.State,
                                                                    ServiceOrderRecords.Current.CountryID,
                                                                    ServiceOrderRecords.Current.PostalCode);

                    if (Geocoder.GetStatus(addressString, SetupRecord.Current.MapApiKey).CompareTo(System.Net.HttpStatusCode.OK.ToString()) == 0)
                    {
                        ServiceOrderRecords.Current.AddressValidated = true;
                        ServiceOrderRecords.Cache.Update(ServiceOrderRecords.Current);
                    }
                }
                else
                {
        ...
        {
            FSPostDet fsPostDetRow = ServiceOrderPostedIn.Current;
        ...
        private void EnableDisable_SODetLine(PXGraph graph, PXCache cache, FSSODet fsSODetRow, FSSrvOrdType fsSrvOrdTypeRow, FSServiceOrder fsServiceOrderRow)
        {
            ...
            if (fsSODetRow.IsPrepaid == true)
            {
                ...
                enableSiteID = false;
                enableSiteLocationID = false;
                enableAcctID = false;
                enableSubID = false;
                enableTranDesc = false;
                requireTranDesc = false;
                enableEnablePO = false;
            }
            else
            {
                switch (fsSODetRow.LineType)
                {
                    ...
                    case ID.LineType_ServiceTemplate.NONSTOCKITEM:
                        ...
                        enableSiteID = true;
                        enableSiteLocationID = false;
                        enableEnablePO = fsSODetRow.InventoryID != null;
            ...
            PXUIFieldAttribute.SetEnabled<FSSODet.siteID>(cache, fsSODetRow, enableSiteID);
            PXUIFieldAttribute.SetEnabled<FSSODet.siteLocationID>(cache, fsSODetRow, enableSiteLocationID);
            ...
            PXUIFieldAttribute.SetEnabled<FSSODet.enablePO>(cache, fsSODetRow, enableEnablePO);
            PXUIFieldAttribute.SetEnabled<FSSODet.poVendorID>(cache, fsSODetRow, enableEnablePO && fsSODetRow.EnablePO == true);
            PXUIFieldAttribute.SetEnabled<FSSODet.poVendorLocationID>(cache, fsSODetRow, enableEnablePO && fsSODetRow.EnablePO == true);
        ...
        private void HideRooms(FSServiceOrder fsServiceOrderRow)
        {
            ...
            openRoomBoard.SetEnabled(isRoomManagementActive);
        }
        ...
        private bool isTheLineValid(PXCache cache, FSSODet fsSODetRow, PXErrorLevel errorLevel = PXErrorLevel.Error)
        {
            ...
            ///////////////////TX.Error.SINGLE_SERVICE_RESTRICTION
            if (fsSODetRow.LineType == ID.LineType_ServiceTemplate.SERVICE)
            {
                CheckSingleServiceBySrvOrdType(fsSODetRow.SOID);
            }
        ...
        private void ClearSalesOrder(FSServiceOrder fsServiceOrderRow)
        {
            ...
            graphSOOrder.Document.Current = graphSOOrder.Document.Search<SOOrder.orderNbr>(fsServiceOrderRow.SourceRefNbr, fsServiceOrderRow.SourceType);
        ...
        /// <summary>
        /// Checks if the SingleService option is checked by the <c>SrvOrdType</c> selected 
        /// and raise an exception when there are more than one service line to be saved.
        /// </summary>
        /// <param name="sOID">Service Order ID.</param>
        private void CheckSingleServiceBySrvOrdType(int? sOID)
        {
            bool? singleService = false;
            bool throwSingleServiceException = false;
            int countServices = 0;

            if (ServiceOrderTypeSelected.Current != null)
            {
                singleService = ServiceOrderTypeSelected.Current.SingleService;
                ...
                if (singleService == true)
                {
                    if (sOID != null && sOID > 0)
                    {
                        ...
                        {
                            if (IsAServiceLine(fsSODetServiceRow) && fsSODetServiceRow.Status != ID.Status_ServiceOrder.CANCELED)
                            {
                                countServices++;
                            }
                        }

                        throwSingleServiceException = countServices > 1;
                    }
                    else
                    {
                        //Case when ServiceOrder is not already created
                        //It is needed to check the Cache.Inserted values

                        //Inserted
                        IEnumerable detServs = ServiceOrderDetServices.Cache.Inserted;
                        IEnumerator detServsEnumerator = detServs.GetEnumerator();

                        while (detServsEnumerator.MoveNext())
                        {
                            FSSODetService fsSODetServiceRow = (FSSODetService)detServsEnumerator.Current;
            ...
            // IF this ServiceOrder's ServiceOrderType allows just a single service
            // AND The user wants to create multiple service lines
            // THEN throw an exception
            if (throwSingleServiceException)
            {
                throw new PXException(TX.Error.SINGLE_SERVICE_RESTRICTION);
            }
        }

        /// <summary>
        /// Check if the given Line is a Service. 
        /// </summary>
        /// <param name="fsSODetServiceRow">Line to be checked.</param>
        /// <returns>True if the Line is a Service.</returns>
        private bool IsAServiceLine(FSSODetService fsSODetServiceRow)
        {
            return fsSODetServiceRow.LineType == ID.LineType_ServiceTemplate.SERVICE;
        }
        ...
        protected virtual void FSServiceOrder_Hold_FieldVerifying(PXCache cache, PXFieldVerifyingEventArgs e)
        {
            ...
            {
                newStatus = ServiceOrderTypeSelected.Current.Behavior == ID.Behavior_SrvOrderType.QUOTE ? ID.Status_ServiceOrder.QUOTE : ID.Status_ServiceOrder.OPEN;
            }
        ...
        protected virtual void FSServiceOrder_RowInserted(PXCache cache, PXRowInsertedEventArgs e)
        {
            ...
            if (fsServiceOrderRow.SOID < 0)
            {
                ...
                ServiceOrderCore.SetDefaultAddressAndContactInfo(this, fsServiceOrderRow, ServiceOrderTypeSelected.Current);
            }
        }
        ...
        protected virtual void FSServiceOrder_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
        {
            ...
            createPrepayment.SetEnabled(fsServiceOrderRow.IsPrepaymentEnable == true && cache.GetStatus(e.Row) != PXEntryStatus.Inserted);
        ...
        protected virtual void FSSODetService_RowDeleting(PXCache cache, PXRowDeletingEventArgs e)
        {
            ...
            {
                throw new PXException(PXMessages.LocalizeFormat(TX.Error.FSSODET_LINKED_TO_APPOINTMENTS, "Service"), PXErrorLevel.Error);
            }
        ...
        {
            ServiceOrderAppointmentHandlers.X_SiteID_FieldUpdated<FSSODetService.curyUnitPrice>(cache, e);
            cache.SetDefaultExt<FSSODetService.curyUnitCost>(e.Row);
        }

        protected virtual void FSSODetPart_SiteID_FieldUpdated(PXCache cache, PXFieldUpdatedEventArgs e)
        {
            ServiceOrderAppointmentHandlers.X_SiteID_FieldUpdated<FSSODetPart.curyUnitPrice>(cache, e);
            cache.SetDefaultExt<FSSODetPart.curyUnitCost>(e.Row);
        }
        ...
        protected virtual void FSSODetPart_RowDeleting(PXCache cache, PXRowDeletingEventArgs e)
        {
            ...
            {
                throw new PXException(PXMessages.LocalizeFormat(TX.Error.FSSODET_LINKED_TO_APPOINTMENTS, "Inventory Item"), PXErrorLevel.Error);
            }
        ...
        {
            if (CurrentServiceOrder.Current != null)
            {
                StaffSelectorFilter.Current.PostalCode = CurrentServiceOrder.Current.PostalCode;
        ...
        public class ServiceOrderQuickProcess : PXGraphExtension<ServiceOrderEntry>
        {
            ...
            private void SetQuickProcessSettingsVisibility(PXCache cache, FSSrvOrdType fsSrvOrdTypeRow, FSServiceOrder fsServiceOrderRow, FSSrvOrdQuickProcessParams fsQuickProcessParametersRow)
            {
                if (fsSrvOrdTypeRow != null)
                {
                    ...
                    PXUIFieldAttribute.SetVisible<FSSrvOrdQuickProcessParams.releaseInvoice>(cache, fsQuickProcessParametersRow, postToSO && isInvoiceBehavior && fsQuickProcessParametersRow.SOQuickProcess == false);
                    PXUIFieldAttribute.SetVisible<FSSrvOrdQuickProcessParams.emailInvoice>(cache, fsQuickProcessParametersRow, postToSO && isInvoiceBehavior && fsQuickProcessParametersRow.SOQuickProcess == false);
            ...
            {
                fsSrvOrdQuickProcessParamsRowTarget.PrepareInvoice = FSQuickProcessParametersRowSource.GenerateInvoiceFromServiceOrder.Value ? FSQuickProcessParametersRowSource.PrepareInvoice : false;
        ...
        public class SalesTax : TaxGraph<ServiceOrderEntry, FSServiceOrder>
        {
            ...
            protected virtual void FSServiceOrderTaxTran_RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
            {
                ...
                if (e.Operation == PXDBOperation.Delete)
                {
                    FSServiceOrderTax tax = (FSServiceOrderTax)Base.TaxLines.Cache.Locate((FSServiceOrderTax)row);
