...
namespace PX.Objects.FS
{
    public class SM_SOOrderEntry : PXGraphExtension<SOOrderEntry>, IInvoiceGraph, IInvoiceContractGraph
    {
        ...
        [PXButton]
        [PXUIField(DisplayName = "Schedule", MapEnableRights = PXCacheRights.Select, MapViewRights = PXCacheRights.Select, Visible = false)]
                                            ...
                                            PXDBOperation operation)
        {
            ...
            {
                if (fsServiceOrderRow.CountryID != soShippingAddressRow.CountryID)
                {
                    graphServiceOrder.ServiceOrderRecords.SetValueExt<FSServiceOrder.countryID>(fsServiceOrderRow, soShippingAddressRow.CountryID);
                    updateRequired = true;
                }

                if (fsServiceOrderRow.State != soShippingAddressRow.State)
                {
                    graphServiceOrder.ServiceOrderRecords.SetValueExt<FSServiceOrder.state>(fsServiceOrderRow, soShippingAddressRow.State);
                    updateRequired = true;
                }

                if (fsServiceOrderRow.City != soShippingAddressRow.City)
                {
                    graphServiceOrder.ServiceOrderRecords.SetValueExt<FSServiceOrder.city>(fsServiceOrderRow, soShippingAddressRow.City);
                    updateRequired = true;
                }

                if (fsServiceOrderRow.PostalCode != soShippingAddressRow.PostalCode)
                {
                    graphServiceOrder.ServiceOrderRecords.SetValueExt<FSServiceOrder.postalCode>(fsServiceOrderRow, soShippingAddressRow.PostalCode);
                    updateRequired = true;
                }

                if (fsServiceOrderRow.AddressLine1 != soShippingAddressRow.AddressLine1)
                {
                    graphServiceOrder.ServiceOrderRecords.SetValueExt<FSServiceOrder.addressLine1>(fsServiceOrderRow, soShippingAddressRow.AddressLine1);
                    updateRequired = true;
                }

                if (fsServiceOrderRow.AddressLine2 != soShippingAddressRow.AddressLine2)
                {
                    graphServiceOrder.ServiceOrderRecords.SetValueExt<FSServiceOrder.addressLine2>(fsServiceOrderRow, soShippingAddressRow.AddressLine2);
                    updateRequired = true;
                }

                if (fsServiceOrderRow.AddressLine3 != soShippingAddressRow.AddressLine3)
                {
                    graphServiceOrder.ServiceOrderRecords.SetValueExt<FSServiceOrder.addressLine3>(fsServiceOrderRow, soShippingAddressRow.AddressLine3);
                    updateRequired = true;
                }

                if (fsServiceOrderRow.AddressValidated != soShippingAddressRow.IsValidated)
                {
                    graphServiceOrder.ServiceOrderRecords.SetValueExt<FSServiceOrder.addressValidated>
                                    (fsServiceOrderRow, soShippingAddressRow.IsValidated);
                    updateRequired = true;
                }
            }
            ...
            {
                if (fsServiceOrderRow.Attention != soShippingContactRow.Salutation)
                {
                    graphServiceOrder.ServiceOrderRecords.SetValueExt<FSServiceOrder.attention>(fsServiceOrderRow, soShippingContactRow.Salutation);
                    updateRequired = true;
                }

                if (fsServiceOrderRow.Phone1 != soShippingContactRow.Phone1)
                {
                    graphServiceOrder.ServiceOrderRecords.SetValueExt<FSServiceOrder.phone1>(fsServiceOrderRow, soShippingContactRow.Phone1);
                    updateRequired = true;
                }

                if (fsServiceOrderRow.Phone2 != soShippingContactRow.Phone2)
                {
                    graphServiceOrder.ServiceOrderRecords.SetValueExt<FSServiceOrder.phone2>(fsServiceOrderRow, soShippingContactRow.Phone2);
                    updateRequired = true;
                }

                if (fsServiceOrderRow.Phone3 != soShippingContactRow.Phone3)
                {
                    graphServiceOrder.ServiceOrderRecords.SetValueExt<FSServiceOrder.phone3>(fsServiceOrderRow, soShippingContactRow.Phone3);
                    updateRequired = true;
                }

                if (fsServiceOrderRow.Fax != soShippingContactRow.Fax)
                {
                    graphServiceOrder.ServiceOrderRecords.SetValueExt<FSServiceOrder.fax>(fsServiceOrderRow, soShippingContactRow.Fax);
                    updateRequired = true;
                }

                if (fsServiceOrderRow.EMail != soShippingContactRow.Email)
                {
                    graphServiceOrder.ServiceOrderRecords.SetValueExt<FSServiceOrder.eMail>(fsServiceOrderRow, soShippingContactRow.Email);
                    updateRequired = true;
                }
            }
        ...
        private void EnableDisableSOline(PXCache cache, SOLine soLineRow, bool fsIntegrationEnabled)
        {
            ...
            PXUIFieldAttribute.SetEnabled<FSxSOLine.sDSelected>(cache, soLineRow, (bool)fsxSOOrderRow.SDEnabled && fsxSOOrderRow.SORefNbr != null && fsIntegrationEnabled);
        ...
        /// </summary>
        protected virtual void EnableDisable_OpenAppointmentBoard(PXCache cache, SOOrder soOrderRow, FSxSOOrder fsxSOOrderRow)
        {
            bool enabled = false;

            if (soOrderRow != null && cache.GetStatus(soOrderRow) != PXEntryStatus.Inserted
                                && fsxSOOrderRow != null && fsxSOOrderRow.SORefNbr != null)
            {
                enabled = true;
            }

            OpenAppointmentBoard.SetEnabled(enabled);
        }

        /// <summary>
        /// Enables/Disables additional Sales Orders fields.
        /// </summary>
        protected virtual void EnableDisable_SDFields(PXCache cache, object row)
        {
            FSxSOOrder fsxSOOrderRow = cache.GetExtension<FSxSOOrder>(row);

            bool enabled = fsxSOOrderRow.SDEnabled == true;

            PXUIFieldAttribute.SetEnabled<FSxSOOrder.assignedEmpID>(cache, row, enabled);
            PXUIFieldAttribute.SetEnabled<FSxSOOrder.installed>(cache, row, enabled);
            PXUIFieldAttribute.SetEnabled<FSxSOOrder.sLAETA>(cache, row, enabled);

            // SORefNbr depends on SrvOrdType
            if (fsxSOOrderRow.SORefNbr != null)
            {
                enabled = false;
            }

            PXUIFieldAttribute.SetEnabled<FSxSOOrder.srvOrdType>(cache, row, enabled);
            PXDefaultAttribute.SetPersistingCheck<FSxSOOrder.srvOrdType>(cache, row, enabled ? PXPersistingCheck.NullOrBlank : PXPersistingCheck.Nothing);
        }
        ...
        protected virtual void EnableDisable_All(PXCache cache, SOOrder soOrderRow, bool fsIntegrationEnabled)
        {
            ...
            PXUIFieldAttribute.SetEnabled<FSxSOOrder.sDEnabled>(
                                                                cache, 
                                                                null, 
                                                                allowCreateServiceOrder && fsIntegrationEnabled);

            EnableDisable_SDFields(cache, soOrderRow);
            EnableDisable_OpenAppointmentBoard(cache, soOrderRow, fsxSOOrderRow);
                        ...
                        && Base.Accessinfo.ScreenID != SharedFunctions.SetScreenIDToDotFormat(ID.ScreenID.INVOICE_BY_SERVICE_ORDER)))
                {
                    FSCreatedDoc fsCreatedDocRow = PXSelectJoin<FSCreatedDoc,
                                                    ...
                                                    Where<
                                                        ...
                                                        And<FSPostBatch.postTo, Equal<FSPostBatch.postTo.SO>,
                                                 ...
                                                 .Select(Base, soOrderRow.RefNbr, soOrderRow.OrderType);
        ...
        protected virtual void SOLine_InventoryID_FieldUpdated(PXCache cache, PXFieldUpdatedEventArgs e)
        {
            ...
            if (soLine != null && soLine.CreatedByScreenID == ID.ScreenID.SALES_ORDER)
            {
                SharedFunctions.ShowServiceOrderNote(PXDBOperation.Insert, Base.Document.View, soLine.InventoryID);
            }
        ...
        public virtual void CreateInvoice(PXGraph graphProcess, List<DocLineExt> docLines, List<DocLineExt> docLinesGrouped, short invtMult, DateTime? invoiceDate, string invoiceFinPeriodID, OnDocumentHeaderInsertedDelegate onDocumentHeaderInserted, OnTransactionInsertedDelegate onTransactionInserted, PXQuickProcess.ActionFlow quickProcessFlow)
        {
            ...
            FSxSOLine fSxSOLineRow = null;
            List<SharedClasses.SOLineEquipmentComponent> componentList = new List<SharedClasses.SOLineEquipmentComponent>();
            ...
            foreach (GroupDocLineExt singleLine in singleLines)
            {
                ...
                if (PXAccess.FeatureInstalled<FeaturesSet.equipmentManagementModule>())
                {
                    if (docLine.EquipmentAction != null)
                    {
                        ...
                        {
                            componentList.Add(new SharedClasses.SOLineEquipmentComponent(docLine, sOLineRow, fSxSOLineRow));
                        }
                        else
                        {
            ...
            { 
                //Assigning the NewTargetEquipmentLineNbr field value for the component type records
                foreach (SharedClasses.SOLineEquipmentComponent currLineModel in componentList.Where(x => x.equipmentAction == ID.Equipment_Action.SELLING_TARGET_EQUIPMENT))
                {
                    foreach (SharedClasses.SOLineEquipmentComponent currLineComponent in componentList.Where(x => (x.equipmentAction == ID.Equipment_Action.CREATING_COMPONENT
        ...
        public virtual FSContractPostDoc CreateInvoiceByContract(PXGraph graphProcess, DateTime? invoiceDate, string invoiceFinPeriodID, FSContractPostBatch fsContractPostBatchRow, FSServiceContract fsServiceContractRow, FSContractPeriod fsContractPeriodRow, List<ContractInvoiceLine> docLines)
        {
            ...
            foreach (ContractInvoiceLine docLine in docLines)
            {
                ...
                Base.Transactions.SetValueExt<SOLine.siteID>(sOLineRow, docLine.SiteID ?? fSBranchLocationRow.DfltSiteID);
                ...
                fSxSOLineRow.Source = ID.Billing_By.CONTRACT;
