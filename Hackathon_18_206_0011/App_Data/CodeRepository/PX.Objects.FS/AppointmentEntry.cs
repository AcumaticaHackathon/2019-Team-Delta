...
{
    public class AppointmentEntry : AppointmentEntryBase, IGraphWithInitialization
    {
        ...
        #region SrvOrdType
        [PXDBString(4, IsKey = false, IsFixed = true)]
        [PXDefault]
        ...
        [PXDBBool]
        [PXUIField(DisplayName = "Mark for PO", Visible = false)]
        ...
        [PXDBString]
        [PXUIField(DisplayName = "PO Nbr.", Enabled = false, Visible = false)]
        [PO.PO.RefNbr(typeof(
        ...
        [PXDBString]
        [PXUIField(DisplayName = "PO Status", Enabled = false, Visible = false)]
        ...
        #region InventoryItem CacheAttached
        #region FSxService_ActionType
        [PXDBString(1, IsFixed = true)]
        [PXDefault(ID.Service_Action_Type.NO_ITEMS_RELATED)]
        [PXUIField(DisplayName = "Pickup/Deliver Items", Enabled = true)]
        protected virtual void InventoryItem_ActionType_CacheAttached(PXCache sender)
        {
        }
        #endregion
        #region InventoryItem_PurchaseUnit
        //Needed to remove the PurchaseUnit field from the StockItem selector
        ...
        [PXButton]
        public virtual IEnumerable ViewDirectionOnMap(PXAdapter adapter)
        {
            if (AppointmentSelected.Current != null && AppointmentSelected.Current.SOID != null)
            {
                ...
                if (map != null)
                {
                    ...
                    map.ShowAddress(fsServiceOrderRow.CountryID, fsServiceOrderRow.State, fsServiceOrderRow.City, fsServiceOrderRow.PostalCode, fsServiceOrderRow.AddressLine1, fsServiceOrderRow.AddressLine2, fsServiceOrderRow.AddressLine3);
                }
            }
        ...
        [PXUIField(DisplayName = "Validate Address", MapEnableRights = PXCacheRights.Select, MapViewRights = PXCacheRights.Select)]
        [PXButton]
        public virtual IEnumerable ValidateAddress(PXAdapter adapter)
        {
            if (ServiceOrderRelated.Current != null && ServiceOrderRelated.Current.AddressValidated == false)
            {
                if (ServiceOrderRelated.Current.CountryID != null 
                        && ServiceOrderRelated.Current.City != null
                            && ServiceOrderRelated.Current.State != null 
                                && ServiceOrderRelated.Current.PostalCode != null)
                {
                    string addressString = ServiceOrderCore.GetAddress(
                                                                    ServiceOrderRelated.Current.AddressLine1, 
                                                                    ServiceOrderRelated.Current.AddressLine2, 
                                                                    ServiceOrderRelated.Current.City, 
                                                                    ServiceOrderRelated.Current.State, 
                                                                    ServiceOrderRelated.Current.CountryID, 
                                                                    ServiceOrderRelated.Current.PostalCode);

                    if (Geocoder.GetStatus(addressString, SetupRecord.Current.MapApiKey).CompareTo(System.Net.HttpStatusCode.OK.ToString()) == 0)
                    {
                        ServiceOrderRelated.Current.AddressValidated = true;
                        ServiceOrderRelated.Cache.Update(ServiceOrderRelated.Current);
                    }
                    else
                    {
            ...
            where AppointmentDetType : FSAppointmentDet, new()
        {
            foreach (AppointmentDetType fsAppointmentDetRow in viewAppointmentDet.Select())
            {
                if (fsAppointmentDetRow.SODetID != null)
                {
                    ...
                    if (fsSODetRow == null)
                    {
                        throw new PXException(PXMessages.LocalizeFormat(TX.Error.RECORD_NOT_FOUND, TX.CustomTextFields.SERVICE_ORDER_DETAIL), PXErrorLevel.Error);
                    }
        ...
        public virtual void UncloseAppointment()
        {
            ...
            {
                string dialogMessage = TX.Messages.ASK_CONFIRM_APPOINTMENT_UNCLOSING;

                if (ServiceOrderTypeSelected.Current.SingleAppointment == true)
                {
                    dialogMessage = TX.Messages.ASK_CONFIRM_APP_SO_UNCLOSING;
                }
                
                if (AutomaticallyUncloseAppointment || WebDialogResult.Yes == AppointmentRecords.Ask(
        ...
        {
            FSPostDet fsPostDetRow = AppointmentPostedIn.Current;
        ...
        public virtual IEnumerable StartService(PXAdapter adapter)
        {
            ...
            foreach (FSAppointment fsAppointmentRow in adapter.Get())
            {
                if (AppointmentRecords.Cache.GetStatus(fsAppointmentRow) != PXEntryStatus.Inserted)
                {
                    ...
                    if (fsAppointmentDetRow != null && fsAppointmentDetRow.LineType == ID.LineType_All.SERVICE)
                    {
                        ...
                        SharedFunctions.SaveUpdatedChanges(AppointmentDetServices.Cache, fsAppointmentDetRow);
                    }
                }

                yield return fsAppointmentRow;
            }
        }
        #endregion
            ...
            foreach (FSAppointment fsAppointmentRow in adapter.Get())
            {
                if (AppointmentRecords.Cache.GetStatus(fsAppointmentRow) != PXEntryStatus.Inserted)
                {
                    ...
                    if (fsAppointmentEmployeeRow != null)
                    {
                        ...
                        SharedFunctions.SaveUpdatedChanges(AppointmentEmployees.Cache, fsAppointmentEmployeeRow);
                    }
                }
        ...
        public PXSetup<FSServiceContract,
                    Where<
        ...
        public PXSetup<FSContractPeriod,
                    Where<
            ...
            if (_licenseLimits != null)
            {
                OnBeforeCommit += _licenseLimits.GetCheckerDelegate<FSAppointment>(
                    new TableQuery(TransactionTypes.LinesPerMasterRecord, typeof(FSAppointmentDet), (graph) =>
                    {
                        ...
                        {
                                new PXDataFieldValue<FSAppointmentDet.srvOrdType>(((AppointmentEntry)graph).AppointmentRecords.Current?.SrvOrdType),
        ...
        public List<FSProfitability> Delegate_ProfitabilityRecords(PXCache cache, FSAppointment fsAppointmentRow)
        {
            ...
            foreach (FSAppointmentDetPart fsAppDetPartRow in AppointmentDetParts.Select().Where(x => ((FSAppointmentDetPart)x).LineType == ID.LineType_All.INVENTORY_ITEM && ((FSAppointmentDetPart)x).Status != ID.Status_AppointmentDet.CANCELED))
            {
                FSProfitability fsProfitabilityRow = new FSProfitability(fsAppDetPartRow);
                returnList.Add(fsProfitabilityRow);
            }

            foreach (FSAppointmentEmployee fsAppEmployee in AppointmentEmployees.Select().Where(x => ((FSAppointmentEmployee)x).CuryUnitCost != 0m))
            {
            ...
            foreach (FSAppointmentDetService fsAppDetServiceRow in AppointmentDetServices.Select().Where(x => ((FSAppointmentDetService)x).LineType == ID.LineType_All.NONSTOCKITEM && ((FSAppointmentDetService)x).Status != ID.Status_AppointmentDet.CANCELED))
            {
        ...
        private void SingleAppointmentValidation()
        {
            FSServiceOrder fsServiceOrderRow = ServiceOrderRelated.Current;
            if (fsServiceOrderRow != null)
            {
                FSSrvOrdType fsSrvOrdTypeRow = ServiceOrderTypeSelected.Select();

                if (fsSrvOrdTypeRow != null && fsSrvOrdTypeRow.SingleAppointment == true)
                {
                    var fsAppointmentRow_tmp = PXSelect<FSAppointment,
                                               Where<
                                                   FSAppointment.sOID, Equal<Required<FSAppointment.sOID>>>>
                                               .SelectWindowed(this, 0, 2, fsServiceOrderRow.SOID);
                    if (fsAppointmentRow_tmp.Count > 1)
                    {
                        throw new PXException(TX.Error.SRVORD_SINGLE_APPOINTMENT);
                    }
                }
            }
        }
                                                    ...
                                                    FSServiceOrder fsServiceOrderRow, FSSrvOrdType fsSrvOrdTypeRow, FSBillingCycle fsBillingCycleRow)
        {
            ...
            if (fsServiceOrderRow != null)
            {
                bool enableValidateAddress = fsServiceOrderRow.CountryID != null
                                           && fsServiceOrderRow.City != null
                                               && fsServiceOrderRow.State != null
                                                   && fsServiceOrderRow.PostalCode != null
                                                       && fsServiceOrderRow.AddressValidated == false;
        ...
        private void SetServiceOrderStatusFromAppointment(FSServiceOrder fsServiceOrderRow, FSAppointment fsAppointmentRow)
        {
            ...
            if (ServiceOrderTypeSelected.Current.SingleAppointment == false)
            {
                lastAppointment = isTheLastestActiveAppointmentInServicesOrder(fsServiceOrderRow, fsAppointmentRow, fsAppointmentRow.Status == ID.Status_Appointment.CLOSED);
            }
                            ...
                            && fsAppointmentRow.HandleManuallyScheduleTime == false)
            {
                cache.RaiseExceptionHandling
                                <FSAppointment.scheduledDateTimeEnd>(
                                                                    ...
                                                                    fsAppointmentRow.ScheduledDateTimeEnd,
                                                                    new PXSetPropertyException(
                                                                        PXMessages.LocalizeFormat(TX.Warning.END_TIME_AUTOMATICALLY_CALCULATED_NOTIFICATION, TX.AppointmentTotalTimesLabels.ESTIMATED),
                                                                        PXErrorLevel.Warning));
            }
                            ...
                            && fsAppointmentRow.HandleManuallyActualTime == false)
            {
                cache.RaiseExceptionHandling
                                <FSAppointment.actualDateTimeEnd>(
                                                                ...
                                                                new PXSetPropertyException(
                                                                    PXMessages.LocalizeFormat(TX.Warning.END_TIME_AUTOMATICALLY_CALCULATED_NOTIFICATION, TX.AppointmentTotalTimesLabels.ACTUAL),
                                                                    PXErrorLevel.Warning));
            }
        }
            ...
            where fieldType : IBqlField
        {
            ...
            if (fsServiceOrderRow == null 
                    || string.IsNullOrEmpty(fsServiceOrderRow.PostalCode) == true)
            {
                return;
            }
            ...
            foreach (PXResult<FSGeoZoneEmp, FSGeoZonePostalCode> bqlResult in fsGeoZoneEmpSet)
            {
                ...
                if (Regex.Match(fsServiceOrderRow.PostalCode.Trim(), @fsGeoZonePostalCodeRow.PostalCode.Trim()).Success)
                {
        ...
        private bool IsSingleService()
        {
            FSSrvOrdType fsSrvOrdTypeRow = ServiceOrderTypeSelected.Select();
            if (fsSrvOrdTypeRow != null)
            {
                return fsSrvOrdTypeRow.SingleService == true;
            }
        ...
        private void ValidateWeekCode(FSAppointment fsAppointmentRow)
        {
            ...
            if (fsScheduleRow != null && fsScheduleRow.WeekCode != null)
            {
                ...
                {
                    throw new PXException(PXMessages.LocalizeFormat(TX.Error.WEEKCODE_NOT_MATCH_WITH_SCHEDULE, fsScheduleRow.RefNbr, scheduleTimeModified.Value.ToShortDateString()), PXErrorLevel.Error);
                }
            }
            ...
            if (bqlResult != null)
            {
                ...
                if (fsRouteRow != null && fsRouteRow.WeekCode != null)
                {
                    ...
                    {
                        throw new PXException(PXMessages.LocalizeFormat(TX.Error.WEEKCODE_NOT_MATCH_WITH_ROUTE_SCHEDULE, fsScheduleRow.RefNbr, scheduleTimeModified.Value.ToShortDateString()), PXErrorLevel.Error);
                    }
                }
            }
            ...
            if (fsAppointmentRow.RouteID != null)
            {
                ...
                if (fsRouteRow != null && fsRouteRow.WeekCode != null)
                {
                    if (!SharedFunctions.WeekCodeIsValid(fsRouteRow.WeekCode, scheduleTimeModified, this))
                    {
                        throw new PXException(PXMessages.LocalizeFormat(TX.Error.WEEKCODE_NOT_MATCH_WITH_ROUTE, fsScheduleRow.RefNbr, scheduleTimeModified.Value.ToShortDateString()), PXErrorLevel.Error);
                    }
                }
            }
        }

        private void ValidateAppWithSrvOrdType()
        {
            ...
            bool? appWithoutSrv  = ServiceOrderTypeSelected.Current.AppWithoutSrv;
            bool? appWithoutEmp  = ServiceOrderTypeSelected.Current.AppWithoutEmp;
            bool? appWithMultEmp = ServiceOrderTypeSelected.Current.AppWithMultEmp;

            //Checking worst case:

            //Checking if there is at least one service
            if (appWithoutSrv == false)
            {
                if (AppointmentDetServices.SelectWindowed(0, 1).Count == 0)
                {
                    throw new PXException(TX.Error.APPOINTMENT_REQUIRES_SERVICE, PXErrorLevel.Error);
                }
            }

            //Checking if there is at least one employee
            if (appWithoutEmp == false)
            {
                if (AppointmentEmployees.SelectWindowed(0, 1).Count == 0)
                {
                    throw new PXException(TX.Error.APPOINTMENT_REQUIRES_EMPLOYEE, PXErrorLevel.Error);
                }
            }

            //Checking if there are more than one employee
            if (appWithMultEmp == false)
            {
                if (AppointmentEmployees.SelectWindowed(0, 2).Count > 1)
                {
                    throw new PXException(TX.Error.APPOINTMENT_DOES_NOT_ALLOW_MULTI_EMPLOYEES, PXErrorLevel.Error);
                }
            }
        }
        ...
        private FSRouteDocument GetOrGenerateRoute(int? routeID, int? routeDocumentID, DateTime? appointmentScheduledDate, int? branchID)
        {
            ...
            else
            {
                fsRouteDocumentRow = PXSelect<FSRouteDocument,
                                     ...
                                     OrderBy<Desc<FSRouteDocument.routeDocumentID>>>.Select(this, routeID, routeDocumentID, begin, end);
            }
                                            ...
                                            string apiKey)
        {
            ...
            nodeAddress = SharedFunctions.GetBranchLocationAddress(fsBranchLocationRow_Begin);
            ...
            foreach (PXResult<FSAppointment, FSServiceOrder> bqlResult in fsAppointmentSet)
            {
                routeAppointmentList.Add((FSAppointment)bqlResult);
                FSServiceOrder fsServiceOrderRow = (FSServiceOrder)bqlResult;

                nodeAddress = SharedFunctions.GetAppointmentAddress(fsServiceOrderRow);
            ...
            nodeAddress = SharedFunctions.GetBranchLocationAddress(fsBranchLocationRow_End);
        ...
        private void GetLocation(FSServiceOrder fsServiceOrderRow, FSSrvOrdType fsSrvOrdTypeRow)
        {
            if (fsSrvOrdTypeRow != null
                    && fsSrvOrdTypeRow.Behavior == ID.Behavior_SrvOrderType.ROUTE_APPOINTMENT)
            {
                ...
                GLocation[] results = Geocoder.Geocode(SharedFunctions.GetAppointmentAddress(fsServiceOrderRow), SetupRecord.Current.MapApiKey);
        ...
        /// <summary>
        /// Hides or shows Appointments, Pickup Delivery Items Tabs.
        /// </summary>
                                            ...
                                            PXTranStatus? tranStatus)
        {
            ...
            {
                throw new PXException(PXMessages.LocalizeFormat(TX.Error.RECORD_NOT_FOUND, TX.CustomTextFields.SERVICE_ORDER), PXErrorLevel.Error);
            }
            ...
            where AppointmentDetType : FSAppointmentDet, new()
        {
            ...
            {
                // When the line is not being inserted, only Status and BranchID are replicated to FSSODet.
            ...
            {
                if (viewSODet.Cache.GetStatus(fsSODetRow) == PXEntryStatus.Notchanged)
                {
                    viewSODet.Cache.SetStatus(fsSODetRow, PXEntryStatus.Updated);
                }
            }
        ...
        private void UpdateAppointmentDetService_StaffID(string serviceLineRef, string oldServiceLineRef)
        {
            ...
            {
                var empReferencingLineRef = AppointmentEmployees.Select()
                                                                .Where(y => ((FSAppointmentEmployee)y).ServiceLineRef == serviceLineRef);

                var fsAppointmentDetRow = AppointmentDetServices.Select()
            ...
            {
                var empReferencingOldLineRef = AppointmentEmployees.Select()
                                                                   .Where(y => ((FSAppointmentEmployee)y).ServiceLineRef == oldServiceLineRef);

                var serviceOldLineRef = AppointmentDetServices.Select()
            ...
            ref List<FSAppointmentEmployee> createReleatedTimeActivity)
        {
            foreach (FSAppointmentDetService fsAppointmentDetService in AppointmentDetServices.Cache.Updated)
            {
                ...
                switch (fsAppointmentDetService.Status)
                {
                    case ID.Status_AppointmentDet.CANCELED:
                    ...
                    default:
                        ...
                        if (oldServiceStatus == ID.Status_AppointmentDet.CANCELED)
                        {
            ...
            ref List<FSAppointmentEmployee> fsAppointmentDetEmployeeList)
        {
            var fsAppointmentEmployeeRows = AppointmentEmployees.Select()
                            .Where(y => ((FSAppointmentEmployee)y).ServiceLineRef == fsAppointmentDetService.LineRef);
        ...
        protected override void FSAppointment_RowInserted(PXCache cache, PXRowInsertedEventArgs e)
        {
            ...
            if (fsAppointmentRow.AppointmentID < 0)
            {
                ...
                ServiceOrderCore.SetDefaultAddressAndContactInfo(this, ServiceOrderRelated.Current, ServiceOrderTypeSelected.Current);
            }
        }
        ...
        protected override void FSAppointment_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
        {
            ...
            TreeWFStageHelper.EnableDisableActionsByWorkflowStage(
                                cache,
                                WFStageRelated?.Current,
                                completeAppointment,
                                cancelAppointment,
                                closeAppointment,
                                reopenAppointment);
            ...
            createPrepayment.SetEnabled(ServiceOrderRelated.Current != null && ServiceOrderRelated.Current.IsPrepaymentEnable == true && cache.GetStatus(e.Row) != PXEntryStatus.Inserted);
        ...
        protected virtual void FSAppointment_RowPersisting(PXCache cache, PXRowPersistingEventArgs e)
        {
            ...
            {
                if (fsAppointmentRow.SORefNbr != null)
                {
                    SingleAppointmentValidation();
                }
            ...
            if (e.TranStatus == PXTranStatus.Open)
            {
                if (updateContractPeriod == true && fsAppointmentRow.BillContractPeriodID != null)
                {
                    ...
                    if (currentStandardContract != null && currentStandardContract.RecordType == ID.RecordType_ServiceContract.SERVICE_CONTRACT)
                    {
                        ...
                        foreach (FSAppointmentDetService fsAppointmentDetServiceRow in AppointmentDetServices.Select().Where(x => ((FSAppointmentDetService)x).ContractRelated == true 
                                                                                                                                    && ((FSAppointmentDetService)x).Status != ID.Status_AppointmentDet.CANCELED))
                        {
                            fsContractPeriodDetRow = graphServiceContract.ContractPeriodDetRecords
                                                        .Search<FSContractPeriodDet.inventoryID,
                                                            ...
                                                            fsAppointmentDetServiceRow.BillingRule).FirstOrDefault();
        ...
        protected virtual void FSAppointmentEmployee_ServiceLineRef_FieldUpdated(PXCache cache, PXFieldUpdatedEventArgs e)
        {
            ...
            if (e.ExternalCall == true)
            {
                ...
                AppointmentDateTimeHelper.UpdateStaffActualDateTimeEnd(ServiceOrderTypeSelected.Current, AppointmentRecords.Current, AppointmentEmployees);
            }
        }
        ...
        protected virtual void FSAppointmentEmployee_RowPersisting(PXCache cache, PXRowPersistingEventArgs e)
        {
            ...
            if (AppointmentRecords.Current != null)
            {
                fsAppointmentEmployeeRow.RefNbr = fsAppointmentEmployeeRow.RefNbr == null
                                                || fsAppointmentEmployeeRow.RefNbr != AppointmentRecords.Current.RefNbr ? AppointmentRecords.Current.RefNbr : fsAppointmentEmployeeRow.RefNbr;
            }
        ...
        protected virtual void FSAppointmentResource_RowPersisting(PXCache cache, PXRowPersistingEventArgs e)
        {
            ...
            fsAppointmentResourceRow.RefNbr = fsAppointmentResourceRow.RefNbr == null
                                                || fsAppointmentResourceRow.RefNbr != AppointmentRecords.Current.RefNbr ? AppointmentRecords.Current.RefNbr : fsAppointmentResourceRow.RefNbr;
        }
        ...
        protected virtual void FSAppointmentDetService_ContractRelated_FieldDefaulting(PXCache cache, PXFieldDefaultingEventArgs e)
        {
            ...
            e.NewValue = duplicatedContractLine == false
                            && StandardContractPeriodDetail.Select().Where(x => ((FSContractPeriodDet)x).InventoryID == fsAppointmentDetServiceRow.InventoryID
        ...
        protected virtual void FSAppointmentDetService_CoveredQty_FieldDefaulting(PXCache cache, PXFieldDefaultingEventArgs e)
        {
            ...
            FSContractPeriodDet fsContractPeriodDetRow = (FSContractPeriodDet)StandardContractPeriodDetail.Select().Where(x => ((FSContractPeriodDet)x).InventoryID == fsAppointmentDetServiceRow.InventoryID
                                                                                                                                && (((FSContractPeriodDet)x).SMEquipmentID == fsAppointmentDetServiceRow.SMEquipmentID
                                                                                                                                        || ((FSContractPeriodDet)x).SMEquipmentID == fsAppointmentDetServiceRow.SMEquipmentID)
        ...
        {
            ServiceOrderAppointmentHandlers.X_SiteID_FieldUpdated<FSAppointmentDetService.curyUnitPrice>(cache, e);
        }

        protected virtual void FSAppointmentDetPart_SiteID_FieldUpdated(PXCache cache, PXFieldUpdatedEventArgs e)
        {
            ServiceOrderAppointmentHandlers.X_SiteID_FieldUpdated<FSAppointmentDetPart.curyUnitPrice>(cache, e);
            cache.SetDefaultExt<FSAppointmentDetPart.curyUnitCost>(e.Row);
        }

        protected virtual void FSAppointmentInventoryItem_SiteID_FieldUpdated(PXCache cache, PXFieldUpdatedEventArgs e)
        {
            ServiceOrderAppointmentHandlers.X_SiteID_FieldUpdated<FSAppointmentInventoryItem.curyUnitPrice>(cache, e);
        }
        ...
        protected virtual void FSAppointmentDetService_InventoryID_FieldUpdated(PXCache cache, PXFieldUpdatedEventArgs e)
        {
            ...
            ServiceOrderCore.ShowServiceNoteFromServiceOrder(
                                                            AppointmentDetServices.View,
                                                            fsAppointmentDetServiceRow.InventoryID,
                                                            fsServiceOrderRow,
                                                            e.ExternalCall, cache.Graph.IsCopyPasteContext);
        ...
        protected virtual void FSAppointmentDetService_Status_FieldUpdated(PXCache cache, PXFieldUpdatedEventArgs e)
        {
            ...
            foreach (FSAppointmentEmployee fsAppointmentEmployeeRow in AppointmentEmployees.Select().Where(y => ((FSAppointmentEmployee)y).ServiceLineRef == fsAppointmentDetRow.LineRef))
            {
        ...
        protected virtual void FSAppointmentDetService_StaffActualDuration_FieldUpdated(PXCache cache, PXFieldUpdatedEventArgs e)
        {
            ...
            else
            {
                ...
                {
                    FSAppointmentEmployee fsAppointmentEmployeeRow = AppointmentEmployees.Select()
                                                                                         .Where(_ => ((FSAppointmentEmployee)_).ServiceLineRef == fsAppointmentDetServiceRow.LineRef)
                                                                                         .First();
        ...
        protected virtual void FSAppointmentDetService_RowDeleting(PXCache cache, PXRowDeletingEventArgs e)
        {
            ...
            if (fsSODetRow != null)
            {
                foreach (FSAppointmentEmployee fsAppointmentEmployeeRow in AppointmentEmployees.Select().Where(y => ((FSAppointmentEmployee)y).ServiceLineRef == fsSODetRow.LineRef))
                {
                    AppointmentEmployees.Delete(fsAppointmentEmployeeRow);
                }
            }
        ...
        protected virtual void FSAppointmentAttendee_RowPersisting(PXCache cache, PXRowPersistingEventArgs e)
        {
            ...
            fsAppointmentAttendeeRow.RefNbr = fsAppointmentAttendeeRow.RefNbr == null
                                                || fsAppointmentAttendeeRow.RefNbr != AppointmentRecords.Current.RefNbr ? AppointmentRecords.Current.RefNbr : fsAppointmentAttendeeRow.RefNbr;
        }
        ...
        protected virtual void FSAppointmentInventoryItem_RowPersisting(PXCache cache, PXRowPersistingEventArgs e)
        {
            ...
            fsAppointmentInventoryItemRow.RefNbr = fsAppointmentInventoryItemRow.RefNbr == null
                                                || fsAppointmentInventoryItemRow.RefNbr != AppointmentRecords.Current.RefNbr ? AppointmentRecords.Current.RefNbr : fsAppointmentInventoryItemRow.RefNbr;
        ...
        protected virtual void FSServiceOrder_BranchLocationID_FieldUpdated(PXCache cache, PXFieldUpdatedEventArgs e)
        {
            ...
            {
                ResetLatLong(fsSrvOrdTypeRow);
                GetLocation((FSServiceOrder)e.Row, fsSrvOrdTypeRow);
            }
        }
        ...
        protected virtual void FSServiceOrder_ContactID_FieldUpdated(PXCache cache, PXFieldUpdatedEventArgs e)
        {
            ...
            if (ServiceOrderTypeSelected.Current != null 
                    && ServiceOrderTypeSelected.Current.AppAddressSource == ID.SrvOrdType_AppAddressSource.CUSTOMER_CONTACT)
            {
                ResetLatLong(ServiceOrderTypeSelected.Current);
                GetLocation((FSServiceOrder)e.Row, ServiceOrderTypeSelected.Current);
            }
        }
        ...
        protected virtual void FSServiceOrder_CustomerID_FieldUpdated(PXCache cache, PXFieldUpdatedEventArgs e)
        {
            ...
            ServiceOrderCore.FSServiceOrder_CustomerID_FieldUpdated_Handler(
                                                                            ...
                                                                            customerRow: BillCustomer.Current);

            if (ServiceOrderTypeSelected.Current != null 
                    && ServiceOrderTypeSelected.Current.AppAddressSource == ID.SrvOrdType_AppAddressSource.BUSINESS_ACCOUNT)
            {
                ResetLatLong(ServiceOrderTypeSelected.Current);

                try
                {
                    GetLocation(fsServiceOrderRow, ServiceOrderTypeSelected.Current);
                }
                catch
                { 
        ...
        protected virtual void FSServiceOrder_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
        {
            ...
            int appointmentCount = (int)PXSelectGroupBy<FSAppointment,
                                        Where<
                                            FSAppointment.sOID, Equal<Required<FSAppointment.sOID>>>,
                                        Aggregate<Count>>
                                        .Select(cache.Graph, fsServiceOrderRow.SOID).RowCount;
            ...
            TreeWFStageHelper.EnableDisableActionsByWorkflowStage(
                    ...
                    closeAppointment,
                    reopenAppointment);
            cache.AllowUpdate = AppointmentSelected.Cache.AllowUpdate;
            cache.AllowDelete = AppointmentSelected.Cache.AllowDelete;
        }

        protected virtual void FSServiceOrder_AddressLine1_FieldUpdated(PXCache cache, PXFieldUpdatedEventArgs e)
        {
            if (e.Row == null)
            {
                return;
            ...
            if (fsServiceOrderRow.AddressLine1 != (string)e.OldValue)
            {
                ResetLatLong(ServiceOrderTypeSelected.Current);
                GetLocation(fsServiceOrderRow, ServiceOrderTypeSelected.Current);
            }
        ...
        protected virtual void FSServiceOrder_AddressLine2_FieldUpdated(PXCache cache, PXFieldUpdatedEventArgs e)
        {
            if (e.Row == null)
            {
                return;
            ...
            if (fsServiceOrderRow.AddressLine2 != (string)e.OldValue)
            {
                ResetLatLong(ServiceOrderTypeSelected.Current);
                GetLocation(fsServiceOrderRow, ServiceOrderTypeSelected.Current);
            }
        ...
        protected virtual void FSServiceOrder_CountryID_FieldUpdated(PXCache cache, PXFieldUpdatedEventArgs e)
        {
            if (e.Row == null)
            {
                return;
            ...
            if (fsServiceOrderRow.CountryID != (string)e.OldValue)
            {
                fsServiceOrderRow.State = null;
                fsServiceOrderRow.PostalCode = null;
        ...
        protected virtual void FSServiceOrder_City_FieldUpdated(PXCache cache, PXFieldUpdatedEventArgs e)
        {
            if (e.Row == null)
            {
                return;
            ...
            if (fsServiceOrderRow.City != (string)e.OldValue)
            {
                ResetLatLong(ServiceOrderTypeSelected.Current);
                GetLocation(fsServiceOrderRow, ServiceOrderTypeSelected.Current);
            }
        ...
        protected virtual void FSServiceOrder_State_FieldUpdated(PXCache cache, PXFieldUpdatedEventArgs e)
        {
            if (e.Row == null)
            {
                return;
            ...
            if (fsServiceOrderRow.State != (string)e.OldValue)
            {
                ResetLatLong(ServiceOrderTypeSelected.Current);
                GetLocation(fsServiceOrderRow, ServiceOrderTypeSelected.Current);
            }
        ...
        protected virtual void FSServiceOrder_PostalCode_FieldUpdated(PXCache cache, PXFieldUpdatedEventArgs e)
        {
            if (e.Row == null)
            {
                return;
            ...
            if (fsServiceOrderRow.PostalCode != (string)e.OldValue)
            {
                ResetLatLong(ServiceOrderTypeSelected.Current);
                GetLocation(fsServiceOrderRow, ServiceOrderTypeSelected.Current);
            }
        ...
        public virtual void OpenStaffSelectorFromServiceTab()
        {
            if (ServiceOrderRelated.Current != null)
            {
                StaffSelectorFilter.Current.PostalCode = ServiceOrderRelated.Current.PostalCode;
        ...
        public class AppointmentQuickProcess : PXGraphExtension<AppointmentEntry>
        {
            ...
            private void SetQuickProcessSettingsVisibility(PXCache cache, FSSrvOrdType fsSrvOrdTypeRow, FSAppointment fsAppointmentRow, FSAppQuickProcessParams fsQuickProcessParametersRow)
            {
                if (fsSrvOrdTypeRow != null)
                {
                    ...
                    PXUIFieldAttribute.SetVisible<FSAppQuickProcessParams.releaseInvoice>(cache, fsQuickProcessParametersRow, postToSO && isInvoiceBehavior && fsQuickProcessParametersRow.SOQuickProcess == false);
                    PXUIFieldAttribute.SetVisible<FSAppQuickProcessParams.emailInvoice>(cache, fsQuickProcessParametersRow, postToSO && isInvoiceBehavior && fsQuickProcessParametersRow.SOQuickProcess == false);
