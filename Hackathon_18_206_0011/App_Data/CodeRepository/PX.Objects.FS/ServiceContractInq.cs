...
namespace PX.Objects.FS
{
    ...
    public class ServiceContractInq : ContractGenerationEnqBase<ServiceContractInq, FSContractSchedule, ServiceContractFilter, FSContractGenerationHistory.recordType.ServiceContract>
    {
        ...
        [LocationID(typeof(Where<Location.bAccountID, Equal<Current<FSServiceContract.customerID>>>),
                      DescriptionField = typeof(Location.descr), DisplayName = "Service Contract Location", DirtyRead = true)]
        ...
        protected virtual void ServiceContractFilter_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
        {
            if (e.Row == null)
            {
                return;
            ...
            if (warning == true)
            {
                cache.RaiseExceptionHandling<StaffScheduleFilter.toDate>(
                         filter,
                         filter.ToDate,
