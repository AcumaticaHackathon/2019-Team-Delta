...
namespace PX.Objects.FS
{
    public static class SharedFunctions
    {
        ...
        #region ServiceOrder
        /// <summary>
        /// Shows Service Order Note of the serviceID.
        /// </summary>
        /// <param name="operation">PXDBOperation instance.</param>
        /// <param name="view">PXView instance where the ask is going to be placed.</param>
        /// <param name="serviceID">Service ID of the Note to be shown.</param>
        public static void ShowServiceOrderNote(PXDBOperation operation, PXView view, int? serviceID)
        {
            InventoryItem inventoryItemRow = GetInventoryItemRow(view.Graph, serviceID);
            if (inventoryItemRow != null)
            {
                ShowServiceOrderNote(operation, view, inventoryItemRow);
            }
        }

        /// <summary>
        /// Shows Service Order Note of the serviceID taking as parameter the InventoryItem instance 
        /// to avoid execution of a <c>PXSelect</c> of <c>InventoryItem</c>.
        /// </summary>
        /// <param name="operation">PXDBOperation instance.</param>
        /// <param name="view">PXView instance where the ask is going to be placed.</param>
        /// <param name="inventoryItemRow">InventoryItem instance.</param>
        public static void ShowServiceOrderNote(PXDBOperation operation, PXView view, InventoryItem inventoryItemRow)
        {
            ...
            if (inventoryItemRow != null)
            {
                ...
                if (fsxServiceRow != null && fsxServiceRow.AutoShowNote == true)
                {
                    Note noteSO = PXSelect<Note,
                                  Where<
                                        Note.noteID, Equal<Required<Note.noteID>>>>
                                  .Select(view.Graph, inventoryItemRow.NoteID);

                    if (noteSO != null)
                    {
                        //For some reason the notetext field has the description of the inventoryItem
                        int index = noteSO.NoteText.IndexOf("\0");
                        string strNote = index >= 0 ? noteSO.NoteText.Substring(0, index).Trim() : noteSO.NoteText.Trim();

                        if (strNote.Length > 0)
                        {
                            view.Ask(
                                TX.WebDialogTitles.SRVORDER_NOTE_WINDOW,
                                view.Cache.Current,
                                TX.WebDialogTitles.SRVORDER_NOTE_WINDOW,
                                strNote,
                                MessageButtons.OK,
        ...
        public static void WarnUserWithServiceOrdersWithoutBillingSettings(PXCache cache, CreateInvoiceFilter createInvoiceFilterRow, PXAction<CreateInvoiceFilter> fixServiceOrdersWithoutBillingInfo)
        {
            ...
            int serviceOrdersWithoutBillingInfo_PossibleRepair = ServiceOrdersWithoutBillingSettings_PossibleFix(tempGraph);
            int serviceOrdersWithoutBillingInfo = ServiceOrdersWithoutBillingSettings(tempGraph);
        ...
        public static int ServiceOrdersWithoutBillingSettings_PossibleFix(PXGraph tempGraph)
        {
            return PXSelectReadonly2<FSServiceOrder,
                                         ...
                                         And<
                                             FSServiceOrder.customerID, IsNotNull,
                                         And<
                                             Where2<
                                                Where2<
                                                    Where<
        ...
        {
            return PXSelectReadonly<FSServiceOrder,
                                         Where<
                                            FSServiceOrder.postedBy, IsNull,
                                            ...
                                            And<
                                                Where2<
                                                    Where<
                                                        FSServiceOrder.cBID, IsNull>,
                                                    Or<
                                                        FSServiceOrder.cutOffDate, IsNull>>>>>>.Select(tempGraph).Count;
        }
        ...
        #endregion
        #region Current Company ID

        public static int GetCurrentCompanyId()
        {
            //Gets the actual CpnyID to be used from the BLC
            string companyKey = PXAccess.GetCompanyName();
            PX.Data.Update.CompanyInfo[] companies = PXDatabase.SelectCompanies();
            ...
            if (totalCompanies > 2)
            {
                ...
                {
                    if (companyKey.Equals(companies[i].LoginName))
                    {
        ...
        public static void GetBillableFieldValues(PXGraph graph, IFSSODetBase fsSODetBaseRow,
                                int? duration, out decimal? billableQty, out int? billableTime)
        {
            billableQty = 0;
            billableTime = 0;
...
#if (!Acumatica_5_20 && !Acumatica_5_10)
            appointmentEntryGraph.IsMobile = graph.IsMobile;
#endif
        ...
        public static Dictionary<FSAppointment, string> CloseAppointments(PXResultset<FSAppointment> bqlResultSet)
        {
            ...
            foreach (FSAppointment fsAppointmentRow in bqlResultSet)
            {
                ...
                try
                {
                    appointmentEntryGraph.CloseAppointment();
                }
                catch (PXException e)
                {
                    appointmentsWithErrors.Add(fsAppointmentRow, e.Message);
            ...
            foreach (FSAppointment fsAppointmentRow in bqlResultSet)
            {
                try
                {
                    if (fsAppointmentRow.Status != ID.Status_Appointment.MANUAL_SCHEDULED
                            ...
                            && fsAppointmentRow.Status != ID.Status_Appointment.CANCELED)
                    {
                        throw new PXException(PXMessages.LocalizeFormat(
                    ...
                    else
                    {
                        appointmentEntryGraph.AppointmentRecords.Current = appointmentEntryGraph.AppointmentRecords.Search<FSAppointment.appointmentID>
                                                                                                (fsAppointmentRow.AppointmentID, fsAppointmentRow.SrvOrdType);
                        appointmentEntryGraph.SkipCallSOAction = true;
                        appointmentEntryGraph.CancelAppointment();
                    }
                }
                catch (PXException e)
                {
        ...
        /// <summary>
        /// Get the next <c>RefNbr</c> in the sequence for a given base <c>RefNbr</c>.
        /// </summary>
        /// <param name="baseRefNbr">The base document <c>RefNbr</c>.</param>
        /// <param name="currentRefNbr">The current document <c>RefNbr</c>.</param>
        public static string GetNextRefNbr(string baseRefNbr, string currentRefNbr)
        {
            if (string.IsNullOrEmpty(currentRefNbr) || currentRefNbr.Trim() == "<NEW>")
            {
                return baseRefNbr.Trim() + "-1";
            }
            else
            {
                int intRefNbr = int.Parse(currentRefNbr.Substring(currentRefNbr.IndexOf("-") + 1)) + 1;

                return baseRefNbr.Trim() + "-" + intRefNbr.ToString().Trim();
            }
        }
        ...
        private static string SetOrdinalNumberString(short? ordinal)
        {
            switch (ordinal)
            {
                case 1: return TX.RecurrenceDescription.ST;
                case 2: return TX.RecurrenceDescription.ND;
                case 3: return TX.RecurrenceDescription.RD;
                default: return TX.RecurrenceDescription.TH;
            }
        }
        ...
        public static void SetRecurrenceDescription(FSSchedule fsScheduleRow)
        {
            ...
            builder.Append(TX.RecurrenceDescription.OCCURS_EVERY + " ");
            ...
            switch (fsScheduleRow.FrequencyType)
            {
                case ID.Schedule_FrequencyType.DAILY:
                    ...
                    builder.Append(fsScheduleRow.DailyFrequency + " " +
                        TX.RecurrenceDescription.DAYS + ".");

                    break;
                case ID.Schedule_FrequencyType.WEEKLY:

                    builder.Append(PXMessages.LocalizeFormat(
                        fsScheduleRow.WeeklyFrequency + " " +
                        TX.RecurrenceDescription.WEEKS + " " +
                        TX.RecurrenceDescription.ON + " "));

                    if (fsScheduleRow.WeeklyOnSun == true) { builder.Append(TX.WeekDays.SUNDAY + ", "); }
                    if (fsScheduleRow.WeeklyOnMon == true) { builder.Append(TX.WeekDays.MONDAY + ", "); }
                    if (fsScheduleRow.WeeklyOnTue == true) { builder.Append(TX.WeekDays.TUESDAY + ", "); }
                    if (fsScheduleRow.WeeklyOnWed == true) { builder.Append(TX.WeekDays.WEDNESDAY + ", "); }
                    if (fsScheduleRow.WeeklyOnThu == true) { builder.Append(TX.WeekDays.THURSDAY + ", "); }
                    if (fsScheduleRow.WeeklyOnFri == true) { builder.Append(TX.WeekDays.FRIDAY + ", "); }
                    if (fsScheduleRow.WeeklyOnSat == true) { builder.Append(TX.WeekDays.SATURDAY + ", "); }
                ...
                case ID.Schedule_FrequencyType.MONTHLY:
                    ...
                    builder.Append(PXMessages.LocalizeFormat(
                        ...
                        TX.RecurrenceDescription.MONTHS + " " +
                        TX.RecurrenceDescription.ON + " " +
                        TX.RecurrenceDescription.THE + " "));
                    ...
                    if (fsScheduleRow.Monthly2Selected == true)
                    {
                        ...
                        else if (fsScheduleRow.MonthlyRecurrenceType2.Equals(ID.ScheduleMonthlySelection.WEEKLY))
                        {
                            builder.Append(PXMessages.LocalizeFormat(
                                TX.RecurrenceDescription.THE + " " +
                                TX.RecurrencyFrecuency.counters[(int)fsScheduleRow.MonthlyOnWeek2 - 1] + " " +
                    ...
                    if (fsScheduleRow.Monthly3Selected == true)
                    {
                        ...
                        else if (fsScheduleRow.MonthlyRecurrenceType3.Equals(ID.ScheduleMonthlySelection.WEEKLY))
                        {
                            builder.Append(PXMessages.LocalizeFormat(
                                TX.RecurrenceDescription.THE + " " +
                                TX.RecurrencyFrecuency.counters[(int)fsScheduleRow.MonthlyOnWeek3 - 1] + " " +
                    ...
                    if (fsScheduleRow.Monthly4Selected == true)
                    {
                        ...
                        else if (fsScheduleRow.MonthlyRecurrenceType4.Equals(ID.ScheduleMonthlySelection.WEEKLY))
                        {
                            builder.Append(PXMessages.LocalizeFormat(
                                TX.RecurrenceDescription.THE + " " +
                                TX.RecurrencyFrecuency.counters[(int)fsScheduleRow.MonthlyOnWeek4 - 1] + " " +
                                TX.RecurrencyFrecuency.daysOfWeek[(int)fsScheduleRow.MonthlyOnDayOfWeek4]));
                        }
                    }

                    builder.Append(PXMessages.LocalizeFormat(
                        " " + TX.RecurrenceDescription.OF + " " +
                        TX.RecurrenceDescription.THAT + " " +
                        TX.RecurrenceDescription.MONTH + "."));
                ...
                case ID.Schedule_FrequencyType.ANNUAL:
                    ...
                    builder.Append(PXMessages.LocalizeFormat(
                        ...
                        TX.RecurrenceDescription.YEARS + " " +
                        TX.RecurrenceDescription.ON + " "));
                    ...
                    if (fsScheduleRow.AnnualRecurrenceType == ID.ScheduleMonthlySelection.WEEKLY)
                    {
                        builder.Append(PXMessages.LocalizeFormat(
                            " " + TX.RecurrenceDescription.THE + " " +
                            TX.RecurrencyFrecuency.counters[(int)fsScheduleRow.AnnualOnWeek - 1] + " " +
                            TX.RecurrencyFrecuency.daysOfWeek[(int)fsScheduleRow.AnnualOnDayOfWeek] + " " +
                            TX.RecurrenceDescription.OF + " "));

                        if (fsScheduleRow.AnnualOnJan == true) { builder.Append(TX.Months.JANUARY + ", "); }
                        if (fsScheduleRow.AnnualOnFeb == true) { builder.Append(TX.Months.FEBRUARY + ", "); }
                        if (fsScheduleRow.AnnualOnMar == true) { builder.Append(TX.Months.MARCH + ", "); }
                        if (fsScheduleRow.AnnualOnApr == true) { builder.Append(TX.Months.APRIL + ", "); }
                        if (fsScheduleRow.AnnualOnMay == true) { builder.Append(TX.Months.MAY + ", "); }
                        if (fsScheduleRow.AnnualOnJun == true) { builder.Append(TX.Months.JUNE + ", "); }
                        if (fsScheduleRow.AnnualOnJul == true) { builder.Append(TX.Months.JULY + ", "); }
                        if (fsScheduleRow.AnnualOnAug == true) { builder.Append(TX.Months.AUGUST + ", "); }
                        if (fsScheduleRow.AnnualOnSep == true) { builder.Append(TX.Months.SEPTEMBER + ", "); }
                        if (fsScheduleRow.AnnualOnOct == true) { builder.Append(TX.Months.OCTOBER + ", "); }
                        if (fsScheduleRow.AnnualOnNov == true) { builder.Append(TX.Months.NOVEMBER + ", "); }
                        if (fsScheduleRow.AnnualOnDec == true) { builder.Append(TX.Months.DECEMBER + ", "); }
        ...
        public static string GetAppointmentAddress(FSServiceOrder fsServiceOrderRow)
        {
            ...
            return SharedFunctions.GetCompleteAdress(
                                                    fsServiceOrderRow.PostalCode,
                                                    fsServiceOrderRow.AddressLine1,
                                                    fsServiceOrderRow.AddressLine2,
                                                    fsServiceOrderRow.City,
                                                    fsServiceOrderRow.State,
                                                    fsServiceOrderRow.CountryID);
        }
        ...
        public static string GetBranchLocationAddress(FSBranchLocation fsBranchLocationRow)
        {
            ...
            return SharedFunctions.GetCompleteAdress(
                                                    fsBranchLocationRow.PostalCode,
                                                    fsBranchLocationRow.AddressLine1,
                                                    fsBranchLocationRow.AddressLine2,
                                                    fsBranchLocationRow.City,
                                                    fsBranchLocationRow.State,
                                                    fsBranchLocationRow.CountryID);
        }

        /// <summary>
        /// Forms a string with the parameters.
        /// </summary>
        /// <returns>A string containing the complete address with the parameters.</returns>
        public static string GetCompleteAdress(string postalCode, string addressLine1, string addressLine2, string city, string state, string countryID)
        {
            ...
            {
                completeAddress = (firstValue == true) ? postalCode.Trim() : completeAddress + ", " + postalCode.Trim();
                firstValue = false;
            }
        ...
        /// <summary>
        /// Checks if the given Line Type is a comment or instruction.
        /// </summary>
        /// <param name="lineType">Line Type.</param>
        /// <returns>True if the Line Type is a comment or instruction.</returns>
        public static bool isThisACommentOrInstruction(string lineType)
        {
            if (lineType == ID.LineType_All.INSTRUCTION_SERVICE ||
        ...
        public static void CheckRouteActualDateTimes(PXCache cache, FSRouteDocument fsRouteDocumentRow, DateTime? businessDate)
        {
            ...
            if (exception != null)
            {
                fsRouteDocumentRow.Mem_ActualDuration = null;
            }

            if (fsRouteDocumentRow.ActualStartTime != null
                && fsRouteDocumentRow.ActualEndTime != null)
            {
                fsRouteDocumentRow.ActualStartTime = new DateTime(
                                                           businessDate.Value.Year,
                ...
                fsRouteDocumentRow.ActualEndTime = new DateTime(
                                                           businessDate.Value.Year,
                                                           businessDate.Value.Month,
                                                           businessDate.Value.Day,
        ...
        public static FSEquipment CreateSoldEquipment(SMEquipmentMaint graphSMEquipmentMaint, SoldInventoryItem soldInventoryItemRow, SOLine soLineRow, FSxSOLine fsxSOLineRow, string action, InventoryItem inventoryItemRow)
        {
            ...
            {
                fsEquipmentRow.InstServiceOrderID = fsxSOLineRow.SOID;
                fsEquipmentRow.InstAppointmentID = fsxSOLineRow.AppointmentID;
            }
            ...
            if (action == ID.Equipment_Action.REPLACING_TARGET_EQUIPMENT)
            {
                ...
                {
                    //Equipment Replaced
                    fsEquipmentRow.EquipmentReplacedID = fsxSOLineRow.SMEquipmentID;
                }
            }
            ...
            graphSMEquipmentMaint.EquipmentRecords.SetValueExt<FSEquipment.salesDate>(fsEquipmentRow, soldInventoryItemRow.SOOrderDate);
            ...
            if (fsEquipmentRow != null && soLineRow != null && fsxSOLineRow != null)
            {
        ...
        public static bool ShowWarningScheduleNotProcessed(PXCache cache, FSSchedule fsScheduleRow)
        {
            ...
            {
                cache.RaiseExceptionHandling<FSSchedule.refNbr>(
                        fsScheduleRow,
        ...
        public static string GetWebMethodPath(string path)
        {
            ...
            int pageIndex = path.IndexOf(ID.AcumaticaFolderName.PAGE.ToLower()) != -1 ? path.IndexOf(ID.AcumaticaFolderName.PAGE.ToLower()) : path.IndexOf(ID.AcumaticaFolderName.PAGE);
            ...
            int index = path.IndexOf(ID.Module.SERVICE_DISPATCH.ToLower(), pageIndex) != -1 ? path.IndexOf(ID.Module.SERVICE_DISPATCH.ToLower(), pageIndex) : path.IndexOf(ID.Module.SERVICE_DISPATCH, pageIndex);
        ...
        public static void UpdateEquipmentAction(PXCache cache, object row, int? inventoryID, string EquipmentItemClass, string lineType, string eQAction, object newTargetEQLineNbr, int? sMEquipmentID, int? componentID, string eQActionFieldName, string sMEquipmentIDFieldName, string newTargetEQFieldName, string componentIDFieldName, string equipmentLineRefFieldName, string commentFieldName, string quantityFieldName, bool updateQty = true)
        {
            if (inventoryID != null || lineType == ID.LineType_All.SERVICE)
            {
                        ...
                        && enableQty == false)
                {
                    if (updateQty)
                    {
                    cache.SetValue(row, quantityFieldName, 1.0m);
                    }
        ...
        public static int? GetCurrentEmployeeID(PXCache cache)
        {
            ...
            if (fsxepEmployeeRow.SDEnabled == true && epEmployeeRow.Status != EPEmployee.status.Inactive)
            {
        ...
        public static int SchedulesWithoutNextExecution(PXGraph tempGraph)
        {
            return PXSelectReadonly<FSSchedule,
                                        ...
                                        And<
                                            FSSchedule.active, Equal<True>>>>.Select(tempGraph).Count;
        }
                    ...
                    using (PXTransactionScope ts = new PXTransactionScope())
                    {
                        var resultSet = PXSelectReadonly<FSSchedule,
                                            Where<
                                                FSSchedule.nextExecutionDate, IsNull,
                                            And<
                                                FSSchedule.active, Equal<True>>>>.Select(callerGraph);

                        foreach (FSSchedule fsScheduleRow in resultSet)
                        {
                            PXUpdate<Set<FSSchedule.nextExecutionDate,
                        ...
                        PXUpdate<Set<FSSetup.disableFixScheduleAction,
                                Required<FSSetup.disableFixScheduleAction>>, FSSetup>
        ...
        public static bool GetEnableSeasonSetting(PXGraph graph, FSSchedule fsScheduleRow, FSRouteSetup fsRouteSetupRow = null, FSSetup fsSetupRow = null)
        {
            ...
            if (fsScheduleRow is FSRouteContractSchedule)
            {
                ...
                {
                    fsRouteSetupRow = ServiceManagementSetup.GetServiceManagementRouteSetup(graph);
                }
        ...
        public static string CheckCacheState(PXCache cache, IBqlTable row, bool getErrorMsg = false, bool getRowWarning = false, bool getFieldWarning = false)
        {
            ...
            foreach (string field in cache.Fields)
            {
                ...
                if (fieldState != null && fieldState.Error != null && getErrorMsg)
                {
                    if (fieldState.ErrorLevel != PXErrorLevel.RowInfo &&
                       (getRowWarning == true || fieldState.ErrorLevel != PXErrorLevel.RowWarning) &&
                       (getFieldWarning == true || fieldState.ErrorLevel != PXErrorLevel.Warning))
                    {
                        ...
                        errorMsgBuilder.Append(fieldState.Error);
                    }
                }
            }
        ...
        public static SharedClasses.SubAccountIDList GetSubAccountIDs(PXGraph graph, FSSrvOrdType fsSrvOrdTypeRow, int? inventoryID, int? branchID, int? locationID, int? branchLocationID, int? salesPersonID, bool isService = true)
        {
            INSite inSite = null;
            ...
            else
            {
                ...
                return new SharedClasses.SubAccountIDList(branchLocation_SubID, branch_SubID, inventoryItem_SubID, customerLocation_SubID, postingClass_SubID, salesPerson_SubID, srvOrdType_SubID, warehouse_SubID);
            }
        }
        ...
        public static void CopyNotesAndFiles(PXCache cache, FSSrvOrdType fsSrvOrdTypeRow, object document, int? customerID, int? locationID)
        {
                    ...
                    || fsSrvOrdTypeRow.CopyAttachmentsFromCustomer == true)
            {
                ...
                CopyNotesAndFiles(
                    customerMaintGraph.BAccount.Cache,
                    cache,
                ...
                || fsSrvOrdTypeRow.CopyAttachmentsFromCustomerLocation == true)
            {
                ...
                CopyNotesAndFiles(
                    customerLocationMaintGraph.Location.Cache,
                    cache,
                    ...
                    || fsSrvOrdTypeRow.CopyAttachmentsToAppoinment == true))
            {
                ...
                CopyNotesAndFiles(
                    new PXCache<FSServiceOrder>(cache.Graph),
                    cache,
                    fsServiceOrderRow,
                    document,
                    needCopyNotes,
        ...
        {
            return (PXResultset<FSPostBatch>) PXSelectJoinGroupBy<
                                                            FSPostBatch,
                                                            InnerJoin<FSPostDoc,
                                                                On<FSPostDoc.batchID, Equal<FSPostBatch.batchID>>>,
        ...
        public static void UpdateFSSODetReferences(PXGraph graph, PXCache soCache, POOrder poOrderRow, List<POLine> poLineUpdatedList)
        {
            try
            {
                ...
                foreach (FSServiceOrder fsServiceOrderRow in results)
                {
                    ...
                    foreach (FSSODet fsSODetRow in PXSelect<FSSODet, Where<
                                                                    FSSODet.srvOrdType, Equal<Required<FSSODet.srvOrdType>>,
                                                                    And<FSSODet.refNbr, Equal<Required<FSSODet.refNbr>>,
                    ...
                    else
                    {
                        PXResultset<FSAppointment> appointmentRows = PXSelect<FSAppointment,
                                                                        Where<FSAppointment.sOID, Equal<Required<FSAppointment.sOID>>>>
