...
namespace PX.Objects.FS
{
    public static class AppointmentCore
    {
        ...
        public class AppointmentRecords_View : PXSelect<FSAppointment,
                                                Where<
                                                  ...
                                                  bool? isBeingCalledFromQuickProcess)
        {
            ...
            bool enableInsertUpdate = CanUpdateAppointment(graph, fsAppointmentRow, fsServiceOrderRow) || skipTimeCardUpdate || (isBeingCalledFromQuickProcess ?? false);
            bool enableDelete = CanDeleteAppointment(graph, fsAppointmentRow, fsServiceOrderRow);
            ...
            appointmentPart.Cache.AllowInsert = enableInsertUpdate && enablePartsTab;
            appointmentPart.Cache.AllowUpdate = enableInsertUpdate && enablePartsTab;
            appointmentPart.Cache.AllowDelete = enableInsertUpdate && enablePartsTab;
            ...
            PXUIFieldAttribute.SetVisible<FSAppointment.billContractPeriodID>(appointmentRecords.Cache, fsAppointmentRow, enableServiceContractFields);
        }
        ...
        /// </summary>
        public static bool CanUpdateAppointment(PXGraph graph, FSAppointment fsAppointmentRow, FSServiceOrder fsServiceOrderRow)
        {
            if (fsAppointmentRow == null
        ...
        /// </summary>
        public static bool CanDeleteAppointment(PXGraph graph, FSAppointment fsAppointmentRow, FSServiceOrder fsServiceOrderRow)
        {
            bool canDeleteServiceOrder = true;
            if (fsServiceOrderRow != null)
            {
                canDeleteServiceOrder = ServiceOrderCore.CanUpdateServiceOrder(graph, fsServiceOrderRow);
            }
        ...
        {
            int? servicesToPost = appointmentServiceDetail.Select().Where(y => ((FSAppointmentDetService)y).needToBePosted() == true)
                                                                   .RowCast<FSPostInfo>().Where(x => x.isPosted() == false).Count();

            int? partsToPost = appointmentPartDetail.Select().Where(y => ((FSAppointmentDetPart)y).needToBePosted() == true)
                                                             .RowCast<FSPostInfo>().Where(x => x.isPosted() == false).Count();
            ...
            PXDBOperation apptLineOperation = PXDBOperation.Insert)
        {
            ...
            if (fsSODetRow != null)
            {
                ...
                if (updateServiceOrder)
                {
                    ...
                    int? apptNumber          = fsSODetRow.ApptNumber      + sign * 1;
                    decimal apptTranAmt;
                    ...
                    int? apptDurationTotal = fsServiceOrderRow.ApptDurationTotal - fsSODetRow.ApptDuration + apptDuration;
