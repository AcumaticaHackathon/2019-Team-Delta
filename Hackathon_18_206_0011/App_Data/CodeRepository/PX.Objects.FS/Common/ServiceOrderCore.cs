...
namespace PX.Objects.FS
{
    public static class ServiceOrderCore
    {
        ...
        public class ServiceOrderRecords_View : PXSelect<FSServiceOrder,
                                                Where<
        ...
        public static IEnumerable ViewDirectionOnMap_Handler(PXGraph graph, PXAdapter adapter, FSServiceOrder fsServiceOrderRow)
        {
            if (fsServiceOrderRow.PostalCode != null)
            {
                var googleMap = new PX.Data.GoogleMapRedirector();

                googleMap.ShowAddress(
                                    ...
                                    fsServiceOrderRow.AddressLine3);
            }
            ...
            if (bqlResultSet.Count > 0)
            {
                if (wdr != WebDialogResult.No)
                {
                    ...
                    if (appWithErrors.Count > 0)
                    {
                        ...
                        throw new PXException(PXMessages.LocalizeFormat(TX.Error.SERVICE_ORDER_CANT_BE_CLOSED_APPOINTMENTS_HAVE_ISSUES));
                    }
                }
                else
                {
        ...
        public static void CompleteAppointmentsInServiceOrder(ServiceOrderEntry graph, FSServiceOrder fsServiceOrderRow)
        {
            ...
            if (bqlResultSet.Count > 0)
            {
                ...
                if (appWithErrors.Count > 0)
                {
                    ...
                    throw new PXException(PXMessages.LocalizeFormat(TX.Error.SERVICE_ORDER_CANT_BE_COMPLETED_APPOINTMENTS_HAVE_ISSUES));
                }
            }
        }
        ...
        public static void CancelAppointmentsInServiceOrder(ServiceOrderEntry graph, FSServiceOrder fsServiceOrderRow)
        {
            ...
            if (bqlResultSet.Count > 0)
            {
                ...
                if (appWithErrors.Count > 0)
                {
                    ...
                    throw new PXException(PXMessages.LocalizeFormat(TX.Error.SERVICE_ORDER_CANT_BE_CANCELED_APPOINTMENTS_HAVE_ISSUES));
                }
            }
        }
                                                                                ...
                                                                                PXSelectBase<FSServiceOrder> serviceOrderRelated)
        {
            ...
            SetDefaultAddressAndContactInfo(graph, fsServiceOrderRow, fsSrvOrdTypeRow);
        }
                                                                            ...
                                                                            FSSrvOrdType fsSrvOrdTypeRow)
        {
            ...
            FSServiceOrder fsServiceOrderRow = (FSServiceOrder)e.Row;
            SetDefaultAddressAndContactInfo(cache.Graph, fsServiceOrderRow, fsSrvOrdTypeRow);
                                                                        ...
                                                                        Customer customerRow)
        {
            ...
            SetDefaultAddressAndContactInfo(cacheServiceOrder.Graph, fsServiceOrderRow, fsSrvOrdTypeRow);
        ...
        public static void ShowServiceNoteFromServiceOrder(
                                                        PXView view,
                                                        int? InventoryID,
                                                        FSServiceOrder fsServiceOrderRow,
                                                        bool externalCall,
                                                        bool IsCopyPasteContext)
        {
            if (fsServiceOrderRow.SourceType == ID.SourceType_ServiceOrder.SERVICE_DISPATCH && externalCall == true && IsCopyPasteContext == false)
            {
                SharedFunctions.ShowServiceOrderNote(PXDBOperation.Insert, view, InventoryID);
            }
        }
        ...
        public static string GetAddress(string addressLine1, string addressLine2, string city, string state, string countryID, string postalCode)
        {
                                                ...
                                                bool allowCustomerChange = false)
        {
            ...
            {
                enableInsertUpdate = AppointmentCore.CanUpdateAppointment(graph, fsAppointmentRow, fsServiceOrderRow);
                enableDelete = AppointmentCore.CanDeleteAppointment(graph, fsAppointmentRow, fsServiceOrderRow);
            }
            else
            {
                enableDelete = CanDeleteServiceOrder(graph, fsServiceOrderRow);
                enableInsertUpdate = CanUpdateServiceOrder(graph, fsServiceOrderRow);
            }
            ...
            if (fsServiceOrderRow.Status == ID.Status_ServiceOrder.COMPLETED)
            {
                cacheServiceOrder.AllowUpdate = true;
            }
            ...
            {
                cacheServiceOrderDetailParts.AllowInsert = enableInsertUpdate && enablePartsTab && fsServiceOrderRow.AllowInvoice == false;
                cacheServiceOrderDetailParts.AllowUpdate = enableInsertUpdate && enablePartsTab;
                cacheServiceOrderDetailParts.AllowDelete = enableInsertUpdate && enablePartsTab && fsServiceOrderRow.AllowInvoice == false;
            }
            ...
            if (enableInsertUpdate == true)
            {
                ...
                PXUIFieldAttribute.SetVisible<FSServiceOrder.billContractPeriodID>(cacheServiceOrder, fsServiceOrderRow, enableServiceContractFields);
        ...
        public static void EnableDisable_ActionButtons(
                ...
                PXAction<FSServiceOrder> openSource,
                ...
                PXAction<FSServiceOrder> allowInvoice)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (openSource != null)
                    {
                        openSource.SetEnabled(fsServiceOrderRow.SourceType != ID.SourceType_ServiceOrder.SERVICE_DISPATCH
                            || fsServiceOrderRow.SourceType == TX.SourceType_ServiceOrder.QUOTE);
                    }
        ...
        /// <summary>
        /// Returns true if a Service order [fsServiceOrderRow] can be updated based based in its status.
        /// </summary>
        public static bool CanUpdateServiceOrder(PXGraph graph, FSServiceOrder fsServiceOrderRow)
        {
            if (fsServiceOrderRow == null
                    || fsServiceOrderRow.Status == ID.Status_ServiceOrder.CLOSED
                        || fsServiceOrderRow.Status == ID.Status_ServiceOrder.CANCELED)
        ...
        public static void GetAddressAndContactFromCustomerLocation(PXGraph graph, int? locationID, ref Address addressRow, ref Contact contactRow)
        {
            PXResult<Location, Address, Contact> bqlResult = 
                                    (PXResult<Location, Address, Contact>)
                                    PXSelectJoin<Location,
                                    LeftJoin<Address,
                                        On<Address.bAccountID, Equal<Location.bAccountID>,
                                        And<Address.addressID, Equal<Location.defAddressID>>>,
                                    LeftJoin<Contact,
                                        On<Contact.bAccountID, Equal<Location.bAccountID>,
        ...
        public static void GetAddressAndContactFromCustomerContact(PXGraph graph, int? contactID, ref Address addressRow, ref Contact contactRow)
        {
            PXResult<Contact, Address> bqlResult = 
                                    (PXResult<Contact, Address>)
                                    PXSelectJoin<Contact,
                                    LeftJoin<Address,
                                        On<Address.bAccountID, Equal<Contact.bAccountID>,
        ...
        public static void GetAddressAndContactFromBranchLocation(PXGraph graph, int? branchLocationID, ref Address addressRow, ref Contact contactRow)
        {
            ...
            if (fsBranchLocationRow != null)
            {
                ...
                {
                    AddressLine1 = fsBranchLocationRow.AddressLine1,
                    AddressLine2 = fsBranchLocationRow.AddressLine2,
                    AddressLine3 = fsBranchLocationRow.AddressLine3,
                    City = fsBranchLocationRow.City,
                    CountryID = fsBranchLocationRow.CountryID,
                    State = fsBranchLocationRow.State,
                    PostalCode = fsBranchLocationRow.PostalCode,
                    IsValidated = fsBranchLocationRow.IsValidated
                };
                contactRow = new Contact()
                {
                    Phone1 = fsBranchLocationRow.Phone1,
                    Phone2 = fsBranchLocationRow.Phone2,
                    Phone3 = fsBranchLocationRow.Phone3,
                    EMail = fsBranchLocationRow.EMail,
                    Fax = fsBranchLocationRow.Fax,
                    Attention = fsBranchLocationRow.Salutation
                };
            }
        }

        public static void SetDefaultAddressAndContactInfo(PXGraph graph, FSServiceOrder fsServiceOrderRow, FSSrvOrdType fsSrvOrdTypeRow)
        {
            ...
            // Set all the address info to null
            fsServiceOrderRow.AddressLine1 = null;
            fsServiceOrderRow.AddressLine2 = null;
            fsServiceOrderRow.AddressLine3 = null;
            fsServiceOrderRow.City = null;
            fsServiceOrderRow.CountryID = null;
            fsServiceOrderRow.State = null;
            fsServiceOrderRow.PostalCode = null;
            fsServiceOrderRow.AddressValidated = false;

            // Set all the contact info to null
            fsServiceOrderRow.Phone1 = null;
            fsServiceOrderRow.Phone2 = null;
            fsServiceOrderRow.Phone3 = null;
            fsServiceOrderRow.EMail = null;
            fsServiceOrderRow.Fax = null;
            fsServiceOrderRow.Attention = null;

            Address addressRow = null;
            Contact contactRow = null;

            switch (fsSrvOrdTypeRow.AppAddressSource)
            {
                case ID.SrvOrdType_AppAddressSource.BUSINESS_ACCOUNT:
                    GetAddressAndContactFromCustomerLocation(graph, fsServiceOrderRow.LocationID, ref addressRow, ref contactRow);
                    break;

                case ID.SrvOrdType_AppAddressSource.CUSTOMER_CONTACT:
                    GetAddressAndContactFromCustomerContact(graph, fsServiceOrderRow.ContactID, ref addressRow, ref contactRow);
                    break;

                case ID.SrvOrdType_AppAddressSource.BRANCH_LOCATION:
                    GetAddressAndContactFromBranchLocation(graph, fsServiceOrderRow.BranchLocationID, ref addressRow, ref contactRow);
                    break;
            ...
            if (addressRow != null)
            {
                fsServiceOrderRow.AddressLine1 = addressRow.AddressLine1;
                fsServiceOrderRow.AddressLine2 = addressRow.AddressLine2;
                fsServiceOrderRow.AddressLine3 = addressRow.AddressLine3;
                fsServiceOrderRow.City = addressRow.City;
                fsServiceOrderRow.CountryID = addressRow.CountryID;
                fsServiceOrderRow.State = addressRow.State;
                fsServiceOrderRow.PostalCode = addressRow.PostalCode;
                fsServiceOrderRow.AddressValidated = addressRow.IsValidated;
            }

            if (fsServiceOrderRow.ContactID != null
                    && fsSrvOrdTypeRow.AppAddressSource == ID.SrvOrdType_AppAddressSource.CUSTOMER_CONTACT)
            {
                contactRow = PXSelect<Contact,
                             Where<
                                    Contact.contactID, Equal<Required<Contact.contactID>>>>
                            .Select(graph, fsServiceOrderRow.ContactID);
            }

            if (contactRow != null)
            {
                fsServiceOrderRow.Phone1 = contactRow.Phone1;
                fsServiceOrderRow.Phone2 = contactRow.Phone2;
                fsServiceOrderRow.Phone3 = contactRow.Phone3;
                fsServiceOrderRow.EMail = contactRow.EMail;
                fsServiceOrderRow.Fax = contactRow.Fax;
                fsServiceOrderRow.Attention = contactRow.Attention;
            }
        }
        ...
        private static void UpdatePendingPostFlags(
            ...
            PXSelectBase<FSSODetPart> serviceOrderPartDetail)
        {
            int? servicesToPost = serviceOrderServiceDetail.Select().Where(y => ((FSSODetService)y).needToBePosted() == true)
                                                                    .RowCast<FSPostInfo>().Where(x => x.isPosted() == false).Count();

            int? partsToPost = serviceOrderPartDetail.Select().Where(y => ((FSSODetPart)y).needToBePosted() == true)
                                                              .RowCast<FSPostInfo>().Where(x => x.isPosted() == false).Count();
        ...
        private static void UpdateWatingForPartsFlag(
            ...
            PXSelectBase<FSSODetPart> serviceOrderPartDetail)
        {
            int? serviceWaitingForParts = serviceOrderServiceDetail.Select().Where(y => ((FSSODetService)y).waitingForParts() == true).Count();

            int? partsWaitingForParts = serviceOrderPartDetail.Select().Where(y => ((FSSODetPart)y).waitingForParts() == true).Count();
            ...
            AppointmentEntry graphAppointmentEntryCaller)
        {
            ...
            {
                PXResultset<FSAppointmentDetService> services = null;

                if (currentAppointment == true && graphAppointmentEntryCaller != null)
                { 
                    services = graphAppointmentEntryCaller.AppointmentDetServices.Select();
                }
                ...
                PXResultset<FSAppointmentDetPart> parts = null;

                if (currentAppointment == true && graphAppointmentEntryCaller != null)
                {
                    parts = graphAppointmentEntryCaller.AppointmentDetParts.Select();
                }
