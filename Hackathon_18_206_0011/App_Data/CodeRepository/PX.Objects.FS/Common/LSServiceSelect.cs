...
namespace PX.Objects.FS
{
            ...
            Or<FSSODetService.lineType, Equal<FSSODetService.lineType.NonStockItem>>>>>>>
    {
        ...
        {
            IQtyAllocated availability = AvailabilityFetch(sender, (FSSODetService)e.Row, !(e.Row != null && (((FSSODetService)e.Row).Completed == true)));
        ...
        protected override void RaiseQtyExceptionHandling(PXCache sender, object row, object newValue, PXSetPropertyException e)
        {
            ...
            {
                sender.RaiseExceptionHandling<FSSODetService.orderQty>(row, newValue, new PXSetPropertyException(e.MessageNoPrefix, PXErrorLevel.Warning, sender.GetStateExt<FSSODetService.inventoryID>(row), sender.GetStateExt<FSSODetService.subItemID>(row), sender.GetStateExt<FSSODetService.siteID>(row), sender.GetStateExt<FSSODetService.locationID>(row), sender.GetValue<FSSODetService.lotSerialNbr>(row)));
            }
            else
            {
                sender.RaiseExceptionHandling<FSSODetServiceSplit.qty>(row, newValue, new PXSetPropertyException(e.MessageNoPrefix, PXErrorLevel.Warning, sender.GetStateExt<FSSODetServiceSplit.inventoryID>(row), sender.GetStateExt<FSSODetServiceSplit.subItemID>(row), sender.GetStateExt<FSSODetServiceSplit.siteID>(row), sender.GetStateExt<FSSODetServiceSplit.locationID>(row), sender.GetValue<FSSODetServiceSplit.lotSerialNbr>(row)));
            }
        }
