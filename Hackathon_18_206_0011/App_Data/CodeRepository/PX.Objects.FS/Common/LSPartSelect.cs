...
namespace PX.Objects.FS
{
        ...
        And<FSSODetPart.lineType, Equal<FSSODetPart.lineType.Inventory_Item>>>>>
    {
        ...
        {
            IQtyAllocated availability = AvailabilityFetch(sender, (FSSODetPart)e.Row, !(e.Row != null && (((FSSODetPart)e.Row).Completed == true)));
        ...
        protected virtual void Parent_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
        {
            if (_LastSelected == null || !object.ReferenceEquals(_LastSelected, e.Row))
            {
                ...
                PXUIFieldAttribute.SetVisible<FSSODetPartSplit.locationID>(this.DetailCache, null, IsLocationEnabled);
        ...
        public virtual void FSSODetPartSplit_LocationID_FieldDefaulting(PXCache sender, PXFieldDefaultingEventArgs e)
        {
            ...
            if (cache.Current != null && (e.Row == null || ((FSSODetPart)cache.Current).LineNbr == ((FSSODetPartSplit)e.Row).LineNbr && ((FSSODetPartSplit)e.Row).IsStockItem == true))
            {
                e.NewValue = ((FSSODetPart)cache.Current).LocationID;
        ...
        protected override void RaiseQtyExceptionHandling(PXCache sender, object row, object newValue, PXSetPropertyException e)
        {
            ...
            {
                sender.RaiseExceptionHandling<FSSODetPart.orderQty>(row, newValue, new PXSetPropertyException(e.MessageNoPrefix, PXErrorLevel.Warning, sender.GetStateExt<FSSODetPart.inventoryID>(row), sender.GetStateExt<FSSODetPart.subItemID>(row), sender.GetStateExt<FSSODetPart.siteID>(row), sender.GetStateExt<FSSODetPart.locationID>(row), sender.GetValue<FSSODetPart.lotSerialNbr>(row)));
            }
            else
            {
                sender.RaiseExceptionHandling<FSSODetPartSplit.qty>(row, newValue, new PXSetPropertyException(e.MessageNoPrefix, PXErrorLevel.Warning, sender.GetStateExt<FSSODetPartSplit.inventoryID>(row), sender.GetStateExt<FSSODetPartSplit.subItemID>(row), sender.GetStateExt<FSSODetPartSplit.siteID>(row), sender.GetStateExt<FSSODetPartSplit.locationID>(row), sender.GetValue<FSSODetPartSplit.lotSerialNbr>(row)));
            }
        }
