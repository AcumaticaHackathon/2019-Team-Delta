...
{
    public class ServiceContractScheduleEntryBase<TGraph, TPrimary, Field, EntityField, CustomerField> : PXGraph<TGraph, TPrimary>
        ...
        where CustomerField : PX.Data.IBqlField
    {
        ...
        [PXHidden]
        [Obsolete("The view is obsolete and will be removed in Acumatica 2019R1.")]
        public PXSelect<FSRouteSetup> RouteSetup;

        public PXSelect<TPrimary, 
                    Where<
                        EntityField, Equal<Optional<EntityField>>,
                        And<CustomerField, Equal<Optional<CustomerField>>>>> ContractScheduleRecords;
        ...
        public PXSelect<FSScheduleDetService,
                            Where<
                                FSScheduleDetService.scheduleID, Equal<Current<Field>>,
                                And<
                                    Where<
                                        ...
                                        Or<FSScheduleDetService.lineType, Equal<ListField_LineType_Service_ServiceContract.NonStockItem>>>>>>>>> ScheduleDetServices;

        public PXSelect<FSScheduleDetPart,
                            Where<
                                FSScheduleDetPart.scheduleID, Equal<Current<Field>>,
                                And<
                                    Where<
                                        ...
                                        Or<FSScheduleDetPart.lineType, Equal<ListField_LineType_Part_ALL.Instruction_Part>>>>>>> ScheduleDetParts;
        ...
        #region Actions
        #region Next
        public PXAction<TPrimary> next;
        [PXUIField(DisplayName = ActionsMessages.Next, MapEnableRights = PXCacheRights.Select, MapViewRights = PXCacheRights.Select)]
        [PXNextButton]
        protected new virtual IEnumerable Next(PXAdapter adapter)
        {
            FSSchedule fsContractScheduleRow = ContractScheduleRecords.Current as FSSchedule;

            if (fsContractScheduleRow != null
                    && fsContractScheduleRow.CustomerID != null
                        && fsContractScheduleRow.EntityID != null)
            {
                foreach (FSSchedule schedule in (new PXNext<TPrimary>(this, "Next")).Press(adapter))
                {
                    if (schedule.CustomerID != null && schedule.EntityID != null)
                    {
                        return new object[] { schedule };
                    }
                }

                return new object[] { fsContractScheduleRow };
            }

            return (new PXInsert<TPrimary>(this, "Insert")).Press(adapter);
        }
        #endregion
        #region Previous
        public PXAction<TPrimary> previous;
        [PXUIField(DisplayName = ActionsMessages.Previous, MapEnableRights = PXCacheRights.Select, MapViewRights = PXCacheRights.Select)]
        [PXPreviousButton]
        protected new virtual IEnumerable Previous(PXAdapter adapter)
        {
            FSSchedule fsContractScheduleRow = ContractScheduleRecords.Current as FSSchedule;
            ...
            if (fsContractScheduleRow != null
                    && fsContractScheduleRow.CustomerID != null
                        && fsContractScheduleRow.EntityID != null)
            {
                foreach (FSSchedule schedule in (new PXPrevious<TPrimary>(this, "Previous")).Press(adapter))
                {
                    if (schedule.CustomerID != null && schedule.EntityID != null)
                    {
                        return new object[] { schedule };
        ...
        #endregion
        #region Cancel
        public PXAction<TPrimary> cancel;
        [PXUIField(DisplayName = ActionsMessages.Cancel, MapEnableRights = PXCacheRights.Select, MapViewRights = PXCacheRights.Select)]
        [PXCancelButton]
        protected new virtual IEnumerable Cancel(PXAdapter adapter)
        {
            if (ContractScheduleRecords.Current != null)
            {
                FSSchedule fsContractScheduleRow = ContractScheduleRecords.Current as FSSchedule;

                if (adapter.Searches.Length >= 2 && adapter.Searches[0] != null)
                {
                    PXResult<BAccountR, FSServiceContract> result;

                    if (fsContractScheduleRow is FSContractSchedule)
                    {
                        result = (PXResult<BAccountR, FSServiceContract>)
                        PXSelectJoin<BAccountR,
                            InnerJoin<FSServiceContract, On<FSServiceContract.customerID, Equal<BAccountR.bAccountID>>>,
                            ...
                            And<FSServiceContract.refNbr, Equal<Required<FSServiceContract.refNbr>>>>>>
                        .SelectWindowed(this, 0, 1, adapter.Searches[0], adapter.Searches[1]);
                    }
                    else
                    {
                        result = (PXResult<BAccountR, FSServiceContract>)
                    ...
                    FSServiceContract serviceContractRow = result;
                    BAccountR customerRow = result;

                    if (result == null)
                    {
                        adapter.Searches[1] = null;

                        if (adapter.Searches.Length > 2)
                        {
                            adapter.Searches[2] = null;
                        }
                    }
                    else
                    {
                        if (customerRow.BAccountID != fsContractScheduleRow.CustomerID)
                        {
                            if (adapter.Searches.Length > 2)
                            {
                                adapter.Searches[2] = null;
                    ...
                    foreach (FSSchedule schedule in (new PXCancel<TPrimary>(this, "Cancel")).Press(adapter))
                    {
                        if (ContractScheduleRecords.Cache.GetStatus(schedule) == PXEntryStatus.Inserted)
                        {
                            return (new PXInsert<TPrimary>(this, "Insert")).Press(adapter);
                        }
                        else
                ...
                {
                    return (new PXCancel<TPrimary>(this, "Cancel")).Press(adapter);
                }
            }
        ...
        public bool isThisFSSalesPriceNeedToBeDeleted(int? entityID, int? inventoryID, int? currentScheduleDetID)
        {
            int numRowsOld = 0;
            ...
            if (isService != null)
            {
                ...
                switch (operation)
                {
                    case PXDBOperation.Insert:
                        ...
                        foreach (InventoryItem inventoryItemRow in bqlResultSet_InventoryItemsFromSchedule)
                        {
                            if (IsThisSalesPricePersisted(cache, entityID, inventoryItemRow.InventoryID) == false)
                            {
                                ...
                                fsSalesPriceRow.LineType = fsScheduleDetRow.LineType;
                    ...
                    case PXDBOperation.Update:
                        ...
                        bool updateServiceItems = bqlResultSet_PreviousInventoryItemList.Count > 0;
                        ...
                        if (updateServiceItems)
                        {
                            ...
                            {
                                if (isThisFSSalesPriceNeedToBeDeleted(entityID, inventoryItemRow.InventoryID, fsScheduleDetRow.ScheduleDetID) == true)
                                {
        ...
        {
            bool showSeasonSettings = SharedFunctions.GetEnableSeasonSetting(this, fsScheduleRow, null, Setup.Current);
        ...
        public void ContractSchedule_RowSelected_PartialHandler(PXCache cache, FSSchedule fsScheduleRow)
        {
            ...
            fsScheduleRow.Days = TX.RecurrenceDescription.DAYS;
            fsScheduleRow.Weeks = TX.RecurrenceDescription.WEEKS;
            fsScheduleRow.Months = TX.RecurrenceDescription.MONTHS;
            fsScheduleRow.Years = TX.RecurrenceDescription.YEARS;
        ...
        {
            if (e.Row == null || ContractScheduleSelected.Current == null)
            {
                return;
            }

            FSScheduleDetService fsScheduleDetServiceRow = (FSScheduleDetService)e.Row;
            FSSchedule fsScheduleRow = ContractScheduleSelected.Current as FSSchedule;
            InsertUpdateDeleteFSSalesPrice(cache, fsScheduleDetServiceRow, fsScheduleRow.EntityID, e.Operation, e.TranStatus);
        }
        ...
        protected virtual void FSScheduleDetService_RowDeleted(PXCache cache, PXRowDeletedEventArgs e)
        {
            ...
            FSServiceContract fsServiceContractRow = PXSelectJoin<FSServiceContract,
                                                     ...
                                                     .Select(this, fsScheduleDetServiceRow.ScheduleID);
            if (fsServiceContractRow != null)
            {
                InsertUpdateDeleteFSSalesPrice(cache, fsScheduleDetServiceRow, fsServiceContractRow.ServiceContractID, PXDBOperation.Delete, PXTranStatus.Open);
            }
        }
        #endregion
        ...
        protected virtual void FSScheduleDetPart_RowPersisted(PXCache cache, PXRowPersistedEventArgs e)
        {
            ...
            FSScheduleDetPart fsScheduleDetPartRow = (FSScheduleDetPart)e.Row;
            FSSchedule fsScheduleRow = ContractScheduleSelected.Current as FSSchedule;
            InsertUpdateDeleteFSSalesPrice(cache, fsScheduleDetPartRow, fsScheduleRow.EntityID, e.Operation, e.TranStatus);
        }
        ...
        protected virtual void FSScheduleDetPart_RowDeleted(PXCache cache, PXRowDeletedEventArgs e)
        {
            ...
            FSServiceContract fsServiceContractRow = PXSelectJoin<FSServiceContract,
                                                     ...
                                                     Where<
                                                           FSSchedule.scheduleID, Equal<Required<FSSchedule.scheduleID>>>>
                                                     .Select(this, fsScheduleDetPartRow.ScheduleID);
            if (fsServiceContractRow != null)
            {
                InsertUpdateDeleteFSSalesPrice(cache, fsScheduleDetPartRow, fsServiceContractRow.ServiceContractID, PXDBOperation.Delete, PXTranStatus.Open);
            }
        }
        #endregion
