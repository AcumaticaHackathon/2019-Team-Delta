  ...
  public static class Geocoder
  {
        ...
        { 
            XmlNode result = responseXml.SelectSingleNode(
                string.Format(
                    ...
                    ID.MapsConsts.XML_SCHEMA_TAG, type),
                nameSpace);
            if (result != null)
            {
    ...
    public static GLocation[] Geocode(string address, string apiKey)
    {
      ...
      XmlNodeList results = responseXml.SelectNodes(
          string.Format("//{0}Location", ID.MapsConsts.XML_SCHEMA_TAG), nameSpace);
      ...
      foreach (XmlElement result in results)
      {
        ...
        XmlElement locationElement = (XmlElement)result.SelectSingleNode(string.Format(
            ".//{0}Point", ID.MapsConsts.XML_SCHEMA_TAG), nameSpace);
