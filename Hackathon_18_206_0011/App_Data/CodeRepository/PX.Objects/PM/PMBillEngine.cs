	...
	public class PMBillEngine : PXGraph<PMBillEngine>, IRateTable
	{
			...
			if (invoices.Count > 0)
			{
				...
				using (PXTransactionScope ts = new PXTransactionScope())
				{
					...
					foreach (KeyValuePair<string, Tuple<int?, List<BillingData>>> kv in invoices)
					{
						...
						{
							Tuple<string, decimal> docTypeAndRetainage = GetDocTypeAndRetainage(kv.Value.Item2);
		...
		public virtual void InsertTransactionsInInvoice(List<BillingData> unbilled, int mult)
		{
			foreach (BillingData data in unbilled)
			{
				...
				{
					tran.RetainagePct = data.Tran.RetainagePct;
				}
				...
				if (data.Tran.IsPrepayment != true && data.Tran.AccountGroupID != null)
					InvoiceEntry.SubtractAmountToInvoice(newTran, ARDocType.SignAmount(newTran.TranType).GetValueOrDefault(1) * newTran.TranAmt.GetValueOrDefault(), data.Tran.AccountGroupID);
		...
		public virtual ARInvoice InsertNewInvoiceDocument(string finPeriod, string docType, decimal retainage, Customer customer, PMProject project,
			DateTime billingDate, string docDesc, int? locationID)
		{
			...
			invoice.RetainageApply = docType == ARDocType.Invoice && retainage > 0;
			if (project.DefaultBranchID != null)
			...
			if(docType == ARDocType.Invoice && retainage > 0)
		...
		public virtual Tuple<string, decimal> GetDocTypeAndRetainage(List<BillingData> value)
		{
			decimal amount = 0;
			decimal retainage = 0;
			foreach (BillingData data in value)
			{
				...
				retainage += data.Tran.Retainage.GetValueOrDefault();
			}

			if (amount >= 0)
				return new Tuple<string, decimal>(ARDocType.Invoice, retainage);
			else
				return new Tuple<string, decimal>(ARDocType.CreditMemo, retainage);
		}
		...
		public virtual List<BillingData> BillTask(PMProject project, Customer customer, PMTask task, PMBillingRule rule, DateTime billingDate, Dictionary<int, decimal> availableQty, Dictionary<int, PMRecurringItem> billingItems)
		{
			...
			if (rule.FullDetail)
			{
				...
				{
					if (rule.IncludeZeroAmountAndQty == false && tran.InvoicedQty == 0 && tran.InvoicedAmount == 0) continue;
					...
					line.Retainage = line.Amount * line.RetainagePct;
			...
			else
			{
				...
				foreach (Group group in grouping.BreakIntoGroups(billingBase))
				{
					...
					string subCD = CalculateTargetSalesSubaccountCD(rule, project, task, null, line.ResourceID, line.BranchID, line.InventoryID, customer);
		...
		public virtual List<BillingData> BillFixPriceTask(PMProject project, Customer customer, PMTask task, PMBillingRule rule, DateTime billingDate)
		{
			...
			foreach (PXResult<PMRevenueBudget, PMAccountGroup> res in select.Select(task.TaskID))
			{
				...
				proformaLine.Retainage = unbilledAmount * line.RetainagePct;
		...
		public virtual List<BillingData> BillPrepayment(PMProject project, Customer customer, PMTask task, PMBillingRule rule, DateTime billingDate)
		{
			...
			foreach (PXResult<PMRevenueBudget, PMAccountGroup> res in select.Select(task.TaskID))
			{
				...
				proformaLine.Description = PXMessages.LocalizeFormatNoPrefix(Messages.Prepayment);
