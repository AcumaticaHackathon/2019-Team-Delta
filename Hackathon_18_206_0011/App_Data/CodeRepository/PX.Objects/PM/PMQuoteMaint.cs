...
namespace PX.Objects.PM
{
	public class PMQuoteMaint : PXGraph<PMQuoteMaint>, PXImportAttribute.IPXPrepareItems
	{
		...
		[PXSelector(typeof(Search<PMQuoteTask.taskCD, Where<PMQuoteTask.quoteID, Equal<Current<PMQuote.quoteID>>>>), DirtyRead = true)]
		...
		public partial class ConvertToProjectFilter : IBqlTable
		{
			...
			public abstract class createLaborRates : IBqlField { }
			[PXBool()]
			...
			public abstract class activateProject : IBqlField { }
			[PXBool()]
			...
			public abstract class activateTasks : IBqlField { }
			[PXBool()]
			...
			public abstract class moveActivities : IBqlField { }
			[PXBool()]
		...
		[PXHidden]
		[PXCopyPasteHiddenView]
		public PXSetup<Contact, Where<Contact.contactID, Equal<Optional<PMQuote.contactID>>>> Contacts;

		[PXHidden]
		[PXCopyPasteHiddenView]
		public PXSetup<Customer, Where<Customer.bAccountID, Equal<Optional<PMQuote.bAccountID>>>> customer;

		[PXHidden]
		[PXCopyPasteHiddenView]
		public PXSetup<BAccount, Where<BAccount.bAccountID, Equal<Optional<PMQuote.bAccountID>>>> baccount;
		...
		public PXSetup<CR.Location,
			Where<CR.Location.bAccountID, Equal<Current<PMQuote.bAccountID>>,
				And<CR.Location.locationID, Equal<Optional<PMQuote.locationID>>>>> location;
		...
		protected virtual void PMQuote_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			PXUIFieldAttribute.SetEnabled<PMQuote.curyID>(cache, row, false);
			...
			if (row.OpportunityMajorStatus == OpportunityMajorStatusesAttribute._CLOSED)
			{
				cache.RaiseExceptionHandling<PMQuote.opportunityID>(row, row.OpportunityID,
					new PXSetPropertyException(CR.Messages.OpportunityIsClosed, PXErrorLevel.Warning));
			}
		}
		...
		protected virtual void PMQuote_RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
		{
			...
			if (row.OpportunityID != null && row.OpportunityID != dbQuote?.OpportunityID)
			{
				...
				if (oppty != null && oppty.MajorStatus == OpportunityMajorStatusesAttribute._CLOSED)
				{
					sender.RaiseExceptionHandling<PMQuote.opportunityID>(row, row.OpportunityID,
						new PXSetPropertyException(Messages.ClosedOpportunityCannotBeSelected, PXErrorLevel.RowError));
					throw new PXSetPropertyException(Messages.ClosedOpportunityCannotBeSelected, PXErrorLevel.RowError);
				}
			}
		}
		...
		protected virtual void PMQuote_RowPersisted(PXCache sender, PXRowPersistedEventArgs e)
		{
				...
				(e.Operation == PXDBOperation.Insert || e.Operation == PXDBOperation.Update))
			{
				...
				Contact contact = PXSelect<Contact, Where<Contact.contactID, Equal<Required<PMQuote.contactID>>>>.Select(this, row.ContactID);
		...
		protected virtual void _(Events.FieldDefaulting<CROpportunityProducts, CROpportunityProducts.curyUnitCost> e)
		{
			...
			PXCurrencyAttribute.CuryConvCury(e.Cache, e.Row, unitcostInBaseCury, out unitCostInCury);
		...
		public virtual void ConvertQuoteToProject(PMQuote row, ConvertToProjectFilter settings)
		{
			...
			PMProject project = new PMProject() { BaseType=PMProject.ProjectBaseType.Project };
			...
			AddingCostBudgetToProject(projectEntry, taskMap);
		...
		public virtual void RedefaultTasksFromTemplate(PMQuote row)
		{
			...
			{
				Tasks.Insert(new PMQuoteTask() { QuoteID = row.QuoteID, TaskCD = task.TaskCD, Description = task.Description, IsDefault = task.IsDefault, TaxCategoryID = task.TaxCategoryID });
			}
		}
		...
		public virtual void AddingTasksToProject(PMQuote row, ProjectEntry projectEntry, Dictionary<string, int> taskMap)
		{
			...
			foreach (PMQuoteTask task in resultset)
			{
				...
				if (templateTask != null)
				{
					...
					ConfigureNewTask(newTask, task);
				}
				else
				{
					newTask = projectEntry.Tasks.Insert(ConfigureNewTask(new PMTask(), task));
				}
			...
			if (!tasksAdded)
			{
				PMQuoteTask defaultTask = new PMQuoteTask() { TaskCD = DefaultTaskCD, Description = "Default", IsDefault = true };
				PMTask newTask = projectEntry.Tasks.Insert(ConfigureNewTask(new PMTask(), defaultTask));
				taskMap.Add(defaultTask.TaskCD, newTask.TaskID.GetValueOrDefault());
			}
		}
		...
		public virtual void AddingCostBudgetToProject(ProjectEntry projectEntry, Dictionary<string, int> taskMap)
		{
			...
			foreach (CROpportunityProducts item in Products.Select())
			{
				...
				BudgetKeyTuple key = new BudgetKeyTuple(projectEntry.Project.Current.ContractID.GetValueOrDefault(), taskMap[taskcd], item.ExpenseAccountGroupID.GetValueOrDefault(), item.InventoryID.GetValueOrDefault(PMInventorySelectorAttribute.EmptyInventoryID), item.CostCodeID.GetValueOrDefault(CostCodeAttribute.GetDefaultCostCode()));
				...
				if (budget.TryGetValue(key, out existing))
				{
					...
					existing.Amount += item.ExtCost.GetValueOrDefault();

					//Note: MaxAmount is just used to store the aggregate of UnitPrice. (has nothing to do with MaxAmount)
					existing.MaxAmount += item.Qty.GetValueOrDefault() * item.UnitPrice.GetValueOrDefault();
				}
				else
				{
					...
					existing.Qty = item.Qty.GetValueOrDefault();
					existing.Amount = item.ExtCost.GetValueOrDefault();
					existing.UnitPrice = item.UnitPrice.GetValueOrDefault();
			...
			foreach(PMCostBudget item in budget.Values)
			{
				if (item.Qty.GetValueOrDefault() != 0)
					item.Rate = item.Amount / item.Qty.Value;

				if (item.Qty.GetValueOrDefault() != 0)
					item.UnitPrice = item.MaxAmount / item.Qty.Value;
				item.MaxAmount = null;

				string description = item.Description;
				var newItem = projectEntry.CostBudget.Insert(item);

				if (!aggregated.Contains(item.GetBudgetKey()))
				{
		...
		public virtual void AddingRevenueBudgetToProject(ProjectEntry projectEntry, Dictionary<string, int> taskMap, string budgetLevel)
		{
			...
			foreach (CROpportunityProducts item in Products.Select())
			{
				...
				if (budgetLevel == BudgetLevels.Item && item.InventoryID != null)
				{
					inventoryID = item.InventoryID.Value;
				}

				int costcodeID = CostCodeAttribute.GetDefaultCostCode();
				if (budgetLevel == BudgetLevels.CostCode && item.CostCodeID != null)
				{
				...
				if (budget.TryGetValue(key, out existing))
				{
					...
					existing.Amount += item.Amount.GetValueOrDefault();
				}
				else
				{
		...
		public class MultiCurrency : CRMultiCurrencyGraph<PMQuoteMaint, PMQuote>
		{
			...
			{
				CurySource source = base.CurrentSourceSelect();
				if (source == null) source = new CurySource();
				source.AllowOverrideCury = false;
				source.CuryID = Base.Accessinfo.BaseCuryID;
				return source;
			}
		}
					...
					case PXTaxCheck.Line:
							...
							.SelectMultiBound(graph, currents))
						{
							...
							if (tail.TryGetValue(record.TaxID, out line))
							{
								...
								for (idx = ret.Count;
									(idx > 0)
									&& String.Compare(((Tax)(PXResult<CR.CROpportunityTax, Tax, TaxRev>)ret[idx - 1]).TaxCalcLevel, ((Tax)line).TaxCalcLevel) > 0;
									idx--) ;
