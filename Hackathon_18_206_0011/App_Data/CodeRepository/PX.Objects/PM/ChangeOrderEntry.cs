...
namespace PX.Objects.PM
{
	public class ChangeOrderEntry : PXGraph<ChangeOrderEntry, PMChangeOrder>, PXImportAttribute.IPXPrepareItems, IGraphWithInitialization
	{
		...
		public PXSetup<PMProject, Where<PMProject.contractID, Equal<Current<PMChangeOrder.projectID>>>> Project;
		...
		public PXSetup<Customer, Where<Customer.bAccountID, Equal<Optional<PMChangeOrder.customerID>>>> Customer;
		...
		public virtual IEnumerable costBudget()
		{
			...
			{
				PMBudget budget = IsValidKey(record) ? GetOriginalCostBudget(record.GetBudgetKey()) : null;
		...
		public virtual IEnumerable revenueBudget()
		{
			...
			{
				PMBudget budget = IsValidKey(record) ? GetOriginalRevenueBudget(record.GetBudgetKey()) : null;
				if (budget == null) budget = new PMBudget();
		...
		public virtual IEnumerable availableCostBudget()
		{
			...
			{
				existing.Add(((PMChangeOrderCostBudget)res).GetBudgetKey());
			}
			...
			foreach (PMBudget budget in GetCostBudget() )
			{
				...
				if (existing.Contains(budget.GetBudgetKey()))
		...
		public virtual IEnumerable availableRevenueBudget()
		{
			...
			{
				existing.Add(((PMChangeOrderRevenueBudget)res).GetBudgetKey());
			}
		...
		public IEnumerable availablePOLines()
		{
			...
			And<POLinePM.cancelled, NotEqual<True>, 
			And<POLinePM.completed, NotEqual<True>>>>>>>>>>>(this);
		...
		public PXSetup<PMChangeOrderClass, Where<PMChangeOrderClass.classID, Equal<Optional<PMChangeOrder.classID>>>> ChangeOrderClass;
		...
		public PXAction<PMChangeOrder> proformaReport;
		...
		protected virtual void _(Events.RowSelected<PMChangeOrder> e)
		{
			...
			if (e.Row != null)
			{
				...
				PXUIFieldAttribute.SetVisible<PMChangeOrderCostBudget.inventoryID>(CostBudget.Cache, null, !CostCodeAttribute.UseCostCode() || budgetLevel == BudgetLevels.Item);
				
				PXUIVisibility costCodeIDVisibility = CostCodeAttribute.UseCostCode() && (budgetLevel == BudgetLevels.CostCode || IsCopyPasteContext || IsExport || IsImport || enforceVisibility) ? PXUIVisibility.Visible : PXUIVisibility.Invisible;

				PXUIFieldAttribute.SetVisible<PMChangeOrderRevenueBudget.inventoryID>(RevenueBudget.Cache, null, budgetLevel == BudgetLevels.Item);
				PXUIFieldAttribute.SetVisible<PMChangeOrderRevenueBudget.costCodeID>(RevenueBudget.Cache, null, CostCodeAttribute.UseCostCode() && budgetLevel == BudgetLevels.CostCode);
				PXUIFieldAttribute.SetVisibility<PMChangeOrderRevenueBudget.costCodeID>(RevenueBudget.Cache, null, costCodeIDVisibility);
				
				PXUIFieldAttribute.SetVisible<PMBudget.committedAmount>(Caches[typeof(PMBudget)], null, Setup.Current.CostCommitmentTracking == true);
				...
				PXUIFieldAttribute.SetVisible<PMBudget.committedInvoicedAmount>(Caches[typeof(PMBudget)], null, Setup.Current.CostCommitmentTracking == true);
				...
				PXUIFieldAttribute.SetVisible<PMBudget.committedOpenAmount>(Caches[typeof(PMBudget)], null, Setup.Current.CostCommitmentTracking == true);
				...
				PXUIFieldAttribute.SetVisible<PMBudget.committedCOAmount>(Caches[typeof(PMBudget)], null, Setup.Current.CostCommitmentTracking == true);
				...
				PXUIFieldAttribute.SetVisibility<PMBudget.committedAmount>(Caches[typeof(PMBudget)], null, Setup.Current.CostCommitmentTracking == true ? PXUIVisibility.Visible : PXUIVisibility.Invisible);
				...
				PXUIFieldAttribute.SetVisibility<PMBudget.committedInvoicedAmount>(Caches[typeof(PMBudget)], null, Setup.Current.CostCommitmentTracking == true ? PXUIVisibility.Visible : PXUIVisibility.Invisible);
				...
				PXUIFieldAttribute.SetVisibility<PMBudget.committedOpenAmount>(Caches[typeof(PMBudget)], null, Setup.Current.CostCommitmentTracking == true ? PXUIVisibility.Visible : PXUIVisibility.Invisible);
				...
				e.Row.IsRevenueVisible = ChangeOrderClass.Current?.IsRevenueBudgetEnabled == true;
				e.Row.IsCostVisible = ChangeOrderClass.Current?.IsCostBudgetEnabled == true;
				e.Row.IsDetailsVisible = ChangeOrderClass.Current?.IsPurchaseOrderEnabled == true;
		...
		{
			if (Project.Current != null && Project.Current.LastChangeOrderNumber == e.Row.ProjectNbr)
			{
		...
		protected virtual void _(Events.RowSelected<PMChangeOrderCostBudget> e)
		{
			...
			PMBudget budget = IsValidKey(e.Row) ? GetOriginalCostBudget(e.Row.GetBudgetKey()) : null;

			PXUIFieldAttribute.SetEnabled<PMChangeOrderCostBudget.uOM>(e.Cache, e.Row, budget == null);
			PXUIFieldAttribute.SetEnabled<PMChangeOrderCostBudget.rate>(e.Cache, e.Row, budget == null);
		}
		...
		{
			PMCostBudget budget = IsValidKey(e.Row) ? GetOriginalCostBudget(e.Row.GetBudgetKey()) : null;
			if (budget != null)
			{
		...
		protected virtual void _(Events.RowSelected<PMChangeOrderRevenueBudget> e)
		{
			...
			PMRevenueBudget budget = IsValidKey(e.Row) ? GetOriginalRevenueBudget(e.Row.GetBudgetKey()) : null;
			PXUIFieldAttribute.SetEnabled<PMChangeOrderRevenueBudget.uOM>(e.Cache, e.Row, budget == null);
			PXUIFieldAttribute.SetEnabled<PMChangeOrderRevenueBudget.rate>(e.Cache, e.Row, budget == null);
		}
		...
		protected virtual void _(Events.RowUpdated<PMChangeOrderLine> e)
		{
			...
			else if (e.Row.POLineNbr != e.OldRow.POLineNbr)
			{
				if (IsValidKey(e.Row))
				{
					...
					if (poLine != null)
					{
						...
						e.Row.UnitCost = poLine.UnitCost;
			...
			if (IsValidKey(e.Row))
			{
				e.Row.LineType = ChangeOrderLineType.Update;
			}
		...
		protected virtual void _(Events.RowSelected<PMChangeOrderLine> e)
		{
			...
			if (e.Row != null)
			{
				...
				PXUIFieldAttribute.SetEnabled<PMChangeOrderLine.unitCost>(e.Cache, e.Row, !referencesPOLine);
		...
		protected virtual void _(Events.FieldVerifying<PMChangeOrderLine, PMChangeOrderLine.amount> e)
		{
			...
			if (newValue < 0)
			{
				...
				{
					POLinePM poLine = GetPOLine(e.Row);
					if (poLine != null && poLine.CalcCuryOpenAmt.GetValueOrDefault() < Math.Abs(newValue.Value))
					{
						throw new PXSetPropertyException(Messages.CommitmentCannotbeDecreased);
					}
		...
		{
			e.Cache.SetValueExt<PMChangeOrderLine.amountInBaseCury>(e.Row, ConvertToBase(e.Row));
		}
		...
		public virtual Dictionary<BudgetKeyTuple, decimal> BuildBudgetStatsOnDraftChangeOrders()
		{
				...
				, typeof(PMChangeOrderBudget.amount), typeof(PMChangeOrderBudget.amount)))
			{
				...
				{
					drafts.Add(record.GetBudgetKey(), record.Amount.GetValueOrDefault());
				}
			}
		...
		public virtual decimal GetDraftChangeOrderBudgetAmount(PMChangeOrderBudget record)
		{
			...
			decimal result = 0;
			draftChangeOrderBudgetStats.TryGetValue(record.GetBudgetKey(), out result);
		...
		public virtual Dictionary<BudgetKeyTuple, PMCostBudget> BuildCostBudgetLookup()
		{
			...
			{
				result.Add(record.GetBudgetKey(), record);
			}
		...
		public virtual void ReleaseDocument(PMChangeOrder doc)
		{
			...
			using (PXTransactionScope ts = new PXTransactionScope())
			{
				...
				foreach (PXResult<PMChangeOrderLine, POLinePM> res in Details.Select())
				{
					...
					Details.Cache.SmartSetStatus(line, PXEntryStatus.Updated);
				}
		...
		public virtual void ReverseDocument()
		{
			...
			foreach (CSAnswers answer in Answers.Select(sourceNoteID))
			{
				CSAnswers dstanswer =
		...
		public virtual void ModifyExistingOrder(POOrder order, List<PMChangeOrderLine> updated, List<PMChangeOrderLine> added)
		{
			...
			target.SkipProjectLockCommitmentsVerification = true;
			
			if (updated.Count > 0)
				target.Transactions.Select();
			...
			foreach (PMChangeOrderLine line in updated)
			{
				...
				poLine.CuryLineAmt += line.Amount.GetValueOrDefault();
			...
			{
				POLine poLine = target.Transactions.Insert(CreatePOLineFromChangeOrderLine(line));
		...
		{
			return string.Format("{0}.{1}.{2}", order.VendorID, order.OrderType, order.OrderNbr);
		}
		...
		public virtual void InitBudgetFields(PMChangeOrderBudget record, PMBudget budget)
		{
			if (record != null)
			{
				...
				if (IsValidKey(record))
					record.CommittedCOAmount = GetCurrentCommittedCOAmount(record.GetBudgetKey());

				if (budget != null)
				{
					record.RevisedAmount = budget.RevisedAmount.GetValueOrDefault();
					var rollupQty = BalanceCalculator.CalculateRollupQty<PMChangeOrderBudget>(record, budget);
					record.RevisedQty = rollupQty.Qty.GetValueOrDefault() + budget.RevisedQty.GetValueOrDefault();

					record.PreviouslyApprovedAmount = budget.ChangeOrderAmount.GetValueOrDefault();
					record.PreviouslyApprovedQty = budget.ChangeOrderQty.GetValueOrDefault();

					if (IsValidKey(record))
						record.CommittedCOQty = GetCurrentCommittedCOQty(budget.GetBudgetKey(), budget);
					...
					{
						record.PreviouslyApprovedAmount -= record.Amount.GetValueOrDefault();
						record.PreviouslyApprovedQty -= record.Qty.GetValueOrDefault();
					}
				}
				else
				{
					if (!string.IsNullOrEmpty(record.UOM) )
					{
						...
						if (IsValidKey(record))
							record.CommittedCOQty = GetCurrentCommittedCOQty(record.GetBudgetKey(), record);
					}
				}
			}
		}
		...
		public virtual decimal GetCurrentCommittedCOQty(BudgetKeyTuple key, IQuantify budget)
		{
			...
			foreach (PMChangeOrderLine line in Details.Select())
			{
				if (line.CostCodeID.GetValueOrDefault(CostCodeAttribute.GetDefaultCostCode()) == key.CostCodeID && line.InventoryID.GetValueOrDefault(PMInventorySelectorAttribute.EmptyInventoryID) == key.InventoryID && line.TaskID == key.ProjectTaskID && GetProjectedAccountGroup(line) == key.AccountGroupID)
				{
					var rollupQty = BalanceCalculator.CalculateRollupQty<PMChangeOrderLine>(line, budget);
					if (rollupQty.Qty.GetValueOrDefault() != 0)
					{
						qty += rollupQty.Qty.GetValueOrDefault();
					}
				}
			}
		...
		public virtual void InitDetailLineFields(PMChangeOrderLine line)
		{
			if (line != null)
			{
				...
				if (poLine != null)
				{
					line.PotentialRevisedAmount += poLine.CuryExtCost.GetValueOrDefault();
		...
		public class POLineFilter : IBqlTable
		{
			...
			#region CostCodeFrom
			public abstract class costCodeFrom : PX.Data.IBqlField
			{
			}
			...
			#endregion
			#region CostCodeTo
			public abstract class costCodeTo : PX.Data.IBqlField
			{
			}
