...
namespace PX.Objects.PM
{
		...
		where Where : IBqlWhere, new()
	{
		...
		public virtual void RollUpCommitmentBalance(PXCache sender, PMCommitment row, int sign)
		{
			...
			ProjectBalance.RollupQty rollupOrigQty = null;
			ProjectBalance.RollupQty rollupQty = null;
			ProjectBalance.RollupQty rollupOpenQty = null;
			ProjectBalance.RollupQty rollupReceivedQty = null;
			ProjectBalance.RollupQty rollupInvoicedQty = null;


			if (CostCodeAttribute.UseCostCode())
			{
				if (pb.Settings.CostBudgetUpdateMode == CostBudgetUpdateModes.Detailed && ag.IsExpense == true)
				{
					if (existing == null || existing.CostCodeID == CostCodeAttribute.GetDefaultCostCode())
					{
						rollupOrigQty = new ProjectBalance.RollupQty(row.UOM, row.OrigQty);
						rollupQty = new ProjectBalance.RollupQty(row.UOM, row.Qty);
						rollupOpenQty = new ProjectBalance.RollupQty(row.UOM, row.OpenQty);
						rollupReceivedQty = new ProjectBalance.RollupQty(row.UOM, row.ReceivedQty);
						rollupInvoicedQty = new ProjectBalance.RollupQty(row.UOM, row.InvoicedQty);

						if (row.CostCodeID != null)
						{
			...
			{
				if (pb.Settings.CostBudgetUpdateMode == CostBudgetUpdateModes.Detailed && ag.IsExpense == true && row.InventoryID != pb.Settings.EmptyInventoryID)
				{
					if (existing == null || existing.InventoryID == pb.Settings.EmptyInventoryID)
					{
						rollupOrigQty = new ProjectBalance.RollupQty(row.UOM, row.OrigQty);
						rollupQty = new ProjectBalance.RollupQty(row.UOM, row.Qty);
						rollupOpenQty = new ProjectBalance.RollupQty(row.UOM, row.OpenQty);
			...
			if (rollupOrigQty == null)
				rollupOrigQty = pb.CalculateRollupQty<PMCommitment>(row, existing, row.OrigQty);
			if (rollupQty == null)
				rollupQty = pb.CalculateRollupQty<PMCommitment>(row, existing, row.Qty);
			if (rollupOpenQty == null)
				rollupOpenQty = pb.CalculateRollupQty<PMCommitment>(row, existing, row.OpenQty);
			if (rollupReceivedQty == null)
				rollupReceivedQty = pb.CalculateRollupQty<PMCommitment>(row, existing, row.ReceivedQty);
			if (rollupInvoicedQty == null)
				rollupInvoicedQty = pb.CalculateRollupQty<PMCommitment>(row, existing, row.InvoicedQty);
			...
			else
			{
				...
				if (CostCodeAttribute.UseCostCode())
				{
					PMCostCode costCode = PXSelect<PMCostCode, Where<PMCostCode.costCodeID, Equal<Required<PMCostCode.costCodeID>>>>.Select(sender.Graph, ps.CostCodeID);
					if (costCode != null)
					{
			...
			ps.CommittedOrigQty += sign * rollupOrigQty.Qty;
	...
	public class POCommitmentAttribute : PMCommitmentAttribute
	{
		...
		protected override bool EraseCommitment(PXCache sender, object row)
		{
			...
			if (order.OrderType == POOrderType.Blanket || order.OrderType == POOrderType.StandardBlanket || order.Hold == true || order.Approved != true || ((order.Cancelled == true || poline.Cancelled == true) && poline.ReceivedQty == 0 && poline.BilledQty == 0) || poline.TaskID == null)
			{
				return true;
			}
		...
		protected Decimal? CalculateOpenAmount(Decimal? openQty, Decimal? polineOrderQty, Decimal? polineExtCost, Decimal? polineUnbilledAmt)
		{
			if (polineOrderQty.GetValueOrDefault() == 0) return polineUnbilledAmt;
			Decimal unitCost = polineExtCost.GetValueOrDefault() / polineOrderQty.GetValueOrDefault();
