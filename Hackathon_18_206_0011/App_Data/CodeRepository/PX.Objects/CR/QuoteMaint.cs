...
namespace PX.Objects.CR
{
    public class QuoteMaint : PXGraph<QuoteMaint>, PXImportAttribute.IPXPrepareItems, IGraphWithInitialization
    {
        ...
        protected virtual IEnumerable Cancel(PXAdapter adapter)
        {
            ...
            {
                if (!IsImport && Quote.Cache.GetStatus(quote) == PXEntryStatus.Inserted
                    && (oppID != quote.OpportunityID || string.IsNullOrEmpty(quote.QuoteNbr)))
                {
                    bool oldallow = Quote.Cache.AllowInsert;
                    Quote.Cache.AllowInsert = false;
                    foreach (CRQuote first in First.Press(adapter))
                    {
                        if (first.OpportunityID != null)
                        {
                            Quote.Cache.AllowInsert = oldallow;
                            return new object[] { first };                            
                        }                            
                    }
                    Quote.Cache.AllowInsert = oldallow;
	                Quote.Cache.SetStatus(quote, PXEntryStatus.Inserted);
        ...
        {
            foreach (CRQuote item in adapter.Get())
            {
        ...
        public void CopyToQuote(CRQuote currentquote, CopyQuoteFilter param)
        {
        ...
        {            
			...
			if (row.OpportunityMajorStatus == OpportunityMajorStatusesAttribute._CLOSED)
			{
				cache.RaiseExceptionHandling<CRQuote.opportunityID>(row, row.OpportunityID, 
					new PXSetPropertyException(Messages.OpportunityIsClosed, PXErrorLevel.Warning));
			}
		}
        ...
        protected virtual void CRQuote_RowDeleting(PXCache sender, PXRowDeletingEventArgs e)
        {
            ...
            {
                sender.RaiseExceptionHandling<CRQuote.isPrimary>(e.Row, null, new PXSetPropertyException(ErrorMessages.PrimaryQuote));
                e.Cancel = true;
            }
        }
        ...
        protected virtual void CRQuote_RowPersisted(PXCache sender, PXRowPersistedEventArgs e)
        {
                ...
                (e.Operation == PXDBOperation.Insert || e.Operation == PXDBOperation.Update))
            {
                ...
                Contact contact = PXSelect<Contact, Where<Contact.contactID, Equal<Required<CRQuote.contactID>>>>.Select(this, row.ContactID);
        ...
        public class SalesTax : TaxGraph<QuoteMaint, CRQuote>
        {
            ...
            protected override void CalcDocTotals(object row, decimal CuryTaxTotal, decimal CuryInclTaxTotal, decimal CuryWhTaxTotal)
            {
                ...
                CRQuote doc = (CRQuote)this.Documents.Cache.GetMain<Extensions.SalesTax.Document>(this.Documents.Current);
