...
namespace PX.Objects.CR
{
	public class OpportunityMaint : PXGraph<OpportunityMaint>, PXImportAttribute.IPXPrepareItems, IGraphWithInitialization
	{
		...
		public partial class CreateSalesOrderFilter : IBqlTable
		{
		    ...
		    public abstract class recalculatePrices : IBqlField { }
		    [PXBool()]
		    [PXDefault(false)]
		    ...
		    public abstract class recalculateDiscounts : IBqlField { }
		    [PXBool()]
			[PXDefault(false)]
			...
			public abstract class confirmManualAmount : IBqlField { }
			[PXBool()]
			[PXDefault(false)]
	    ...
	    public partial class CreateQuotesFilter : IBqlTable
	    {
            ...
            public abstract class addProductsFromOpportunity : IBqlField { }
	        [PXBool()]
            ...
            public abstract class makeNewQuotePrimary : IBqlField { }
	        [PXBool()]
            [PXDefault(true, PersistingCheck = PXPersistingCheck.Nothing)]
		...
		[PXCopyPasteHiddenView]
	    public PXFilter<CreateInvoicesFilter> InvoiceInfo;
		...
		public OpportunityMaint()
		{
		    ...
		    actionsFolder.AddMenuAction(createAccount);
            actionsFolder.AddMenuAction(createContact);
            actionsFolder.AddMenuAction(createSalesOrder);
            actionsFolder.AddMenuAction(createInvoice);
            actionsFolder.AddMenuAction(submitQuote);
            actionsFolder.AddMenuAction(editQuote);
		...
		public virtual IEnumerable CreateInvoice(PXAdapter adapter)
		{
		    ...
		    if (opportunity.BAccountID == null)
		    {
		        ...
		        {
		            Actions[nameof(CreateAccount)].Press();
		        }
			...
			var stokItems = PXSelectJoin<InventoryItem,
				InnerJoin<CROpportunityProducts, On<CROpportunityProducts.inventoryID, Equal<InventoryItem.inventoryID>>,
				InnerJoin<CROpportunity, On<CROpportunity.quoteNoteID, Equal<CROpportunityProducts.quoteID>>>>,
			...
			if (stokItems.Count > 0)
				throw new PXException(Messages.StockitemsCanNotBeIncludedIninvoice);
	    ...
	    public virtual void ConvertToAccount(CROpportunity opportunity, CreateAccountsFilter param)
		{
			BusinessAccountMaint accountMaint = CreateInstance<BusinessAccountMaint>();
	    ...
	    public virtual IEnumerable CreateQuote(PXAdapter adapter)
	    {
            foreach (CROpportunity opportunity in adapter.Get())
            {
				...
				if (QuoteInfo.Current.QuoteType == CRQuoteTypeAttribute.Project && OpportunityCurrent.Current.CuryID != (Accessinfo.BaseCuryID ?? new PXSetup<PX.Objects.GL.Company>(this).Current?.BaseCuryID))
				{
        ...
        public void CreateNewQuote(CROpportunity opportunity, CreateQuotesFilter param, WebDialogResult result)
	    {
	        ...
	        if (param.AddProductsFromOpportunity == true && !IsSingleQuote(quote.OpportunityID))
	        {
				...
				CloneView(Taxes.View, graph, quote.QuoteID);
	        ...
	        if (opportunity.AllowOverrideContactAddress == true)
	        {
				CloneView(Opportunity_Contact.View, graph, quote.QuoteID);
				quote.OpportunityContactID = graph.Quote_Contact.Current.ContactID;
				CloneView(Opportunity_Address.View, graph, quote.QuoteID);
		...
		private void CloneView(PXView view, PXGraph graph, Guid? quoteId)
		{
		...
		{
			if (this.Quotes.Current?.ProjectID != null)
			{
				var quote = this.Quotes.Current;
				this.Persist();
				var pmProjectMaint = PXGraph.CreateInstance<PM.ProjectEntry>();
				pmProjectMaint.Project.Current = PXSelect<PM.PMProject, Where<PM.PMProject.contractID, Equal<Required<PM.PMProject.contractID>>>>.Select(pmProjectMaint, this.Quotes.Current?.ProjectID);
				if (pmProjectMaint.Project.Current != null)
					PXRedirectHelper.TryRedirect(pmProjectMaint, PXRedirectHelper.WindowMode.InlineWindow);
			}
			return adapter.Get();
		}
        ...
        protected virtual void CROpportunity_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			PXUIFieldAttribute.SetEnabled<CROpportunity.status>(sender, e.Row, true);
			...
			createQuote
				.SetEnabled(Opportunity.Current.MajorStatus != OpportunityMajorStatusesAttribute._CLOSED);

			primaryQuote
				.SetEnabled(hasQuotes && Opportunity.Current.MajorStatus != OpportunityMajorStatusesAttribute._CLOSED);
			...
			createSalesOrder
				.SetEnabled(hasProducts && (!hasQuotes || primaryQt.QuoteType == CRQuoteTypeAttribute.Distribution));

		    editQuote
				.SetEnabled(primaryQt != null && primaryQt.Status != CRQuoteStatusAttribute.Draft);

            submitQuote
				.SetEnabled(primaryQt != null && primaryQt.Status == CRQuoteStatusAttribute.Draft && Opportunity.Current.MajorStatus != OpportunityMajorStatusesAttribute._CLOSED);
			...
			createAccount
				.SetEnabled(row.BAccountID == null);

            createContact
				.SetEnabled(row.ContactID == null || row.AllowOverrideContactAddress == true);

            PXUIFieldAttribute.SetVisible<CROpportunityProducts.curyUnitCost>(this.Products.Cache, null, PXAccess.FeatureInstalled<FeaturesSet.serviceManagementModule>() && PXAccess.FeatureInstalled<FeaturesSet.distributionModule>());
            PXUIFieldAttribute.SetVisible<CROpportunityProducts.vendorID>(this.Products.Cache, null, PXAccess.FeatureInstalled<FeaturesSet.distributionModule>());
            PXUIFieldAttribute.SetVisible<CROpportunityProducts.pOCreate>(this.Products.Cache, null, PXAccess.FeatureInstalled<FeaturesSet.distributionModule>());
        }
		...
		protected virtual void CROpportunity_RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
		{
		    ...
		    if (IsExternalTax(Opportunity.Current.TaxZoneID) && (!sender.ObjectsEqual<CROpportunity.contactID, CROpportunity.taxZoneID, CROpportunity.branchID, CROpportunity.locationID,
                                                       CROpportunity.curyAmount, CROpportunity.opportunityAddressID>(e.Row, e.OldRow) || 
		...
		protected virtual void CROpportunity_RowPersisted(PXCache sender, PXRowPersistedEventArgs e)
		{
				 ...
				 (e.Operation == PXDBOperation.Insert || e.Operation == PXDBOperation.Update ))
			{
				...
				if (contact != null && contact.ContactType == ContactTypesAttribute.Lead)
				{
					...
					Contacts.Cache.Persist(PXDBOperation.Update);					
				}
			}
        ...
        protected virtual void CreateAccountsFilter_AccountClass_FieldDefaulting(PXCache sender, PXFieldDefaultingEventArgs e)
        {
            ...
            CROpportunityClass opcls = PXSelect<CROpportunityClass, 
                                            ...
                                            Equal<Required<CROpportunity.classID>>>>.Select(this, opportunity.ClassID);
            if (opcls != null )
            {
		...
		protected virtual void CreateQuotesFilter_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			if (row.QuoteType == CRQuoteTypeAttribute.Project && OpportunityCurrent.Current.CuryID != (Accessinfo.BaseCuryID ?? new PXSetup<PX.Objects.GL.Company>(this).Current?.BaseCuryID))
			{
		...
		public virtual bool PrepareImportRow(string viewName, IDictionary keys, IDictionary values)
        {
            if (string.Compare(viewName, "Products", true) == 0)
            {
                if (values.Contains("opportunityID"))
                    values["opportunityID"] = Opportunity.Current.OpportunityID;
                else
                    values.Add("opportunityID", Opportunity.Current.OpportunityID);
            }
        ...
        public class Discount : DiscountGraph<OpportunityMaint, CROpportunity>
        {
            public override void Initialize()
            {
               ...
               if(this.Discounts == null)
                ...
                Base.actionsFolder.AddMenuAction(recalculatePrices);
            }
            protected override DocumentMapping GetDocumentMapping()
            {
	        ...
	        protected virtual void _(Events.RowSelected<Extensions.Discount.Document> e)
	        {
		        this.recalculatePrices.SetEnabled((e.Cache.GetMain(e.Row) as CROpportunity).MajorStatus != OpportunityMajorStatusesAttribute._CLOSED
					&& (e.Cache.GetMain(e.Row) as CROpportunity).PrimaryQuoteType == CRQuoteTypeAttribute.Distribution);
	        }
        } 
	    ...
	    public class SalesTax : TaxGraph<OpportunityMaint, CROpportunity>
	    {
            ...
            protected override void CalcDocTotals(object row, decimal CuryTaxTotal, decimal CuryInclTaxTotal, decimal CuryWhTaxTotal)
            {
                ...
                CROpportunity doc = (CROpportunity )this.Documents.Cache.GetMain<Extensions.SalesTax.Document>(this.Documents.Current);
		...
		public class ContactAddress : ContactAddressGraph<OpportunityMaint>
		{
            ...
            protected override CRContact GetCurrentContact()
			{
				var contact = Base.Opportunity_Contact.SelectSingle();
				return contact;
			}
			protected override CRContact GetEtalonContact()
			{
			...
			protected override CRAddress GetCurrentAddress()
			{
				var address = Base.Opportunity_Address.SelectSingle();
				return address;
			}
			protected override CRAddress GetEtalonAddress()
			{
