...
namespace PX.Objects.CR
{
	public class OpportunityMaintExternalTax : ExternalTaxBase<OpportunityMaint, CROpportunity>
	{
		...
		{
			if (IsExternalTax(Base.Opportunity.Current.TaxZoneID))
			{
				Base.Opportunity.Current.IsTaxValid = false;
				if (Base.Opportunity.Cache.GetStatus(Base.Opportunity.Current) == PXEntryStatus.Notchanged)
					Base.Opportunity.Cache.SetStatus(Base.Opportunity.Current, PXEntryStatus.Updated);
			}
		}
		...
		protected GetTaxRequest BuildGetTaxRequest(CROpportunity order)
		{
			if (order == null)
		...
		{
			CRAddress crShipAddress = PXSelect<CRAddress, Where<CRAddress.addressID, Equal<Required<CRAddress.addressID>>>>.Select(Base, order.OpportunityAddressID);
