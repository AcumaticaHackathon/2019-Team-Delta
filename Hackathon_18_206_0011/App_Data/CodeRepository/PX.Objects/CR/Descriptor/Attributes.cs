...
namespace PX.Objects.CR
{
            ...
            private void GetCountryList()
            {
                ...
                foreach (Definition.CountyStateLocale item in this.locales)
                {
					var queryExpr = new SubQuery(new Query().Field(new Column("ValueString", "CountryKvExt"))
		                ...
		                .Where(new Column("RecordID", "CountryKvExt").Equal(new Column("NoteID", "Country")
			                .And(new Column("FieldName", "CountryKvExt").Equal(new PX.Data.SQLTree.Constant(item.Country)))))
		                .Limit(1));
            ...
            private void GetStateList()
            {
                ...
                foreach (Definition.CountyStateLocale item in this.locales)
                {
	                var queryExpr = new SubQuery(new Query().Field(new Column("ValueString", "StateKvExt"))
		                ...
		                .Where(new Column("RecordID", "StateKvExt").Equal(new Column("NoteID", "State")
			                .And(new Column("FieldName", "StateKvExt").Equal(new PX.Data.SQLTree.Constant(item.State)))))
		                .Limit(1));
	                fields.Add(new PXDataField(queryExpr));
				}
                int columnsCount = fields.ToArray().Length;
    ...
    public class CRQuoteStatusAttribute : PXStringListAttribute
	{
		...
		public sealed class draft : Constant<string>
		{
			public draft() : base(Draft) { }
		}

		public sealed class approved : Constant<string>
		{
			public approved() : base(Approved) { }
		}

		public sealed class sent : Constant<string>
		{
			public sent() : base(Sent) { }
		}

		public sealed class pendingApproval : Constant<string>
		{
    ...
    public class CRQuoteTypeAttribute : PXStringListAttribute
    {
        ...
        public sealed class distribution : Constant<string>
        {
            public distribution() : base(Distribution) { }
        }

        public sealed class project : Constant<string>
        {
    ...
    public class CROpportunityProductLineTypeAttribute : PXStringListAttribute
    {
        ...
        public sealed class scopeOfWork : Constant<string>
        {
        ...
        private Definition Definitions
        {
            get
            {
                ...
                if (defs == null)
                {
					...
					defs = PXContext.SetSlot<Definition>(PXDatabase.GetSlot<Definition>(typeof(Definition).FullName + currentLanguage, typeof(CROpportunityProbability), typeof(CROpportunityClassProbability)));
                }
                return defs;
            }
        }
                ...
                using (new PXConnectionScope())
                {
                    ...
                    List<CROppClassStage> allStages =
                        PXDatabase.SelectMulti<CROpportunityProbability>(
							...
							PXDBLocalizableStringAttribute.GetValueSelect(typeof(CROpportunityProbability).Name, typeof(CROpportunityProbability.name).Name, false),
	...
	public class CRRoleTypeList
	{
		...
		public sealed class licensee : Constant<string>
		{
		...
		public sealed class source : Constant<string>
		{
