...
namespace PX.Objects.FA
{
	...
	public class AssetMaint : PXGraph<AssetMaint, FixedAsset>
	{
		...
		{
			MasterFinPeriod currentPeriod = PXSelectJoin<MasterFinPeriod,
				InnerJoin<FABookBalance, On<MasterFinPeriod.finPeriodID, Equal<FABookBalance.currDeprPeriod>>>,
				Where<FABookBalance.assetID, Equal<Current<FixedAsset.assetID>>,
					And<FABookBalance.updateGL, Equal<True>,
					And<MasterFinPeriod.endDate, Greater<MasterFinPeriod.startDate>>>>>.Select(this);
		...
		public override void Persist()
		{
			...
			if (asset != null && det.ReceiptDate != null)
			{
				...
				if (glbal == null)
				{
					currentLocation.PeriodID = FinPeriodRepository.FindFinPeriodByDate(det.ReceiptDate, FinPeriod.organizationID.MasterValue)?.FinPeriodID;
					currentLocation.TransactionDate = det.ReceiptDate;
				}
				else if ((previousLocation == null && string.IsNullOrEmpty(currentLocation.PeriodID))
							|| trbal == null
		...
		protected virtual void PtdCalculatedFieldSelecting(PXCache sender, PXFieldSelectingEventArgs e, int fieldNbr, bool calc)
		{
			...
			FABookPeriod exist_period = PXSelect<
				...
				.Select(this, bookID, ((FAHistory)e.Row).FinPeriodID);
		...
		public virtual IEnumerable booksheethistory()
		{
			...
			foreach (PXResult<FABookPeriod, FABookHistory> res in PXSelectJoin<
				...
				.Select(this, bal.DeprFromYear, toYear))
			{
		...
		public virtual IEnumerable assethistory()
		{
			...
			foreach (PXResult<FABookPeriod, FABookHistory> res in PXSelectReadonly2<
				...
				.Select(this, history_from, history_to))
			{
				FABookPeriod period = res;
				FABookHistory hist = res;
		...
		protected virtual void GLTranFilter_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			if (filter.TranDate != null)
			{
				finPeriod = FinPeriodRepository.GetFinPeriodByDate(filter.TranDate, PXAccess.GetParentOrganizationID(filter.BranchID));
			}
			...
			PXUIFieldAttribute.SetWarning<GLTranFilter.periodID>(sender, filter,
																		finPeriod?.FAClosed == true && glsetup.Current.PostClosedPeriods == false
	...
	public partial class DisposeParams : IBqlTable
	{
		...
		[FinPeriodSelector(typeof(Search2<
	...
	public partial class ReverseDisposalInfo : DisposeParams
	{
		...
		[FinPeriodSelector(typeof(Search2<
			FinPeriod.finPeriodID,
			LeftJoin<FABookHistory, 
				On<FinPeriod.finPeriodID, Equal<FABookHistory.finPeriodID>,
				And<FABookHistory.assetID, Equal<Current<FixedAsset.assetID>>>>,
			LeftJoin<FABook, 
			...
			Where<FinPeriod.fAClosed, NotEqual<True>,
				...
				And<FinPeriod.startDate, NotEqual<FinPeriod.endDate>,
				And<Where<FABook.updateGL, Equal<True>, 
					Or<FABook.updateGL, IsNull>>>>>>>), 
			typeof(ReverseDisposalInfo.reverseDisposalDate),
