...
namespace PX.Objects.FA
{
			...
			if (balance.BookID == null)
	{
		...
		public int GetFinPeriodLengthInDays(int yearNumber, int periodNumberInYear)
		{
			...
			int endPeriod = yearNumber == this.RecoveryYears ? this.DepreciationStopPeriod : GetPeriodsInYearByNumber(yearNumber);
			int currentYear = this.DepreciationStartYear + yearNumber - 1;

			if (this.AveragingConvention == FAAveragingConvention.FullDay && yearNumber == 1 && periodNumberInYear == startPeriod)
			{
				DateTime previousEndDate = DateTime.MinValue;
			...
			if (this.AveragingConvention == FAAveragingConvention.FullDay && yearNumber == this.RecoveryYears && periodNumberInYear == endPeriod)
			{
				DateTime previousEndDate = DateTime.MinValue;
				DateTime periodStartDate = GetPeriodFromDate(this.RecoveryStartDate).StartDate.Value;
				return GetDaysOnPeriod(periodStartDate, this.RecoveryEndDate, currentYear, periodNumberInYear, ref previousEndDate) + 1;
			}
		...
		{
				startPeriodID = FABookPeriodIDAttribute.PeriodPlusPeriod(graph, startPeriodID, 1, assetBalance.BookID);
				}
