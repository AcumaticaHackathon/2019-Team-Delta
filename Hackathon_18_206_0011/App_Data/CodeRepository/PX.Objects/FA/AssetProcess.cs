	...
	public class AssetProcess : PXGraph<AssetProcess>
	{
		...
		public static List<FABookBalance> PrepareDisposal(TransactionEntry docgraph, FixedAsset asset, FADetails assetdet, bool IsMassProcess, bool deprBeforeDisposal)
		{
			...
			string disposalPeriod = assetdet.DisposalPeriodID ??
									docgraph.GetService<IFinPeriodRepository>().FindFinPeriodByDate(assetdet.DisposalDate, PXAccess.GetParentOrganizationID(asset.BranchID))?.FinPeriodID;
		...
		public static void DisposeAsset(TransactionEntry docgraph, FixedAsset asset, IEnumerable<FABookBalance> books, FASetup setup, string reason)
		{
			...
			foreach (FABookBalance bal in books)
			{
				...
				FABookPeriod bookperiod = PXSelect<
					FABookPeriod, 
					Where<FABookPeriod.finPeriodID, Equal<Current<FABookBalance.deprToPeriod>>>>
					.SelectSingleBound(docgraph, new object[] { bookbal });
		...
		public static DocumentList<FARegister> SplitAsset(FixedAsset asset, DateTime? splitDate, string splitPeriodID, bool deprBeforeSplit, Dictionary<FixedAsset, decimal> splits)
		{
			...
			foreach (KeyValuePair<FixedAsset, decimal> split in splits)
			{
				foreach (FABookBalance sbal in PXSelect<FABookBalance, Where<FABookBalance.assetID, Equal<Required<FixedAsset.assetID>>>>.Select(docgraph, split.Key.AssetID))
				{
					...
					string splitPeriodIDByDate = FABookPeriodIDAttribute.PeriodFromDate(docgraph, splitDate, bal.BookID);
					...
					case PXDBOperation.Insert:
						...
						if (initMode)
						{
							...
							foreach (FABookHist hist in PXSelect<
								...
								.Select(docgraph, book.Key.BookID, book.Key.DeprFromPeriod, book.Key.LastDeprPeriod)
			...
			foreach (FABookBalance item in books)
			{
				...
				try
				{
					using (PXTransactionScope ts = new PXTransactionScope())
					{
						...
						foreach (PXResult<FABookHistory, FABookPeriod, FixedAsset> res in PXSelectJoin<
							FABookHistory, 
			...
			using (PXTransactionScope ts = new PXTransactionScope())
			{
																																	...
																																	And<FATran.released, Equal<False>>>>.Select(this, doc.RefNbr))
				{
					...
					if (doc.Origin == FARegister.origin.Transfer && !string.IsNullOrEmpty(bookbal.CurrDeprPeriod))
					{
						closedPeriods.AddRange(PXSelect<
							...
							Where<FABookPeriod.bookID, Equal<Required<FABookPeriod.bookID>>,
								...
								And<FABookPeriod.finPeriodID, Less<Required<FABookPeriod.finPeriodID>>,
								And<FABookPeriod.endDate, Greater<FABookPeriod.startDate>>>>>>
							.Select(this, fatran.BookID, bookbal.CurrDeprPeriod, fatran.FinPeriodID));
					}
					if (doc.Origin == FARegister.origin.Transfer && string.IsNullOrEmpty(bookbal.CurrDeprPeriod) && !string.IsNullOrEmpty(bookbal.LastDeprPeriod))
					{
						closedPeriods.AddRange(PXSelect<
							...
							Where<FABookPeriod.bookID, Equal<Required<FABookPeriod.bookID>>,
								...
								And<FABookPeriod.finPeriodID, Less<Required<FABookPeriod.finPeriodID>>,
								And<FABookPeriod.endDate, Greater<FABookPeriod.startDate>>>>>>
							.Select(this, fatran.BookID, bookbal.LastDeprPeriod, fatran.FinPeriodID));
					}
					if (doc.Origin == FARegister.origin.Split && fatran.TranType == FATran.tranType.DepreciationPlus)
					{
						closedPeriods.AddRange(PXSelect<
							...
							Where<FABookPeriod.bookID, Equal<Required<FABookPeriod.bookID>>, 
								...
								And<FABookPeriod.finPeriodID, Less<Required<FABookPeriod.finPeriodID>>, 
								And<FABookPeriod.endDate, Greater<FABookPeriod.startDate>>>>>>
							.Select(this, fatran.BookID, bookbal.DeprFromPeriod, fatran.FinPeriodID));
					}
					if ((fatran.TranType == FATran.tranType.PurchasingDisposal || (doc.Origin == FARegister.origin.Split && (fatran.TranType == FATran.tranType.DepreciationPlus || fatran.TranType == FATran.tranType.DepreciationMinus))) && string.Compare(fatran.FinPeriodID, bookbal.DeprToPeriod) > 0 && bookbal.Status == FixedAssetStatus.FullyDepreciated)
					{
						closedPeriods.AddRange(PXSelect<
							...
							Where<FABookPeriod.bookID, Equal<Required<FABookPeriod.bookID>>,
								...
								And<FABookPeriod.endDate, Greater<FABookPeriod.startDate>>>>>>
							.Select(this, fatran.BookID, bookbal.DeprToPeriod, fatran.FinPeriodID));
					}
					if (fatran.TranType == FATran.tranType.PurchasingReversal)
					{
						...
						closedPeriods.AddRange(PXSelect<
							...
							Where<FABookPeriod.bookID, Equal<Required<FABookPeriod.bookID>>,
								...
								And<FABookPeriod.finPeriodID, Less<Required<FABookPeriod.finPeriodID>>,
								And<FABookPeriod.endDate, Greater<FABookPeriod.startDate>>>>>>
							.Select(this, fatran.BookID, disptran != null ? disptran.FinPeriodID : fatran.FinPeriodID, fatran.FinPeriodID));
					}
					...
					if (doc.Origin == FARegister.origin.Reversal)
					{
						...
						{
							// Nearest open period
							OrganizationFinPeriod glperiod = GetService<IFinPeriodUtils>()
						...
						fatran.TranType == "R+" || fatran.TranType == "R-")
					{
						...
						{
							// Nearest open period
							OrganizationFinPeriod period = GetService<IFinPeriodUtils>()
								.GetNearestOpenOrganizationFinPeriodInSubledger<OrganizationFinPeriod.fAClosed>(
									fatran.FinPeriodID,
									fatran.BranchID);
							...
							bookbal.CurrDeprPeriod = bookbal.Depreciate != true 
														...
														: isFullyDepreciated 
															...
															: IsDepreciatedInCurrentDeprPeriodNotByDisposal(this, asset.AssetID, bookbal.BookID, hist.FinPeriodID)
																? FABookPeriodIDAttribute.PeriodPlusPeriod(this, hist.FinPeriodID, 1, bookbal.BookID)
															: hist.FinPeriodID;
		...
		public static void TransferAsset(PXGraph graph, FixedAsset asset, FALocationHistory location, ref FARegister register)
		{
			...
			int? newFASubID;
			int? newAccumulatedDepreciationSubID;
			int? newDepreciatedExpenseSubID;

			if (fasetup.UpdateGL == true)
			{
				newFASubID = AssetMaint.MakeSubID<FixedAsset.fASubMask, FixedAsset.fASubID>(assetCache, asset);
				newAccumulatedDepreciationSubID = AssetMaint.MakeSubID<FixedAsset.accumDeprSubMask, FixedAsset.accumulatedDepreciationSubID>(assetCache, asset);
				newDepreciatedExpenseSubID = AssetMaint.MakeSubID<FixedAsset.deprExpenceSubMask, FixedAsset.depreciatedExpenseSubID>(assetCache, asset);
			}
			else
			{
				newFASubID = (int?)assetCache.GetValue<FixedAsset.fASubID>(faclass);
				newAccumulatedDepreciationSubID = (int?)assetCache.GetValue<FixedAsset.accumulatedDepreciationSubID>(faclass);
				newDepreciatedExpenseSubID = (int?)assetCache.GetValue<FixedAsset.depreciatedExpenseSubID>(faclass);
			}
			...
			if (location.FAAccountID != oldLocation.FAAccountID ||
				location.FASubID != oldLocation.FASubID ||
				(location.AccumulatedDepreciationAccountID != oldLocation.AccumulatedDepreciationAccountID ||
				location.AccumulatedDepreciationSubID != oldLocation.AccumulatedDepreciationSubID ||
				location.DepreciatedExpenseAccountID != oldLocation.DepreciatedExpenseAccountID ||
				location.DepreciatedExpenseSubID != oldLocation.DepreciatedExpenseSubID) && asset.Depreciable == true) // really transfer
			{
							...
							And<FABook.updateGL, Equal<True>>>>.SelectMultiBound(graph, new object[]{asset})).Count > 0)
			{
				...
				foreach (FABookBalance bal in balset.RowCast<FABookBalance>().Where(bal => bal.YtdDeprBase != 0m && !string.IsNullOrEmpty(location.PeriodID) && oldBranchID != null))
				{
					...
					if ((newAccumulatedDepreciationAccountID != oldAccumulatedDepreciationAccountID || 
						newAccumulatedDepreciationSubID != oldAccumulatedDepreciationSubID || 
						newBranchID != oldBranchID) && asset.Depreciable == true)
					{
				...
				if ((newDepreciatedExpenseAccountID != oldDepreciatedExpenseAccountID || newDepreciatedExpenseSubID != oldDepreciatedExpenseSubID) && asset.Depreciable == true || 
