...
namespace PX.Objects.GL
{
	...
	public class FinPeriodStatusProcess : PXGraph<FinPeriodStatusProcess>
	{
		...
		public class FinPeriodStatusProcessParameters : IBqlTable
		{
			...
			public abstract class filterOrganizationID : IBqlField { }
			...
			public abstract class action : IBqlField
			{
				...
				public class undefined : Constant<string> { public undefined() : base(Undefined) { } }
				public class open : Constant<string> { public open() : base(Open) { } }
				public class close : Constant<string> { public close() : base(Close) { } }
				public class @lock : Constant<string> { public @lock() : base(Lock) { } }
				public class deactivate : Constant<string> { public deactivate() : base(Deactivate) { } }
				public class reopen : Constant<string> { public reopen() : base(Reopen) { } }
				public class unlock : Constant<string> { public unlock() : base(Unlock) { } }
			...
			public abstract class affectedStatus : IBqlField { }
			[PXString(8)]
			...
			public abstract class reopenInSubledgers : IBqlField { }
			[PXBool]
			[PXDefault(false)]
			...
			#region FirstYear
			public abstract class firstYear : IBqlField { }
			...
			#region LastYear
			public abstract class lastYear : IBqlField { }
			[PXString(4, IsFixed = true)]
			[PXFormula(typeof(Default<filterOrganizationID, action>))]
			[PXDefault(typeof(Search2<
				FinYear.year,
		...
		{
			public new abstract class origBatchNbr : IBqlField { }
			public new abstract class origModule : IBqlField { }
			public new abstract class autoReverseCopy : IBqlField { }
		}
		#endregion
		...
		protected virtual void ShowOpenDocuments(IEnumerable<FinPeriod> periods)
		{
			...
			List<(string ReportID, IPXResultset ReportData)> reports = SubledgerClosingGraphTypes
				...
				.SelectMany(closingGraph => closingGraph.GetReportsData(Filter.Current.OrganizationID, fromFinPeriodID, toFinPeriodID))
				.ToList();
			...
			if (reports.Any())
			{
				...
				PXReportRequiredException report = new PXReportRequiredException(reportData, reportID);

				reports
					.Where(tuple => tuple.ReportID != reportID)
					.ForEach(tuple => report.AddSibling(tuple.ReportID, tuple.ReportData));

				throw report;
			}
			else
			{
				Filter.Ask(AP.Messages.NoUnreleasedDocuments, MessageButtons.OK);
			}
		}
		...
		protected virtual bool ConfirmProcessing(List<FinPeriod> finPeriods)
		{
			...
			switch (parameters.Action)
			{
				case FinPeriodStatusProcessParameters.action.Close:
					...
					if(subledgerTitles.Any())
					{
						string message = string.Format(Messages.ConfirmClosingInSubledgers, string.Join(", ", subledgerTitles), this.GetSubledgerTitle<BatchModule.moduleGL>());
