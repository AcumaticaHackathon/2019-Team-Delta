...
namespace PX.Objects.GL
{
	public class JournalEntry : PXGraph<JournalEntry, Batch>, PXImportAttribute.IPXPrepareItems, PX.Objects.GL.IVoucherEntry
	{
		...
		private void SetTransactionsChanged()
		{
			...
			{
				Caches[typeof(GLTran)].MarkUpdated(tran);
			}
		}
		...
		public virtual List<PMTask> CreateProjectTrans()
		{
			...
			if (BatchModule.Current != null && BatchModule.Current.Module != GL.BatchModule.GL)
			{
				...
				foreach (PXResult res in resultset)
				{
					...
					ProjectBalance.Result balance = pb.Calculate(project, pmt, ag, acc, sign);
		...
		protected virtual void Batch_LedgerID_FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
		{
			...
			int?[] branchIDs = GLTranModuleBatNbr.Select().Select(o => ((GLTran)o).BranchID).Distinct().ToArray();
  ...
  protected virtual void Batch_RowInserting(PXCache cache, PXRowInsertingEventArgs e)
	...
	public class PostGraph : PXGraph<PostGraph>
	{
		...
		public virtual Batch ReverseBatchProc(Batch b)
		{
			Batch copy = PXCache<Batch>.CreateCopy(b);
			{
				using (PXTransactionScope ts = new PXTransactionScope())
				{
					...
					try
					{
						copy.FinPeriodID = FinPeriodRepository.NextPeriod(copy.FinPeriodID, organizationID);
					}
		...
		{
			string prevYearID = FinPeriodUtils.GetPreviousYearID(validationStartPeriod);
		...
		{
			if (validationStartPeriod == FinPeriodUtils.GetFirstFinPeriodIDOfYear(validationStartPeriod))
			{
				return;
			}
		...
		{
			string nextYearID = FinPeriodUtils.GetNextYearID(fromFinPeriodId);

			string firstPeriodIdOfNextYear = FinPeriodUtils.GetFirstFinPeriodIDOfYear(nextYearID);
			...
			foreach (GLHistory netIncomeHistory in PXSelectGroupBy<
				...
				.Select(
					this,
					FinPeriodUtils.GetFirstFinPeriodIDOfYear(fromFinPeriodId),
					fromFinPeriodId,
					niacct.AccountID,
					ledger.LedgerID))
			{
		...
		public virtual void IntegrityCheckProc(Ledger ledger, string startingPeriod)
		{
			...
			PXRowInserting inserted_handler = new PXRowInserting((sender, e) =>
			{
				...
				historyRow.YearClosed = 
					...
					&& historyRow.FinPeriodID == FinPeriodUtils.GetFirstFinPeriodIDOfYear(historyRow.FinPeriodID);
			});
		...
		public virtual void UpdateHistoryProc(IEnumerable<PXResult<GLTran, CATran, Account, Ledger>> glHistoryUpdateData)
		{
			...
			foreach (PXResult<GLTran, CATran, Account, Ledger> glHistoryUpdateInfo in glHistoryUpdateData)
			{
				...
				if ((acct.Type == AccountType.Income || acct.Type == AccountType.Expense) && ledger.BalanceType != LedgerBalanceType.Statistical)
				{
					...
					{
						UpdateHistory(zeroTran, reacct, FinPeriodUtils.GetFirstFinPeriodIDOfYear(tran.FinPeriodID), HistoryUpdateAmountType.FinAmounts, HistoryUpdateMode.Common);
						UpdateHistory(zeroTran, reacct, FinPeriodUtils.GetFirstFinPeriodIDOfYear(tran.TranPeriodID), HistoryUpdateAmountType.TranAmounts, HistoryUpdateMode.Common);

						var firstFinPeriodIDOfNextYear = FinPeriodUtils.GetFirstFinPeriodIDOfYear(FinPeriodUtils.GetNextYearID(tran.FinPeriodID));
						...
						var firstTranPeriodIDOfNextYear = FinPeriodUtils.GetFirstFinPeriodIDOfYear(FinPeriodUtils.GetNextYearID(tran.TranPeriodID));
				...
				else
				{
					...
					if ((ledger.BalanceType == LedgerBalanceType.Actual || ledger.BalanceType == LedgerBalanceType.Report) && acct.AccountID == reacct.AccountID)
					{
						...
						UpdateHistory(retran, niacct, FinPeriodUtils.GetFirstFinPeriodIDOfYear(tran.FinPeriodID), HistoryUpdateAmountType.FinAmounts, HistoryUpdateMode.Common);
						UpdateHistory(retran, reacct, FinPeriodUtils.GetFirstFinPeriodIDOfYear(tran.FinPeriodID), HistoryUpdateAmountType.FinAmounts, HistoryUpdateMode.Common);
					}
				}
		...
		protected virtual Batch CreateInterCompany(Batch b)
		{
			...
			var transForBalancing = PXSelectJoin<GLTran,
													...
													OrderBy<Asc<GLTran.module,
															Asc<GLTran.batchNbr,
															Asc<GLTran.lineNbr>>>>>
													.Select(this, b.Module, b.BatchNbr, batchBranch.OrganizationID, b.BranchID, batchOrganization.OrganizationType)
		...
		public virtual void PostBatchProc(Batch b, bool createintercompany)
		{
			...
			ICollection<PXResult<GLTran, CATran, Account, Ledger>> transWithData =
				GLTran_CATran_Module_BatNbr.Select(b.Module, b.BatchNbr)
					...
					if (tran.IsReclassReverse == true)
					{
                        ...
                        int rowCount = PXUpdate<
							...
							Where<GLTran.module, Equal<Required<GLTran.module>>,
								And<GLTran.batchNbr, Equal<Required<GLTran.batchNbr>>,
								And<GLTran.lineNbr, Equal<Required<GLTran.lineNbr>>,
								And<Mult<IIf<Where<Add<GLTran.curyDebitAmt, GLTran.curyCreditAmt>, Greater<Zero>>, int1, Minus<int1>>,
			...
			if ((e.Operation & PXDBOperation.Command) == PXDBOperation.Insert && e.TranStatus == PXTranStatus.Open)
			{
				...
				if (hist != null && hist.AccountID == reacct && hist.FinPeriodID.Substring(4, 2) == "01" && hist.YearClosed != true)
				{
					...
					LeftJoin<AcctHist3, //Retained Earnings
						...
						And<AcctHist3.finPeriodID, LessEqual<Required<GLHistoryByPeriod.finPeriodID>>>>>>>>,
					...
					Where<AcctHist.accountID, Equal<Required<AcctHist.accountID>>,
						...
						And<AcctHist.yearClosed, Equal<False>>>>>>>>.Update(sender.Graph, niacct, prevFinPeriod?.FinPeriodID, prevFinPeriod?.FinPeriodID, reacct, hist.SubID, hist.LedgerID, hist.BranchID, hist.FinPeriodID);
				}
			}
		}
	}
