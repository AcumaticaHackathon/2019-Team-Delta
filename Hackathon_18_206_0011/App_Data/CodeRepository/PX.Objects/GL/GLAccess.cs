...
namespace PX.Objects.GL
{
	...
	public class GLAccessDetail : PX.SM.UserAccess
	{
		...
		[PXSelector(typeof(Search<PX.SM.SMPrinter.printerName>))]
	...
	public class GLAccess : PX.SM.BaseAccess
	{
		...
		public GLAccess()
		{
			...
			PXUIFieldAttribute.SetEnabled<PX.SM.SMPrinter.printerName>(Printers.Cache, null);
		}
		...
		protected virtual void SMPrinter_RowInserted(PXCache sender, PXRowInsertedEventArgs e)
		{
			if (e.Row != null)
			{
				PX.SM.SMPrinter printer = PXSelectorAttribute.Select<PX.SM.SMPrinter.printerName>(sender, e.Row) as PX.SM.SMPrinter;
				if (printer != null)
				{
