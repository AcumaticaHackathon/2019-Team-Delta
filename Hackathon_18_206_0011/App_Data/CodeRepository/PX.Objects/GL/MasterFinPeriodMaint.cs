...
using PX.Objects.GL.ClassExtensions;
...
{
	public class MasterFinPeriodMaint : PXGraph<MasterFinPeriodMaint, MasterFinYear>
	{
		...
		protected virtual void CheckParametersOfCalendarGeneration(int fromYear, int toYear, int minLastYear, int maxFirstYear)
		{
			if (fromYear > minLastYear + 1)
			{
				throw new PXException(Messages.FromYearGreaterThanLastExistingYear, minLastYear + 1);
			}
			if (toYear < maxFirstYear - 1)
			{
				throw new PXException(Messages.ToYearLessThanFirstExistingYear, maxFirstYear - 1);
			}
		...
		protected virtual void GenerateMasterCalendar(int fromYear, int toYear, int minLastYear, int maxFirstYear)
		{
			if (HasInsertedYear)
			{
		...
		protected virtual void FillCurrentYearWithPeriods(string status = null)
		{
		...
		public PXAction<MasterFinYear> GenerateYears;
		[PXButton]
		...
		{
			List<MasterFinYear> existingYears = PXSelectReadonly3<MasterFinYear, OrderBy<Asc<MasterFinYear.year>>>
				.Select(this)
				.RowCast<MasterFinYear>()
				.ToList();

			int? firstExistingYear = existingYears.Any() ? (int?)int.Parse(existingYears.FirstOrDefault()?.Year) : null;
			int? lastExistingYear = existingYears.Any() ? (int?)int.Parse(existingYears.LastOrDefault()?.Year) : null;

			if (GenerateParams.AskExtFullyValid((graph, viewName) =>
			{
				FinPeriodGenerateParameters parameters = GenerateParams.Current;
				parameters.FromYear =
				parameters.ToYear = lastExistingYear == null ? FiscalYear.Current.Year : (lastExistingYear + 1).ToString();
			...
			DialogAnswerType.Positive))
			{
				...
				MasterFinPeriodMaint processingGraph = this.Clone();
				PXLongOperation.StartOperation(
					this, 
					delegate()
					{
						processingGraph.CheckParametersOfCalendarGeneration(fromYear, toYear, lastExistingYear ?? fromYear - 1, firstExistingYear ?? fromYear - 1);
						string firstFinYear = processingGraph.YearSetup.Current.FirstFinYear;
						if (int.Parse(firstFinYear) > fromYear)
						{
							FiscalYearSetupMaint setupGraph = PXGraph.CreateInstance<FiscalYearSetupMaint>();
							setupGraph.FiscalYearSetup.Current = processingGraph.YearSetup.Current;
							setupGraph.ShiftBackFirstYearTo(fromYear.ToString());
						}
						processingGraph.GenerateMasterCalendar(fromYear, toYear, lastExistingYear ?? fromYear - 1, firstExistingYear ?? fromYear - 1);
					});
			}
		...
		{
			MasterFinYear year = e.Row;

			if (!IsLastMasterFinYear(year))
		{
			...
			if (canDelete)
				canDelete = !IsMasterFinYearReferenced(year.Year);
			...
			if (GetNextYear(year) != null)
			{
				throw new PXException(Messages.DeleteSubseqYears);
			}

			if (year.StartDate.Value.Month != YearSetup.Current.PeriodsStartDate.Value.Month ||
			  year.StartDate.Value.Day != YearSetup.Current.PeriodsStartDate.Value.Day)
			{
				FinYearSetup setupYear = YearSetup.Current;
		...
		private bool IsMasterFinYearReferenced(string year)
		{
			using (new PXConnectionScope())
			{
				GLTran tran = ReferencingByTranQuery
				.WhereNew<Where<MasterFinPeriod.finYear, Equal<Required<MasterFinYear.year>>>>()
				.SelectSingleReadonly<GLTran>(this, year);
				Batch batch = ReferencingByBatchQuery
					.WhereAnd<Where<MasterFinPeriod.finYear, Equal<Required<MasterFinYear.year>>>>()
					.SelectSingleReadonly<Batch>(this, year);
		...
		private bool IsMasterFinPeriodReferenced(string finPeriodID)
		{
			...
			{
				GLTran tran = ReferencingByTranQuery
				.WhereNew<Where<MasterFinPeriod.finPeriodID, Equal<Required<MasterFinPeriod.finPeriodID>>>>()
				.SelectSingleReadonly<GLTran>(this, finPeriodID);
				Batch batch = ReferencingByBatchQuery
					.WhereAnd<Where<MasterFinPeriod.finPeriodID, Equal<Required<MasterFinPeriod.finPeriodID>>>>()
					.SelectSingleReadonly<Batch>(this, finPeriodID);
				return (batch != null || tran != null);
			}
		}

		private bool IsLastMasterFinYear(MasterFinYear masterYear)
		{
			return !PXSelectReadonly<
				MasterFinYear,
				Where<MasterFinYear.year, Greater<Current<MasterFinYear.year>>>>
				.SelectSingleBound(this, new object[] { masterYear })
				.RowCast<MasterFinYear>()
				.Any();
		...
		protected virtual void MasterFinYear_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			MasterFinYear nextYear = GetNextYear(year);
			...
			GenerateYears.SetEnabled(!isInsertedCustomYear);

			PXResult<MasterFinYear, FinYearSetup> first = (PXResult<MasterFinYear, FinYearSetup>)PXSelectOrderBy<
				MasterFinYear, 
		...
		protected virtual void MasterFinPeriod_RowInserting(PXCache cache, PXRowInsertingEventArgs e)
		{
			...
			FiscalPeriodCreator<MasterFinYear, MasterFinPeriod, FinPeriodSetup> creator = 
				new FiscalPeriodCreator<MasterFinYear, MasterFinPeriod, FinPeriodSetup>(
					YearSetup.Current, 
					year.Year, 
		...
		protected virtual void MasterFinPeriod_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			if (year == null) return;
			MasterFinYear nextYear = this.GetNextYear(year);
			...
			foreach (Organization organization in organizationsSelect.Select(organisationIDs))
			{
				...
				try
				{
					foreach (MasterFinYear updatedMasterYear in masterYearCache.Updated)
					{
						...
						updatableOrgYear.StartMasterFinPeriodID = FinPeriodUtils.GetFirstFinPeriodIDOfYear(updatedMasterYear);
					...
					foreach (MasterFinYear insertedMasterYear in masterYearCache.Inserted)
					{
						OrganizationFinYear insertedOrgYear = (OrganizationFinYear)orgYearCache.Insert(new OrganizationFinYear
						{
							...
							StartMasterFinPeriodID = FinPeriodUtils.GetFirstFinPeriodIDOfYear(insertedMasterYear),
		...
		public override void Persist()
		{
			...
			using (PXTransactionScope dbTran = new PXTransactionScope())
			{
				...
				{
					MasterFinPeriod lastPeriod = null;
					PXResultset<MasterFinPeriod> allPeriods = Periods.Select(year.Year);
					...
					foreach (MasterFinPeriod period in allPeriods)
					{
						...
						{
							Periods.Cache.RaiseExceptionHandling<MasterFinPeriod.endDateUI>(period, Periods.Current.EndDateUI,
					...
					SynchronizeBaseAndOrganizationPeriods();
				}
		...
		private bool CreateNextYear(MasterFinYear newYear)
		{
			...
			return FiscalYearCreator<MasterFinYear, MasterFinPeriod, FinPeriodSetup>.CreateNextYear(setupValue, lastYear, newYear);
		}
		...
		private bool CreatePrevYear(MasterFinYear newYear)
		{
			...
			MasterFinYear lastYear = FindEarliestYear();

			return FiscalYearCreator<MasterFinYear, MasterFinPeriod, FinPeriodSetup>.CreatePrevYear(setupValue, lastYear, newYear);
		}
		...
		private MasterFinYear GetNextYear(MasterFinYear aYear)
		{
	...
	public partial class FinPeriodGenerateParameters : IBqlTable
	{
		...
		#region FirstMasterFinYear
		public abstract class firstFinYear : IBqlField { }
		/// <summary>
		/// The financial year starting from which the periods will be generated in the system.
		/// </summary>
		[PXString(4, IsFixed = true)]
		...
		#endregion
		#region ToYear
		public abstract class lastFinYear : IBqlField { }
