...
namespace PX.Objects.GL
{
		...
		[PXDefault(
			typeof(Coalesce<Coalesce<
				Search<Organization.actualLedgerID,
					Where<Organization.organizationID, Equal<Current2<AccountByPeriodFilter.organizationID>>>>,
				Search<Branch.ledgerID,
					Where<Branch.branchID, Equal<Current2<AccountByPeriodFilter.branchID>>>>>,
				Search<Branch.ledgerID,
	...
	public class AccountByPeriodEnq : PXGraph<AccountByPeriodEnq>
	{
			...
			get
			{
				...
				if (filter.ShowSummary == true)
				{
					cmd = new PXSelectJoinGroupBy<GLTranR,
                        InnerJoin<ADL.Sub, On<GLTranR.subID, Equal<ADL.Sub.subID>,
						...
						Where<GLTranR.ledgerID, Equal<Current<AccountByPeriodFilter.ledgerID>>,
							...
							And<ADL.Batch.scheduled, NotEqual<True>>>>>>>,
		...
		[PXDBDefault(typeof(ADL.Batch.finPeriodID))]
		[FinPeriodID]
