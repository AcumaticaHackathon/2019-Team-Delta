	...
	public class AllocationProcess : PXGraph<AllocationProcess>
	{
		...
		protected virtual bool RetrieveDestributionByAcctState(out AccountWeightedList aAccounts, GLAllocation allocation, string aFiscalPeriod)
		{
			...
			PXSelectBase<GLHistoryByPeriod> acctHistSelect = new PXSelectJoinGroupBy<GLHistoryByPeriod,
								InnerJoin<Account, On<GLHistoryByPeriod.accountID, Equal<Account.accountID>, And<Match<Account, Current<AccessInfo.userName>>>>,
								InnerJoin<Sub,	On<GLHistoryByPeriod.subID, Equal<Sub.subID>, And<Match<Sub, Current<AccessInfo.userName>>>>,
										...
										And<
											Where2<
												Where<Account.accountID, NotEqual<Current<GLSetup.ytdNetIncAccountID>>, 
													And<Where<Account.type, Equal<AccountType.asset>, Or<Account.type, Equal<AccountType.liability>>>>>,
											Or<Where<GLHistoryByPeriod.lastActivityPeriod, GreaterEqual<Required<GLHistoryByPeriod.lastActivityPeriod>>,
												And<Where<Account.type, Equal<AccountType.expense>, Or<Account.type, Equal<AccountType.income>,	Or<Account.accountID, Equal<Current<GLSetup.ytdNetIncAccountID>>>>>>>>>
										>>>>>>,
												Aggregate<
			...
			string	yearBegPeriod	= FirstPeriodOfYear(FinPeriodUtils.FiscalYear(aFiscalPeriod));
			foreach (PXResult<GLAllocationDestination, Account, Sub> iRes in destSelect.Select(allocation.GLAllocationID))
			{
				...
				{
					foreach (PXResult<GLHistoryByPeriod, Account, Sub, GLHistory, AH> it in acctHistSelect.Select(iBasisBranch, basisAcctCDWildCard, basisSubCDWildCard, aFiscalPeriod, basisLedgerID, yearBegPeriod))
				{
			...
			ref decimal totalCredit)
		{
			...
			aBatchGraph.GetService<IFinPeriodUtils>().ValidateFinPeriod<GLAllocationAccountHistory>(aSources, m => batch.FinPeriodID, m => m.BranchID.SingleToArray());
		...
		protected static void DistributeAmount(JournalEntry aBatchGraph, GLAllocation aAlloc, decimal aAmount, AccountWeightedList aDestAccounts, Ledger aAllocLedger)
		{
			...
			aBatchGraph.GetService<IFinPeriodUtils>().ValidateFinPeriod<AccountWeight>(
