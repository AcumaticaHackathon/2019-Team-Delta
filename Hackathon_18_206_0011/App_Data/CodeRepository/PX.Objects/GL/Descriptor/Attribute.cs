...
namespace PX.Objects.GL
{
	...
	public class FiscalPeriodClosedException : PXSetPropertyException
	{
		...
		public FiscalPeriodClosedException(string finPeriodID, string organizationCD, PXErrorLevel errorLevel, string errorMessageFormat)
	...
	public class FiscalPeriodInactiveException : PXSetPropertyException
	{
		...
		public FiscalPeriodInactiveException(string finPeriodID, string organizationCD, PXErrorLevel errorLevel)
			: base(Messages.FinPeriodIsInactiveInCompany, errorLevel, FinPeriodIDAttribute.FormatForError(finPeriodID), organizationCD)
		{
		}
	...
	public class FiscalPeriodLockedException : PXSetPropertyException
	{
		...
		public FiscalPeriodLockedException(string finPeriodID, string organizationCD, PXErrorLevel errorLevel)
			: base(Messages.FinPeriodIsLockedInCompany, errorLevel, FinPeriodIDAttribute.FormatForError(finPeriodID), organizationCD)
		{
		}
	...
	/// </summary>
	public class FinPeriodIDFormattingAttribute : PXEventSubscriberAttribute, IPXFieldSelectingSubscriber, IPXFieldUpdatingSubscriber, IPXFieldDefaultingSubscriber, IPXRowPersistingSubscriber
	{
		#region State
		...
		public Func<object[], DateTime?, int?, object[]> DefaultingQueryParametersDelegate { get; set; }
		...
		public bool ReredefaultOrRevalidateOnOrganizationSourceUpdated { get; set; }

		public bool CheckFinPeriodExistenceOnSource { get; set; }
		...
		public FinPeriodIDFormattingAttribute(Type sourceType, Type searchType)
			:this(sourceType, searchType, null)
		{
		}
		...
		public FinPeriodIDFormattingAttribute(
			...
			ICalendarOrganizationIDProvider calendarOrganizationIDProvider = null)
		{
			...
			ReredefaultOrRevalidateOnOrganizationSourceUpdated = redefaultOrRevalidateOnOrganizationSourceUpdated;
			CheckFinPeriodExistenceOnSource = checkFinPeriodExistenceForDate;


			CalendarOrganizationIDProvider = calendarOrganizationIDProvider ??
			                                 new CalendarOrganizationIDProvider(
				                                 BranchSourceType,
				                                 BranchSourceFormulaType,
				                                 OrganizationSourceType,
				                                 UseMasterCalendarSourceType,
		...
		public override void CacheAttached(PXCache sender)
		{
				     ...
				     || sender.GetItemType().IsAssignableFrom(dateCache.GetItemType()))
			{
				sender.Graph.FieldUpdated.AddHandler(_sourceType, _sourceField, SourceFieldUpdated);
					...
					if (CheckFinPeriodExistenceOnSource
					    && !sender.Graph.IsImport
					    && !sender.Graph.IsContractBasedAPI)
					{
						sender.Graph.RowUpdating.AddHandler(dateCache.GetItemType(), DateBranchRowUpdating);
			}
		}

				foreach (CalendarOrganizationIDProvider.SourceSpecificationItem sourceSpecification in CalendarOrganizationIDProvider.GetSourcesSpecification(sender, null).SpecificationItems)
				{
					if (sourceSpecification.BranchSourceType != null)
					{
						PXCache branchCache = sender.Graph.Caches[BqlCommand.GetItemType(sourceSpecification.BranchSourceType)];

						if (branchCache.GetItemType().IsAssignableFrom(sender.GetItemType())
						    || sender.GetItemType().IsAssignableFrom(branchCache.GetItemType()))

						{
							if (CheckFinPeriodExistenceOnSource
							    && dateCache.GetItemType() != branchCache.GetItemType()
							    && !sender.Graph.IsImport
							    && !sender.Graph.IsContractBasedAPI)
							{
								sender.Graph.RowUpdating.AddHandler(branchCache.GetItemType(), DateBranchRowUpdating);
							}

							if (ReredefaultOrRevalidateOnOrganizationSourceUpdated)
							{
								sender.Graph.RowUpdated.AddHandler(branchCache.GetItemType(), BranchRowUpdated);
							}
						}
					}
				}
		...
		protected virtual void DateBranchRowUpdating(PXCache cache, PXRowUpdatingEventArgs e)
		{
			DateTime? newDate = (DateTime?) BqlHelper.GetCurrentValue(cache.Graph, SourceFieldType, e.NewRow);
			DateTime? oldDate = (DateTime?) BqlHelper.GetCurrentValue(cache.Graph, SourceFieldType, e.Row);

			int? newCalendarOrgID = CalendarOrganizationIDProvider.GetCalendarOrganizationID(cache.Graph, cache, e.NewRow);
			int? oldCalendarOrgID = CalendarOrganizationIDProvider.GetCalendarOrganizationID(cache.Graph, cache, e.Row);

			if (newDate == oldDate && newCalendarOrgID == oldCalendarOrgID)
				return;
			...
			ValidationResult validationResult = Validate(
				...
				cache, 
				e.NewRow,
				organizationID => CheckFinPeriodExists(cache, e.NewRow, newDate, organizationID));
				...
				foreach (var branchValue in validationResult.BranchValuesWithErrors)
		{
					if (branchValue.SpecificationItem.BranchSourceType != null 
					    && (branchValue.SpecificationItem.BranchSourceFormulaType != null || PXAccess.FeatureInstalled<FeaturesSet.branch>()))
			{
						...
						string organizationCD = PXAccess.GetOrganizationCD(PXAccess.GetParentOrganizationID(branchValue.BranchIDs.Single()));

						branchCache.RaiseExceptionHandling(
							branchValue.SpecificationItem.BranchSourceType.Name, 
							e.NewRow, 
							branchSourceNewValue, 
							new TranDateOutOfRangeException(newDate, organizationCD));

						branchCache.SetValue(
							e.NewRow, 
							branchValue.SpecificationItem.BranchSourceType.Name, 
							branchCache.GetValue(e.Row, branchValue.SpecificationItem.BranchSourceType.Name));
					}
				}

				PXCache dateCache = cache.Graph.Caches[BqlCommand.GetItemType(SourceFieldType)];

				object dateNewValue = GetNewValueByIncomig(dateCache, e.NewRow, SourceFieldType.Name, e.ExternalCall);

				dateCache.RaiseExceptionHandling(
					SourceFieldType.Name, 
					e.NewRow, 
					dateNewValue, 
					new TranDateOutOfRangeException(
						newDate, 
						validationResult.GetOrganizationCDsWithErrors().JoinIntoStringForMessageNoQuotes()));

				dateCache.SetValue(
					e.NewRow, 
					SourceFieldType.Name, 
					dateCache.GetValue(e.Row, SourceFieldType.Name));
			}
		}
		...
		public class ValidationResult
			...
			public List<CalendarOrganizationIDProvider.SourceValuesCollectionItem> BranchValuesWithErrors;
			...
			public ValidationResult()
			{
				...
				BranchValuesWithErrors = new List<CalendarOrganizationIDProvider.SourceValuesCollectionItem>();
				}
			...
			Func<int?, bool?> /*<Calendar OrganizationID, result>*/ validationDelegate)
		{
			...
			List<CalendarOrganizationIDProvider.SourceValuesCollectionItem> branchValues =
			...
			foreach (CalendarOrganizationIDProvider.SourceValuesCollectionItem branchValue in branchValues)
			{
				bool hasError = false;

				foreach (int? branchID in branchValue.BranchIDs)
				{
		...
		protected virtual void RedefaultPeriodID(PXCache cache, object row)
		{
			...
			int? calendarOrganizationID = CalendarOrganizationIDProvider.GetCalendarOrganizationID(cache.Graph, cache, row);

			DateTime? sourcedate = dateCache.GetItemType().IsAssignableFrom(row.GetType())
				? (DateTime?) dateCache.GetValue(row, _sourceField)
				: (DateTime?) dateCache.GetValue(dateCache.Current, _sourceField);

			if ((sourcedate != null || SourceType == null)
			    && calendarOrganizationID != null)
			{
				PeriodResult result = GetPeriod(cache.Graph,
												DefaultTypeRestrictedByOrganization ?? DefaultType,
												sourcedate,
				...
				finally
				{
					...
					if (!haspending && result.StartDate > sourcedate)
					{
			...
			{
				cache.SetValue(row, _FieldName, null);
			}
		}

		protected virtual void BranchRowUpdated(PXCache cache, PXRowUpdatedEventArgs e)
		{
			IEnumerable<int?> branchIDs = CalendarOrganizationIDProvider
				.GetBranchIDsValuesFromField(cache.Graph, cache, e.Row)
				.SelectMany(sourceValues => sourceValues.BranchIDs)
				.Distinct()
				.OrderBy(id => id);

			IEnumerable<int?> oldBranchIDs = CalendarOrganizationIDProvider
				.GetBranchIDsValuesFromField(cache.Graph, cache, e.OldRow)
				.SelectMany(sourceValues => sourceValues.BranchIDs)
				.Distinct()
				.OrderBy(id => id);

			CalendarOrganizationIDProvider.SourcesSpecificationCollection specification = 
				CalendarOrganizationIDProvider.GetSourcesSpecification(cache, e.OldRow);

			IEnumerable<object> oldAdditionalFieldsValues =
				specification.DependsOnFields.Select(fieldType => cache.GetValue(e.OldRow, fieldType.Name));

			IEnumerable<object> additionalFieldsValues =
				specification.DependsOnFields.Select(fieldType => cache.GetValue(e.Row, fieldType.Name));

			if (branchIDs.SequenceEqual(oldBranchIDs)
					&& oldAdditionalFieldsValues.SequenceEqual(additionalFieldsValues))
				return;
			...
			foreach (PXEventSubscriberAttribute attribute in cache.GetAttributesReadonly(e.Row, _FieldName))
			{
				...
				{
					bool hasError = uiFieldAttribute.ErrorLevel == PXErrorLevel.Error|| uiFieldAttribute.ErrorLevel == PXErrorLevel.RowError;

					if (hasError
						|| uiFieldAttribute.ErrorLevel == PXErrorLevel.Warning
					    || uiFieldAttribute.ErrorLevel == PXErrorLevel.RowWarning)
					{
						object errorValue = uiFieldAttribute.ErrorValue;
						object value = hasError
							? errorValue
							: FormatForDisplay((string)cache.GetValue(e.Row, _FieldName));

						cache.RaiseExceptionHandling(_FieldName, e.Row, value, null);

						if (errorValue == null && hasError)
					{
							RedefaultPeriodID(cache, e.Row);
					}
					else
					{
							cache.SetValueExt(e.Row, _FieldName, value);
						}
			...
			string currentValue = (string)cache.GetValue(e.Row, _FieldName);

			if (currentValue != null)
			{
				cache.SetValueExt(e.Row, _FieldName, FormatForDisplay(currentValue));
			}
			else
			{
				RedefaultPeriodID(cache, e.Row);
		...
		public virtual void FieldSelecting(PXCache sender, PXFieldSelectingEventArgs e)
		{
			...
			{
				e.ReturnState = PXStringState.CreateInstance(e.ReturnState, FinPeriodUtils.FULL_LENGHT, null, _FieldName, null, null, CS_DISPLAY_MASK, null, null, null, null);
			}
		}
		...
		public virtual void FieldDefaulting(PXCache sender, PXFieldDefaultingEventArgs e)
		{
			    ...
			    (SourceType == null && (BranchSourceType != null || OrganizationSourceType != null)))
			{
				e.NewValue = (string) GetPeriod(sender.Graph,
					DefaultTypeRestrictedByOrganization ?? DefaultType,
					_sourceDate == DateTime.MinValue ? (DateTime?) null : _sourceDate,
					CalendarOrganizationIDProvider.GetCalendarOrganizationID(sender.Graph, sender, e.Row),
					new[] {e.Row},
					DefaultingQueryParametersDelegate);
			}
		}
		...
		{
			if (_sourceField != null && BranchSourceType != null && CheckFinPeriodExistenceOnSource == true)
			{
				if ((e.Operation & PXDBOperation.Command) == PXDBOperation.Insert
				     || (e.Operation & PXDBOperation.Command) == PXDBOperation.Update)
				{
					DateTime? sourceDate = (DateTime?)sender.GetValue(e.Row, _sourceField);

					ValidationResult validationResult = Validate(
						CalendarOrganizationIDProvider,
						sender,
						e.Row,
						organizationID => CheckFinPeriodExists(sender, e.Row, sourceDate, organizationID));
		...
		{
			return string.IsNullOrEmpty(period) ? null : $"{FinPeriodUtils.PeriodInYear(period)}{FinPeriodUtils.FiscalYear(period)}";
		}
		...
		public static string FormatForStoringNoTrim(string period)
		{
			...
			return string.IsNullOrEmpty(period) ? null : $"{period.Substring(FinPeriodUtils.PERIOD_LENGTH, FinPeriodUtils.YEAR_LENGTH)}{period.Substring(0, FinPeriodUtils.PERIOD_LENGTH)}";
		}
		...
		public static PeriodResult GetPeriod(PXGraph graph, 
											...
											Func<object[], DateTime?, int?, object[]> getParametersDelegate = null,
											Boolean applyFormat = true)
		{
			...
			{
				parameters = getParametersDelegate(currents, fromdate, calendarOrganizationID);
		}
			else
			{
				...
				if (fromdate != null)
		{
				...
				paramList.Add(calendarOrganizationID);
		...
		protected virtual Type GetQueryWithRestrictionByOrganization(Type bqlQueryType)
		{
		...
		//TODO Pan 333 CheckFinPeriodExists by all companies
		protected virtual bool? CheckFinPeriodExists(PXCache cache, object row, DateTime? date, int? calendarOrganizationID)
		{
			if (date == null || calendarOrganizationID == null)
				return null;

			string finPeriodID = GetPeriod(cache.Graph,
				SearchTypeRestrictedByOrganization ?? SearchType,
				date,
				calendarOrganizationID,
				new object[] { row });

			return !string.IsNullOrEmpty(finPeriodID);
		}
		...
		protected virtual void SetUseMasterCalendarValue(PXCache sender, object row)
		{
			...
			//TODO Pank 111 Translations
			bool useMasterCalendarValue = 
				CalendarOrganizationIDProvider.GetBranchIDsValueFromField(sender.Graph, sender, row, CalendarOrganizationIDProvider.GetSourcesSpecification(sender, row).PrimarySpecificationItem) == null
			    && CalendarOrganizationIDProvider.GetOrganizationIDsValueFromField(sender.Graph, sender, row, CalendarOrganizationIDProvider.GetSourcesSpecification(sender, row).PrimarySpecificationItem) == null;
	...
	public class TranPeriodIDFormattingAttribute : FinPeriodIDFormattingAttribute
	{
		public TranPeriodIDFormattingAttribute(Type sourceType = null,
			Type searchType = null,
			Type branchSourceType = null,
			Type branchSourceFormulaType = null,
			Type organizationSourceType = null,
		...
		protected override void RedefaultPeriodID(PXCache cache, object row)
		{
			...
			DateTime? sourcedate = dateCache.GetItemType().IsAssignableFrom(row.GetType())
				? (DateTime?)dateCache.GetValue(row, SourceFieldType.Name)
				: (DateTime?)dateCache.GetValue(dateCache.Current, SourceFieldType.Name);

			int? calendarOrganizationID = CalendarOrganizationIDProvider.GetCalendarOrganizationID(cache.Graph, cache, row);

			if (sourcedate != null && calendarOrganizationID != null)
			{
				PeriodResult result
					= GetPeriod(cache.Graph,
						SearchTypeRestrictedByOrganization ?? SearchType,
						(DateTime)sourcedate,
						calendarOrganizationID,
						new object[] {row });
				...
				cache.SetValuePending(row, _FieldName, newValue);
				cache.SetValueExt(row, _FieldName, newValue);
			}
			else
			{
				cache.SetValue(row, _FieldName, null);
	...
	public class PeriodIDAttribute : PXAggregateAttribute, IPXCommandPreparingSubscriber, IPXRowSelectingSubscriber
	{
		...
		public Type SearchTypeRestrictedByOrganization
		{
			get { return GetFinPeriodIDFormattingAttribute().SearchTypeRestrictedByOrganization; }
			set { GetFinPeriodIDFormattingAttribute().SearchTypeRestrictedByOrganization = value; }
		}

		public Type DefaultType
		{
			get { return GetFinPeriodIDFormattingAttribute().DefaultType; }
			set { GetFinPeriodIDFormattingAttribute().DefaultType = value; }
		}

		public Type DefaultTypeRestrictedByOrganization
		{
			get { return GetFinPeriodIDFormattingAttribute().DefaultTypeRestrictedByOrganization; }
			set { GetFinPeriodIDFormattingAttribute().DefaultTypeRestrictedByOrganization = value; }
		}
		...
		/// <param name="int?">Calendar OrganizationID</param>
		/// <param name="object[]">Result collection</param>
		public Func<object[], DateTime?, int?, object[]> DefaultingQueryParametersDelegate
		{
			get { return GetFinPeriodIDFormattingAttribute().DefaultingQueryParametersDelegate; }
			set { GetFinPeriodIDFormattingAttribute().DefaultingQueryParametersDelegate = value; }
		}

		public Type BranchSourceType
		{
			get { return GetFinPeriodIDFormattingAttribute().BranchSourceType; }
			set { GetFinPeriodIDFormattingAttribute().BranchSourceType = value; }
		}

		public Type OrganizationSourceType
		{
			get { return GetFinPeriodIDFormattingAttribute().OrganizationSourceType; }
			set { GetFinPeriodIDFormattingAttribute().OrganizationSourceType = value; }
		}

		public Type UseMasterCalendarSourceType
		{
			get { return GetFinPeriodIDFormattingAttribute().UseMasterCalendarSourceType; }
			set { GetFinPeriodIDFormattingAttribute().UseMasterCalendarSourceType = value; }
		}
		...
		public PeriodIDAttribute(Type SourceType, Type SearchType)
			: this(SourceType, SearchType, null, null)
		{
		}
		
		public PeriodIDAttribute(Type sourceType = null, 
			Type searchType = null, 
			Type branchSourceType = null, 
			Type branchSourceFormulaType = null,
			Type organizationSourceType = null, 
			...
			Type[] sourceSpecificationTypes = null)
		{
			...
			PXDBStringAttribute dbstr = new PXDBStringAttribute(FinPeriodUtils.FULL_LENGHT);
			dbstr.IsFixed = true;
			_Attributes.Add(dbstr);

			_Attributes.Add(CreateFormattingAttribute(
		...
		protected virtual FinPeriodIDFormattingAttribute CreateFormattingAttribute(Type sourceType = null,
			Type searchType = null,
			Type branchSourceType = null,
			Type branchSourceFormulaType = null,
			Type organizationSourceType = null,
			...
			ICalendarOrganizationIDProvider calendarOrganizationIDProvider = null)
		{
			return new FinPeriodIDFormattingAttribute(
				sourceType: sourceType,
				searchType: searchType,
				branchSourceType: branchSourceType,
				branchSourceFormulaType: branchSourceFormulaType,
		...
		protected FinPeriodIDFormattingAttribute GetFinPeriodIDFormattingAttribute()
			{
	...
	public class TranPeriodIDAttribute : FinPeriodIDAttribute
	{
		...
		public TranPeriodIDAttribute(Type sourceType)
			: this(sourceType, null, null, null)
		{

		}

		public TranPeriodIDAttribute(
				Type sourceType,
				Type branchSourceType = null,
				Type branchSourceFormulaType = null,
				Type organizationSourceType = null,
		...
		protected override FinPeriodIDFormattingAttribute CreateFormattingAttribute(Type sourceType = null,
			Type searchType = null,
			Type branchSourceType = null,
			Type branchSourceFormulaType = null,
			Type organizationSourceType = null,
			...
			ICalendarOrganizationIDProvider calendarOrganizationIDProvider = null)
			{
			return new TranPeriodIDFormattingAttribute(
				sourceType: sourceType,
				searchType: searchType,
				branchSourceType: branchSourceType,
				branchSourceFormulaType: branchSourceFormulaType,
				...
				sourceSpecificationTypes: sourceSpecificationTypes)
		{
			...
			PXSelectorAttribute selectorAttribute =
				...
				?new GenericFinPeriodSelectorAttribute(
					cmd,
					GetFinPeriodIDFormattingAttribute().CalendarOrganizationIDProvider,
	...
	public class FinPeriodNonLockedSelectorAttribute : FinPeriodSelectorAttribute
	{
		#region Ctor
	...
	public class OpenPeriodAttribute : FinPeriodSelectorAttribute, IPXFieldVerifyingSubscriber, IPXFieldDefaultingSubscriber, IPXFieldSelectingSubscriber, IPXRowPersistingSubscriber, IPXRowSelectedSubscriber, IPXFieldUpdatedSubscriber 
	{
		...
		public bool RaiseErrorOnInactiveOrLockedPeriod { get; set; }
		...
		public virtual void OpenPeriodDefaulting(PXCache sender, PXFieldDefaultingEventArgs e)
		{
			...
			if (_sourceDate != DateTime.MinValue)
			{
				int? organizationID = GetFinPeriodIDFormattingAttribute().CalendarOrganizationIDProvider.GetCalendarOrganizationID(sender.Graph, sender, e.Row);

				e.NewValue = (string) FinPeriodIDFormattingAttribute.GetPeriod(sender.Graph,
					DefaultTypeRestrictedByOrganization ?? DefaultType,
					_sourceDate,
					organizationID,
					new[] {e.Row},
		...
		protected virtual void IsCurrentPeriod(PXCache sender, object row, object value)
		{
			...
			{
				int? calendarOrganizationID = GetFinPeriodIDFormattingAttribute().CalendarOrganizationIDProvider.GetCalendarOrganizationID(sender.Graph, sender, row);

				string finPeriodIDFromDate = FinPeriodIDFormattingAttribute.GetPeriod(sender.Graph, 
																					SearchTypeRestrictedByOrganization ?? SearchType, 
																					_sourceDate,
																					calendarOrganizationID,
																					new[] {row},
				...
				if (!object.Equals(finPeriodIDFromDate, userPeriod))
				{
					...
					if (period != null)
						{
						if (object.Equals(period.StartDate, period.EndDate) 
							&& DateTime.Compare((DateTime)_sourceDate, (DateTime)year.StartDate) >= 0 
							&& DateTime.Compare((DateTime)_sourceDate, (DateTime)period.EndDate) < 0)
							{
								return;
							}
						}
		...
		public virtual void Document_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			if (GetValidatePeriod(sender, e.Row) == PeriodValidation.DefaultSelectUpdate)
			{
				...
				{
					sender.RaiseExceptionHandling(_FieldName, e.Row, OldValue, new FiscalPeriodInactiveException(ex.Message, PXErrorLevel.Warning));
				}
				catch (FiscalPeriodLockedException ex)
				{
					sender.RaiseExceptionHandling(_FieldName, e.Row, OldValue, new FiscalPeriodLockedException(ex.Message, PXErrorLevel.Warning));
				}
				catch (PXSetPropertyException ex)
				{
		...
		public virtual void RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
		{
			...
			try
			{
				if ((e.Operation & PXDBOperation.Command) == PXDBOperation.Insert && _ValidatePeriod != PeriodValidation.Nothing
				    || (e.Operation & PXDBOperation.Command) == PXDBOperation.Update && sender.AllowDelete && _ValidatePeriod == PeriodValidation.DefaultSelectUpdate)
				{
			...
			{
				if (RaiseErrorOnInactiveOrLockedPeriod && sender.RaiseExceptionHandling(_FieldName, e.Row, FormatForDisplay(NewValue), ex))
				{
		...
		{
		    ICalendarOrganizationIDProvider calendarOrganizationIDProvider = GetFinPeriodIDFormattingAttribute().CalendarOrganizationIDProvider;
		...
		{
			CalendarOrganizationIDProvider.SourceValuesCollection sourceValues =
				calendarOrganizationIDProvider.GetBasisOrganizationIDsValues(sender.Graph, sender, row);

			int?[] organizationIDs = sourceValues.OrganizationIDs.ToArray();
		...
		protected class PeriodValidationResult : ProcessingResultBase<PeriodValidationMessage>
		{
	...
	public class ClosedPeriodAttribute : FinPeriodSelectorAttribute
	{
		public ClosedPeriodAttribute(Type aSourceType)
			: base(typeof(Search<
				...
				OrderBy<
					Desc<FinPeriod.finPeriodID>>>), aSourceType)
		{
		}
	...
	public class AnyPeriodFilterableAttribute : FinPeriodSelectorAttribute
	{
		...
		public AnyPeriodFilterableAttribute(Type searchType, Type sourceType, Type[] fieldList)
			: this(searchType, sourceType, null, null, null, null, null, false, true, fieldList)
		{ }
	...
	public class BranchAttribute : BranchBaseAttribute
	{
		...
		public BranchAttribute(bool onlyActive)
			: base(onlyActive)
		{
		}
	}
	...
	public abstract class BranchBaseAttribute : AcctSubAttribute, IPXFieldSelectingSubscriber
	{
		...
		protected BranchBaseAttribute(Type sourceType, Type searchType, bool onlyActive = false)
			: base()
		{
			if (onlyActive)
			{
			...
			if (parentrow != null && (parentrow.TranModule == GL.BatchModule.AP))
			{
				taxRevSelect = new PXSelectReadonly2<Tax,
											LeftJoin<TaxRev, On<TaxRev.taxID, Equal<Tax.taxID>, Data.And<TaxRev.outdated, Equal<boolFalse>,
												And2<Where<TaxRev.taxType, Equal<TaxType.purchase>, Data.And<Tax.reverseTax, Equal<boolFalse>, Data.Or<TaxRev.taxType, Equal<TaxType.sales>, And<Where<Tax.reverseTax, Equal<boolTrue>, Data.Or<Tax.taxType, Equal<CSTaxType.use>, Or<Tax.taxType, Equal<CSTaxType.withholding>>>>>>>>, And<Current<GLTranDoc.tranDate>, Between<TaxRev.startDate, TaxRev.endDate>>>>>>,Where>(graph);
			}
	...
	#region printerType
	public sealed class printerType : Constant<string>
	{
		public printerType()
