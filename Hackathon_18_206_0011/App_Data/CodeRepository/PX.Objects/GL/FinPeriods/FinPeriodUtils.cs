...
namespace PX.Objects.GL.FinPeriods
{
	public class FinPeriodUtils : IFinPeriodUtils
	{
		...
		public static bool FinPeriodEqual(string period1, string period2, FinPeriodIDAttribute.FinPeriodComparison comparison)
		{
			if (period1 != null && period2 != null && period1.Length >= FinPeriodUtils.FULL_LENGHT && period2.Length >= FinPeriodUtils.FULL_LENGHT)
			{
				switch (comparison)
				{
					case FinPeriodIDAttribute.FinPeriodComparison.Full:
						break;
					case FinPeriodIDAttribute.FinPeriodComparison.Year:
						period1 = period1.Substring(0, FinPeriodUtils.YEAR_LENGTH);
						period2 = period2.Substring(0, FinPeriodUtils.YEAR_LENGTH);
						break;
					case FinPeriodIDAttribute.FinPeriodComparison.Month:
			...
			where TBranchField : class, IBqlField
		{
			...
			if (finPeriod != null)
			{
					...
					|| isClosed && glsetup != null && glsetup.RestrictAccessToClosedPeriods == true)
				{
					...
					OrganizationFinPeriod firstopen = rowCache.Graph.GetService<IFinPeriodRepository>().FindFirstOpenFinPeriod(finPeriodID, organizationID);
