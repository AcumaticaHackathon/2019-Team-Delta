...
namespace PX.Objects.TX
{
		...
		where TTaxPeriod : TaxPeriod, new()
	{
		...
		public virtual TaxPeriod GetOrCreateCurrentTaxPeriod(PXCache taxYearCache, PXCache taxPeriodCache, int? organizationID, int? taxAgencyID)
		{
			...
			if (first_date != null)
			{
				CreateAndAddToCache(taxYearCache, taxPeriodCache, organizationID, vendor, first_date.Value.Year);
		...
		public virtual TaxYearWithPeriods<TTaxYear, TTaxPeriod> Create(int? organizationID, Vendor vendor, int? calendarYear = null)
		{
			...
			{
				FillCalendarDataWhenLastOrganizationTaxYearDoesNotExist(calendarYear.Value, vendor, creationParams);

				return CreateWithCorrespondingTaxPeriodType(creationParams);
			}
		}

		public void FillCalendarDataWhenLastOrganizationTaxYearDoesNotExist(int calendarYear, Vendor vendor, CreationParams creationParams)
		{
			...
			if (otherOrganizationLastYear != null)
			{
				...
				creationParams.TaxPeriodType = otherOrganizationLastYear.TaxPeriodType;
			}
			else
			{
