...
>>>>>>>>>>>>))]
	...
	public class ReportTaxDetail : PXGraph<ReportTaxDetail>
	{
		...
		public virtual IEnumerable history_Detail()
		{
			...
			foreach (PXResult<TaxReportLine, TaxBucketLine, TaxTranReport, BAccount> res in History_Detail_Expanded.Select(filterRow.OrganizationID, filterRow.BranchID))
			{
	...
	public class ReportTaxReview : PXGraph<ReportTax>
	{
		...
		protected virtual IEnumerable period_Details()
		{
			...
			{
				return Period_Details_Expanded.Select(filter.OrganizationID, filter.BranchID).Where(line => ShowTaxReportLine(line.GetItem<TaxReportLine>(), filter.TaxPeriodID));				
			}
		}
	...
	public class ReportTax : PXGraph<ReportTax>
	{
		...
		public static TaxTran GetEarliestNotReportedTaxTran(PXGraph graph, Vendor taxAgency, int? organizationID, int? branchID)
		{
			return taxAgency.TaxReportFinPeriod == true 
					? GetEarliestNotReportedTaxTranByField<TaxTran.finDate>(graph, taxAgency.BAccountID, organizationID, branchID)
					: GetEarliestNotReportedTaxTranByField<TaxTran.tranDate>(graph, taxAgency.BAccountID, organizationID, branchID);
		}
			...
			where TOrderByField: IBqlField
		{
			...
			return PXSelectJoin<TaxTran,
								...
								.SelectWindowed(graph, 0, 1, taxAgencyID, organizationID, branchID);
			}
		}
				...
				if (taxper.Status != TaxPeriodStatus.Dummy 
					...
					switch (taxper.Status)
					{
						...
						case TaxPeriodStatus.Closed:
							{
						TaxTran tran = GetEarliestNotReportedTaxTran(this, vendor, filter.OrganizationID, filter.BranchID);
		...
		protected virtual void TaxPeriodFilter_RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
		{
			...
			if (!sender.ObjectsEqual<TaxPeriodFilter.organizationID, TaxPeriodFilter.branchID, TaxPeriodFilter.vendorID>(e.Row, e.OldRow))
			{
				List<PXView> views = this.Views.Select(view => view.Value).ToList();
				foreach (var view in views) view.Clear();
	...
	public class ReportTaxProcess : PXGraph<ReportTaxProcess>
	{
				...
				PXResult<TaxPeriod, TaxHistory> result = (PXResult<TaxPeriod, TaxHistory>)
														PXSelectJoin<TaxPeriod,
																	...
																	Where<TaxPeriod.organizationID, Equal<Required<TaxPeriod.organizationID>>,
																			...
																			And<TaxPeriod.taxPeriodID, Equal<Required<TaxPeriod.taxPeriodID>>>>>,
																	OrderBy<Desc<TaxHistory.revisionID>>>
																	.Select(graph, organizationID, vendorId, taxPeriodID, organizationID, branchID);
		...
		public virtual void VoidReportProc(TaxPeriodFilter taxPeriod)
		{
			using (new PXConnectionScope())
			{
				using (PXTransactionScope ts = new PXTransactionScope())
				{
					using (new PXReadBranchRestrictedScope(taxPeriod.OrganizationID.SingleToArray(), taxPeriod.BranchID.SingleToArrayOrNull(), requireAccessForAllSpecified:true))
					{
						PXUpdateJoin<
					...
					using (new PXReadBranchRestrictedScope(taxPeriod.OrganizationID.SingleToArray(), null, requireAccessForAllSpecified:true))
					{
		...
		public virtual void ClosePeriodProc(TaxPeriodFilter p)
		{
			...
			using (PXTransactionScope ts = new PXTransactionScope())
			{
				...
				CheckUnreleasedTaxAdjustmentsDoNotExist(p.TaxPeriodID, p.OrganizationID);

				bool arPPDExist;
				bool apPPDExist;
			    CheckForUnprocessedPPD(this, p.OrganizationID, null, p.VendorID, p.EndDate, out arPPDExist, out apPPDExist);
				...
				using (new PXReadBranchRestrictedScope(p.OrganizationID.SingleToArray(), null, requireAccessForAllSpecified:true))
				{
		...
		private void CheckUnreleasedTaxAdjustmentsDoNotExist(string taxPeriodID, int? organizationID)
		{
			using (new PXReadBranchRestrictedScope(organizationID.SingleToArray(), null, requireAccessForAllSpecified: true))
		{
		...
		public virtual string FileTaxProc(TaxPeriodFilter filter)
		{
			...
			using (new PXConnectionScope())
			{
				using (PXTransactionScope ts = new PXTransactionScope())
				{
					bool arPPDExist;
					bool apPPDExist;
					CheckForUnprocessedPPD(this, filter.OrganizationID, filter.BranchID, filter.VendorID,
										   filter.EndDate, out arPPDExist, out apPPDExist);
					string error = String.Empty;
					error += arPPDExist ? PXMessages.Localize(AR.Messages.UnprocessedPPDExists) : String.Empty;
					error += apPPDExist ? " " + PXMessages.Localize(AP.Messages.UnprocessedPPDExists) : String.Empty;
					...
					int revisionId = ReportTaxProcess.CurrentRevisionId(this, filter.OrganizationID, filter.BranchID,
																		taxPeriod.VendorID, taxPeriod.TaxPeriodID) ?? 1;
					...
					using (new PXReadBranchRestrictedScope(filter.OrganizationID.SingleToArray(), 
														   filter.BranchID.SingleToArrayOrNull(), requireAccessForAllSpecified: true))
					{
					...
					using (new PXReadBranchRestrictedScope(filter.OrganizationID.SingleToArray(), filter.BranchID.SingleToArrayOrNull(), requireAccessForAllSpecified: true))
					{
						foreach (PXResult<TaxReportLine, TaxBucketLine, TaxTranReport> res
								in Period_Details_Expanded.Select(revisionId, taxPeriod.TaxPeriodID, taxPeriod.VendorID)
		...
		{
			arPPDExist = CheckForUnprocessedPPD(graph, organizationID, branchID, vendorID, endDate);
			apPPDExist = CheckAPInvoicesForUnprocessedPPD(graph, organizationID, branchID, vendorID, endDate);
		}
