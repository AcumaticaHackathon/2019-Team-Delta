			...
			foreach (SVATConversionHistExt histSVAT in list)
			{
				...
				try
				{
					...
					using (PXTransactionScope ts = new PXTransactionScope())
					{
						...
						FinPeriod finPeriod = svat.GetService<IFinPeriodRepository>().GetFinPeriodByDate(histSVAT.TaxInvoiceDate, PXAccess.GetParentOrganizationID(histSVAT.AdjdBranchID));
						...
						try
						{
							svat.GetService<IFinPeriodUtils>().ValidateFinPeriod<SVATTaxTran>(taxTranSelection.RowCast<SVATTaxTran>(),
								m => svat.GetService<IFinPeriodRepository>().GetFinPeriodByDate(histSVAT.TaxInvoiceDate, PXAccess.GetParentOrganizationID(m.BranchID)).FinPeriodID,
