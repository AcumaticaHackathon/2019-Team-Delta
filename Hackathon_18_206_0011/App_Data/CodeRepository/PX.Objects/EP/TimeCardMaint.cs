...
namespace PX.Objects.EP
{
	...
	public class TimeCardMaint : PXGraph<TimeCardMaint, EPTimeCard>, PXImportAttribute.IPXPrepareItems
	{
		...
		public virtual IList<EPTimeCardSummary> SelectSummaryRecords(EPTimeCard document, bool autocorrect)
		{
			if (document == null)
			...
			if (employee != null)
			{
				...
				if (currentTimeZone != null)
				{
					...
					if (pref != null && !string.IsNullOrEmpty(pref.TimeZone) && currentTimeZone.Id != pref.TimeZone)
					{
						...
						PXUIFieldAttribute.SetWarning<EPTimeCard.employeeID>(sender, e.Row, PXMessages.LocalizeNoPrefix(Messages.TimeInTimezoneOfEmployee) + empTimeZone.DisplayName);
					}
		...
		protected virtual void EPTimeCardSummary_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			PXUIFieldAttribute.SetEnabled<EPTimeCardSummary.isBillable>(sender, row, isEnabled && ProjectDefaultAttribute.IsBillableProject(this, row.ProjectID));
		...
		//TODO: Review this handler - probabaly obsolete.
		protected virtual void EPTimeCardSummary_RowInserting(PXCache sender, PXRowInsertingEventArgs e)
		{
			...
			if (earningType != null)
			{
                ...
                if (row.IsBillable == null && ProjectDefaultAttribute.IsProject(this, row.ProjectID))
		...
		protected virtual void EPTimecardDetail_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			if (row != null)
			{
				...
				else
				{
					...
					PXUIFieldAttribute.SetEnabled<EPTimecardDetail.isBillable>(sender, row, ProjectDefaultAttribute.IsBillableProject(this, row.ProjectID));
				}
		...
		protected virtual void _(Events.FieldDefaulting<EPTimeCardSummary, EPTimeCardSummary.workCodeID> e)
		{
		...
		protected virtual void _(Events.FieldUpdated<EPTimeCardSummary, EPTimeCardSummary.costCodeID> e)
		{
			e.Cache.SetDefaultExt<EPTimeCardSummary.workCodeID>(e.Row);
		}
		protected virtual void _(Events.FieldDefaulting<EPTimeCardSummary, EPTimeCardSummary.employeeRate> e)
		{
			if (Document.Current != null)
			{
		...
		protected virtual void _(Events.FieldUpdated<EPTimeCardSummary, EPTimeCardSummary.earningType> e)
		{
			e.Cache.SetDefaultExt<EPTimeCardSummary.labourItemID>(e.Row);
			e.Cache.SetDefaultExt<EPTimeCardSummary.employeeRate>(e.Row);
		}
		
		protected virtual void _(Events.FieldUpdated<EPTimeCardSummary, EPTimeCardSummary.projectTaskID> e)
		{
			e.Cache.SetDefaultExt<EPTimeCardSummary.employeeRate>(e.Row);
		}
		protected virtual void _(Events.FieldUpdated<EPTimeCardSummary, EPTimeCardSummary.labourItemID> e)
		{
			e.Cache.SetDefaultExt<EPTimeCardSummary.employeeRate>(e.Row);
		}
		protected virtual void _(Events.FieldUpdated<EPTimeCardSummary, EPTimeCardSummary.certifiedJob> e)
		{
			e.Cache.SetDefaultExt<EPTimeCardSummary.employeeRate>(e.Row);
		}
		...
		public virtual void ProcessRegularTimecard(PM.RegisterEntry releaseGraph, EPTimeCard timecard)
		{
			...
			foreach (EPTimecardDetail act in Activities.Select())
			{
				...
				var cost = CostEngine.CalculateEmployeeCost(act.TimeCardCD, act.EarningTypeID, act.LabourItemID, act.ProjectID, act.ProjectTaskID, act.CertifiedJob, act.UnionID, timecard.EmployeeID, act.Date.Value);
				if (cost == null)
				{
					throw new PXException(Messages.EmployeeCostRateNotFound);
				}

				if (act.EmployeeRate == null)
				{
		...
		{			
			foreach (EPTimeCardSummary summary in SelectSummaryRecords(Document.Current, false))
			{
		...
		public virtual PM.PMTran CreateItemTransaction(EPTimeCardItem record, int? employeeID, DateTime? date, decimal? qty, PXCache tranCache)
		{
			...
			tran.Date = date;
			FinPeriod finPeriod = GetService<IFinPeriodRepository>().GetFinPeriodByDate(tran.Date, PXAccess.GetParentOrganizationID(branchID));
		...
		public class EPTimecardDetail : PMTimeActivity
		{
			...
			#region UnionID
			public new abstract class unionID : PX.Data.IBqlField
			{
			}
			...
			#region LabourItemID
			public new abstract class labourItemID : IBqlField { }
