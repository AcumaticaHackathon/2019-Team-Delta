		...
		public virtual IEnumerable ViewDocumentToApply(PXAdapter adapter)
		{
			...
			if (header?.OrigModule == GL.BatchModule.AP)
			{
				APRegister doc = PXSelect<APRegister, Where<APRegister.docType, Equal<Required<APRegister.docType>>, 
								And<APRegister.refNbr, Equal<Required<APRegister.refNbr>>>>>.Select(this, row.AdjdDocType, row.AdjdRefNbr);
			...
			if (header.OrigModule == GL.BatchModule.AR)
			{
				ARRegister doc = PXSelect<ARRegister, Where<ARRegister.docType, Equal<Required<ARRegister.docType>>,
								And<ARRegister.refNbr, Equal<Required<ARRegister.refNbr>>>>>.Select(this, row.AdjdDocType, row.AdjdRefNbr);
