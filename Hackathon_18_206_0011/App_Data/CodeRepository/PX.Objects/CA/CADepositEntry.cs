...
namespace PX.Objects.CA
{
	...
	public class CADepositEntry : PXGraph<CADepositEntry, CADeposit>
	{
		...
		{
			#region DocType
			public abstract new class docType : IBqlField
			{
			}
			#endregion
			#region RefNbr
			...
			#endregion
			#region DepositDate
			public abstract new class depositDate : IBqlField
			{
			}
			#endregion
			#region DepositType
			public abstract new class depositType : IBqlField
			{
			}
			#endregion
			#region DepositNbr
			public abstract new class depositNbr : IBqlField
			{
			}
			#endregion
			#region Deposited
			public abstract new class deposited : IBqlField
			{
			}
			#endregion
			#region Methods
		...
		public virtual void VoidDepositProc(CADeposit doc)
		{
																...
																Asc<CADepositDetail.tranType, Asc<CADepositDetail.refNbr, Asc<CADepositDetail.lineNbr>>>>>>>.Select(this, doc.DocType, doc.RefNbr))
			{
				...
				{
					if (this.Details.Search<CADepositDetail.origModule, CADepositDetail.origDocType, CADepositDetail.origRefNbr>(BatchModule.AP, APPaymentType.GetVoidingAPDocType(pmtAP.DocType), pmt.RefNbr).Count == 0)
						continue; //Skip paymnets, which are already voided
				}
