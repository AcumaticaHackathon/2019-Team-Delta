...
namespace PX.Objects.CA
{
	public class CAReconEntry : PXGraph<CAReconEntry>
	{
		...
		public partial class CATranExt : CATran
		{
			...
			#region tstamp
			public new class Tstamp : IBqlField
			{
			}
		...
		public virtual IEnumerable toggleCleared(PXAdapter adapter)
		{
			...
			foreach (CATranExt tran in GetReconTrans())
			{
				...
				if (IsDisbledBecauseOfVoidingNotReleased(tran))
				{
					continue;
				}
			...
			IEnumerable<CATranExt> transToReconcile = PXSelect<CATranExt,
												...
												And<Where<CATranExt.reconNbr, Equal<Current<CARecon.reconNbr>>,
													...
													And<Where<CATranExt.tranDate, LessEqual<Current2<CARecon.loadDocumentsTill>>,
														Or<Current2<CARecon.loadDocumentsTill>, IsNull>>>>>>>>>.Select(this).Select(result => (CATranExt)result);
		...
		private void SelectAdditionalTables(CARecon doc, IEnumerable<CATranExt> transToReconcile, out Dictionary<long?, CABankTran> bankTrans, out Dictionary<long?, CATranVoiding> voidingTrans)
		{
			...
			foreach (var batch in idsCollection.BreakInto(batchSize))
			{
				...
				bankTrans.AddRange(PXSelectJoin<CABankTran,
											...
											.ToDictionary(result => ((CABankTranMatch)((PXResult<CABankTran, CABankTranMatch>)result)).CATranID, result => (CABankTran)result));
		...
		private bool IsDisbledBecauseOfVoidingNotReleased(CATranExt tran)
		{
		...
		protected virtual void CATranExt_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			if (isReconcilable)
			{
				if (row.Released != true || IsDisbledBecauseOfVoidingNotReleased(row))
				{
					PXUIFieldAttribute.SetEnabled(cache, row, false);
		...
		[PXDefault]
		[CashAccount(suppressActiveVerification: true, branchID: null, search: typeof(Search<CashAccount.cashAccountID,
		...
		#region Obsolete
		[Obsolete(PX.Objects.Common.InternalMessages.ClassIsObsoleteAndWillBeRemoved2019R1)]
		...
		[Obsolete(PX.Objects.Common.InternalMessages.MethodIsObsoleteAndWillBeRemoved2019R1)]
		...
		[Obsolete(PX.Objects.Common.InternalMessages.ObjectIsObsoleteAndWillBeRemoved2019R1)]
