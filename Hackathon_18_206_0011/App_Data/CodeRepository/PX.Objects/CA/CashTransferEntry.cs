	...
	public class CashTransferEntry : PXGraph<CashTransferEntry, CATransfer>
	{
		...
		private void CheckExpensesOnHold()
		{
			...
			foreach (CAAdj adj in adjs)
			{
				if (adj.Hold == true)
				{
					var expense = (CAExpense)Expenses.Select().First(m => ((CAExpense)m)?.AdjRefNbr == adj.AdjRefNbr);
		...
		private CATransfer ReverseTransfer()
		{
			...
			Expenses.ReverseExpenses(currentTransfer, reverseTransfer);
			RemoveLinkOnAdj();
		...
		private void CreateCuryInfo(CATransfer currentTransfer, CATransfer reverseTransfer)
		{
			...
			CurrencyInfo currinfoOutNew = PXSelect<CurrencyInfo, Where<CurrencyInfo.curyInfoID, Equal<Required<CATransfer.outCuryInfoID>>, And<CurrencyInfo.curyID, Equal<Required<CATransfer.outCuryID>>>>>.Select(this, reverseTransfer.OutCuryInfoID, reverseTransfer.OutCuryID);
			CurrencyInfo currinfoInNew = PXSelect<CurrencyInfo, Where<CurrencyInfo.curyInfoID, Equal<Required<CATransfer.inCuryInfoID>>, And<CurrencyInfo.curyID, Equal<Required<CATransfer.inCuryID>>>>>.Select(this, reverseTransfer.InCuryInfoID, reverseTransfer.InCuryID);

			currinfoOutNew.CuryID = currinfoIn.CuryID;
			currinfoOutNew.CuryRate = currinfoIn.CuryRate;
			CuryInfo.Update(currinfoOutNew);

			currinfoInNew.CuryID = currinfoOut.CuryID;
			currinfoInNew.CuryRate = currinfoOut.CuryRate;
			CuryInfo.Update(currinfoInNew);
