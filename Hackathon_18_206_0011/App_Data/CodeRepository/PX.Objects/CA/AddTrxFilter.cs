...
namespace PX.Objects.CA
{
	...
	{
		#region OnlyExpense
		public abstract class onlyExpense : IBqlField
		{
		}
		...
		[CAAPAROpenPeriod(typeof(origModule), typeof(tranDate), typeof(branchID), ValidatePeriod = PeriodValidation.DefaultSelectUpdate)]
		...
		[Account(typeof(AddTrxFilter.branchID), typeof(Search2<Account.accountID, LeftJoin<CashAccount, On<CashAccount.accountID, Equal<Account.accountID>>,
												...
												Where2<Where2<
													...
													And<Match<Current<AccessInfo.userName>>>>>), DisplayName = "Offset Account", DescriptionField = typeof(Account.description))]
					...
					if (row.OrigModule == GL.BatchModule.CA)
					{
						...
						if (String.IsNullOrEmpty(row.EntryTypeID) == false)
						{
							if (row.AccountID.HasValue)
							{
								...
								bool isSingleCashAcct = cashAccts.Count == 1;
								CashAccount offsetCashAcct = (CashAccount)cashAccts;
								singleCashAcct = (offsetCashAcct != null && offsetCashAcct.AccountID.HasValue && isSingleCashAcct == true);
							}
							else
							{
