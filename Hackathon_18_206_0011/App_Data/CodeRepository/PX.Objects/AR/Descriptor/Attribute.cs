...
namespace PX.Objects.AR
{
        ...
        where ReleasedField : class, IBqlField
    {
		...
		protected virtual void GetCustomerBalance(PXCache cache, Customer customer, out decimal? CustomerBal, out DateTime? OldInvoiceDate)
		{
			...
			{
				summarybal = CustomerMaint.GetCustomerBalances<Override.Customer.sharedCreditCustomerID>(cache.Graph, customer.SharedCreditCustomerID);
			}
	...
	public class ARTaxAttribute : TaxAttribute
	{
		...
		protected override void ParentSetValue(PXGraph graph, string fieldname, object value)
		{
			...
			if (fieldname == _CuryDocBal && cache.Current != null && IsRoundingNeeded(graph))
			{
				...
				if (currency?.UseARPreferencesSettings == false)
				{
					value = ARReleaseProcess.RoundAmount((decimal?)value, currency.ARInvoiceRounding, currency.ARInvoicePrecision);
				}
				else
				{
	...
	public class AROpenPeriodAttribute : OpenPeriodAttribute
	{
		...
		public static void DefaultFirstOpenPeriod(PXCache sender, string FieldName)
		{
			foreach (PeriodIDAttribute attr in sender.GetAttributesReadonly(FieldName).OfType<PeriodIDAttribute>())
			{
				...
				attr.DefaultingQueryParametersDelegate =
					delegate(object[] currents, DateTime? sourceDate, int? calendarOrganizationID)
					{
						return new object[] {sourceDate, calendarOrganizationID};
					};
			}
		}
	...
	public class ARReports : PX.SM.ReportUtils
	{
