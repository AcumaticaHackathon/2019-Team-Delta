...
namespace PX.Objects.AR
{
		...
		protected virtual IEnumerable ardocumentlist()
		{
			PXSelectBase<BalancedARDocument> selectDocumentsCommand = new PXSelectJoinGroupBy<BalancedARDocument,
				...
				Where2<
					...
					And<Where<PaymentMethod.paymentType, NotEqual<PaymentMethodType.creditCard>,
						Or<PaymentMethod.paymentMethodID, IsNull,
						Or<Current<ARSetup.integratedCCProcessing>, Equal<False>,
						Or<PaymentMethod.aRIsProcessingRequired, Equal<False>>>>>>>>>>>,
				Aggregate<
	...
	public class ARReleaseProcess : PXGraph<ARReleaseProcess>
	{
			...
			out PMRegister pmDoc)
		{
			...
			if (doc.Released != true)
			{
				...
				foreach (PXResult<ARTran, ARTax, Tax, DRDeferredCode, SO.SOOrderType> r in ARTran_TranType_RefNbr.Select((object)ardoc.DocType, ardoc.RefNbr))
				{
					...
					if (_IsIntegrityCheck == false &&
						!object.Equals(prev_n, n))
																													{ 
						ProcessInvoiceDetailDiscount(doc, vend, n, sotype, ardoc);
						}
				...
				if (proforma != null)
				{
					...
					foreach(PXResult<PMTran, PMProformaTransactLine> res2 in select.Select(proforma.RefNbr))
					{
						...
						{
							if( pmt.Reverse != PMReverse.OnBilling )// if not already reveresed on billing.
								reverseToNonBillable.Add(pmt);
						}
						else if (line.Option == PMProformaLine.option.HoldRemainder && line.LineTotal < line.BillableAmount && (string.IsNullOrEmpty(pmt.AllocationID) || pmt.Reverse == PMReverse.OnBilling) )
						{
				...
				PXResultset<ARTran> docLinesWithData = ARTran_TranType_RefNbr.Select((object) ardoc.DocType, ardoc.RefNbr);

				FinPeriodUtils.ValidateFinPeriod(docLinesWithData.Select(line => (ARTran)line), typeof(OrganizationFinPeriod.aRClosed));

				prev_n = new ARTran();

				foreach (PXResult<ARTran, ARTax, Tax, DRDeferredCode, SO.SOOrderType> r in docLinesWithData)
				{
					ARTran n = r;
					ARTax x = r;
					Tax salestax = r;
					...
					if (!object.Equals(prev_n, n))
					{
									...
									if (intran.UpdateShippedNotInvoiced == true)
									{
										...
										foreach (IN.INTranCost trancost in trancosts)
										{
											...
											if (trancost != null && trancost.COGSAcctID != null && intran != null)
											{
												...
												InsertInvoiceDetailsINTranCostTransaction(je, tranFromIN, 
													new GLTranInsertionContext { ARRegisterRecord = doc, ARTranRecord = n, INTranRecord = intran, INTranCostRecord = trancost });
							...
							n.OrigInvoiceDate != null)//OrigInvoiceDate is used as a marker to distinguish between ARTran that were added automatically on Reversal and added manually by user.
						{
							...
							RestoreAmountToInvoice(n, n.TranAmt.GetValueOrDefault(), account?.AccountGroupID);
						}
							...
							if (n.PMDeltaOption == ARTran.pMDeltaOption.CompleteLine)
							{
								foreach (PMTran pmtran in linkedTransactions)
								{
									...
									if (pmtran.IsNonGL != true && pmtran.OffsetAccountID != pmtran.AccountID)
									{
										...
										InsertInvoiceDetailsPMTranTransaction(je, tran,
											new GLTranInsertionContext { ARRegisterRecord = doc, ARTranRecord = n, PMTranRecord = pmtran });
		...
		public virtual void RestoreAmountToInvoice(ARTran line, decimal value, int? revenueAccountGroup)
		{
			if (line.TaskID == null)
				return;
			...
			Tax salestax)
		{
			...
			else if (isChildRetainageDocumentWithTax)
			{
				...
				{
					PXSelectBase<ARTaxTran> selectTaxTran = new PXSelect<ARTaxTran,
						Where<ARTaxTran.module, Equal<Required<ARTaxTran.module>>,
							And<ARTaxTran.tranType, Equal<Required<ARTaxTran.tranType>>,
							And<ARTaxTran.refNbr, Equal<Required<ARTaxTran.refNbr>>,
							And<ARTaxTran.taxID, Equal<Required<ARTaxTran.taxID>>>>>>>(this);

					ARTaxTran origTaxTran = selectTaxTran.SelectSingle(x.Module, origRetainageDoc.DocType, origRetainageDoc.RefNbr, x.TaxID);
					...
					if (origTaxTran != null)
					{
						...
						foreach (ARRegister childRetainageDoc in PXSelect<ARRegister,
						...
						{
							ARTaxTran childTaxTran = selectTaxTran.SelectSingle(x.Module, childRetainageDoc.DocType, childRetainageDoc.RefNbr, x.TaxID);
						...
						{
							ARInvoice origRetainageInvoice = PXSelect<ARInvoice,
								Where<ARInvoice.docType, Equal<Required<ARInvoice.docType>>,
									And<ARInvoice.refNbr, Equal<Required<ARInvoice.refNbr>>>>>
								.SelectSingleBound(this, null, origRetainageDoc.DocType, origRetainageDoc.RefNbr);

							bool origIsDebit = (origRetainageInvoice.DrCr == DrCr.Debit);

							GLTran roundingTran = PXCache<GLTran>.CreateCopy(origTran);
							roundingTran.ReclassificationProhibited = true;
							roundingTran.AccountID = salestax.RetainageTaxPayableAcctID;
							roundingTran.SubID = salestax.RetainageTaxPayableSubID;
			...
			Func<decimal, decimal> round)
		{
			...
			if (!postedPPD && lineTax?.TaxID != null && 
				salesTax != null && 
				salesTax.TaxCalcLevel == "0")
			{
				...
				return new Amount(
					...
					lineTax.TaxableAmt + lineTax.ExemptedAmt + (lineTax.RetainedTaxableAmt + addUp ?? 0m) + lineTax.TaxableDiscountAmt);
			}
		...
		private void ProcessAdjustmentWriteOff(JournalEntry je, ARAdjust adj, ARPayment payment, Customer paymentCustomer, CurrencyInfo new_info, CurrencyInfo vouch_info)
		{
			...
			if (adj.AdjWOAmt != 0)
			{
				...
				GLTran tranNew = InsertAdjustmentsWriteOffTransaction(je, tran, 
					new GLTranInsertionContext { ARRegisterRecord = payment, ARAdjustRecord = adj });
						...
						foreach (PXResult<ARAdjust> adjres in ARAdjust_AdjgDocType_RefNbr_CustomerID.Select(doc.DocType, doc.RefNbr, _IsIntegrityCheck, doc.AdjCntr))
							...
							Batch paymentBatch = je.BatchModule.Current;
								...
								foreach (KeyValuePair<Tuple<string, string>, List<PXResult<ARAdjust>>> pair in appsByDocTypeAndPeriod)
								{
									...
									JournalEntry.SegregateBatch(je, BatchModule.AR, doc.BranchID, doc.CuryID, datesByDocTypeAndPeriod[appsKey], appsKey.Item2,
		...
		public virtual void IntegrityCheckProc(Customer cust, string startPeriod)
		{
			...
			using (new PXConnectionScope())
			{
				...
				using (PXTransactionScope ts = new PXTransactionScope())
				{
					...
					foreach (CuryARHist old_hist in PXSelectReadonly<
						CuryARHist, 
						Where<CuryARHist.customerID, Equal<Required<CuryARHist.customerID>>, 
							And<CuryARHist.finPeriodID, GreaterEqual<Required<CuryARHist.finPeriodID>>>>>
						.Select(this, cust.BAccountID, minPeriod))
					{
		...
		/// <summary>
		/// The method to insert invoice details PMTran GL transactions 
		/// for the <see cref="PMTran"/> entity inside the
		/// <see cref="ReleaseInvoice(JournalEntry, ref ARRegister, PXResult{ARInvoice, CurrencyInfo, Terms, Customer, Account}, out PMRegister)"/> method.
		/// <see cref="GLTranInsertionContext"/> class content:
		/// <see cref="GLTranInsertionContext.ARRegisterRecord"/>,
		/// <see cref="GLTranInsertionContext.ARTranRecord"/>,
		/// <see cref="GLTranInsertionContext.PMTranRecord"/>.
		/// </summary>
		public virtual GLTran InsertInvoiceDetailsPMTranTransaction(
			JournalEntry je,
			GLTran tran,
			GLTranInsertionContext context)
		{
