...
namespace PX.Objects.AR
{
	...
	public class ARPaymentEntry : ARDataEntryGraph<ARPaymentEntry, ARPayment>
	{
		...
		public partial class LoadOptions : IBqlTable
		{
			...
			[Branch(null, Required = false, DisplayName = "Branch")]
		...
		public override IEnumerable VoidCheck(PXAdapter adapter)
		{
				...
				&& ARPaymentType.VoidEnabled(payment))
			{
						...
						|| paymentmethod.Current.ARDefaultVoidDateToDocumentDate != true))
				{
					...
					string businessPeriodID = FinPeriodRepository.GetPeriodIDFromDate(document.DocDate, PXAccess.GetParentOrganizationID(document.BranchID));
				...
				{
					GetService<IFinPeriodUtils>().VerifyAndSetFirstOpenedFinPeriod<ARPayment.adjFinPeriodID, ARPayment.branchID>(
						Document.Cache,
						document,
						finperiod,
		...
		public virtual IEnumerable CaptureCCPayment(PXAdapter adapter)
		{
			...
			foreach (ARPayment doc in adapter.Get<ARPayment>())
			{
				...
				CheckValidPeriodForCCTran(this, doc);
				list.Add(doc);
		...
		public override void Persist()
		{
			...
			{
				PXDBDefaultAttribute.SetDefaultForUpdate<ARAdjust.adjgFinPeriodID>(Adjustments_History.Cache, adj, false);
				PXDBDefaultAttribute.SetDefaultForUpdate<ARAdjust.adjgTranPeriodID>(Adjustments_History.Cache, adj, false);
			...
			foreach (ARPayment doc in Document.Cache.Updated)
			{
				...
				if (doc.OpenDoc == true && 
					(bool?)Document.Cache.GetValueOriginal<ARPayment.openDoc>(doc) == false && 
		...
		protected virtual void ARAdjust_RowInserting(PXCache sender, PXRowInsertingEventArgs e)
		{
			...
			ARPayment document = Document.Current;

			if (!string.IsNullOrEmpty(adjust.AdjgFinPeriodID)
			    && AdjgDocTypesToValidateFinPeriod.Contains(document.DocType))
			{
				IFinPeriod organizationFinPeriod = FinPeriodRepository.GetByID(adjust.AdjgFinPeriodID, PXAccess.GetParentOrganizationID(document.BranchID));

				FinPeriodUtils.CanPostToPeriod(organizationFinPeriod, typeof(FinPeriod.aRClosed)).RaiseIfHasError();
			}
		...
		protected virtual void ARAdjust_RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
		{
			...
			//
			if (!UnattendedMode &&
				!IsAdjdRefNbrFieldVerifyingDisabled(doc))
			{
		...
		protected virtual void ARPayment_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
                ...
                && IsImport != true)
			{
				...
				{
					FinPeriod finPeriod = FinPeriodRepository.FindFinPeriodByDate((DateTime)Accessinfo.BusinessDate, PXAccess.GetParentOrganizationID(doc.BranchID));
			...
			if (enableCCProcess && this.arsetup.Current.IntegratedCCProcessing == true && docNotReleased)
			{
				...
				{
					release.SetEnabled(docNotOnHold && docOpen && CurrPaymentState.IsSettlementDue);
				}
				else
				{
		...
		protected virtual void ARPayment_DocDate_FieldUpdating(PXCache sender, PXFieldUpdatingEventArgs e)
		{
			...
			{
				if (CurrPaymentState.IsSettlementDue && CurrPaymentState.lastTran != null && DateTime.Compare(((DateTime?)e.NewValue).Value, CurrPaymentState.lastTran.EndTime.Value.Date) != 0)
				{
