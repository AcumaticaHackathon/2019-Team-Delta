...
namespace PX.Objects.AR
{
	...
	public class ARStatementPrint : PXGraph<ARStatementDetails>
	{
		...
		protected virtual void PrintParameters_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			if (row != null && row.PrinterName == null && PXAccess.FeatureInstalled<FeaturesSet.deviceHub>())
			{
				row.PrinterName = new NotificationUtility(this).SearchPrinter(
			...
			PXUIFieldAttribute.SetVisible<PrintParameters.printerName>(sender, row, showPrintSettings);

			PXUIFieldAttribute.SetEnabled<PrintParameters.definePrinterManually>(sender, row, row.PrintWithDeviceHub == true);
			PXUIFieldAttribute.SetEnabled<PrintParameters.printerName>(sender, row, row.PrintWithDeviceHub == true && row.DefinePrinterManually == true);
