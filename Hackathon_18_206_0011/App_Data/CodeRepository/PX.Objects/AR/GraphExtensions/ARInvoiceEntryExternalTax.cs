...
namespace PX.Objects.AR
{
	public class ARInvoiceEntryExternalTax : ExternalTax<ARInvoiceEntry, ARInvoice>
	{
		...
		public override ARInvoice CalculateExternalTax(ARInvoice invoice)
		{
			...
			{
				Base.Document.SetValueExt<ARInvoice.isTaxValid>(invoice, true);
				if (invoice.IsTaxSaved == true)
					Base.Document.SetValueExt<ARInvoice.isTaxSaved>(invoice, false);
				SkipTaxCalcAndSave();
			}
		...
		protected virtual void _(Events.RowUpdated<ARInvoice> e)
		{
			...
			{
				if (IsExternalTax(e.Row.TaxZoneID) && e.Row.InstallmentNbr == null &&
				    !e.Cache.ObjectsEqual<ARInvoice.avalaraCustomerUsageType, ARInvoice.curyDiscTot, ARInvoice.customerLocationID>(e.Row, e.OldRow))
				{
		...
		{
			if (Base.Document.Current != null && IsExternalTax(Base.Document.Current.TaxZoneID) && Base.Document.Current.InstallmentNbr == null)
			{
		...
		public virtual void CancelTax(ARInvoice invoice, VoidReasonCode code)
		{
			...
			var service = ExternalTax.TaxProviderFactory(Base, invoice.TaxZoneID);
		...
		{
			return ((Address)PXSelect<Address, Where<Address.addressID, Equal<Required<Address.addressID>>>>.Select(Base, Base.location.Current.DefAddressID)).With(ValidAddressFrom<Location.defAddressID>);
		}

		protected virtual IAddressBase GetToAddress(ARInvoice invoice, ARTran tran)
		{
			if (invoice.OrigModule == BatchModule.SO)
			{
				var soLine =
				...
				bool commonCarrier = ((Carrier)shipLine)?.IsCommonCarrier == true || ((Carrier)soLine)?.IsCommonCarrier == true;
				if (tran.SiteID != null && commonCarrier == false)
			...
			where TFieldSource : IBqlField
		{
			...
			if (typeof(TFieldSource) == typeof(SOInvoice.shipAddressID))
				return PXSelectReadonly<SOInvoice, Where<SOInvoice.shipAddressID, Equal<Required<Address.addressID>>>>
					.SelectWindowed(Base, 0, 1, ((SOAddress)address).AddressID).First().GetItem<SOInvoice>()
					.With(e => PXMessages.LocalizeFormat(Messages.AvalaraAddressSourceError, EntityHelper.GetFriendlyEntityName<SOInvoice>(), new EntityHelper(Base).GetRowID(e)));

			if (typeof(TFieldSource) == typeof(Location.defAddressID))
				return PXSelectReadonly<Location, Where<Location.defAddressID, Equal<Required<Address.addressID>>>>
					.SelectWindowed(Base, 0, 1, ((Address)address).AddressID).First().GetItem<Location>()
					.With(e => PXMessages.LocalizeFormat(Messages.AvalaraAddressSourceError, EntityHelper.GetFriendlyEntityName<Location>(), new EntityHelper(Base).GetRowID(e)));
