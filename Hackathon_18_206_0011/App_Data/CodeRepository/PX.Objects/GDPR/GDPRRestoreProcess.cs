...
namespace PX.Objects.GDPR
{
	public class GDPRRestoreProcess : GDPRPersonalDataProcessBase
	{
		...
		public class RestoreType : IBqlTable
		{
			...
			public abstract class deleteNonRestored : IBqlField { }
		...
		{
			ObfuscatedItems.SetProcessDelegate(delegate(List<SMPersonalDataIndex> entries) { EraseImpl(entries); });

			TopLevelProcessor = TopCommonProcessorImpl;
			ChildLevelProcessor = ChildEraseProcessorImpl;
		...
		private static void EraseImpl(IEnumerable<SMPersonalDataIndex> entities)
		{
			using (PXTransactionScope ts = new PXTransactionScope())
			{
				ProcessImpl(RemapToPrimary(entities), true, null);
		...
		private static void TopCommonProcessorImpl(string combinedKey, Guid? topParentNoteID, string info)
		{
			HandleSearchIndex(combinedKey);
		}

		private static void ChildRestoreProcessorImpl(PXGraph processingGraph, Type childTable, IEnumerable<PXPersonalDataFieldAttribute> fields, IEnumerable<object> childs, Guid? topParentNoteID)
		{
			RestoreObfuscatedEntries(processingGraph, childTable, fields, childs);
		}
		
		private static void ChildEraseProcessorImpl(PXGraph processingGraph, Type childTable, IEnumerable<PXPersonalDataFieldAttribute> fields, IEnumerable<object> childs, Guid? topParentNoteID)
		{
			ErasePseudonymizedData(processingGraph, childTable, childs);
		}

		private static void HandleSearchIndex(string combinedKey)
		{
			PXDatabase.Delete<SMPersonalDataIndex>(
				new PXDataFieldRestrict<SMPersonalDataIndex.combinedKey>(combinedKey));
		}

		private static void RestoreObfuscatedEntries(PXGraph processingGraph, Type childTable, IEnumerable<PXPersonalDataFieldAttribute> fields, IEnumerable<object> childs)
		{
			foreach (object child in childs)
			{
				...
				foreach (PXDataRecord record in PXDatabase.SelectMulti<SMPersonalData>(
					...
					new PXDataFieldValue<SMPersonalData.noteID>(entityNoteID)))
				{
				...
				if (isSuccess)
				{
					PXDatabase.Delete<SMPersonalData>(
						...
						new PXDataFieldRestrict<SMPersonalData.noteID>(entityNoteID));
				}
			}
		}

		private static List<PXDataFieldParam> InterruptRestorationHandler(PXCache cache, List<PXDataFieldParam> restricts)
		{
		...
		private static void ErasePseudonymizedData(PXGraph processingGraph, Type childTable, IEnumerable<object> childs)
		{
			foreach (object child in childs)
			{
				List<PXDataFieldParam> assignsDB = new List<PXDataFieldParam>();
		...
		private static void CleanNonRestored(IEnumerable<IBqlTable> entities)
		{
