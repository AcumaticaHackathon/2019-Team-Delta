...
namespace PX.Objects.GDPR
{
	public class GDPRPseudonymizeProcess : GDPRPersonalDataProcessBase
	{
		...
		{
			#region Search
			public abstract class search : IBqlField { }
			...
			#region MasterEntity
			public abstract class masterEntity : IBqlField { }
			...
			#region NoConsent
			public abstract class noConsent : IBqlField { }
			...
			public abstract class consentExpired : IBqlField { }
		...
		{
			SelectedItems.SetProcessDelegate(delegate(List<ObfuscateEntity> entries) { Process(entries); });
		...
		public static void Process(IEnumerable<ObfuscateEntity> entities)
		{
		...
		protected static bool StoreRestorableSearchIndex(string combinedKey, string info)
		{
		...
		protected static bool DeleteSearchIndex(Guid? topParentNoteID)
		{
			return PXDatabase.Delete<SearchIndex>(
				new PXDataFieldRestrict<SearchIndex.noteID>(topParentNoteID));
		}

		private static void StoreChildsValues(PXGraph processingGraph, Type childTable, IEnumerable<PXPersonalDataFieldAttribute> fields, IEnumerable<object> childs, Guid? topParentNoteID)
		{
			foreach (object child in childs)
			{
				...
				foreach (PXPersonalDataFieldAttribute field in fields)
				{
					...
					assigns.Add(new PXDataFieldAssign<SMPersonalData.noteID>(childNoteID));
		...
		protected static void PseudonymizeChilds(PXGraph processingGraph, Type childTable, IEnumerable<PXPersonalDataFieldAttribute> fields, IEnumerable<object> childs)
		{
			foreach (object child in childs)
			{
		...
		private static List<PXDataFieldParam> InterruptPseudonimyzationHandler(PXCache cache, List<PXDataFieldParam> restricts)
		{
		...
		private static object GetObfuscateValue(Type table, PXPersonalDataFieldAttribute field)
		{
		...
		protected static void WipeAudit(PXGraph processingGraph, Type childTable, IEnumerable<PXPersonalDataFieldAttribute> fields, IEnumerable<object> childs)
		{
			foreach (object child in childs)
			{
