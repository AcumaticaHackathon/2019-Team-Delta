	...
	public class RMReportReaderGL : PXGraphExtension<RMReportReader>
    {
        ...
        public virtual void GLEnsureInitialized()
        {
            if (_historyLoaded == null)
            {
				...
				var res = PXSelectJoin<CR.BAccount, InnerJoin<Branch, On<CR.BAccount.bAccountID, Equal<Branch.bAccountID>>>>.Select(this.Base).ToList();
                ...
                if (Base.Report.Current.ApplyRestrictionGroups == true)
                {
					...
					var accounts = PXSelect<Account, Where<Match<Current<AccessInfo.userName>>>>.Select(this.Base).ToList();

					PXSelect<Sub, Where<Match<Current<AccessInfo.userName>>>>.Clear(this.Base);
					var subs = PXSelect<Sub, Where<Match<Current<AccessInfo.userName>>>>.Select(this.Base).ToList();

					PXSelect<Branch, Where2<MatchWithBranch<Branch.branchID>, And<Match<Current<AccessInfo.userName>>>>>.Clear(this.Base);
					var branches = PXSelect<Branch, Where2<MatchWithBranch<Branch.branchID>, And<Match<Current<AccessInfo.userName>>>>>.Select(this.Base).ToList();
				}
				else
                {
                    PXSelect<Account>.Clear(this.Base);
					var accounts = PXSelect<Account>.Select(this.Base).ToList();

					PXSelect<Sub>.Clear(this.Base);
					var subs = PXSelect<Sub>.Select(this.Base).ToList();

					PXSelect<Branch>.Clear(this.Base);
					var branches = PXSelect<Branch>.Select(this.Base).ToList();
				}
		...
		private IEnumerable<GLHistory> NormalizeHistory(IEnumerable<PXResult<GLHistoryByPeriod, GLHistory, Account>> history)
		{
			foreach(PXResult<GLHistoryByPeriod, GLHistory, Account> historyItem in history)
			{
				...
				else
				{
					...
					holeHistory.BegBalance =
					holeHistory.YtdBalance =
						...
						? lastHistory.YtdBalance
						: 0m;

					holeHistory.CuryBegBalance =
					holeHistory.CuryYtdBalance =
						((account.Type == AccountType.Income || account.Type == AccountType.Expense) &&
						(new ReportFunctions()).ArePeriodsInSameYear(currHistory.LastActivityPeriod, currHistory.FinPeriodID))
						|| account.Type == AccountType.Asset || account.Type == AccountType.Liability
						? lastHistory.CuryYtdBalance
						: 0m;
        ...
        private void LoadHistory(int ledgerID, int year)
        {
			...
			PXSelectBase<GLHistoryByPeriod> glHistorySelect = new PXSelectReadonly2<
					...
					Where<GLHistoryByPeriod.ledgerID, Equal<Required<GLHistoryByPeriod.ledgerID>>>>(this.Base);
			...
			IEnumerable<GLHistory> yearHistory = NormalizeHistory(
				...
				.SelectMulti(ledgerID, year.ToString(), (year + 1).ToString())
            ...
            if (lastPeriodOfPrevYear != null)
            {
				...
				.SelectMulti(ledgerID, lastPeriodOfPrevYear.FinPeriodID)
				.Cast<PXResult<GLHistoryByPeriod, GLHistory, Account>>());
