...
namespace PX.Objects.CS
{
				...
				Or<OrganizationBAccount.type, Equal<BAccountType.organizationBranchCombinedType>>>>
	{
		...
		{
			public abstract class clearAccessRoleOnChildBranches : IBqlField { }
			[PXBool]
			public bool? ClearAccessRoleOnChildBranches { get; set; }

			public abstract class isBranchTabVisible: IBqlField { }
			[PXBool]
			public bool? IsBranchTabVisible { get; set; }

			public abstract class isDeliverySettingsTabVisible : IBqlField { }
			[PXBool]
			public bool? IsDeliverySettingsTabVisible { get; set; }

			public abstract class isGLAccountsTabVisible : IBqlField { }
			[PXBool]
			public bool? IsGLAccountsTabVisible { get; set; }

			public abstract class isRutRotTabVisible : IBqlField { }
			[PXBool]
		...
		{
			if (orgStatus != PXEntryStatus.Inserted) return;
		...
		public override void Persist()
		{
			...
			if (organization != null)
			{
				...
				//TODO 111 Pank move to FieldUpdated
			...
			if (!IsImport && !IsExport && !IsContractBasedAPI && !IsMobile)
			{
				...
				if (!needRefresh)
				{
					...
					{
						string originalAcctName = BAccount.Cache.GetValueOriginal<OrganizationBAccount.acctName>(updated) as string;
						string newAcctName = BAccount.Cache.GetValue<OrganizationBAccount.acctName>(updated) as string;
			...
			OrganizationView.Cache.ClearQueryCache();
			OrganizationView.Current = OrganizationView.Select();
			...
			if (needRefresh) // to refresh branch combo
			{
				...
				//PXDatabase.SelectTimeStamp(); //clear db slots (for PXAccess.AvailableOrganizationes)
			}
		...
		protected virtual void CreateSingleBranchRecord(Organization organization)
		{
			...
			BranchesView.Cache.Insert(branch);
		...
		protected virtual void MakeBranchSeparate(OrganizationBAccount orgBAccount)
		{
			...
			int? newBAccountID = CreateSeparateBAccountForBranch(newAcctCD, orgBAccount.AcctName);

			MapBranchToNewBAccountAndChangeBranchCD(orgBAccount.AcctCD, newAcctCD, newBAccountID);

			AssignNewBranchToEmployees(orgBAccount.BAccountID, newBAccountID, newAcctCD);
		}
		...
		protected virtual void AssignNewBranchToEmployees(int? oldBAccountID, int? newBAccountID, string branchCD)
		{
			...
			foreach (var employee in employees)
			{
				...
				string countryID = defAddress?.CountryID;

				employee.ParentBAccountID = newBAccountID;

				string state = null;
				string postalCode = null;

				Address defAddressCopy = null;

				if (defAddress != null)
				{
					if (branch.CountryID == countryID)
					{
						state = defAddress.State;
						postalCode = defAddress.PostalCode;
					}

					defAddressCopy = PXCache<Address>.CreateCopy(defAddress);

					defAddressCopy.State = null;
					defAddressCopy.PostalCode = null;

					employeeMaint.Address.Update(defAddressCopy);
				}

				employeeMaint.Employee.Update(employee);

				if (employeeMaint.Address.Current != null 
					&& employeeMaint.Address.Current.CountryID == null)
				{
					employeeMaint.Address.Current.CountryID = countryID;
				}

				if (employeeMaint.Address.Current?.CountryID == countryID
					&& (state != null || postalCode != null))
				{
					defAddressCopy = PXCache<Address>.CreateCopy(employeeMaint.Address.Current);

					defAddressCopy.State = state;
					defAddressCopy.PostalCode = postalCode;
		...
		protected virtual int? CreateSeparateBAccountForBranch(string acctCD, string acctName)
		{
			...
			return baccountMaint.BAccount.Current.BAccountID;
		}
		...
		protected virtual void ProcessPublicableToBranchesFieldsChanging(Organization origOrganization, Organization organization, PXEntryStatus status)
		{
			...
			if (forceCopy || shouldAdjustRoleName || shouldAdjustActive || shouldAdjustLedgerID || shouldAdjustCountryID || shouldLogoNameReport || shouldInvokeSyncEx)
			{
				...
				foreach (BranchMaint.BranchBAccount childBranchBAccount in childBranchBAccounts)
				{
					...
					BAccount.Cache.ClearQueryCache();
		...
		public class LedgerCreateParameters : IBqlTable
		{
			...
			#region LedgerCD
			public abstract class ledgerCD : IBqlField { }
