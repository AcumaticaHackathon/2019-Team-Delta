...
namespace PX.Objects.CS
{
	...
	public class AutoNumberAttribute : PXEventSubscriberAttribute, IPXFieldDefaultingSubscriber, IPXFieldVerifyingSubscriber, IPXRowInsertingSubscriber, IPXRowPersistingSubscriber, IPXRowPersistedSubscriber, IPXFieldSelectingSubscriber
	{
		...
		public virtual void RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
		{
			...
			if (_dateTime == null && !string.IsNullOrEmpty(_emptyDateMessage))
			{
				Exception ex = new AutoNumberException(_emptyDateMessage);
		...
		public void CommandPreparing(PXCache sender, PXCommandPreparingEventArgs e)
		{
			...
			{				
				e.Expr = new SubQuery(
					...
					.From<CSAnswers>()
					.Where(new Column<CSAnswers.attributeID>().Equal(new Data.SQLTree.Constant(GetAttributeID(sender)))
						.And(new Column<CSAnswers.refNoteID>().Equal(new Column(_refNoteId, new SimpleTable(e.Table ?? _BqlTable)))))
				).Embrace();
			}
		}
