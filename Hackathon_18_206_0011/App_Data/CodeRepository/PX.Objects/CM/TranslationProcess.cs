...
namespace PX.Objects.CM
{
	...
	public class TranslationProcess : PXGraph<TranslationProcess>
	{
		...
		public virtual IEnumerable translate(PXAdapter adapter)
		{
			...
			{
				FinPeriod finPeriod = FinPeriodRepository.FindByID(PXAccess.GetParentOrganizationID(parameters.BranchID), parameters.FinPeriodID);
		...
		protected virtual void TranslationParams_CuryEffDate_FieldDefaulting(PXCache cache, PXFieldDefaultingEventArgs e)
		{
			TranslationParams row = (TranslationParams)e.Row;
			if (row == null) return;
			MasterFinPeriod period = PXSelect<MasterFinPeriod, Where<MasterFinPeriod.finPeriodID, Equal<Current<TranslationParams.finPeriodID>>>>.Select(cache.Graph);
		...
		protected virtual void TranslationParams_LastFinPeriodID_FieldDefaulting(PXCache cache, PXFieldDefaultingEventArgs e)
		{
			...
			{
				MasterFinPeriod period = PXSelectJoin<MasterFinPeriod,
					InnerJoin<TranslationHistory,
						On<TranslationHistory.finPeriodID, Equal<MasterFinPeriod.finPeriodID>>>,
					Where<TranslationHistory.translDefId, Equal<Required<TranslationParams.translDefId>>>,
					OrderBy<Desc<MasterFinPeriod.finPeriodID>>>.SelectSingleBound(cache.Graph, new object[0], row.TranslDefId);
				if (period != null)
				{
		...
		protected virtual void VerifyCurrencyEffectiveDate(PXCache sender, TranslationParams translationParameters)
		{
			...
			MasterFinPeriod currentPeriod = (MasterFinPeriod)PXSelect<
					MasterFinPeriod,
				Where<
						MasterFinPeriod.finPeriodID, Equal<Required<MasterFinPeriod.finPeriodID>>>>
