...
namespace PX.Objects.CT
{
	...
	public partial class Contract : PX.Data.IBqlTable
	{
		...
		[PXDBIdentity()]
		[PXSelector(typeof(Search<Contract.contractID, Where<Contract.baseType, Equal<CTPRType.contract>>>))]
		...
		#region BaseType
		public abstract class baseType : IBqlField
		{
		}
...
//		[PXFormula(typeof(Selector<templateID, ContractTemplate.durationType>))]
		...
		public abstract class changeOrderWorkflow : PX.Data.IBqlField
		{
		}
		[PXDBBool()]
		...
		public abstract class lockCommitments : PX.Data.IBqlField
		{
		}
		[PXDBBool()]
		[PXDefault(false)]
		...
		public abstract class lastChangeOrderNumber : PX.Data.IBqlField
		{
		}
		[PXDBString(10, IsUnicode = true)]
		...
		[PXDBScalar(typeof(Search4<PMTaskTotal.asset, Where<PMTaskTotal.projectID, Equal<contractID>>, Aggregate<Sum<PMTaskTotal.asset>>>))]
		[PXBaseCury()]
		[PXDefault(TypeCode.Decimal, "0.0", PersistingCheck = PXPersistingCheck.Nothing)]
		...
		/// This value is calculated on the fly only in the <see cref="ProjectEntry"/> graph and is not stored in the DB. 
		/// </summary>
		[PXDBScalar(typeof(Search4<PMTaskTotal.liability, Where<PMTaskTotal.projectID, Equal<contractID>>, Aggregate<Sum<PMTaskTotal.liability>>>))]
		[PXBaseCury()]
		[PXDefault(TypeCode.Decimal, "0.0", PersistingCheck = PXPersistingCheck.Nothing)]
		...
		/// This value is calculated on the fly only in the <see cref="ProjectEntry"/> graph and is not stored in the DB. 
		/// </summary>
		[PXDBScalar(typeof(Search4<PMTaskTotal.income, Where<PMTaskTotal.projectID, Equal<contractID>>, Aggregate<Sum<PMTaskTotal.income>>>))]
		[PXBaseCury()]
		[PXDefault(TypeCode.Decimal, "0.0", PersistingCheck = PXPersistingCheck.Nothing)]
		...
		/// This value is calculated on the fly only in the <see cref="ProjectEntry"/> graph and is not stored in the DB. 
		/// </summary>
		[PXDBScalar(typeof(Search4<PMTaskTotal.expense, Where<PMTaskTotal.projectID, Equal<contractID>>, Aggregate<Sum<PMTaskTotal.expense>>>))]
		[PXBaseCury()]
		[PXDefault(TypeCode.Decimal, "0.0", PersistingCheck = PXPersistingCheck.Nothing)]
		...
		[PXSearchable(SM.SearchCategory.AR, Messages.ContractSearchTitle, new Type[] { typeof(Contract.contractCD), typeof(Contract.customerID), typeof(BAccount.acctName) },
		   ...
		   WhereConstraint = typeof(Where<Current<Contract.baseType>, Equal<CTPRType.contractTemplate>>),
