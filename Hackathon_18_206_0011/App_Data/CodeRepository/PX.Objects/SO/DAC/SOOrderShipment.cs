...
namespace PX.Objects.SO
{
	...
	[PXPrimaryGraph(new Type[] {
				...
				new Type[] {
					typeof(Select2<SOShipment, LeftJoin<INSite, On<INSite.siteID, Equal<SOShipment.siteID>>>, Where<SOShipment.noteID, Equal<Current<SOOrderShipment.shippingRefNoteID>>, And<SOShipmentType.dropShip, NotEqual<Current<SOOrderShipment.shipmentType>>, And<Where<INSite.siteID, IsNull, Or<Match<INSite, Current<AccessInfo.userName>>>>>>>>),
	...
	public partial class SOOrderShipment : PX.Data.IBqlTable
	{
		...
		public abstract class shippingRefNoteID : IBqlField
		{
		}
		[PXDBGuid(IsKey = true)]
		[PXDefault(typeof(SOShipment.noteID))]
		[PXUIField(DisplayName = "Document Nbr.")]
		[PXSelector(typeof(Search<SOOrderShipment.shippingRefNoteID, Where<SOOrderShipment.orderType, Equal<Current<SOOrderShipment.orderType>>, And<SOOrderShipment.orderNbr, Equal<Current<SOOrderShipment.orderNbr>>>>>), DirtyRead = true)]
		...
		public class ShippingRefNoteAttribute : Common.PXRefNoteBaseAttribute
		{
			/// <summary>
			/// Suppresses converting <see cref="Guid"/> to a human readable reference number.
			/// It is necessary when working with <see cref="System.Collections.IDictionary"/> instead of usual <see cref="PXCache"/>.
			/// </summary>
			public bool SuppressConvertingGuid { get; set; }

			public override void FieldSelecting(PXCache sender, PXFieldSelectingEventArgs e)
			{
				if (SuppressConvertingGuid)
					return;

				var row = e.Row as SOOrderShipment;
				if (row != null && row.ShipmentType == INDocType.Issue && !string.Equals(row.ShipmentNbr, Constants.NoShipmentNbr))
				{
				...
				{
					e.ReturnValue = GetEntityRowID(sender.Graph.Caches[typeof(POReceipt)], new object[] { PO.POReceiptType.POReceipt, row.ShipmentNbr });
				}
				else if (row != null
				...
				{
					e.ReturnValue = GetEntityRowID(sender.Graph.Caches[typeof(ARInvoice)], new object[] { row.InvoiceType, row.InvoiceNbr });
				}
				else
				{
		...
		[CustomerOrOrganizationRestrictor]
		...
		public abstract class orderFreightAllocated : IBqlField
		{
		}
		/// <summary>
