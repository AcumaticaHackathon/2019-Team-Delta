...
namespace PX.Objects.SO
{
	public class SOShipmentEntry : PXGraph<SOShipmentEntry, SOShipment>, IGraphWithInitialization
	{
		...
		public virtual IEnumerable ValidateAddresses(PXAdapter adapter)
		{
			foreach (SOShipment current in adapter.Get<SOShipment>())
			{
				if (current != null)
				{
					...
					{
						PXAddressValidator.Validate<SOShipmentAddress>(this, address, true);
					}
				}
        ...
        protected virtual void SOShipment_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			PXUIFieldAttribute.SetEnabled(sender, e.Row, isNotConfirmed);
			PXUIFieldAttribute.SetEnabled<SOShipment.curyID>(sender, e.Row, isNotConfirmed && curyenabled);
			...
			PXUIFieldAttribute.SetEnabled<SOShipment.curyFreightAmt>(sender, e.Row, isNotConfirmed && row.OverrideFreightAmount == true);
			...
			sender.AllowDelete = isNotConfirmed;
			selectSO.SetEnabled(row.SiteID != null && sender.AllowDelete);

			Transactions.Cache.AllowInsert = false;
			Transactions.Cache.AllowUpdate = isNotConfirmed;
			Transactions.Cache.AllowDelete = isNotConfirmed;

			splits.Cache.AllowInsert = isNotConfirmed;
			splits.Cache.AllowUpdate = isNotConfirmed;
			splits.Cache.AllowDelete = isNotConfirmed;

			Packages.Cache.AllowInsert = isNotConfirmed;
			Packages.Cache.AllowUpdate = isNotConfirmed;
			Packages.Cache.AllowDelete = isNotConfirmed;
			...
			PXUIFieldAttribute.SetEnabled<SOShipment.destinationSiteID>(sender, e.Row,
				sender.AllowUpdate && (Transactions.Select().Count == 0) && isTransfer);

			SOShipmentAddress shipAddress = this.Shipping_Address.Select();
			bool enableAddressValidation = isNotConfirmed
			...
			if (((SOShipment)e.Row).ShipVia != null)
			{
				...
				if (carrier != null)
				{
					...
					PXUIFieldAttribute.SetEnabled<SOShipment.curyFreightCost>(sender, e.Row, carrier.CalcMethod == CarrierCalcMethod.Manual && isNotConfirmed);
				}
			}
		...
		protected virtual void SOPackageDetail_Weight_FieldVerifying(PXCache cache, PXFieldVerifyingEventArgs e)
		{
			var row = (SOPackageDetail) e.Row;
			if (row != null)
			{
				CSBox box = SOPackageDetail.BoxFK.FindParent(cache.Graph, row);
		...
		public virtual decimal? CreateSplitsForAvailableNonLots(
			...
			{
				InventoryItem item = InventoryItem.PK.Find(this, newline.InventoryID.Value);
				if (item?.NegQty == true)
				{
					...
					if (orderType?.ShipFullIfNegQtyAllowed == true)
					{
						...
						if (locCounter > 0)
						{
							INLocation location = PXSelectReadonly<INLocation,
		...
		public virtual void CreateShipment(SOOrder order, int? SiteID, DateTime? ShipDate, bool? useOptimalShipDate, string operation, DocumentList<SOShipment> list, PXQuickProcess.ActionFlow quickProcessFlow)
		{
			...
			if (!addOrder)
			{
				...
				{
					newdoc = list.Find
						<SOShipment.customerID, SOShipment.shipDate, SOShipment.shipAddressID, SOShipment.shipContactID, SOShipment.siteID, SOShipment.fOBPoint, SOShipment.shipVia, SOShipment.shipTermsID, SOShipment.shipZoneID, SOShipment.useCustomerAccount, SOShipment.shipmentType, SOShipment.freightAmountSource, SOShipment.hidden>
						(order.CustomerID, ShipDate, order.ShipAddressID, order.ShipContactID, SiteID, order.FOBPoint, order.ShipVia, order.ShipTermsID, order.ShipZoneID, order.UseCustomerAccount, INTranType.DocType(orderoperation.INDocType), order.FreightAmountSource, false) ?? new SOShipment();
				}
				else
				{
		...
		public virtual void InvoiceShipment(SOInvoiceEntry docgraph, SOShipment shiporder, DateTime invoiceDate, DocumentList<ARInvoice, SOInvoice> list)
			=> InvoiceShipment(docgraph, shiporder, invoiceDate, list, PXQuickProcess.ActionFlow.NoFlow);
				...
				if (packagesResultset.Count > 0)
				{
					...
					if (uids.Length > 0)
					{
						...
						else
						{
							...
							throw new PXReportRequiredException(parameters, SOReports.PrintLabels, "Report");
						}
					}
					else
					{
		...
		protected virtual void PrintPickList(List<SOShipment> list, PXAdapter adapter)
		{
			...
			foreach (SOShipment order in list)
			{
				...
				actualReportID = new NotificationUtility(this).SearchReport(SONotificationSource.Customer, cstmr, SOReports.PrintPickList, company.BranchID);
				ex = PXReportRequiredException.CombineReport(ex, actualReportID, parameters);
			}
		...
		public virtual void PrintCarrierLabels(List<SOShipment> list, PXAdapter adapter)
		{
					...
					if (packagesResultset.Count > 0)
					{
						...
						if (uids.Length > 0)
						{
							...
							else
							{
								...
								parameters["SOShipment.ShipmentNbr"] = shiporder.ShipmentNbr;
			...
			if (laserLabels.Count > 0 && ex != null)
			{
				if (PXAccess.FeatureInstalled<FeaturesSet.deviceHub>())
					PX.SM.SMPrintJobMaint.CreatePrintJobGroup(adapter, new NotificationUtility(this).SearchPrinter, SONotificationSource.Customer, SOReports.PrintLabels, Accessinfo.BranchID, ex, null);
        ...
        protected virtual CarrierRequest BuildRequest(SOShipment shiporder)
		{
		...
		protected virtual IList<CarrierBox> GetPackages(SOShipment shiporder, Carrier carrier, CarrierPlugin plugin)
		{
			...
			foreach (PXResult<SOPackageDetail, PX.Objects.CS.CSBox> res in select.Select(shiporder.ShipmentNbr))
			{
                ...
                list.Add(BuildCarrierPackage(detail, package, plugin.UOM));
			}
        ...
        protected virtual CarrierBox BuildCarrierPackage(SOPackageDetail detail, CSBox package, string pluginUOM)
        {
            decimal weightInStandardUnit = CarrierMaint.ConvertGlobalUnits(this, commonsetup.Current.WeightUOM, pluginUOM, detail.Weight ?? 0);
		...
		public PackageDetail PackageDetailExt => GetExtension<PackageDetail>();
		public class PackageDetail : PXGraphExtension<SOShipmentEntry>
		{
			...
			protected void UpdateParentShipmentLine(PXCache sender, SOPackageDetailSplit row, SOPackageDetailSplit oldRow)
			{
				...
				{
					var shipmentLine = PXParentAttribute.SelectParent<SOShipLine>(sender, row);
				...
				else
				{
					...
					if (oldRow != null)
					{
						var shipmentLine = PXParentAttribute.SelectParent<SOShipLine>(sender, oldRow);
						if (shipmentLine != null)
						{
			...
			protected void UpdateShipmentLine(PXCache sender, SOShipLine shipmentLine, SOPackageDetailSplit packageDetailSplit, decimal adjustment)
			{
				if (adjustment != 0)
				{
					...
					decimal? originalPackagedQty = shipmentLine.PackedQty;

					shipmentLine.PackedQty = shipmentLine.PackedQty.GetValueOrDefault() + adjustment;
					...
					sender.Graph.Caches[typeof(SOShipLine)].RaiseFieldUpdated<SOShipLine.packedQty>(shipmentLine, originalPackagedQty);
					sender.Graph.Caches[typeof(SOShipLine)].RaiseRowUpdated(shipmentLine, originalShipmentLine);

					if (shipmentLine.PackedQty > shipmentLine.ShippedQty)
					{
						sender.RaiseExceptionHandling<SOPackageDetailSplit.qty>(packageDetailSplit, packageDetailSplit.Qty, new PXSetPropertyException(Messages.QuantityPackedExceedsShippedQuantityForLine));
					}
				}
			}
			...
			protected void SOPackageDetailSplit_ShipmentLineNbr_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
			{
				...
				if (row != null)
				{
					...
					row.Qty = shipmentLine?.ShippedQty - shipmentLine?.PackedQty;
				}
			}
	...
	public partial class SOMiscLine2 : IBqlTable, ISortOrder
	{
		...
		public abstract class invoiceLineNbr : IBqlField
		{
		}
		/// <summary>
	...
	public partial class AddSOFilter : IBqlTable
	{
		...
		public abstract class freightAmountSource : IBqlField
		{
		}
