...
namespace PX.Objects.SO
{
	public class SOInvoiceEntry : ARInvoiceEntry
	{
		...
		[PXRestrictor(typeof(Where2<SameOrganizationBranch<INSite.branchID, Current<ARInvoice.branchID>>,
		...
		[SOOpenPeriod(typeof(ARRegister.docDate), typeof(ARRegister.branchID))]
				...
				var ingraph = new Lazy<INIssueEntry>(() =>
				{
					...
					g.RowPersisting.AddHandler<INRegister>((PXCache sender, PXRowPersistingEventArgs e) =>
					{
						...
						PXResultset<INTran> trans = 
							PXSelectJoin<INTran, 
							InnerJoin<INSite,
								On<INTran.siteID, Equal<INSite.siteID>>>,
						...
						GetService<IFinPeriodUtils>().ValidateFinPeriod(
		...
		protected override void ARInvoice_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			PXUIFieldAttribute.SetVisible<ARInvoice.projectID>(cache, e.Row, PM.ProjectAttribute.IsPMVisible(BatchModule.SO) || PM.ProjectAttribute.IsPMVisible(BatchModule.AR));
		...
		protected virtual void SOInvoice_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			bool isAuthorizedCashSale = (doc.DocType == ARDocType.CashSale && (isCCPreAuthorized || isCCCaptured));
			PXUIFieldAttribute.SetEnabled<ARInvoice.curyDiscTot>(sender, doc, !PXAccess.FeatureInstalled<FeaturesSet.customerDiscounts>() && !isAuthorizedCashSale);
		}
		...
		protected override void ARTran_RowDeleted(PXCache sender, PXRowDeletedEventArgs e)
		{
            ...
            if (siblings.Count == 0)
			{
				...
				SOFreightDetail freightDet = FreightDetails.Select()
		...
		{           
			...
			discount.CuryTranAmt = (decimal?)sender.GetValue<ARInvoice.curyDiscTot>(e.Row);
			...
			Discount.Cache.MarkUpdated(discount);

			discount.ManualDisc = true; //escape SOManualDiscMode.RowUpdated
			Discount.Cache.RaiseRowUpdated(discount, old_row);

            decimal auotDocDisc = ARDiscountEngine.GetTotalGroupAndDocumentDiscount<ARInvoiceDiscountDetail>(DiscountDetails);
		...
		public virtual void InvoiceOrder(DateTime invoiceDate, PXResult<SOOrderShipment, SOOrder, CurrencyInfo, SOAddress, SOContact, SOOrderType> order, Customer customer, DocumentList<ARInvoice, SOInvoice> list, PXQuickProcess.ActionFlow quickProcessFlow)
		{
		...
		public virtual void InvoiceOrder(DateTime invoiceDate, PXResult<SOOrderShipment, SOOrder, CurrencyInfo, SOAddress, SOContact, SOOrderType> order, PXResultset<SOShipLine, SOLine> details, Customer customer, DocumentList<ARInvoice, SOInvoice> list, PXQuickProcess.ActionFlow quickProcessFlow)
		{
			ARInvoice newdoc;
            ...
            if (soOrderType.RecalculateDiscOnPartialShipment != true)
            {
				...
				foreach (SOOrderDiscountDetail docGroupDisc in discountCodesWithApplicableSOLines.LeftValues)
                {
					...
					decimal invoicedMiscAmt = 0m;
						foreach (SOLine soLine in discountCodesWithApplicableSOLines.RightsFor(docGroupDisc))
						{
								...
								!PXAccess.FeatureInstalled<FeaturesSet.customerDiscounts>())
							{
								...
								{
									invoicedMiscAmt += (tran.CuryTranAmt ?? 0m);
								}
					...
					if (docGroupDisc.CuryDiscountableAmt > 0m)
					{
						if (docGroupDisc.Type == DiscountType.Group)
							rate = invoicedGroupAmt / docGroupDisc.CuryDiscountableAmt;
		...
		public virtual ARInvoice FindOrCreateInvoice(DateTime orderInvoiceDate, PXResult<SOOrderShipment, SOOrder, CurrencyInfo, SOAddress, SOContact, SOOrderType> order, DocumentList<ARInvoice, SOInvoice> list)
		{
			...
			SOOrder soOrder = order;
			SOOrderType soOrderType = order;
			string orderTermsID = ((SOOrderType)order).ARDocType == ARDocType.CreditMemo ? null : soOrder.TermsID;
			...
			if (soOrder.PaymentCntr == 0 && soOrder.BillSeparately == false && !IsCreditCardProcessing(soOrder))
			{
				if (soOrder.PaymentMethodID == null && soOrder.CashAccountID == null)
					return list.Find<ARInvoice.hidden, ARInvoice.docType, ARInvoice.docDate, ARInvoice.branchID, ARInvoice.customerID, ARInvoice.customerLocationID, SOInvoice.billAddressID, SOInvoice.billContactID, ARInvoice.taxZoneID, ARInvoice.curyID, ARInvoice.termsID, SOInvoice.extRefNbr>(
							false, soOrderType.ARDocType, orderInvoiceDate, soOrder.BranchID, soOrder.CustomerID, soOrder.CustomerLocationID, soOrder.BillAddressID, soOrder.BillContactID, soOrder.TaxZoneID, soOrder.CuryID, orderTermsID, soOrder.ExtRefNbr) ?? new ARInvoice();
					...
					return list.Find<ARInvoice.hidden, ARInvoice.docType, ARInvoice.docDate, ARInvoice.branchID, ARInvoice.customerID, ARInvoice.customerLocationID, SOInvoice.billAddressID, SOInvoice.billContactID, ARInvoice.taxZoneID, ARInvoice.curyID, ARInvoice.termsID, SOInvoice.extRefNbr, SOInvoice.pMInstanceID>(
							false, soOrderType.ARDocType, orderInvoiceDate, soOrder.BranchID, soOrder.CustomerID, soOrder.CustomerLocationID, soOrder.BillAddressID, soOrder.BillContactID, soOrder.TaxZoneID, soOrder.CuryID, orderTermsID, soOrder.ExtRefNbr, soOrder.PMInstanceID) ?? new ARInvoice();
				else
					return list.Find<ARInvoice.hidden, ARInvoice.docType, ARInvoice.docDate, ARInvoice.branchID, ARInvoice.customerID, ARInvoice.customerLocationID, SOInvoice.billAddressID, SOInvoice.billContactID, ARInvoice.taxZoneID, ARInvoice.curyID, ARInvoice.termsID, SOInvoice.extRefNbr, SOInvoice.pMInstanceID, SOInvoice.cashAccountID>(
							false, soOrderType.ARDocType, orderInvoiceDate, soOrder.BranchID, soOrder.CustomerID, soOrder.CustomerLocationID, soOrder.BillAddressID, soOrder.BillContactID, soOrder.TaxZoneID, soOrder.CuryID, orderTermsID, soOrder.ExtRefNbr, soOrder.PMInstanceID, soOrder.CashAccountID) ?? new ARInvoice();
			}
			else
			{
		...
		public virtual ARTran InsertInvoiceDirectLine(ARTran tran)
		{
			...
			{
				PXResult<SOLine, SOOrderType> res = lsselect.GetLinkedSOLine(tran);
				if (res != null)
				{
					SOLine line = res;
					...
					bool useLineDiscPct;
					CopyTranFieldsFromSOLine(tran, (SOOrderType)res, line, out useLineDiscPct);
				}
			}
		...
		protected virtual void PostInvoiceDirectLines(INIssueEntry docgraph, ARInvoice invoice, DocumentList<INRegister> list)
		{
			List<PXResult<ARTran, SOLine, INItemPlan>> directLines = PXSelectJoin<ARTran,
				...
				.SelectMultiBound(this, new object[] { invoice })
		...
		public virtual bool IsFullOrderFreightAmountFirstTime(SOOrder order)
		{
			...
			SOOrderTypeOperation nonDefaultOperation = PXSelectReadonly<SOOrderTypeOperation,
				Where<SOOrderTypeOperation.orderType, Equal<Required<SOOrderTypeOperation.orderType>>,
					And<SOOrderTypeOperation.operation, NotEqual<Required<SOOrderTypeOperation.operation>>, And<SOOrderTypeOperation.active, Equal<True>>>>>
