	...
	public class SOOrderProcess : PXGraph<SOOrderProcess>
	{
		...
		public virtual void SOProcessFilter_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			if (filter != null && !String.IsNullOrEmpty(filter.Action))
			{
				...
				PXUIFieldAttribute.SetVisible<SOProcessFilter.printerName>(sender, filter,
				...
				PXUIFieldAttribute.SetVisible<SOProcessFilter.printerName>(sender, filter, showPrintSettings);

				PXUIFieldAttribute.SetEnabled<SOProcessFilter.definePrinterManually>(sender, filter, filter.PrintWithDeviceHub == true);
				PXUIFieldAttribute.SetEnabled<SOProcessFilter.printerName>(sender, filter, filter.PrintWithDeviceHub == true && filter.DefinePrinterManually == true);
		...
		public virtual void SOProcessFilter_RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
		{
			...
			{
				Filter.Current.PrinterName = new NotificationUtility(this).SearchPrinter(SONotificationSource.Customer, SOReports.PrintSalesOrder, Accessinfo.BranchID);
			}
		}
	}
