...
{
	[Serializable]
	public class SOOrderEntry : PXGraph<SOOrderEntry, SOOrder>, PXImportAttribute.IPXPrepareItems, IGraphWithInitialization
	{
		[Serializable]
		...
		public virtual IEnumerable POSupply()
		{
			...
			foreach (PXResult<POLine3, PO.POOrder, SOLineSplit> res in
				...
				And2<Where<POLine3.lineType, Equal<PO.POLineType.goodsForInventory>, //we should change this to In3<> once it is available
                    ...
                    Or<POLine3.lineType, Equal<PO.POLineType.goodsForServiceOrder>,
					Or<POLine3.lineType, Equal<PO.POLineType.nonStockForSalesOrder>,
                    Or<POLine3.lineType, Equal<PO.POLineType.nonStockForServiceOrder>,
				...
				case 3:
					{
						...
						PXLongOperation.StartOperation(this, delegate ()
						{
							...
							SOOrderEntry.InvoiceOrder(adapter.Arguments, list, created, adapter.MassProcess, adapter.QuickProcessFlow);
		...
		public virtual IEnumerable CreateCCPaymentMethodHF(PXAdapter adapter)
		{
			...
			graph.GetCreatePaymentProfileForm(this, new GenericCustomerPaymentMethodAdaper<CustomerPaymentMethodC>(DefPaymentMethodInstance));
			return adapter.Get();
		}
		...
		public virtual IEnumerable SyncCCPaymentMethods(PXAdapter adapter)
		{
			...
			var graph = PXGraph.CreateInstance<CCCustomerInformationManagerGraph>();
			graph.GetNewPaymentProfiles(this, new GenericCustomerPaymentMethodAdaper<CustomerPaymentMethodC>(DefPaymentMethodInstance), 
				new GenericCustomerPaymentMethodDetailAdapter<CustomerPaymentMethodDetail>(DefPaymentMethodInstanceDetailsAll));
			this.Save.Press();
			return orderList;
		}
		#endregion
		...
		public virtual IEnumerable ValidateAddresses(PXAdapter adapter)
		{
			foreach (SOOrder current in adapter.Get<SOOrder>())
			{
				if (current != null)
				{
					...
					{
						PXAddressValidator.Validate<SOBillingAddress>(this, address, true);
					}
					...
					{
						PXAddressValidator.Validate<SOShippingAddress>(this, shipAddress, true);
					}
				}
			...
			if (_licenseLimits != null)
			{
				OnBeforeCommit += _licenseLimits.GetCheckerDelegate<SOOrder>(
					new TableQuery(TransactionTypes.LinesPerMasterRecord, typeof(SOLine), (graph) =>
					{
						...
						{
								new PXDataFieldValue<SOLine.orderType>(((SOOrderEntry)graph).Document.Current?.OrderType),
					...
					new TableQuery(TransactionTypes.SerialsPerDocument, typeof(SOLineSplit), (graph) =>
					{
						...
						return new PXDataFieldValue[]
						{
							new PXDataFieldValue<SOLineSplit.orderType>(order?.Behavior.IsIn(SOBehavior.CM, SOBehavior.IN) == true ? order?.OrderType : null),
		...
		[SOOrderShipment.ShippingRefNote(IsKey = true)]
			...
			if (this.IsContractBasedAPI)
			{
				CustomerPaymentMethodDetail PaymentProfileID = PXSelectJoin<
				   ...
				   Where<CustomerPaymentMethod.bAccountID, Equal<Current2<SOOrder.customerID>>,
					   ...
					   And<CustomerPaymentMethod.pMInstanceID, Equal<Current<SOOrder.pMInstanceID>>>>>>>.Select(this);
			...
			if (order != null)
			{
				...
				if (paymentProfileID != null)
				{
					...
					InnerJoin<PaymentMethodDetail,
						On<CustomerPaymentMethodDetail.paymentMethodID, Equal<PaymentMethodDetail.paymentMethodID>,
						And<PaymentMethodDetail.detailID, Equal<CustomerPaymentMethodDetail.detailID>>>>>,
					Where<CustomerPaymentMethod.bAccountID, Equal<Required<SOOrder.customerID>>,
						And<PaymentMethodDetail.isCCProcessingID, Equal<True>,
						And<CustomerPaymentMethod.paymentMethodID, Equal<Required<SOOrder.paymentMethodID>>,
		...
		protected virtual bool AllowChangingOverrideFreightAmount(SOOrder doc)
		{
			...
			ShipTerms terms = (ShipTerms)PXSelectorAttribute.Select<SOOrder.shipTermsID>(Document.Cache, doc);
				...
				|| !sender.ObjectsEqual<SOOrder.curyFreightCost, SOOrder.overrideFreightAmount>(e.OldRow, e.Row))
			{
				...
				else
				{
					if (!(soordertype.Current?.CalculateFreight == false))
					{
						...
						fc.CalcFreightCost<SOOrder, SOOrder.curyFreightCost>(sender, row);
						row.FreightCostIsValid = true;
						if (row.OverrideFreightAmount != true)
						{
							PXResultset<SOLine> res = Transactions.Select();
							fc.ApplyFreightTerms<SOOrder, SOOrder.curyFreightAmt>(sender, row, res.Count);
						}
					}
				}
			}
			...
			if (row != null)
			{
				...
				if (doc != null && doc.PMInstanceID == row.PMInstanceID)
				{
					...
					if (!String.IsNullOrEmpty(row.PaymentMethodID))
					{
                        ...
                        if(!isInserted)
					{
                            CCProcTran ccTran = PXSelectReadonly<CCProcTran, Where<CCProcTran.pMInstanceID, Equal<Required<CCProcTran.pMInstanceID>>,
									And<CCProcTran.procStatus, Equal<CCProcStatus.finalized>,
                                    And<CCProcTran.tranStatus, Equal<CCTranStatusCode.approved>>>>>.SelectWindowed(this, 0, 1, row.PMInstanceID);
		...
		protected virtual void CustomerPaymentMethodDetail_Value_FieldUpdated(PXCache cache, PXFieldUpdatedEventArgs e)
		{
			...
			if (def != null)
			{
				...
				{
					var graph = PXGraph.CreateInstance<CCCustomerInformationManagerGraph>();
					graph.GetOrCreatePaymentProfile(this, new GenericCustomerPaymentMethodAdaper<CustomerPaymentMethodC>(DefPaymentMethodInstance),
						new GenericCustomerPaymentMethodDetailAdapter<CustomerPaymentMethodDetail>(DefPaymentMethodInstanceDetailsAll));
				}
			}
		}
		...
		protected virtual void SOLineValidateMinGrossProfit(PXCache sender, SOLine row, MinGrossProfitClass mgpc)
		{
			...
			if (row.InventoryID != null && row.UOM != null && mgpc.CuryUnitPrice >= 0)
			{
				...
				mgpc.CuryUnitPrice = MinGrossProfitValidator<SOLine>.ValidateUnitPrice<SOLine.curyInfoID, SOLine.inventoryID, SOLine.uOM>(sender, row, inItem, itemCost, mgpc.CuryUnitPrice, sosetup.Current.MinGrossProfitValidation);

				if (mgpc.DiscPct != 0)
				{
					mgpc.DiscPct = MinGrossProfitValidator<SOLine>.ValidateDiscountPct<SOLine.inventoryID, SOLine.uOM>(sender, row, inItem, itemCost, row.UnitPrice, mgpc.DiscPct, sosetup.Current.MinGrossProfitValidation);
				}

				if (mgpc.CuryDiscAmt != 0 && row.Qty != null && Math.Abs(row.Qty.GetValueOrDefault()) != 0)
				{
					mgpc.CuryDiscAmt = MinGrossProfitValidator<SOLine>.ValidateDiscountAmt<SOLine.inventoryID, SOLine.uOM>(sender, row, inItem, itemCost, row.UnitPrice, mgpc.CuryDiscAmt, sosetup.Current.MinGrossProfitValidation);
				}
			}
		}
		...
		{
			if (PXAccess.FeatureInstalled<FeaturesSet.customerDiscounts>() && line.InventoryID != null && line.Qty != null && line.CuryLineAmt != null && line.IsFree != true)
			{
        ...
        protected virtual CarrierRequest BuildRateRequest(SOOrder order)
		{
        ...
        protected virtual CarrierRequest BuildQuoteRequest(SOOrder order, CarrierPlugin plugin)
		{
			SOShippingAddress shipAddress = Shipping_Address.Select();
			BAccount companyAccount = PXSelectJoin<BAccountR, InnerJoin<Branch, On<Branch.bAccountID, Equal<BAccountR.bAccountID>>>, Where<Branch.branchID, Equal<Required<Branch.branchID>>>>.Select(this, Accessinfo.BranchID);
			Address companyAddress = PXSelect<Address, Where<Address.addressID, Equal<Required<Address.addressID>>>>.Select(this, companyAccount.DefAddressID);
		...
		protected virtual CarrierBoxEx BuildCarrierBoxes(List<SOPackageInfoEx> list, Address origin, string method, string pluginUOM)
		{
			List<CarrierBox> boxes = new List<CarrierBox>();
			foreach (SOPackageInfoEx boxInfo in list)
			{
				boxes.Add(BuildCarrierPackage(boxInfo, pluginUOM));
			}
        ...
        protected virtual CarrierBox BuildCarrierPackage(SOPackageInfoEx boxInfo, string pluginUOM)
        {
		...
		protected virtual IList<SOPackageEngine.PackSet> CalculatePackages(SOOrder order, string carrierID)
		{
			...
			PXSelectBase<SOLine> selectLines = new PXSelectJoin<SOLine,
						InnerJoin<InventoryItem, On<SOLine.InventoryFK>>,
		...
		public static void InvoiceOrder(Dictionary<string, object> parameters, List<SOOrder> list, DocumentList<ARInvoice, SOInvoice> created, bool isMassProcess = false)
			=> InvoiceOrder(parameters, list, created, isMassProcess, PXQuickProcess.ActionFlow.NoFlow);
		public static void InvoiceOrder(Dictionary<string, object> parameters, List<SOOrder> list, DocumentList<ARInvoice, SOInvoice> created, bool isMassProcess, PXQuickProcess.ActionFlow quickProcessFlow)
		{
			SOShipmentEntry docgraph = PXGraph.CreateInstance<SOShipmentEntry>();
			SOInvoiceEntry ie = PXGraph.CreateInstance<SOInvoiceEntry>();
				...
				try
				{
							...
							.Select(docgraph, order.OrderNbr, order.OrderType, order.CuryInfoID, order.BillAddressID, order.BillContactID, order.CustomerID))
					{
						...
						if (((SOOrderType)res).RequireShipping == false || ((SOOrderTypeOperation)res).INDocType == INTranType.NoUpdate)
						{
                            ...
                            foreach (SOLine line in PXSelectJoin<SOLine, InnerJoin<InventoryItem, On<SOLine.InventoryFK>>, Where<SOLine.orderType, Equal<Required<SOLine.orderType>>, And<SOLine.orderNbr, Equal<Required<SOLine.orderNbr>>, And<SOLine.lineType, NotEqual<SOLineType.miscCharge>>>>>.Select(docgraph, order.OrderType, order.OrderNbr))
							{
                        ...
                        InnerJoin<SOLine, On<SOLineSplit.SOLineFK>>>,
					...
					foreach (PXResult<SOOrderShipment, SOOrder, CurrencyInfo, SOAddress, SOContact, SOOrderType, SOOrderTypeOperation> res in shipments)
					{
						try
						{
							...
							{
								PXTimeStampScope.SetRecordComesFirst(typeof(SOOrder), true);
								ie.InvoiceOrder((DateTime)ie.Accessinfo.BusinessDate, res, details, null, created, quickProcessFlow);
		...
		public virtual void PostOrder(INIssueEntry docgraph, SOOrder order, DocumentList<INRegister> list, SOOrderShipment orderShipment)
		{
			...
			foreach (PXResult<SOLine, SOLineSplit, SOOrderType, ARTran, INTran, INItemPlan, INPlanType> res in
				PXSelectJoin<SOLine,
				LeftJoin<SOLineSplit, On<SOLineSplit.SOLineFK>,
		...
		public class SOQuickProcess : PXGraphExtension<SOOrderEntry>
		{
			...
			{
				public const string DoNotEmail = "The {0} customer prefers not to receive invoices by email. Make sure that emailing invoice is needed.";
				public const string DoEmail = "The {0} customer prefers to receive invoices by email. Make sure that emailing invoice should be skipped.";
				public const string DoNotPrint = "The {0} customer prefers not to receive printed invoices. Make sure that printing invoice is needed.";
				public const string DoPrint = "The {0} customer prefers to receive printed invoices. Make sure that printing invoice should be skipped.";
			...
			protected virtual Tuple<string, int> OrderAvailabilityStatus(int? SiteID, DateTime? ShipDate)
			{
				...
				Func<int?, int?, decimal?> availabilityFetch = (InventoryID, SubItemID) =>
				{
					...
					if (allowShipNegQty)
					{
						INLotSerClass lotSerClass = PXSelectReadonly<INLotSerClass,
							Where<INLotSerClass.lotSerClassID, Equal<Required<INLotSerClass.lotSerClassID>>>>
							.Select(Base, delta.LotSerClassID);
