...
namespace PX.Objects.SO
{
	...
	public class SOLineSplitPlanIDAttribute : INItemPlanIDAttribute
	{
		...
		public override void Parent_RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
		{
				...
				|| !sender.ObjectsEqual<SOOrder.hold, SOOrder.cancelled, SOOrder.completed, SOOrder.backOrdered, SOOrder.shipComplete>(e.Row, e.OldRow))
			{
				...
				foreach (SOLineSplit split in PXSelect<SOLineSplit,
					Where<SOLineSplit.orderType, Equal<Current<SOOrder.orderType>>, And<SOLineSplit.orderNbr, Equal<Current<SOOrder.orderNbr>>>>>
		...
		where Where : IBqlWhere, new()
	{
		...
		private bool CheckInvoicedAndReturnedQty(PXCache sender, int? returnInventoryID, IEnumerable<SOInvoicedRecords.Record> invoiced, IEnumerable<ReturnRecord> returned)
		{
		...
		private class ReturnRecord
		{
			...
			public static ReturnRecord FromSOLine(SOLine l)
			{
			...
			public static ReturnRecord FromARTran(ARTran t)
			{
				return new ReturnRecord
				{
					InventoryID = t.InventoryID,
					UOM = t.UOM,
					Qty = Math.Abs(t.Qty ?? 0m)
				};
			}
		}
