		...
		And<ARTranAsSplit.lineType, NotEqual<SOLineType.freight>, And<ARTranAsSplit.lineType, NotEqual<SOLineType.discount>>>>>>
	{
		...
		public virtual void ARTranSplit_LotSerialNbr_FieldDefaulting(PXCache sender, PXFieldDefaultingEventArgs e)
		{
			...
			if (item != null)
			{
				...
				{
					foreach (ARTranAsSplit lssplit in INLotSerialNbrAttribute.CreateNumbers<ARTranAsSplit>(sender, item, mode, 1m))
					{
		...
		protected override void RaiseQtyExceptionHandling(PXCache sender, object row, object newValue, PXSetPropertyException e)
		{
			if (row is ARTran)
			{
				sender.RaiseExceptionHandling<ARTran.qty>(row, null, new PXSetPropertyException(e.MessageNoPrefix, PXErrorLevel.Warning, sender.GetStateExt<ARTran.inventoryID>(row), sender.GetStateExt<ARTran.subItemID>(row), sender.GetStateExt<ARTran.siteID>(row), sender.GetStateExt<ARTran.locationID>(row), sender.GetValue<ARTran.lotSerialNbr>(row)));
			}
			else
			{
				sender.RaiseExceptionHandling<ARTranAsSplit.qty>(row, null, new PXSetPropertyException(e.MessageNoPrefix, PXErrorLevel.Warning, sender.GetStateExt<ARTranAsSplit.inventoryID>(row), sender.GetStateExt<ARTranAsSplit.subItemID>(row), sender.GetStateExt<ARTranAsSplit.siteID>(row), sender.GetStateExt<ARTranAsSplit.locationID>(row), sender.GetValue<ARTranAsSplit.lotSerialNbr>(row)));
			}
		}
