...
namespace PX.Objects.PO
{
	...
	public partial class POLine : PX.Data.IBqlTable, IItemPlanMaster, ISortOrder, ITaxableDetail
	{
		#region Selected
		...
		[PXRestrictor(typeof(Where2<SameOrganizationBranch<INSite.branchID, Current<POOrder.branchID>>,
			Or2<FeatureInstalled<FeaturesSet.interBranch>, Or<Current<POOrder.orderType>, Equal<POOrderType.standardBlanket>>>>), Common.Messages.InterBranchFeatureIsDisabled)]
        ...
        [ManualDiscountMode(typeof(POLine.curyDiscAmt), typeof(POLine.curyExtCost), typeof(POLine.discPct))]
        [PXDBBool()]
        [PXDefault(false)]
		...
		[Account(typeof(POLine.branchID), DescriptionField = typeof(Account.description), DisplayName = "Accrual Account", Filterable = false)]
		[PXDefault(PersistingCheck = PXPersistingCheck.Nothing)]
		...
		[CostCode(typeof(expenseAcctID), typeof(taskID), GL.AccountType.Expense)]
		...
		[PXFormula(typeof(Switch<
			...
			Switch<Case<Where<Current<POSetup.addServicesFromNormalPOtoPR>, Equal<True>, And<Current<POLine.orderType>, In3<POOrderType.regularOrder, POOrderType.regularSubcontract>,
		...
		public abstract class curyReceivedCost : IBqlField
		{
		}
		[PXDecimal(4)]
		[Obsolete("The field is preserved only for the support of the Default Endpoints.")]
	...
	public static class POLineType
	{
        ...
        public class goodsForServiceOrder : Constant<string>
        {
        ...
        public class nonStockForServiceOrder : Constant<string>
        {
		...
		public class Goods : SetOfConstants<string, Goods.Provider>
		{
			public class Provider : ISetProvider
			{
				public Constant<String>[] Constants =>
					new Constant<String>[]
					{
						new goodsForInventory(),
						new goodsForDropShip(),
						new goodsForSalesOrder(),
                        new goodsForServiceOrder(),
						new goodsForReplenishment(),
						new goodsForManufacturing()
					};
			}
		}

		public static bool IsNonStock(string lineType) => NonStocks.ContainsValue(lineType);
		public class NonStocks : SetOfConstants<string, NonStocks.Provider>
		{
			public class Provider : ISetProvider
			{
				public Constant<String>[] Constants =>
					...
					{
						new nonStock(),
						new nonStockForDropShip(),
						new nonStockForSalesOrder(),
                        new nonStockForServiceOrder(),
						new service(),
						new nonStockForManufacturing()
					};
			}
		}
