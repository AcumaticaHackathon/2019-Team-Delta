...
using System.Linq;
using POReceiptLineAdd = PX.Objects.AP.APInvoiceEntry.POReceiptLineAdd;
using LinkLineReceipt = PX.Objects.AP.APInvoiceEntry.LinkLineReceipt;
using POOrderRS = PX.Objects.AP.APInvoiceEntry.POOrderRS;
using POAccrualComparer = PX.Objects.AP.APInvoiceEntry.POAccrualComparer;
using POProcessingInfo = PX.Objects.AP.APInvoiceEntry.POProcessingInfo;
using POReceiptFilter = PX.Objects.AP.APInvoiceEntry.POReceiptFilter;
using POReceiptLineS = PX.Objects.AP.APInvoiceEntry.POReceiptLineS;
using POLineS = PX.Objects.AP.APInvoiceEntry.POLineS;
		...
		public virtual IEnumerable AddReceiptLine2(PXAdapter adapter)
		{
				...
				Base.Document.Current.Prebooked == false)
			{
				...
				foreach (POReceiptLineAdd it in poReceiptLinesSelection.Select())
				{
					if (it.Selected == true)
					{
						Base.AddPOReceiptLine(it, duplicates, ref serviceLinesInfo);
						containsRetainage |= ((it.RetainagePct ?? 0m) != 0m);
				...
				Base.CheckInvoicedServices(serviceLinesInfo);
