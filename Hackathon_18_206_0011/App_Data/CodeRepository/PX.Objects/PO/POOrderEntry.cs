...
namespace PX.Objects.PO
{
	...
	public class POOrderEntry : PXGraph<POOrderEntry, POOrder>, PXImportAttribute.IPXPrepareItems, IGraphWithInitialization
	{
		...
		protected virtual void _(Events.FieldSelecting<POOrderAPDoc.statusText> e)
		{
			...
			var query =
				new PXSelectGroupBy<APTranSigned,
					...
					Aggregate<Sum<APTranSigned.signedBaseQty, Sum<APTranSigned.signedCuryLineAmt, Sum<APTranSigned.pOPPVAmt>>>>>(this);
			...
			e.ReturnValue = PXMessages.LocalizeFormatNoPrefix(Messages.StatusTotalBilledTotalPPV,
				...
				FormatQty(apTranTotal?.SignedCuryLineAmt ?? 0),
		...
		public virtual IEnumerable ViewOrigChangeOrder(PXAdapter adapter)
		{
			...
			{
				PMChangeOrder cOrder = PXSelect<PMChangeOrder, Where<PMChangeOrder.refNbr, Equal<Required<PMChangeOrder.refNbr>>>>.Select(this, ChangeOrders.Current.RefNbr);
				if (cOrder != null && cOrder.OrigRefNbr != null)
				{
					...
					target.Document.Current = PXSelect<PMChangeOrder, Where<PMChangeOrder.refNbr, Equal<Required<PMChangeOrder.origRefNbr>>>>.Select(this, cOrder.OrigRefNbr);
		...
		public POOrderEntry()
		{
			...
			this.ChangeOrders.Cache.AllowSelect = false;
			if (PXAccess.FeatureInstalled<FeaturesSet.projectModule>())
			{
				PMSetup pmsetup = PXSelect<PMSetup>.Select(this);
				if (pmsetup != null && pmsetup.CostCommitmentTracking == true)
				{
					this.ChangeOrders.Cache.AllowSelect = true;
				}
			}
			...
			bool requireSingleProject = (apSetup.RequireSingleProjectPerDocument == true);
			PXUIFieldAttribute.SetVisible<POOrder.projectID>(Document.Cache, null, isPMVisible && requireSingleProject);
			PXDefaultAttribute.SetPersistingCheck<POOrder.projectID>(Document.Cache, null,
			...
			if (_licenseLimits != null)
			{
				OnBeforeCommit += _licenseLimits.GetCheckerDelegate<POOrder>(
					new TableQuery(TransactionTypes.LinesPerMasterRecord, typeof(POLine), (graph) =>
					{
						...
						{
							new PXDataFieldValue<POLine.orderType>(((POOrderEntry)graph).Document.Current?.OrderType),
        ...
        public virtual void CreatePrePaymentProc(APInvoiceEntry target)
        {
            ...
            {
                if (line.UnbilledQty == Decimal.Zero)
                {
                    recalculateTaxDetails = true;
					if (line.LineType != POLineType.Service || line.Closed == true)
                    continue;                    
                }
            ...
            if (recalculateTaxDetails)
            {
                ...
                {
					TaxAttribute.Calculate<APTran.taxCategoryID>(target.Transactions.Cache,
		...
		public static bool NeedsPOReceipt(POLine aLine, bool skipCompleted, POSetup poSetup)
		{
			...
			if (aLine.LineType.IsIn(
				...
				|| aLine.LineType == POLineType.Service
					...
					|| aLine.OrderType.IsIn(POOrderType.RegularOrder, POOrderType.RegularSubcontract) && poSetup.AddServicesFromNormalPOtoPR == true)))
			{
				return true;
			}
			return false;
		...
		public virtual bool NeedsAPInvoice()
		{
			...
			foreach (POLine persistedLine in PXSelectReadonly2<POLine,
				LeftJoin<APTran, On<APTran.pOOrderType, Equal<POLine.orderType>, And<APTran.pONbr, Equal<POLine.orderNbr>, And<APTran.pOLineNbr, Equal<POLine.lineNbr>,
        ...
        protected virtual void POOrder_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
        {
            ...
            if (doc.Cancelled != true && doc.Status != POOrderStatus.Closed)
            {
                if (!this._blockUIUpdate)
                {
					...
					PXUIFieldAttribute.SetEnabled<POOrder.projectID>(Document.Cache, null, isPMVisible && requireSingleProject);
		...
		protected virtual void POOrder_PayToVendorID_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			...
			{
				if (IsAccrualAccountRequired(line))
				{
					try
					{
		...
		protected virtual void POOrder_RowDeleting(PXCache sender, PXRowDeletingEventArgs e)
		{
			...
			{
				throw new PXException(Messages.CanNotDeleteWithChangeOrderBehavior);
			}
			...
			if (document.Status == POOrderStatus.Open)
			{
				...
				POLine line = PXSelect<POLine,
					Where<POLine.orderType, Equal<Required<POLine.orderType>>, And<POLine.orderNbr, Equal<Required<POLine.orderNbr>>,
						And<POLine.closed, Equal<Required<POLine.closed>>>>>>
		...
		protected virtual void POLine_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			else
			{
				if (!this._blockUIUpdate)
				{
					...
					PXUIFieldAttribute.SetEnabled<POLine.pOAccrualAcctID>(sender, e.Row, IsAccrualAccountRequired(row));
					PXUIFieldAttribute.SetEnabled<POLine.pOAccrualSubID>(sender, e.Row, IsAccrualAccountRequired(row));
				}
        ...
        protected virtual void POLine_RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
        {
            ...
            if (!sender.ObjectsEqual<POLine.branchID>(e.Row, e.OldRow) || !sender.ObjectsEqual<POLine.inventoryID>(e.Row, e.OldRow) ||
					!sender.ObjectsEqual<POLine.baseOrderQty>(e.Row, e.OldRow) || !sender.ObjectsEqual<POLine.curyUnitCost>(e.Row, e.OldRow) || !sender.ObjectsEqual<POLine.curyExtCost>(e.Row, e.OldRow) ||
		...
		public virtual void VerifyProjectLockCommitments(int? newProjectID)
		{
			...
			PMProject project;
			if (ProjectDefaultAttribute.IsProject(this, newProjectID, out project) && project.LockCommitments == true)
			{
		...
		public partial class POOrderS : IBqlTable
		{
			...
			#endregion
			#region LineTotalQty
			public abstract class lineTotalQty : IBqlField
			{
			}
			...
			public abstract class baseLineTotalQty : IBqlField
			{
			}
			[PXDBDecimal(6, BqlField = typeof(POLine.baseOrderQty))]
			...
			public abstract class curyLineTotalCost : IBqlField
			{
			}
			...
			public abstract class lineTotalCost : IBqlField
			{
			}
			...
			public abstract class curyLeftToReceiveCost : IBqlField
			{
			}
			...
			public abstract class leftToReceiveCost : IBqlField
			{
			}
			[PXBaseCury]
		...
		[PXDefault(typeof(Search<CREmployee.userID,
				Where<CREmployee.userID, Equal<Current<AccessInfo.userID>>,
					And<Current<POOrder.workgroupID>, IsNull>>>),
		...
		public virtual void SetBehaviorBasedOnLines()
		{
			if (Document.Current != null && PXAccess.FeatureInstalled<FeaturesSet.changeOrder>())
			{
				...
				if (pmsetup != null && pmsetup.CostCommitmentTracking == true)
				{
					HashSet<int?> projectsUsed = new HashSet<int?>();
					foreach (POLine line in Transactions.Select())
					{
						if (line.TaskID != null && line.ProjectID != null)
							projectsUsed.Add(line.ProjectID);
					}
										
					if (projectsUsed.Count > 0)
					{
						var selectProjectWithChangeOrderMode = new PXSelect<PMProject, Where<PMProject.changeOrderWorkflow, Equal<True>,
							And<PMProject.contractID, In<Required<PMProject.contractID>>>>>(this);

						changeOrderBehaviorIsRequired = selectProjectWithChangeOrderMode.SelectWindowed(0, 1, projectsUsed.ToArray()).Any();
					}
				}

				if (changeOrderBehaviorIsRequired && Document.Current.Behavior != POBehavior.ChangeOrder)
				{
					Document.Current.Behavior = POBehavior.ChangeOrder;
					Document.Update(Document.Current);
				}
				else if (!changeOrderBehaviorIsRequired && Document.Current.Behavior == POBehavior.ChangeOrder)
				{
					Document.Current.Behavior = POBehavior.Standard;
					Document.Update(Document.Current);
				}
			}
		}
		...
		public virtual void FillPOLineFromDemand(POLine dest, POFixedDemand demand, string OrderType, SOLineSplit3 solinesplit)
		{
            ...
            else
            {
                if (solinesplit != null)
                {
                    ...
                    INPostClass postclass = (PXResult<InventoryItem, INPostClass>)
                        PXSelectJoin<InventoryItem,
                        InnerJoin<INPostClass, On<INPostClass.postClassID, Equal<InventoryItem.postClassID>>>,
                        Where<InventoryItem.inventoryID, Equal<Required<InventoryItem.inventoryID>>>>.Select(this, solinesplit.InventoryID);
                    dest.ExpenseSubID = (solinesplit.LineType == SOLineType.NonInventory && postclass != null && postclass.COGSSubFromSales == true
                        ? solinesplit.SalesSubID
                        : null);
                }
            }
