...
namespace PX.Objects.PO
{
	...
	public class POCreate : PXGraph<POCreate>
	{
				...
				try
				{
					...
					if (OrderType == POOrderType.RegularOrder)
					{
						...
						if (order != null && order.ShipDestType == POShippingDestination.CompanyLocation && order.SiteID == null)
						{
							...
							{
								order = created.Find<POOrder.orderType, POOrder.vendorID, POOrder.vendorLocationID, POOrder.projectID, POOrder.bLOrderNbr>(OrderType, demand.VendorID, demand.VendorLocationID, project, BLOrderNbr) ?? new POOrder();
							}
							else
							{
						...
						else
						{
							if (requireSingleProject)
							{
								order = created.Find<POOrder.orderType, POOrder.vendorID, POOrder.vendorLocationID, POOrder.siteID, POOrder.projectID, POOrder.bLOrderNbr>(OrderType, demand.VendorID, demand.VendorLocationID, demand.POSiteID, project, BLOrderNbr) ?? new POOrder();
							}
							else
							{
				    ...
				    if (OrderType == POOrderType.RegularOrder && demand.PlanType != INPlanConstants.Plan6B)
					{
						line = setup.CopyLineDescrSO == true && soline != null
								? ordered.Find
									...
									POLine.requestedDate, POLine.tranDesc, POLine.projectID, POLine.taskID, POLine.costCodeID>
									(demand.VendorID, demand.VendorLocationID, demand.POSiteID, demand.InventoryID, demand.SubItemID,
									 ...
									 soline != null ? soline.CostCodeID : null)
								: ordered.Find
									<POLine.vendorID, POLine.vendorLocationID, POLine.siteID, POLine.inventoryID, POLine.subItemID,
									POLine.requestedDate, POLine.projectID, POLine.taskID, POLine.costCodeID>
									(demand.VendorID, demand.VendorLocationID, demand.POSiteID, demand.InventoryID, demand.SubItemID,
									 soline != null ? soline.ShipDate : null,
									 soline != null ? soline.ProjectID : null,
									 soline != null ? soline.TaskID : null,
		...
		public partial class POCreateFilter : IBqlTable
		{
            ...
            [PXDBString(15, IsUnicode = true, InputMask = ">CCCCCCCCCCCCCCC")]
            [PXUIField(DisplayName = "Service Order Nbr.", FieldClass = "SERVICEMANAGEMENT")]
    ...
    [POCreate.POCreateProjectionAttribute]
        ...
        //Fields for Service Management info
        #region SrvOrdType
        public abstract class srvOrdType : PX.Data.IBqlField
        {
        }
