...
namespace PX.Objects.PO.LandedCosts
{
	public class LandedCostINAdjustmentFactory
	{
		...
		{
			var result = PXSelect<LandedCostCode, Where<LandedCostCode.landedCostCodeID, Equal<Required<LandedCostCode.landedCostCodeID>>>>.Select(_pxGraph, landedCostCodeID);
		...
		protected virtual void GetLCVarianceAccountSub(ref int? aAccountID, ref int? aSubID, POReceiptLine receiptLine)
		{
			if (receiptLine.InventoryID.HasValue)
			{
				...
				PXResult<InventoryItem, INPostClass> res = (PXResult<InventoryItem, INPostClass>)PXSelectJoin<InventoryItem,
									...
									Where<InventoryItem.inventoryID, Equal<Required<POLine.inventoryID>>>>.Select(_pxGraph, receiptLine.InventoryID);
				if (res != null)
				{
					...
					if ((bool)invItem.StkItem)
					{
						...
						if (receiptLine.SiteID != null)
							invSite = PXSelect<INSite, Where<INSite.siteID, Equal<Required<POReceiptLine.siteID>>>>.Select(_pxGraph, receiptLine.SiteID);
