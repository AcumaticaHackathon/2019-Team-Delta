...
namespace PX.Objects.PO
{
	...
	public class POLandedCostDocEntry : PXGraph<POLandedCostDocEntry, POLandedCostDoc>
	{
		...
		{
			return new LandedCostINAdjustmentFactory(this);
		}
		...
		[PXCopyPasteHiddenView]
		public TaxTranSelect<POLandedCostDoc, POLandedCostDoc.termsID, POLandedCostTaxTran, POLandedCostTaxTran.taxID,
		...
		protected virtual void _(Events.RowSelected<POLandedCostDoc> e)
		{
			...
			PXUIFieldAttribute.SetEnabled<POLandedCostDoc.curyID>(e.Cache, e.Row, !isReleased);
		...
		protected virtual void _(Events.RowPersisting<POLandedCostDetail> e)
		{
			...
			if (e.Operation != PXDBOperation.Delete && doc.Hold == false && e.Row.CuryLineAmt <= Decimal.Zero)
			{
				throw new PXRowPersistingException(typeof(POLandedCostDetail.curyLineAmt).Name, null, CS.Messages.Entry_GT, 0);
			}
		}
		...
		protected virtual void ValidateDocument()
		{
			if (Document.Current != null)
			{
				...
				if (doc.Hold == false)
				{
					...
					{
						if (detail.CuryLineAmt <= 0)
						{
							Details.Cache.RaiseExceptionHandling<POLandedCostDetail.curyLineAmt>(detail, detail.CuryLineAmt,
								new PXSetPropertyException(CS.Messages.Entry_GT, PXErrorLevel.Error, 0));
							showException = true;
						}

					}

					if (showException) throw new PXException(CS.Messages.Entry_GT, 0);
				}
		...
		public virtual IEnumerable AddPOReceipt(PXAdapter adapter)
		{
			    ...
			    Document.Current.Released != true)
			{
				if (poReceiptSelection.AskExt((graph, view) =>
				{
					...
					poReceiptSelection.Cache.ClearQueryCache();
		...
		protected virtual void _(Events.FieldVerifying<POLandedCostDoc.billDate> e)
		{
			...
			if (e.NewValue != null)
			{
				...
				if (fp == null)
					throw new PXSetPropertyException<POLandedCostDoc.billDate>(GL.Messages.TranDateOutOfRange, e.NewValue);

				var glsetup = (GLSetup)PXSetup<GLSetup>.Select(this);

				if (fp.Active != true)
					throw new PXSetPropertyException<POLandedCostDoc.billDate>(GL.Messages.FiscalPeriodInactive, fp.FinPeriodID);

				if (row.CreateBill == true && fp.APClosed == true && glsetup.PostClosedPeriods == false)
					throw new PXSetPropertyException<POLandedCostDoc.billDate>(GL.Messages.PeriodClosedInAPModule, fp.FinPeriodID);
			}
		}
		...
		public virtual void AllocateLandedCosts()
		{
			...
			var doc = Document.Current;
			var landedCostReceiptLines = this.ReceiptLines.Select().Select(t => (POLandedCostReceiptLine) t).ToList();
			var details = Details.Select().Select(t => (POLandedCostDetail) t).ToList();
			...
			var taxes = taxesQuery.Select(doc.DocType, doc.RefNbr).Select(t=> (PXResult<POLandedCostTax, Tax>)t).ToList();
			...
			IEnumerable<LandedCostAllocationService.POLandedCostReceiptLineAdjustment> adjustments)
		{
			...
			var inAdjustmentFactory = GetInAdjustmentFactory();
		...
		{
			var currentSplits = Splits.Select().RowCast<POLandedCostSplit>().ToList();
			var obloleteSplits =
				currentSplits.Where(t => !landedCostSplits.Contains(t, new POLandedCostSplitEqualityComparer()));

			foreach (var poLandedCostSplit in landedCostSplits)
			{
				var currentSplit = currentSplits.SingleOrDefault(t =>
					t.ReceiptLineNbr == poLandedCostSplit.ReceiptLineNbr && t.DetailLineNbr == poLandedCostSplit.DetailLineNbr);

				if (currentSplit != null)
				{
					currentSplit.CuryInfoID = poLandedCostSplit.CuryInfoID;
					currentSplit.CuryLineAmt = poLandedCostSplit.CuryLineAmt;
					currentSplit.LineAmt = poLandedCostSplit.LineAmt;

					Splits.Cache.Update(currentSplit);
				}
				else
				{
					Splits.Cache.Insert(poLandedCostSplit);
				}
			}

			foreach (var obloleteSplit in obloleteSplits)
			{
				Splits.Cache.Delete(obloleteSplit);
			}
		}
