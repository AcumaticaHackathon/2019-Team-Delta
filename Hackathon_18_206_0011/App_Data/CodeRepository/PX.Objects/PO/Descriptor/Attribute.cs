...
namespace PX.Objects.PO
{
	...
	public class POTaxAttribute : TaxAttribute
	{
		...
		protected abstract class curyRetainageAmt : IBqlField
		{
		}
		...
		protected virtual bool IsRetainedTaxes(PXGraph graph)
		{
	...
	public class POUnbilledTaxAttribute : POTaxAttribute
	{
			...
			decimal CuryTaxDiscountTotal)
		{
			...
			if (curyOpenDiscTotal > 0.00005m && curyOpenLineTotal > 0.00005m)
			{
				curyOpenTaxTotal *= (curyOpenLineTotal - curyOpenDiscTotal) / curyOpenLineTotal;
			}
		...
		Where<POReceiptLineSplit.receiptNbr, Equal<Current<POReceipt.receiptNbr>>>>
	{
			...
			if (fetchMode.HasFlag(AvailabilityFetchMode.TryOptimize) && _detailsRequested++ == 5)
			{
				...
				var select = new PXSelectReadonly2<POReceiptLine,
					InnerJoin<INSiteStatus,
						On<POReceiptLine.SiteStatusFK>,
					LeftJoin<INLocationStatus,
						On<POReceiptLine.LocationStatusFK>,
					LeftJoin<INLotSerialStatus,
						On<POReceiptLine.LotSerialStatusFK>>>>,
        ...
        {
            var inventory = (InventoryItem)PXSelectorAttribute.Select(sender, null, typeof(POReceiptLine.inventoryID).Name, InventoryID);
            if (inventory == null)
                return null;
            var lsclass = (INLotSerClass)PXSelectReadonly<INLotSerClass, 
                Where<INLotSerClass.lotSerClassID, Equal<Required<INLotSerClass.lotSerClassID>>>>.Select(sender.Graph, inventory.LotSerClassID);

            return new PXResult<InventoryItem, INLotSerClass>(inventory, lsclass ?? new INLotSerClass());
        }
    }
