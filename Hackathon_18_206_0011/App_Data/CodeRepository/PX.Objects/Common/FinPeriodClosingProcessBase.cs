	...
	{ 
		...
		public class FinPeriodClosingProcessParameters : IBqlTable
		{
			...
			public abstract class action : IBqlField
			{
				...
				public class close : Constant<string> { public close() : base(FinPeriodStatusProcess.FinPeriodStatusProcessParameters.action.Close) { } }
				public class reopen : Constant<string> { public reopen() : base(FinPeriodStatusProcess.FinPeriodStatusProcessParameters.action.Reopen) { } }
		...
		public class UnprocessedObjectsQueryParameters : IBqlTable
		{
			...
			public abstract class fromFinPeriodStartDate : IBqlField { }
			[PXDate]
			public virtual DateTime? FromFinPeriodStartDate { get; set; }
			#endregion

			#region ToFinPeriodEndDate
			public abstract class toFinPeriodEndDate : IBqlField { }
			[PXDate]
			...
			where TOrganizationID : IBqlOperand
		{
			private static IBqlCreator Where =>
				new Where<TFinPeriodID, GreaterEqual<Current<UnprocessedObjectsQueryParameters.fromFinPeriodID>>,
		...
		where TSubledgerClosedFlagField : IBqlField
	{
		...
		public override void ClosePeriod(FinPeriod finPeriod)
		{
			if (PXAccess.FeatureInstalled<FeaturesSet.centralizedPeriodsManagement>())
			{
				...
				{
					VerifyOpenDocuments(organizationFinPeriod);
