...
namespace PX.Objects.Common.Discount
{
	public abstract class DiscountEngine : PXGraph<DiscountEngine>
	{
			...
			bool single)
		{
			...
			BqlCommand command = BqlTemplate.OfCommand<
				...
				Where2<
					...
					BqlPlaceholder1>, // Additional conditions placeholder
				OrderBy<Desc<DiscountSequenceDetail.discount>>>>.Replace<BqlPlaceholder1>(BqlCommand.Compose(discountTypes.ToArray())).ToCommand();
		...
		protected ARSalesPrice GetARPrice(PXCache cache, int inventoryID, int customerID, string customerPriceClassID, string curyID, string UOM, decimal quantity, DateTime date)
		{
			ARSalesPrice salesPrice =
				PXSelect<ARSalesPrice,
				Where<ARSalesPrice.inventoryID, Equal<Required<ARSalesPrice.inventoryID>>, 
				...
				.SelectWindowed(cache.Graph, 0, 1, inventoryID, curyID, UOM, quantity, date, date, date, customerID, customerPriceClassID);
			return salesPrice;
		}

		//Returns best APVendorPrice
		protected APVendorPrice GetAPPrice(PXCache cache, int inventoryID, int vendorID, string curyID, string UOM, decimal quantity, DateTime date)
		{
			APVendorPrice salesPrice =
				...
				Where<APVendorPrice.inventoryID, Equal<Required<APVendorPrice.inventoryID>>,
					...
					And<APVendorPrice.breakQty, LessEqual<Required<APVendorPrice.breakQty>>,

					And2<Where2<Where<APVendorPrice.effectiveDate, LessEqual<Required<APVendorPrice.effectiveDate>>,
						And<APVendorPrice.isPromotionalPrice, Equal<False>>>,
					Or<Where<APVendorPrice.effectiveDate, LessEqual<Required<APVendorPrice.effectiveDate>>,
						And<APVendorPrice.expirationDate, GreaterEqual<Required<APVendorPrice.expirationDate>>, And<APVendorPrice.isPromotionalPrice, Equal<True>>>>>>,

					And<Where<APVendorPrice.vendorID, Equal<Required<APVendorPrice.vendorID>>>>>>>>>,
				OrderBy<Desc<APVendorPrice.vendorID, Desc<APVendorPrice.isPromotionalPrice, Desc<APVendorPrice.breakQty>>>>>
				.SelectWindowed(cache.Graph, 0, 1, inventoryID, curyID, UOM, quantity, date, date, date, vendorID);
			return salesPrice;
		}
		#endregion
