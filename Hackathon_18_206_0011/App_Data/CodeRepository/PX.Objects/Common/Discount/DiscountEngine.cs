...
namespace PX.Objects.Common.Discount
{
		...
		where TDiscountDetail : class, IBqlTable, IDiscountDetail, new()
	{
			...
			DiscountCalculationOptions discountCalculationOptions = DefaultDiscountCalculationParameters)
		{
			...
			if (!discountCalculationOptions.HasFlag(DiscountCalculationOptions.DisableGroupAndDocumentDiscounts) && ((!cache.Graph.IsImport || discountCalculationOptions.HasFlag(DiscountCalculationOptions.CalculateDiscountsFromImport)) || cache.Graph.IsMobile))
			{
			...
			DiscountCalculationOptions discountCalculationOptions)
		{
			try
			{
				...
				if (recalcFilter.RecalcTarget == RecalcDiscountsParamFilter.AllLines)
				{
					...
					{
						RecalculatePricesAndDiscountsOnLine(cache, line, recalcFilter, discountCalculationOptions);
					}
				}
		...
		public virtual void AutoRecalculatePricesAndDiscounts(
			...
			DiscountCalculationOptions discountCalculationOptions)
		{
			if (locationID == null || date == null || !IsDiscountFeatureEnabled(discountCalculationOptions))
				return;
			...
			if (!discountResult.IsEmpty)
			{
				...
				else
				{
					...
					if (lineDiscountTarget == AR.LineDiscountTargetType.SalesPrice)
					{
						...
						decimal extPriceAfterDiscount = qtyVal * Math.Round(salesPriceAfterDiscount, precision, MidpointRounding.AwayFromZero);
						curyDiscAmt = qtyVal * (curyUnitPrice ?? 0) - PXCurrencyAttribute.Round(cache, row, extPriceAfterDiscount, CMPrecision.TRANCURY);
		...
		/// </summary>
		[Obsolete] //REMOVE
		protected virtual Dictionary<DiscountSequenceKey, DiscountedLines> GetExistingDiscountCodesWithApplicableLines(
			PXCache cache,
			List<TLine> documentDetails,
			PXSelectBase<TDiscountDetail> discountDetails,
			string discountType)
		{
			Dictionary<DiscountSequenceKey, DiscountedLines> discountsWithDiscountedLines = new Dictionary<DiscountSequenceKey, DiscountedLines>();
			List<TDiscountDetail> discountLines = GetDiscountDetailsByType(cache, discountDetails, discountType);
			...
			foreach (TLine line in documentDetails)
			{
				...
				foreach (TDiscountDetail discountLine in discountLines)
				{
					if (discountLine.LineNbr != null && discountedLine.DiscountsAppliedToLine != null && discountedLine.DiscountsAppliedToLine.Contains(discountLine.LineNbr ?? 0))
					{
						DiscountSequenceKey dSequence = new DiscountSequenceKey(discountLine.DiscountID, discountLine.DiscountSequenceID);

						if (discountsWithDiscountedLines.ContainsKey(dSequence))
						{
							discountsWithDiscountedLines[dSequence] = new DiscountedLines
							{
								DiscountableLines = discountsWithDiscountedLines[dSequence].DiscountableLines.Concat(new[] { line }).ToList(),
								DiscountableValues = new DiscountableValues
								{
									CuryDiscountableAmount = discountsWithDiscountedLines[dSequence].DiscountableValues.CuryDiscountableAmount + discountableValues.CuryDiscountableAmount,
									DiscountableQuantity = discountsWithDiscountedLines[dSequence].DiscountableValues.DiscountableQuantity + discountableValues.DiscountableQuantity
								}
							};
						}
						else
						{
							discountsWithDiscountedLines.Add(dSequence, new DiscountedLines { DiscountableLines = new List<TLine> { line }, DiscountableValues = discountableValues });
						}
					}
				}
			}

			return discountsWithDiscountedLines;
		}
			...
			foreach (TLine line in documentDetails)
			{
				...
				foreach (TDiscountDetail discount in discountsLinkedToLine.LeftsFor(line).Where(x => x.Type.IsIn(DiscountType.Document, DiscountType.ExternalDocument) && x.SkipDiscount != true))
				{
					...
					foreach (TLine dline in discountsLinkedToLine.RightsFor(discount))
					{
						...
						if (dlineDiscountFields.DiscountID == null || cachedDiscountTypes.ContainsKey(dlineDiscountFields.DiscountID) && !cachedDiscountTypes[dlineDiscountFields.DiscountID].ExcludeFromDiscountableAmt)
						{
		...
		protected virtual UnitPriceVal GetUnitPrice(PXCache cache, TLine line, int? locationID, string curyID, DateTime date)
		{
			...
			if (arsetup.ApplyLineDiscountsIfCustomerPriceDefined == false || arsetup.ApplyLineDiscountsIfCustomerClassPriceDefined == false)
			{
				...
				if (eFields.CustomerID != null && eFields.InventoryID != null)
				{
					...
					ARSalesPrice arSalesPrice = GetARPrice(cache, (int)eFields.InventoryID, (int)eFields.CustomerID, customerPriceClassID, curyID, aFields.UOM, (decimal)aFields.Quantity, date);
					if (arSalesPrice == null) return unitPriceVal;

					unitPriceVal.CuryUnitPrice = arSalesPrice.SalesPrice;
					unitPriceVal.isBAccountSpecific = arSalesPrice.CustomerID != null;
					unitPriceVal.isPriceClassSpecific = arSalesPrice.CustPriceClassID != null;
					unitPriceVal.isPromotional = arSalesPrice.IsPromotionalPrice ?? false;
		...
		//Returns one Discount Details line
		[Obsolete]
		private TDiscountDetail GetDiscountDetail(PXCache sender, PXSelectBase<TDiscountDetail> discountDetails, string discountID, string discountSequenceID, string type, int? lineNbr)
		{
			PXCache cache = sender.Graph.Caches[typeof(TDiscountDetail)];

			Type discountIDField = cache.GetBqlField(DiscountID);
			Type discountSequenceIDField = cache.GetBqlField(DiscountSequenceID);
			Type discountTypeField = cache.GetBqlField(TypeFieldName);
			Type lineNbrField = cache.GetBqlField(LineNbrFieldName);

			Type whereType = lineNbr == null
				? BqlTemplate
					.FromType(typeof(
						Where<BqlPlaceholder1, Equal<Required<BqlPlaceholder1>>,
							And<BqlPlaceholder2, Equal<Required<BqlPlaceholder2>>,
							And<BqlPlaceholder3, Equal<Required<BqlPlaceholder3>>>>>))
					.Replace<BqlPlaceholder1>(discountIDField)
					.Replace<BqlPlaceholder2>(discountSequenceIDField)
					.Replace<BqlPlaceholder3>(discountTypeField)
					.ToType()
				: BqlTemplate
					.FromType(typeof(Where<BqlPlaceholder1, Equal<Required<BqlPlaceholder1>>>))
					.Replace<BqlPlaceholder1>(lineNbrField)
					.ToType();
			...
			TDiscountDetail discountDetail = lineNbr == null
				? discountDetails.SelectSingle(discountID, discountSequenceID, type)
				: discountDetails.SelectSingle(lineNbr);

			if (discountDetail == null)
			{
				discountDetails.Cache.ClearQueryCache();
				discountDetail = lineNbr == null
					? discountDetails.SelectSingle(discountID, discountSequenceID, type)
					: discountDetails.SelectSingle(lineNbr);
			}
