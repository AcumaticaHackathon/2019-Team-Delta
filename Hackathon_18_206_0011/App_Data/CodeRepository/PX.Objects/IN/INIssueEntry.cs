...
{
	public class INIssueEntry : PXGraph<INIssueEntry, INRegister>, IGraphWithInitialization
	{
			...
			if (_licenseLimits != null)
			{
				OnBeforeCommit += _licenseLimits.GetCheckerDelegate<INRegister>(
					new TableQuery(TransactionTypes.LinesPerMasterRecord, typeof(INTran), (graph) =>
					{
						...
						{
							new PXDataFieldValue<INTran.docType>(((INIssueEntry)graph).issue.Current?.DocType),
							new PXDataFieldValue<INTran.refNbr>(((INIssueEntry)graph).issue.Current?.RefNbr),
							new PXDataFieldValue<INTran.createdByScreenID>("IN302000")
						};
					}),
					new TableQuery(TransactionTypes.SerialsPerDocument, typeof(INTranSplit), (graph) =>
					{
						return new PXDataFieldValue[]
						{
							new PXDataFieldValue<INTranSplit.docType>(((INIssueEntry)graph).issue.Current?.DocType),
							new PXDataFieldValue<INTranSplit.refNbr>(((INIssueEntry)graph).issue.Current?.RefNbr),
							new PXDataFieldValue<INTranSplit.createdByScreenID>("IN302000")
						};
					}));
			}
		}
		...
		protected virtual void INTran_ToLocationID_FieldDefaulting(PXCache sender, PXFieldDefaultingEventArgs e)
		{
			...
			INTran row = (INTran)e.Row;
			if (row == null || row.DocType != INDocType.Transfer) return;
