...
namespace PX.Objects.IN
{
	...
	public class PIGenerator : PXGraph<PIGenerator>
	{
		...
		[PXDBInt]
		protected virtual void INPIStatusItem_InventoryID_CacheAttached(PXCache sender)
		{
		}
		...
		public virtual List<PIPreliminaryResult> CalcPIRows(bool updateDB, bool saveEmpty)
		{
					...
					IntermediateResult)
			{
				...
				if (lss_rec.InventoryID != null) // nulls left-joined - non-lot-serial
				{
					...
					if (item.BookQty > lss_rec.QtyOnHand)
					{
						item.BookQty = lss_rec.QtyOnHand;
					}
				}
				else
				{
					...
					if (item.BookQty > ls_rec.QtyOnHand)
					{
						item.BookQty = ls_rec.QtyOnHand;
					}
				}
			...
			if (updateDB)
			{
										...
										gs.Method != PIMethod.ByCycle))
				{
					...
					if (siteLocationsAll)
					{
						...
						var alreadyAddedItem = this.Caches[typeof(INPIStatusItem)]
							.Inserted
							.Cast<INPIStatusItem>()
							.Select(i => i.InventoryID)
							.ToHashSet();

						var alreadyAddedLoc = this.Caches[typeof(INPIStatusLoc)]
							.Inserted
							.Cast<INPIStatusLoc>()
						...
						{
							InventoryItem i = r;
								LockInventory(this, null, i.InventoryID, true, alreadyAddedItem, alreadyAddedLoc);
						}
					}
					else
					{
		...
		{
			// The method is called when all warehouse locations are selected.
			// So if there is a full inventory lock we will find no items that can be added to new PI and locked.
			var fullInventoryLock = (INPIStatusItem)PXSelect<INPIStatusItem,
							Where<INPIStatusItem.active, Equal<True>,
								And<INPIStatusItem.siteID, Equal<Current<PIGeneratorSettings.siteID>>,
								And<INPIStatusItem.inventoryID, IsNull>>>>
							.SelectWindowed(this, 0, 1);

			if (fullInventoryLock != null)
				return cmd.WhereAnd<Where<True, Equal<False>>>();

			BqlCommand result = BqlCommand.AppendJoin<LeftJoin<INPIStatusItem,
									On<INPIStatusItem.active, Equal<True>,
										And<INPIStatusItem.siteID, Equal<Current<PIGeneratorSettings.siteID>>,
										And<INPIStatusItem.inventoryID, Equal<InventoryItem.inventoryID>>>>>>(cmd);

			return result.WhereAnd<Where<INPIStatusItem.recordID, IsNull>>();
		}
                ...
                if(t.FullName == typeof(INItemSite).FullName)
                {
                    if (!cmd.GetTables().Contains(typeof(INItemSite)))
                        result = BqlCommand.AppendJoin<InnerJoin<INItemSite,
                                                ...
                                                And<INItemSite.siteID, Equal<Current<PIGeneratorSettings.siteID>>>>>>(cmd);
                    continue;
                }
		...
		public static void LockInventory(PXGraph graph, int? locationID, int? inventoryID)
			=> LockInventory(graph, locationID, inventoryID, false, null, null);

		public static void LockInventory(
			PXGraph graph,
			int? locationID,
			int? inventoryID,
			bool skipStatusCheck,
			HashSet<int?> alreadyAddedItems,
			HashSet<int?> alreadyAddedLocs)
		{
										...
										And<INPIStatus.active, Equal<boolTrue>,
											And2<Where<INPIStatus.locationID, IsNull,
												...
												And<Where<INPIStatus.inventoryID, IsNull,
														Or<INPIStatus.inventoryID,
															Equal<Required<INPIStatus.inventoryID>>>>
												>>>>>
			...
			//LockInventory is called with both nulls only once
			bool itemIsAdded = false;
			bool locIsAdded = false;
			if (inventoryID != null || locationID != null)
			{
				itemIsAdded = alreadyAddedItems?.Contains(inventoryID)
								?? graph.Caches[typeof(INPIStatusItem)]
									.Inserted
									.Cast<INPIStatusItem>()
									.Any(rec => rec.InventoryID == inventoryID);

				locIsAdded = alreadyAddedLocs?.Contains(locationID)
								?? graph.Caches[typeof(INPIStatusLoc)]
									.Inserted
									.Cast<INPIStatusLoc>()
									.Any(rec => rec.LocationID == locationID);
			}
