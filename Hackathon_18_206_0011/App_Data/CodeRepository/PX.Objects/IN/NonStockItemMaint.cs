...
namespace PX.Objects.IN
{
	...
	public class InventoryItemMaintBase : PXGraph<InventoryItemMaintBase> { }
	...
	Where<INItemCategory.inventoryID, Equal<Current<InventoryItem.inventoryID>>>> Category;
		...
		protected virtual void InventoryItem_RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
		{
			...
			if (e.Operation == PXDBOperation.Insert || e.Operation == PXDBOperation.Update)
			{
                ...
                if (cls != null && cls.LotSerTrack != INLotSerTrack.NotNumbered && cls.LotSerNumShared == false && string.IsNullOrEmpty(row.LotSerNumVal))
				{
                    INLotSerSegment lsSegment = PXSelectReadonly<INLotSerSegment, Where<INLotSerSegment.lotSerClassID, Equal<Required<INLotSerSegment.lotSerClassID>>,
