...
{
    [Serializable]
	public class INTransferEntry : PXGraph<INTransferEntry, INRegister>, IGraphWithInitialization
    {
	    ...
	    public class INOpenPeriodTransferAttribute : INOpenPeriodAttribute
	    {
		    ...
		    protected class InTransferCalendarOrganizationIDProvider : CalendarOrganizationIDProvider
		    {
			    ...
			    public override SourcesSpecificationCollection GetSourcesSpecification(PXCache cache, object row)
			    {
				    ...
				    sourceSpecifications.PrimarySpecificationItem = SiteSpecification;
			...
			public INOpenPeriodTransferAttribute()
			    : base(typeof(INRegister.tranDate),
				    selectionModeWithRestrictions: SelectionModesWithRestrictions.All)
		    {

		    }

		    protected override FinPeriodIDFormattingAttribute CreateFormattingAttribute(
		    ...
		    {
			    return base.CreateFormattingAttribute(
				    sourceType,
				    searchType, 
				    branchSourceType, 
				    ...
				    checkFinPeriodExistenceForDate,
				    filterByOrganizationID,
				    useMasterOrganizationIDByDefault,
				    sourceSpecificationTypes,
				    new InTransferCalendarOrganizationIDProvider());
		    }
	    }
		...
		[INOpenPeriodTransferAttribute(RaiseErrorOnInactiveOrLockedPeriod = true)]
		...
		[TranPeriodID(typeof(INRegister.tranDate),
			branchSourceType: typeof(INRegister.siteID),
			branchSourceFormulaType: typeof(Selector<INRegister.siteID, INSite.branchID>))]
		public virtual void INRegister_TranPeriodID_CacheAttached(PXCache sender)
		{
			...
			if (_licenseLimits != null)
			{
				OnBeforeCommit += _licenseLimits.GetCheckerDelegate<INRegister>(
					new TableQuery(TransactionTypes.LinesPerMasterRecord, typeof(INTran), (graph) =>
					{
						...
						{
							new PXDataFieldValue<INTran.docType>(((INTransferEntry) graph).transfer.Current?.DocType),
							new PXDataFieldValue<INTran.refNbr>(((INTransferEntry) graph).transfer.Current?.RefNbr),
							new PXDataFieldValue<INTran.createdByScreenID>("IN304000")
						};
					}),
					new TableQuery(TransactionTypes.SerialsPerDocument, typeof(INTranSplit), (graph) =>
					{
						return new PXDataFieldValue[]
						{
							new PXDataFieldValue<INTranSplit.docType>(((INTransferEntry) graph).transfer.Current?.DocType),
							new PXDataFieldValue<INTranSplit.refNbr>(((INTransferEntry) graph).transfer.Current?.RefNbr),
							new PXDataFieldValue<INTranSplit.createdByScreenID>("IN304000")
						};
					}));
			}
		}
		...
		protected virtual void INTran_ToSiteID_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			if (e.Row != null)
			{
				foreach (INTranSplit item in this.splits.Select())
				{
