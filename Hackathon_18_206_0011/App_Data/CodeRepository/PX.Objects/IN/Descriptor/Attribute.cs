...
namespace PX.Objects.IN
{
	...
	public abstract class INItemPlanIDAttribute : INItemPlanIDBaseAttribute, IPXRowInsertedSubscriber, IPXRowUpdatedSubscriber, IPXRowDeletedSubscriber
	{
		...
		{
			return (plantype.InclQtySOBackOrdered.GetValueOrDefault() +
				plantype.InclQtySOBooked.GetValueOrDefault() +
				plantype.InclQtySODropShip.GetValueOrDefault() +
				plantype.InclQtySOFixed.GetValueOrDefault() +
				plantype.InclQtySOFixedProduction.GetValueOrDefault() +
				plantype.InclQtySOPrepared.GetValueOrDefault() +
				plantype.InclQtySOShipped.GetValueOrDefault() +
		...
		where Where : IBqlWhere, new()
	{
		...
		{
			if (_itemselect == null)
				_itemselect = new PXSelectJoin<InventoryItem, 
					...
					And<
						...
						Or<INLotSerClass.lotSerClassID, IsNotNull>>>>>(sender.Graph);
			if (inventoryID == null)
                return null;
		...
		[Obsolete("Use AvailabilityFetch(PXCache, ILSMaster, AvailabilityFetchMode) instead")]
		public virtual IQtyAllocated AvailabilityFetch(PXCache sender, ILSMaster Row, bool ExcludeCurrent)
			=> AvailabilityFetch(sender, Row, ExcludeCurrent ? AvailabilityFetchMode.ExcludeCurrent : AvailabilityFetchMode.None);
		...
		[Obsolete("Use AvailabilityFetch(PXCache, ILSDetail, AvailabilityFetchMode) instead")]
		...
		[Obsolete("Use AvailabilityFetch<TNode>(ILSDetail, IQtyAllocated, IStatus, AvailabilityFetchMode) instead")]
		protected virtual IQtyAllocated AvailabilityFetch<TNode>(ILSDetail Row, IQtyAllocated allocated, IStatus status, bool ExcludeCurrent)
			...
			=> AvailabilityFetch<TNode>(Row, allocated, status, ExcludeCurrent ? AvailabilityFetchMode.ExcludeCurrent : AvailabilityFetchMode.None);
	...
	public class INUnitAttribute : AcctSub2Attribute, IPXFieldVerifyingSubscriber, IPXRowSelectedSubscriber, IPXRowPersistingSubscriber
	{
        ...
        protected INUnitAttribute(bool @override): base()
        { }

        #endregion

        #region Constructors
        private INUnitAttribute(VerifyingMode verifyingMode):base()
        {
		...
		public virtual void FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
		{
            ...
            if (!VerifyOnCopyPaste && sender.Graph.IsCopyPasteContext)
                return;
            if (!_verifyOnSettedInventory && GetSelectorParameterValue(sender, e.Row, _InventoryType) != null)
                return;
		...
		protected virtual INUnit ReadUnit(PXCache sender, object data, string fromUnitID)
        {
			...
			switch (_verifyingMode)
			{
				...
				case VerifyingMode.GlobalUnitConversion:
					var toUnitID = (string)GetSelectorParameterValue(sender, data, _BaseUnitType);
					unit = INUnit.UK.ByGlobal.Find(sender.Graph, fromUnitID, toUnitID);
					break;
				case VerifyingMode.InventoryUnitConversion:
					var inventory = InventoryItem.PK.Find(sender.Graph, (int?)GetSelectorParameterValue(sender, data, _InventoryType));
					if (inventory == null)
						unit = ReadGlobalUnit(sender, fromUnitID);
					else
		...
		public virtual INUnit ReadConversion(PXCache cache, object data, string fromUnitID)
		{
			...
			switch (_verifyingMode)
			{
				case VerifyingMode.UnitCatalog:
					break;
				case VerifyingMode.GlobalUnitConversion:
					var toUnitID = (string)GetSelectorParameterValue(cache, data, _BaseUnitType);
					if (fromUnitID != toUnitID)
						return INUnit.UK.ByGlobal.Find(cache.Graph, fromUnitID, toUnitID);
					break;
				case VerifyingMode.InventoryUnitConversion:
					var inventory = InventoryItem.PK.Find(cache.Graph, (int?)GetSelectorParameterValue(cache, data, _InventoryType));
					if (inventory != null)
					{
						...
						return INUnit.UK.ByInventory.Find(cache.Graph, inventory.InventoryID, fromUnitID);
					}
					break;
			}
			return new INUnit
	...
	[PXDBInt]
	[PXUIField(DisplayName = "Inventory ID", Visibility = PXUIVisibility.Visible)]
	[PXRestrictor(typeof(Where<InventoryItem.itemStatus, NotEqual<InventoryItemStatus.unknown>>), IN.Messages.InventoryItemIsInStatus, typeof(InventoryItem.itemStatus), ShowWarning = true)]
				...
				: base(dimension, type, substituteKey, fieldList)
			{
				...
				var interBranchRestrictionCondition = BqlTemplate.OfCondition<
					Where2<FeatureInstalled<FeaturesSet.interBranch>,
						Or<Where2<SameOrganizationBranch<INSite.branchID, Current<BqlPlaceholder1>>,
							And<Current<BqlPlaceholder1>, Equal<Current<BqlPlaceholder1>>>>>>> // Workaround for the bug found in SameOrganizationBranch
					.Replace<BqlPlaceholder1>(restrictionBranchId)
					.ToType();

				_Attributes.Add(new PXRestrictorAttribute(interBranchRestrictionCondition, Common.Messages.InterBranchFeatureIsDisabled));
		...
		private BranchScopeDimensionSelector PrepareSelectorAttr(Type transferTypeField, Type restrictionBranchId)
		{
			Type selectorType = BqlTemplate.OfCommand<Search<INSite.siteID,
				Where2<Where<INSite.siteID, NotEqual<SiteAttribute.transitSiteID>, And<Match<Current<AccessInfo.userName>>>>,
					Or<BqlPlaceholder1, Equal<INTransferType.twoStep>, And<INSite.siteID, NotEqual<SiteAttribute.transitSiteID>>>>>>
						.Replace<BqlPlaceholder1>(typeof(IBqlField).IsAssignableFrom(transferTypeField)
			...
			var branchScopeCondition = BqlTemplate.OfCommand<
				Select<INRegister,
				Where<BqlPlaceholder1, Equal<INTransferType.twoStep>>>>
				.Replace<BqlPlaceholder1>(transferTypeField).ToCommand();
					...
					if (attr is PXDBFieldAttribute)
					{
						...
						var siteQuery = new Query()
							...
							.Where(siteExt.Column<INSite.siteID>()
								.Equal(new Column(((PXDBFieldAttribute)attr).DatabaseFieldName,
									e.Table == null ? _BqlTable.Name : e.Table.Name)));
	...
	public class PXDBBaseQuantityAttribute : PXDBQuantityAttribute
	{
        internal override INUnit ReadConversion(PXCache cache, object data)
        {
			var unit = base.ReadConversion(cache, data);
			if(unit != null && unit.FromUnit != unit.ToUnit)
            {
                unit = PXCache<INUnit>.CreateCopy(unit);
                unit.UnitMultDiv = (unit.UnitMultDiv == MultDiv.Multiply) ? MultDiv.Divide : MultDiv.Multiply;
            }
			return unit;
		}
						...
						else
						{
							...
							if (conv != null)
                            {
                                if(conv.FromUnit == conv.ToUnit)
                                {
                                    ...
                                    {
                                        decimal revValue = Math.Round(resultFieldCurrentValue ?? 0m, (int)_Precision, MidpointRounding.AwayFromZero);
		...
		protected virtual void CalcTranQty(PXCache sender, object data)
		{
			...
			if (_ResultField != null)
			{
				...
				if (NewValue != null)
				{
					...
					if (this._HandleEmptyKey)
					{
						object value = sender.GetValue(data, _KeyField.Name);
						if (string.IsNullOrEmpty((string)value))
						{
