...
namespace PX.Objects.AP
{
	public class APQuickCheckEntry : APDataEntryGraph<APQuickCheckEntry, APQuickCheck>, IGraphWithInitialization
	{
		...
		public PXSelectJoin<APTaxTran, LeftJoin<Tax, On<Tax.taxID, Equal<APTaxTran.taxID>>>, Where<APTaxTran.module, Equal<BatchModule.moduleAP>, And<APTaxTran.tranType, Equal<Current<APQuickCheck.docType>>, And<APTaxTran.refNbr, Equal<Current<APQuickCheck.refNbr>>,
		...
		public override IEnumerable VoidCheck(PXAdapter adapter)
		{
			if (Document.Current != null && (Document.Current.Released == true || Document.Current.Prebooked == true) && Document.Current.Voided == false && Document.Current.DocType == APDocType.QuickCheck)
			{
				...
				GetService<IFinPeriodUtils>().VerifyAndSetFirstOpenedFinPeriod<APQuickCheck.finPeriodID, APQuickCheck.branchID>(Document.Cache, doc, finperiod, typeof(OrganizationFinPeriod.aPClosed));
			...
			if (_licenseLimits != null)
			{
				OnBeforeCommit += _licenseLimits.GetCheckerDelegate<APQuickCheck>(
					new TableQuery(TransactionTypes.LinesPerMasterRecord, typeof(APTran), (graph) =>
					{
						...
						{
							new PXDataFieldValue<APTran.tranType>(((APQuickCheckEntry)graph).Document.Current?.DocType),
