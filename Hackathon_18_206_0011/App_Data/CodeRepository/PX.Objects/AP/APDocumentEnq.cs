...
namespace PX.Objects.AP
{
	...
	public class APDocumentEnq : PXGraph<APDocumentEnq>
	{
		...
		public partial class APDocumentFilter : IBqlTable
		{
			...
			public abstract class curyVendorRetainedBalance : PX.Data.IBqlField
			{
			}
			[PXCury(typeof(APDocumentFilter.curyID))]
			[PXDefault(TypeCode.Decimal, "0.0")]
			...
			public abstract class vendorRetainedBalance : PX.Data.IBqlField
			{
			}
			...
			public virtual void ClearSummary() 
			{
				...
				this.CuryVendorRetainedBalance = Decimal.Zero;
				this.VendorRetainedBalance = Decimal.Zero;
			}
		}
		...
		protected virtual IEnumerable documents()
		{
			...
			else if (header.OrganizationID != null)
			{
				...
				sel.WhereAnd<Where<APRegister.branchID, In<Required<APRegister.branchID>>>>();
			}
			...
			bool anyDoc = false;
			foreach (PXResult<APDocumentResult, Sub, APInvoice, APPayment> reg in sel.Select(branchIDs))
			{
