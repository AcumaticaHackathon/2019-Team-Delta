...
namespace PX.Objects.AP
{
	public class APInvoiceEntryExternalTax : ExternalTax<APInvoiceEntry, APInvoice>
	{
		...
		public override APInvoice CalculateExternalTax(APInvoice invoice)
		{
			...
			{
				Base.Document.SetValueExt<APInvoice.isTaxValid>(invoice, true);
				if (invoice.IsTaxSaved == true)
					Base.Document.SetValueExt<APInvoice.isTaxSaved>(invoice, false);
		...
		protected virtual void _(Events.RowPersisting<APInvoice> e)
		{
			...
			//Cancel tax if IsExternalTax has changed to False (Document was changed from External Tax Provider to Acumatica Tax Engine )
			if (((e.Operation & PXDBOperation.Command) == PXDBOperation.Insert || (e.Operation & PXDBOperation.Command) == PXDBOperation.Update)
		...
		protected virtual void _(Events.RowUpdated<APTran> e)
		{
			...
			if (Base.Document.Current != null && IsExternalTax(Base.Document.Current.TaxZoneID) && Base.Document.Current.InstallmentNbr == null &&
			    !e.Cache.ObjectsEqual<APTran.accountID, APTran.inventoryID, APTran.tranDesc, APTran.tranAmt, APTran.tranDate, APTran.taxCategoryID>(e.Row, e.OldRow))
			{
		...
		protected virtual void CancelTax(APInvoice invoice, VoidReasonCode code)
		{
			...
			if (!result.IsSuccess)
			{
				...
				if (result.Messages.Any(t => t.Contains("DocumentNotFoundError")))
				{
