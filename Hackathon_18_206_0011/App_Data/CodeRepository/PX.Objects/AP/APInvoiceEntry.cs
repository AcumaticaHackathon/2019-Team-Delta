	...
	public partial class APInvoiceEntry : APDataEntryGraph<APInvoiceEntry, APInvoice>, PXImportAttribute.IPXPrepareItems, IGraphWithInitialization
	{
		...
		public virtual void AddLandedCosts(IEnumerable<POLandedCostDetailS> details)
		{
			...
			foreach (var detailGroup in detailGroups)
			{
				...
				var transctions = landedCostApBillFactory.CreateTransactions(landedCostDoc, landedCostDetails);
		...
		public virtual void LinkLandedCostDetailLine(APInvoice doc, APTran apTran, POLandedCostDetailS detail)
		{
			...
			var aLCCode =
				(LandedCostCode)PXSelect<LandedCostCode,
						Where<LandedCostCode.landedCostCodeID, Equal<Required<LandedCostCode.landedCostCodeID>>>>
					.Select(this, detail.LandedCostCodeID);
		...
		{
			APInvoice current = CurrentDocument.Current;
			CurrentDocument.SetValueExt<APInvoice.taxCalcMode>(current, TaxCalculationMode.TaxSetting);
		}
			...
			if (_licenseLimits != null)
			{
				OnBeforeCommit += _licenseLimits.GetCheckerDelegate<APInvoice>(
					new TableQuery(TransactionTypes.LinesPerMasterRecord, typeof(APTran), (graph) =>
					{
						...
						{
							new PXDataFieldValue<APTran.tranType>(((APInvoiceEntry)graph).Document.Current?.DocType),
        ...
        protected virtual void APInvoice_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
					...
					APTran tran = PXSelectReadonly
						<APTran,
							Where<APTran.tranType, Equal<Required<APTran.tranType>>, And<APTran.refNbr, Equal<Required<APTran.refNbr>>,
								And<Where<APTran.pONbr, IsNotNull, Or<APTran.receiptNbr, IsNotNull>>>>>>.Select
			...
			Transactions.Cache.AllowDelete &= document.IsRetainageDocument != true && !invoiceState.IsRetainageDebAdj && !IsTaxDocumentVendorNotBaseCuryID;
			Transactions.Cache.AllowInsert &= document.IsRetainageDocument != true && !invoiceState.IsRetainageDebAdj && !IsTaxDocumentVendorNotBaseCuryID;
			Transactions.Cache.AllowUpdate &= document.IsRetainageDocument != true && !invoiceState.IsRetainageDebAdj && !IsTaxDocumentVendorNotBaseCuryID;

			bool isPayToVendor = PXSelect<Vendor, Where<Vendor.payToVendorID, Equal<Current<APInvoice.vendorID>>>, OrderBy<Asc<Vendor.bAccountID>>>.SelectSingleBound(this, new object[] {document}).Any();
		...
		protected virtual void APTran_RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
		{
			...
			if(Document.Current?.DocType == APDocType.Invoice && !sender.ObjectsEqual<APTran.lCDocType, APTran.lCRefNbr, APTran.lCLineNbr>(e.Row, e.OldRow))
			{
		...
		protected virtual void APTran_RowDeleted(PXCache sender, PXRowDeletedEventArgs e)
		{
			...
			if(Document.Current?.DocType == APDocType.Invoice && !String.IsNullOrEmpty(row.LCRefNbr))
			{
			...
			bool errorIfUnreleasedAPExists = true)
		{
			...
			var receiptLinks = PXSelectGroupBy<POOrderReceiptLink,
				Where<POOrderReceiptLink.receiptNbr, Equal<Required<POReceipt.receiptNbr>>, 
					And<Where<Required<POReceipt.receiptType>, Equal<POReceiptType.poreturn>,
						Or<POOrderReceiptLink.unbilledOrderQty, NotEqual<decimal0>,
						Or<POOrderReceiptLink.curyUnbilledOrderTotal, NotEqual<decimal0>>>>>>, 
				Aggregate<
				...
				.Select(this, receipt.ReceiptNbr, receipt.ReceiptType)
			...
			if (list != null)
			{
				...
				else
				{
					...
					Document.Cache.SetValueExt<APInvoice.curyID>(newdoc, receipt.CuryID);
			...
			HashSet<APTran> duplicates = new HashSet<APTran>(new POAccrualComparer());
			foreach (APTran tran in Transactions.Select())
			{
				duplicates.Add(tran);
			}
				...
				.Select(this, receipt.ReceiptNbr, newdoc.RefNbr, newdoc.DocType))
			{
				...
				containsRetainage |= ((orderline.RetainagePct ?? 0m) != 0m);
		...
		public virtual void AddOrderTaxLines(string orderType, string orderNbr)
		{
			foreach (var tax in
				...
				.OrderBy(r => r.Tax.TaxCalcLevel)) // Order by value, not by label (BQL sorts such fields by corresponding labels)
			{
		...
		public virtual void ProcessPOOrderLines(POOrder order, HashSet<APTran> duplicates, bool addBilled, bool keepOrderTaxes = false)
		{
			...
			HashSet<string> ordersWithDiscounts = new HashSet<string>();
			foreach (POLineS orderline in PXSelectReadonly2<POLineS,
				LeftJoin<APTran, On<APTran.pOOrderType, Equal<POLineS.orderType>, And<APTran.pONbr, Equal<POLineS.orderNbr>, And<APTran.pOLineNbr, Equal<POLineS.lineNbr>,
					And<APTran.released, Equal<False>>>>>>,
				Where<POLineS.orderType, Equal<Required<POLineS.orderType>>,
					...
					And2<Where<POLineS.billed, Equal<False>, Or<Required<POLineS.billed>, Equal<True>>>,
					And<POLineS.pOAccrualType, Equal<POAccrualType.order>,
					And<Where<APTran.refNbr, IsNull, Or<APTran.refNbr, Equal<Current<APInvoice.refNbr>>, And<APTran.tranType, Equal<Current<APInvoice.docType>>>>>>>>>>>>,
				OrderBy<Asc<POLineS.sortOrder>>>
				.Select(this, order.OrderType, order.OrderNbr, addBilled))
			{
				TaxAttribute.SetTaxCalc<APTran.taxCategoryID, TaxAttribute>(this.Transactions.Cache, null,
				...
				try
				{
					...
					{
						this.Document.Current.SetWarningOnDiscount = true;
						ordersWithDiscounts.Add(orderline.OrderNbr);
					}
				}
				catch (PXException ex)
				{
		...
		public virtual void ProcessPOOrderLines(IEnumerable<POLineRS> orderlines, HashSet<APTran> duplicates = null, bool keepOrderTaxes = false)
		{
			if (duplicates == null)
				duplicates = new HashSet<APTran>();
		...
		public virtual APTran AddPOReceiptLine(IAPTranSource aLine, HashSet<APTran> checkForDuplicates, ref POProcessingInfo pOProcessingInfo)
		{
			...
			decimal? billRatio = keepAmounts ? 1m : aLine.BaseBillQty / aLine.BaseOrigQty;
		...
		[Obsolete("Billed Cost and Qty are located in POAccrualStatus.")]
		public struct POVoucheredValues
		{
			...
			public decimal lineAmtLeftToBill;
			public decimal curyLineAmtLeftToBill;
			...
			public POVoucheredValues(bool isPOService)
			{
				...
				this.lineAmtLeftToBill = 0m;
				this.curyLineAmtLeftToBill = 0m;
			}
		}
		...
		protected virtual void CheckQtyFromPO(PXCache sender, APTran tran)
		{
			...
			switch (tran.POAccrualType)
			{
				case POAccrualType.Receipt:
					...
					if (tran.BaseQty > rctLine.BaseUnbilledQty && tran.Sign == ((rctLine.InvtMult < 0) ? -1m : 1m))
					{
				...
				case POAccrualType.Order:
					...
					decimal? maxBaseRcptQty = poLine.BaseOrderQty * poLine.RcptQtyMax / 100m;
					if (poLine.RcptQtyAction.IsIn(POReceiptQtyAction.Reject, POReceiptQtyAction.AcceptButWarn)
						&& tran.BaseQty + poLine.BaseBilledQty > maxBaseRcptQty)
					{
		...
		public partial class APAdjust : PX.Objects.AP.APAdjust
		{
			...
			[FinPeriodID(typeof(APAdjust.adjdDocDate), typeof(APAdjust.adjdBranchID))]
			[PXDBDefault(typeof(AP.APInvoice.finPeriodID))]
