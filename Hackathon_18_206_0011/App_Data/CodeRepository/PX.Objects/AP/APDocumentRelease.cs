	...
	public class APReleaseProcess : PXGraph<APReleaseProcess>
	{
		...
		public PXSelectJoin<
			...
			Where<
				...
				And<Where<
					APAdjust.released, NotEqual<True>,
					Or<Required<APAdjust.released>, Equal<True>, 
			...
			out List<INRegister> inDocs)
		{
			...
			if (doc.Released != true && (!isPrebooking || doc.Prebooked != true))
			{
				...
				if (apdoc.DocType != APDocType.Prepayment)
				{
					...
					foreach (PXResult<APTran> apTranWithData in apTranTaxes)
					{
						APTran tran = (APTran) apTranWithData;
					...
					foreach (PXResult<APTran, APTax, Tax, DRDeferredCode, LandedCostCode, InventoryItem> r in apTranTaxes)
					{
						APTran n = (APTran)r;
						APTax x = (APTax)r;
						Tax salestax = (Tax)r;
						...
						if (object.Equals(prev_n, n) == false)
						{
							...
							if (poRes != null)
							{
								var srcDoc = (POOrder)poRes;
								var srcLine = (POLine)poRes;
								if (_IsIntegrityCheck == false && updatePOOrder && srcLine.POAccrualType != null)
								{
									var updLine = (POLineUOpen)this.poOrderLineUPD.Cache.CreateCopy((POLineUOpen)poRes);
									...
									updLine.UnbilledQty -= n.Sign * qtyDelta;
									if (updLine.UnbilledQty < 0m)
										updLine.UnbilledQty = 0m;
									...
									decimal unbilledAmtDelta = amtDelta;
									if (Math.Abs((decimal)n.POPPVAmt) > 0.00005m && doc.DocType == APDocType.DebitAdj)
									{
										PXCurrencyAttribute.CuryConvCury(Caches[typeof(POLineUOpen)], updLine, (n.TranAmt + n.RetainageAmt - n.POPPVAmt) ?? 0m, out unbilledAmtDelta);
									}

									updLine.CuryBilledAmt += n.Sign * amtDelta;
									updLine.CuryUnbilledAmt -= n.Sign * unbilledAmtDelta;
									if (updLine.CuryUnbilledAmt < 0m)
										updLine.CuryUnbilledAmt = 0m;

									var accrualService = this.CreatePOAccrualService();
									var poLineAccrualStatus = accrualService.GetAccrualStatusSummary(updLine);
									...
									if (!needsPOReceipt || qtyCoincide && (updLine.Completed == true || !closePOLineByQty))
									{
										...
										if (closePOLineByQty)
										{
											if (updLine.Completed == true && n.Sign > 0m)
											{
												billedEnough = true;
											}
											else
										...
										{
											billedEnough = (poLineAccrualStatus.BillCuryID != null && poLineAccrualStatus.BillCuryID == srcDoc.CuryID)
												? ((srcLine.CuryExtCost + srcLine.CuryRetainageAmt) * srcLine.RcptQtyThreshold / 100m <= poLineAccrualStatus.CuryBilledAmt)
												: ((srcLine.ExtCost + srcLine.RetainageAmt) * srcLine.RcptQtyThreshold / 100m <= poLineAccrualStatus.BilledAmt);
										}
									...
									if (closePOLine || (!closePOLine && updLine.Closed == true && n.Sign < 0m))
									{
										...
										orderCheckClosed[orderKey] = closePOLine;
									}
								...
								if (srcLine != null && srcLine.CommitmentID != null && !isPrebooking)
								{
									...
									container.InvoicedAmount = n.Sign * n.LineAmt;
					...
					foreach (KeyValuePair<KeyValuePair<string, string>, bool> orderNbr in orderCheckClosed)
					{
						POLineUOpen line =
							PXSelect<POLineUOpen,
								...
								And<POLineUOpen.closed, Equal<False>>>>>>.Select(this, orderNbr.Key.Key, orderNbr.Key.Value);
		...
		{
			if (doc.DocType == APDocType.DebitAdj && !String.IsNullOrEmpty(tran.LCDocType) && !String.IsNullOrEmpty(tran.LCRefNbr) && tran.LCLineNbr.HasValue)
			{
			...
			APTran n, LandedCostCode lcCode, Amount postedAmount)
		{
				...
				&& !isPrebooking && !isPrebookingVoiding)
			{
				...
				var landedCostLineAmt = GetLandedCostAmount(landedCostDetail);
			...
			List<AllocationServiceBase.POReceiptLineAdjustment> ppvTransactions, List<APTran> ppvLines)
		{
			...
			if (uomCoincide && poAccrual.ReceivedQty != null && poAccrual.ReceivedUOM.IsIn(null, tran.UOM) && poAccrual.BilledQty != null && poAccrual.BilledUOM.IsIn(null, tran.UOM))
			{
				...
				closingPOAccrual = (poAccrual.ReceivedQty == poAccrual.BilledQty + tran.Sign * tran.Qty);
			}
			else
			{
				closingPOAccrual = (poAccrual.BaseReceivedQty == poAccrual.BaseBilledQty + tran.Sign * tran.BaseQty);
			}
		...
		protected virtual IEnumerable<GLTran> PostTaxExpenseToItemAccounts(JournalEntry je, APInvoice apdoc, CurrencyInfo new_info, APTaxTran x, Tax salestax, bool doInsert)
		{
			...
			PXResultset<APTax> deductibleLines = PXSelectJoin<APTax, 
		...
		protected virtual bool IsDebitTaxTran(APTaxTran x)
		{
			bool isDebit = (APInvoiceType.DrCr(x.OrigTranType != null && x.OrigTranType.Trim() != string.Empty
				? x.OrigTranType
				: x.TranType) == DrCr.Debit);
			...
			Tax salestax)
		{
			...
			bool isOriginalRetainageDocumentWithTax = apdoc.IsOriginalRetainageDocument() && x.CuryRetainedTaxAmt != 0m && !isUseTax;
			bool isChildRetainageDocumentWithTax = apdoc.IsChildRetainageDocument() && x.CuryTaxAmt != 0m && !isUseTax;
			...
			else if (isChildRetainageDocumentWithTax)
			{
				...
				if (origRetainageDoc != null && origRetainageDoc.CuryRetainageUnreleasedAmt == 0m)
				{
					PXSelectBase<APTaxTran> selectTaxTran = new PXSelect<APTaxTran,
						Where<APTaxTran.module, Equal<Required<APTaxTran.module>>,
							And<APTaxTran.tranType, Equal<Required<APTaxTran.tranType>>,
							And<APTaxTran.refNbr, Equal<Required<APTaxTran.refNbr>>,
							And<APTaxTran.taxID, Equal<Required<APTaxTran.taxID>>>>>>>(this);

					APTaxTran origTaxTran = selectTaxTran.SelectSingle(x.Module, origRetainageDoc.DocType, origRetainageDoc.RefNbr, x.TaxID);

					if (origTaxTran != null)
					{
						decimal sumCuryTaxAmt = (x.CuryTaxAmt ?? 0m) + (x.CuryExpenseAmt ?? 0m);
						decimal sumTaxAmt = (x.TaxAmt ?? 0m) + (x.ExpenseAmt ?? 0m);

						foreach (APRegister childRetainageDoc in PXSelect<APRegister,
							...
							.Select(this, origRetainageDoc.DocType, origRetainageDoc.RefNbr))
						{
							APTaxTran childTaxTran = selectTaxTran.SelectSingle(x.Module, childRetainageDoc.DocType, childRetainageDoc.RefNbr, x.TaxID);
							if (childTaxTran != null)
							{
								sumCuryTaxAmt += (childTaxTran.CuryTaxAmt ?? 0m) + (childTaxTran.CuryExpenseAmt ?? 0m);
								sumTaxAmt += (childTaxTran.TaxAmt ?? 0m) + (childTaxTran.ExpenseAmt ?? 0m);
		}
						}

						decimal curyTaxAmtDiff = (origTaxTran.CuryRetainedTaxAmt ?? 0m) - sumCuryTaxAmt;
						decimal taxAmtDiff = (origTaxTran.RetainedTaxAmt ?? 0m) - sumTaxAmt;

						if (curyTaxAmtDiff != 0m || taxAmtDiff != 0m)
						{
							bool origIsDebit = IsDebitTaxTran(origTaxTran) && !isReversedTax;

							GLTran roundingTran = PXCache<GLTran>.CreateCopy(origTran);
							roundingTran.ReclassificationProhibited = true;
							roundingTran.AccountID = salestax.RetainageTaxClaimableAcctID;
							roundingTran.SubID = salestax.RetainageTaxClaimableSubID;

							roundingTran.CuryDebitAmt = origIsDebit && curyTaxAmtDiff > 0m ? 0m : Math.Abs(curyTaxAmtDiff);
							roundingTran.DebitAmt = origIsDebit && taxAmtDiff > 0m ? 0m : Math.Abs(taxAmtDiff);
							roundingTran.CuryCreditAmt = origIsDebit && curyTaxAmtDiff > 0m ? Math.Abs(curyTaxAmtDiff) : 0m;
							roundingTran.CreditAmt = origIsDebit && taxAmtDiff > 0m ? Math.Abs(taxAmtDiff) : 0m;

							je.GLTranModuleBatNbr.Insert(roundingTran);

							decimal curyCreditDiff = (decimal)(roundingTran.CuryDebitAmt - roundingTran.CuryCreditAmt);
							decimal creditDiff = (decimal)(roundingTran.DebitAmt - roundingTran.CreditAmt);
		...
		protected virtual void PostDirectTax(CurrencyInfo info, APTaxTran x, APInvoice orig_doc)
		{
			...
			if (tran == null)
			{
				...
				tran.FinPeriodID = orig_doc.FinPeriodID;
		...
		public static Amount GetExpensePostingAmount(PXGraph graph, POLine documentLine)
		{
			...
			PXResult<POLine, POTax, Tax> queryResult =
				documentLineWithTaxes
					.Select(documentLine.OrderType, documentLine.OrderNbr, documentLine.LineNbr)
		...
		protected virtual void VerifyRoundingAllowed(APInvoice document, Batch batch, string baseCuryID)
		{
			...
			if (useCurrencyPrecision && Math.Abs(Math.Round((decimal)(document.CuryTaxRoundDiff) - diff, 4)) >= 0.00005m)
			{
		...
		private void UpdateWithholding(JournalEntry je, APAdjust adj, APRegister adjddoc, APPayment adjgdoc, Vendor vend, CurrencyInfo vouch_info)
		{
			...
			foreach (PXResult<APTaxTran, Tax> whres in whtaxtrans)
			{
				...
				else
				{
					...
					whtran.FinPeriodID = adj.AdjgFinPeriodID;
		...
		public virtual void UpdateBalances(APAdjust adj, APRegister adjddoc, Vendor vendor)
		{
            ...
            if (apdoc.DocType == APDocType.Prepayment && adj.AdjgDocType != APDocType.Refund)
            {
		...
		private void ProcessPrepaymentRequestApplication(APRegister prepaymentRequest, APAdjust prepaymentAdj)
		{
			...
			else if (prepaymentAdj.AdjgDocType == APDocType.Check)
			{
				if (_IsIntegrityCheck)
				{
					...
					BalanceCalculation.AdjustBalance(prepaymentRequest, prepaymentAdj);
			...
			Currency paycury)
		{
				...
				adjustments.Where(row => ((APAdjust)row).IsInitialApplication != true))
			{
				...
				if (adjddoc.RefNbr != null)
				{
					UpdateBalances(adj, adjddoc, paymentVendor);
						...
						(doc.DocType == APDocType.Check || doc.DocType == APDocType.VoidCheck || doc.DocType == APDocType.Prepayment))
					{
						...
						foreach(PXResult<APAdjust> adjres in APAdjust_AdjgDocType_RefNbr_VendorID.Select(doc.DocType, doc.RefNbr, _IsIntegrityCheck, doc.AdjCntr))
						{
							...
							List<PXResult<APAdjust>> apps;
								var appsKey = new Tuple<string, string>(adj.AdjdDocType, adj.AdjgFinPeriodID);
						...
						foreach (KeyValuePair<Tuple<string, string>, List<PXResult<APAdjust>>> pair in appsByDocTypeAndPeriod)
						{
							...
							JournalEntry.SegregateBatch(je, BatchModule.AP, doc.BranchID, doc.CuryID, datesByDocTypeAndPeriod[appsKey], appsKey.Item2,
		...
		private void SetClosedPeriodsFromLatestApplication(APRegister doc, int? adjNbr)
		{
			...
			foreach (string docType in docTypes)
			{
				...
				{
					doc.ClosedFinPeriodID = GL.FinPeriods.FinPeriodUtils.Max(doc.ClosedFinPeriodID, adj.AdjgFinPeriodID);
					doc.ClosedTranPeriodID = GL.FinPeriods.FinPeriodUtils.Max(doc.ClosedTranPeriodID, adj.AdjgTranPeriodID);
				}
			}

			doc.ClosedFinPeriodID = GL.FinPeriods.FinPeriodUtils.Max(doc.ClosedFinPeriodID, doc.FinPeriodID);
			doc.ClosedTranPeriodID = GL.FinPeriods.FinPeriodUtils.Max(doc.ClosedTranPeriodID, doc.TranPeriodID);
		}
		...
		private void SetAdjgPeriodsFromLatestApplication(APRegister doc, APAdjust adj)
		{
			if (adj.VoidAppl == true)
			{
				...
				foreach (string adjgDocType in doc.PossibleOriginalDocumentTypes())
				{
					...
					if (orig != null)
				{
						adj.AdjgFinPeriodID = GL.FinPeriods.FinPeriodUtils.Max(orig.AdjgFinPeriodID, adj.AdjgFinPeriodID);
						adj.AdjgTranPeriodID = GL.FinPeriods.FinPeriodUtils.Max(orig.AdjgTranPeriodID, adj.AdjgTranPeriodID);
			...
			adj.AdjgFinPeriodID = GL.FinPeriods.FinPeriodUtils.Max(adj.AdjdFinPeriodID, adj.AdjgFinPeriodID);
			adj.AdjgTranPeriodID = GL.FinPeriods.FinPeriodUtils.Max(adj.AdjdTranPeriodID, adj.AdjgTranPeriodID);
