	...
	public class APPaymentEntry : APDataEntryGraph<APPaymentEntry, APPayment>
	{
		...
		public override IEnumerable VoidCheck(PXAdapter adapter)
		{
				...
				APPaymentType.VoidEnabled(Document.Current.DocType))
			{
				...
				GetService<IFinPeriodUtils>().VerifyAndSetFirstOpenedFinPeriod<APPayment.finPeriodID, APPayment.branchID>(
					Document.Cache,
					doc,
					finperiod,
		...
		public override void Persist()
		{
			...
			{
				if (doc.OpenDoc == true && 
					(bool?)Document.Cache.GetValueOriginal<APPayment.openDoc>(doc) == false &&
		...
		protected virtual void APAdjust_RowInserting(PXCache sender, PXRowInsertingEventArgs e)
		{
			...
			APPayment document = Document.Current;
			    ...
			    && AdjgDocTypesToValidateFinPeriod.Contains(document.DocType))
			{
				...
				FinPeriodUtils.CanPostToPeriod(organizationFinPeriod, typeof(FinPeriod.aPClosed)).RaiseIfHasError();
			}
		...
		protected virtual void APPayment_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
				...
				&& ((DateTime)doc.AdjDate).CompareTo((DateTime)Accessinfo.BusinessDate) < 0)
			{
				if (Adjustments_Raw.View.SelectSingleBound(new object[] { e.Row }) == null)
				{
					...
					FinPeriod adjFinPeriod = cache.Graph.GetService<IFinPeriodRepository>().FindFinPeriodByDate(doc.AdjDate, PXAccess.GetParentOrganizationID(doc.BranchID));
		...
		protected virtual void APPayment_RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
		{
			...
			if (row.OpenDoc == true && row.Hold != true)
			{
				...
				if (canHaveBalance && row.VoidAppl != true && 
					...
					!canHaveBalance && (row.CuryUnappliedBal != 0m && row.CuryUnappliedBal != null))
				{
		...
		protected bool IsRequestPrepayment(APPayment apdoc)
		{
			APInvoice result = PXSelectReadonly<APInvoice,
					Where<APInvoice.docType, Equal<Current<APPayment.docType>>,
						And<APInvoice.refNbr, Equal<Current<APPayment.refNbr>>,
						And<APInvoice.docType, Equal<APDocType.prepayment>>>>>
					.SelectSingleBound(this, new object[] { apdoc });
